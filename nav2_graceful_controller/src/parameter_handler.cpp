...
ParameterHandler::ParameterHandler(const nav2::LifecycleNode::SharedPtr& node, std::string& plugin_name, rclcpp::Logger& logger,
                                   const double costmap_size_x)
    : nav2_util::ParameterHandler<Parameters>(node, logger) {
    plugin_name_ = plugin_name;

    params_.transform_tolerance = node->declare_or_get_parameter(plugin_name_ + ".transform_tolerance", 0.1);
    params_.min_lookahead = node->declare_or_get_parameter(plugin_name_ + ".min_lookahead", 0.25);
    params_.max_lookahead = node->declare_or_get_parameter(plugin_name_ + ".max_lookahead", 1.0);
    params_.max_robot_pose_search_dist = node->declare_or_get_parameter(plugin_name_ + ".max_robot_pose_search_dist", costmap_size_x / 2.0);
    ...
    params_.k_phi = node->declare_or_get_parameter(plugin_name_ + ".k_phi", 2.0);
    params_.k_delta = node->declare_or_get_parameter(plugin_name_ + ".k_delta", 1.0);
    params_.beta = node->declare_or_get_parameter(plugin_name_ + ".beta", 0.4);
    params_.lambda = node->declare_or_get_parameter(plugin_name_ + ".lambda", 2.0);
    params_.v_linear_min = node->declare_or_get_parameter(plugin_name_ + ".v_linear_min", 0.1);
    params_.v_linear_max = node->declare_or_get_parameter(plugin_name_ + ".v_linear_max", 0.5);
    params_.v_angular_max = node->declare_or_get_parameter(plugin_name_ + ".v_angular_max", 1.0);
    params_.v_angular_min_in_place = node->declare_or_get_parameter(plugin_name_ + ".v_angular_min_in_place", 0.25);
    params_.slowdown_radius = node->declare_or_get_parameter(plugin_name_ + ".slowdown_radius", 1.5);
    params_.initial_rotation = node->declare_or_get_parameter(plugin_name_ + ".initial_rotation", true);
    params_.initial_rotation_tolerance = node->declare_or_get_parameter(plugin_name_ + ".initial_rotation_tolerance", 0.75);
    params_.prefer_final_rotation = node->declare_or_get_parameter(plugin_name_ + ".prefer_final_rotation", true);
    params_.rotation_scaling_factor = node->declare_or_get_parameter(plugin_name_ + ".rotation_scaling_factor", 0.5);
    params_.allow_backward = node->declare_or_get_parameter(plugin_name_ + ".allow_backward", false);
    params_.in_place_collision_resolution = node->declare_or_get_parameter(plugin_name_ + ".in_place_collision_resolution", 0.1);
    params_.use_collision_detection = node->declare_or_get_parameter(plugin_name_ + ".use_collision_detection", true);
...
rcl_interfaces::msg::SetParametersResult ParameterHandler::validateParameterUpdatesCallback(const std::vector<rclcpp::Parameter>& parameters) {
...
void ParameterHandler::updateParametersCallback(const std::vector<rclcpp::Parameter>& parameters) {
