...
class SCLFixture : public nav2_graceful_controller::SmoothControlLaw {
   public:
    SCLFixture(double k_phi, double k_delta, double beta, double lambda, double slowdown_radius, double v_linear_min, double v_linear_max,
               double v_angular_max)
        : nav2_graceful_controller::SmoothControlLaw(k_phi, k_delta, beta, lambda, slowdown_radius, v_linear_min, v_linear_max, v_angular_max) {}
    ...
    double calculateCurvature(geometry_msgs::msg::Pose target, geometry_msgs::msg::Pose current) {
        ...
        return nav2_graceful_controller::SmoothControlLaw::calculateCurvature(ego_coords.r, ego_coords.phi, ego_coords.delta);
    }
};

class GMControllerFixture : public nav2_graceful_controller::GracefulController {
   public:
    GMControllerFixture() : nav2_graceful_controller::GracefulController() {}
    ...
    visualization_msgs::msg::Marker createSlowdownMarker(const geometry_msgs::msg::PoseStamped& motion_target) {
        return nav2_graceful_controller::createSlowdownMarker(motion_target, params_->slowdown_radius);
    }

    geometry_msgs::msg::Twist rotateToTarget(const double& angle_to_target) {
        return nav2_graceful_controller::GracefulController::rotateToTarget(angle_to_target);
    }
...
TEST(GracefulControllerTest, dynamicParameters) {
    ...
    nav2::declare_parameter_if_not_declared(node, "test.max_robot_pose_search_dist", rclcpp::ParameterValue(-2.0));

    // Set initial rotation and allow backward to true so it warns and allow backward is false
    nav2::declare_parameter_if_not_declared(node, "test.initial_rotation", rclcpp::ParameterValue(true));
    nav2::declare_parameter_if_not_declared(node, "test.allow_backward", rclcpp::ParameterValue(true));
    nav2::declare_parameter_if_not_declared(node, "test.use_collision_detection", rclcpp::ParameterValue(true));
    ...
    auto params = std::make_shared<rclcpp::AsyncParametersClient>(node->get_node_base_interface(), node->get_node_topics_interface(),
                                                                  node->get_node_graph_interface(), node->get_node_services_interface());

    // Set parameters
    auto results = params->set_parameters_atomically(
        {rclcpp::Parameter("test.transform_tolerance", 1.0), rclcpp::Parameter("test.min_lookahead", 1.0),
         rclcpp::Parameter("test.max_lookahead", 2.0), rclcpp::Parameter("test.k_phi", 4.0), rclcpp::Parameter("test.k_delta", 5.0),
         rclcpp::Parameter("test.beta", 6.0), rclcpp::Parameter("test.lambda", 7.0), rclcpp::Parameter("test.v_linear_min", 8.0),
         rclcpp::Parameter("test.v_linear_max", 9.0), rclcpp::Parameter("test.v_angular_max", 10.0),
         rclcpp::Parameter("test.v_angular_min_in_place", 14.0), rclcpp::Parameter("test.slowdown_radius", 11.0),
         rclcpp::Parameter("test.initial_rotation", false), rclcpp::Parameter("test.initial_rotation_tolerance", 12.0),
         rclcpp::Parameter("test.prefer_final_rotation", false), rclcpp::Parameter("test.rotation_scaling_factor", 13.0),
         rclcpp::Parameter("test.allow_backward", false), rclcpp::Parameter("test.use_collision_detection", false),
    ...
    results = params->set_parameters_atomically({rclcpp::Parameter("test.allow_backward", true)});
    rclcpp::spin_until_future_complete(node->get_node_base_interface(), results);
    EXPECT_EQ(controller->getInitialRotation(), false);
    EXPECT_EQ(controller->getAllowBackward(), true);

    // Set initial rotation to true
    results = params->set_parameters_atomically({rclcpp::Parameter("test.initial_rotation", true)});
    rclcpp::spin_until_future_complete(node->get_node_base_interface(), results);
    ...
    results = params->set_parameters_atomically({rclcpp::Parameter("test.initial_rotation", false), rclcpp::Parameter("test.allow_backward", false)});
    rclcpp::spin_until_future_complete(node->get_node_base_interface(), results);
    // Check parameters.
    EXPECT_EQ(node->get_parameter("test.initial_rotation").as_bool(), false);
    EXPECT_EQ(node->get_parameter("test.allow_backward").as_bool(), false);
...
TEST(GracefulControllerTest, createSlowdownMsg) {
    ...
    auto results = params->set_parameters_atomically({rclcpp::Parameter("test.slowdown_radius", 0.2)});
    rclcpp::spin_until_future_complete(node->get_node_base_interface(), results);
...
TEST(GracefulControllerTest, rotateToTarget) {
    ...
    auto params = std::make_shared<rclcpp::AsyncParametersClient>(node->get_node_base_interface(), node->get_node_topics_interface(),
                                                                  node->get_node_graph_interface(), node->get_node_services_interface());
    auto results =
        params->set_parameters_atomically({rclcpp::Parameter("test.v_angular_max", 1.0), rclcpp::Parameter("test.rotation_scaling_factor", 0.5)});
    rclcpp::spin_until_future_complete(node->get_node_base_interface(), results);
    ...
    results = params->set_parameters_atomically({rclcpp::Parameter("test.v_angular_min_in_place", 1.0)});
    rclcpp::spin_until_future_complete(node->get_node_base_interface(), results);
...
TEST(GracefulControllerTest, setSpeedLimit) {
    ...
    auto params = std::make_shared<rclcpp::AsyncParametersClient>(node->get_node_base_interface(), node->get_node_topics_interface(),
                                                                  node->get_node_graph_interface(), node->get_node_services_interface());
    auto results = params->set_parameters_atomically({rclcpp::Parameter("test.v_linear_min", 2.0), rclcpp::Parameter("test.v_linear_max", 5.0)});
    rclcpp::spin_until_future_complete(node->get_node_base_interface(), results);
...
TEST(GracefulControllerTest, emptyPlan) {
    ...
    auto results = costmap_ros->set_parameters({rclcpp::Parameter("global_frame", "test_global_frame"),
                                                rclcpp::Parameter("robot_base_frame", "test_robot_frame"), rclcpp::Parameter("width", 10),
                                                rclcpp::Parameter("height", 10), rclcpp::Parameter("resolution", 0.1)});
    ...
    nav2::declare_parameter_if_not_declared(node, "test.max_robot_pose_search_dist", rclcpp::ParameterValue(5.0));
...
TEST(GracefulControllerTest, pruningPlan) {
    ...
    auto costmap_ros = std::make_shared<nav2_costmap_2d::Costmap2DROS>("test_costmap");
    auto results = costmap_ros->set_parameters({rclcpp::Parameter("global_frame", "test_global_frame"),
                                                rclcpp::Parameter("robot_base_frame", "test_robot_frame"), rclcpp::Parameter("width", 20),
                                                rclcpp::Parameter("height", 20), rclcpp::Parameter("resolution", 0.1)});
    for (const auto& result : results) {
        EXPECT_TRUE(result.successful) << result.reason;
    }
    costmap_ros->on_configure(rclcpp_lifecycle::State());
    ...
    // Set max search distant
    nav2::declare_parameter_if_not_declared(node, "test.max_robot_pose_search_dist", rclcpp::ParameterValue(9.0));
...
TEST(GracefulControllerTest, pruningPlanOutsideCostmap) {
    ...
    // Set max search distant
    nav2::declare_parameter_if_not_declared(node, "test.max_robot_pose_search_dist", rclcpp::ParameterValue(15.0));
...
TEST(GracefulControllerTest, computeVelocityCommandRotate) {
    ...
    nav2::declare_parameter_if_not_declared(node, "test.v_angular_max", rclcpp::ParameterValue(1.0));
    nav2::declare_parameter_if_not_declared(node, "test.rotation_scaling_factor", rclcpp::ParameterValue(0.5));
...
TEST(GracefulControllerTest, computeVelocityCommandRegularBackwards) {
    ...
    nav2::declare_parameter_if_not_declared(node, "test.initial_rotation", rclcpp::ParameterValue(false));
    nav2::declare_parameter_if_not_declared(node, "test.allow_backward", rclcpp::ParameterValue(true));
