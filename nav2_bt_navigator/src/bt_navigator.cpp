...
BtNavigator::BtNavigator(rclcpp::NodeOptions options)
    : nav2::LifecycleNode("bt_navigator", "", options.automatically_declare_parameters_from_overrides(true))
...
nav2::CallbackReturn BtNavigator::on_configure(const rclcpp_lifecycle::State& state) {
    ...
    auto user_defined_plugins = this->declare_or_get_parameter("plugin_lib_names", std::vector<std::string>{});
    // append user_defined_plugins to plugin_lib_names
    plugin_lib_names.insert(plugin_lib_names.end(), user_defined_plugins.begin(), user_defined_plugins.end());
    ...
    // Navigator defaults
    const std::vector<std::string> default_navigator_ids = {"navigate_to_pose", "navigate_through_poses"};
    const std::vector<std::string> default_navigator_types = {"nav2_bt_navigator::NavigateToPoseNavigator",
    ...
    if (navigator_ids == default_navigator_ids) {
        for (size_t i = 0; i < default_navigator_ids.size(); ++i) {
            declare_parameter_if_not_declared(node, default_navigator_ids[i] + ".plugin", rclcpp::ParameterValue(default_navigator_types[i]));
        }
    }
    ...
    for (size_t i = 0; i != navigator_ids.size(); i++) {
        try {
            ...
            RCLCPP_INFO(get_logger(), "Creating navigator id %s of type %s", navigator_ids[i].c_str(), navigator_type.c_str());
            navigators_.push_back(class_loader_.createUniqueInstance(navigator_type));
            if (!navigators_.back()->on_configure(node, plugin_lib_names, feedback_utils, &plugin_muxer_, odom_smoother_)) {
...
nav2::CallbackReturn BtNavigator::on_activate(const rclcpp_lifecycle::State& state) {
...
nav2::CallbackReturn BtNavigator::on_deactivate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn BtNavigator::on_cleanup(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn BtNavigator::on_shutdown(const rclcpp_lifecycle::State& /*state*/) {
