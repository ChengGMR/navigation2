...
bool NavigateToPoseNavigator::configure(nav2::LifecycleNode::WeakPtr parent_node, std::shared_ptr<nav2_util::OdomSmoother> odom_smoother) {
    start_time_ = rclcpp::Time(0);
    auto node = parent_node.lock();

    goal_blackboard_id_ = node->declare_or_get_parameter(getName() + ".goal_blackboard_id", std::string("goal"));
    ...
    goal_sub_ = node->create_subscription<geometry_msgs::msg::PoseStamped>(
        "goal_pose", std::bind(&NavigateToPoseNavigator::onGoalPoseReceived, this, std::placeholders::_1));
...
std::string NavigateToPoseNavigator::getDefaultBTFilepath(nav2::LifecycleNode::WeakPtr parent_node) {
    auto node = parent_node.lock();
    std::string pkg_share_dir = ament_index_cpp::get_package_share_directory("nav2_bt_navigator");

    auto default_bt_xml_filename = node->declare_or_get_parameter("default_nav_to_pose_bt_xml",
...
void NavigateToPoseNavigator::goalCompleted(typename ActionT::Result::SharedPtr result, const nav2_behavior_tree::BtStatus /*final_bt_status*/) {
    if (result->error_code == 0) {
        if (bt_action_server_->populateInternalError(result)) {
            RCLCPP_WARN(logger_, "NavigateToPoseNavigator::goalCompleted, internal error %d:%s.", result->error_code, result->error_msg.c_str());
        }
    } else {
        RCLCPP_WARN(logger_, "NavigateToPoseNavigator::goalCompleted error %d:%s.", result->error_code, result->error_msg.c_str());
    }
}
...
bool NavigateToPoseNavigator::initializeGoalPose(ActionT::Goal::ConstSharedPtr goal) {
    ...
    if (!nav2_util::transformPoseInTargetFrame(goal->pose, goal_pose, *feedback_utils_.tf, feedback_utils_.global_frame,
                                               feedback_utils_.transform_tolerance)) {
        bt_action_server_->setInternalError(ActionT::Result::TF_ERROR, "Failed to transform a goal pose provided with frame_id '"
                                                                           + goal->pose.header.frame_id + "' to the global frame '"
                                                                           + feedback_utils_.global_frame + "'.");
        return false;
    }

    RCLCPP_INFO(logger_, "Begin navigating from current location (%.2f, %.2f) to (%.2f, %.2f)", current_pose.pose.position.x,
                current_pose.pose.position.y, goal_pose.pose.position.x, goal_pose.pose.position.y);
