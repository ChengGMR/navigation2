...
bool NavigateThroughPosesNavigator::configure(nav2::LifecycleNode::WeakPtr parent_node, std::shared_ptr<nav2_util::OdomSmoother> odom_smoother) {
    start_time_ = rclcpp::Time(0);
    auto node = parent_node.lock();

    goals_blackboard_id_ = node->declare_or_get_parameter(getName() + ".goals_blackboard_id", std::string("goals"));
    ...
    waypoint_statuses_blackboard_id_ =
        node->declare_or_get_parameter(getName() + ".waypoint_statuses_blackboard_id", std::string("waypoint_statuses"));

    // Odometry smoother object for getting current speed
    odom_smoother_ = odom_smoother;

    bool enable_groot_monitoring = node->declare_or_get_parameter(getName() + ".enable_groot_monitoring", false);
    int groot_server_port = node->declare_or_get_parameter(getName() + ".groot_server_port", 1669);
...
std::string NavigateThroughPosesNavigator::getDefaultBTFilepath(nav2::LifecycleNode::WeakPtr parent_node) {
    auto node = parent_node.lock();
    ...
    auto default_bt_xml_filename = node->declare_or_get_parameter(
        "default_nav_through_poses_bt_xml", pkg_share_dir + "/behavior_trees/navigate_through_poses_w_replanning_and_recovery.xml");
...
void NavigateThroughPosesNavigator::goalCompleted(typename ActionT::Result::SharedPtr result, const nav2_behavior_tree::BtStatus final_bt_status) {
    if (result->error_code == 0) {
        if (bt_action_server_->populateInternalError(result)) {
            RCLCPP_WARN(logger_, "NavigateThroughPosesNavigator::goalCompleted, internal error %d:'%s'.", result->error_code,
                        result->error_msg.c_str());
        }
    } else {
        RCLCPP_WARN(logger_, "NavigateThroughPosesNavigator::goalCompleted error %d:'%s'.", result->error_code, result->error_msg.c_str());
    }

    // populate waypoint statuses in result
    auto blackboard = bt_action_server_->getBlackboard();
    auto waypoint_statuses = blackboard->get<std::vector<nav2_msgs::msg::WaypointStatus>>(waypoint_statuses_blackboard_id_);

    // populate remaining waypoint statuses based on final_bt_status
    auto integrate_waypoint_status = final_bt_status == nav2_behavior_tree::BtStatus::SUCCEEDED ? nav2_msgs::msg::WaypointStatus::COMPLETED
                                                                                                : nav2_msgs::msg::WaypointStatus::FAILED;
...
void NavigateThroughPosesNavigator::onLoop() {
    ...
    feedback_msg->waypoint_statuses = blackboard->get<std::vector<nav2_msgs::msg::WaypointStatus>>(waypoint_statuses_blackboard_id_);
    ...
    if (!nav2_util::getCurrentPose(current_pose, *feedback_utils_.tf, feedback_utils_.global_frame, feedback_utils_.robot_frame,
...
void NavigateThroughPosesNavigator::onPreempt(ActionT::Goal::ConstSharedPtr goal) {
    ...
    if (goal->behavior_tree == bt_action_server_->getCurrentBTFilenameOrID()
        || (goal->behavior_tree.empty() && bt_action_server_->getCurrentBTFilenameOrID() == bt_action_server_->getDefaultBTFilenameOrID())) {
...
bool NavigateThroughPosesNavigator::initializeGoalPoses(ActionT::Goal::ConstSharedPtr goal) {
    ...
    if (!nav2_util::getCurrentPose(current_pose, *feedback_utils_.tf, feedback_utils_.global_frame, feedback_utils_.robot_frame,
                                   feedback_utils_.transform_tolerance)) {
        bt_action_server_->setInternalError(ActionT::Result::TF_ERROR, "Initial robot pose is not available.");
        return false;
    }
    ...
    for (auto& goal_pose : goals_array.goals) {
        if (!nav2_util::transformPoseInTargetFrame(goal_pose, goal_pose, *feedback_utils_.tf, feedback_utils_.global_frame,
                                                   feedback_utils_.transform_tolerance)) {
            bt_action_server_->setInternalError(ActionT::Result::TF_ERROR, "Failed to transform a goal pose (" + std::to_string(i)
                                                                               + ") provided with frame_id '" + goal_pose.header.frame_id
                                                                               + "' to the global frame '" + feedback_utils_.global_frame + "'.");
            return false;
        }
        i++;
    }
    ...
    if (goals_array.goals.size() > 0) {
        RCLCPP_INFO(logger_, "Begin navigating from current location through %zu poses to (%.2f, %.2f)", goals_array.goals.size(),
                    goals_array.goals.back().pose.position.x, goals_array.goals.back().pose.position.y);
    }
    ...
    blackboard->set<decltype(waypoint_statuses)>(waypoint_statuses_blackboard_id_, std::move(waypoint_statuses));
