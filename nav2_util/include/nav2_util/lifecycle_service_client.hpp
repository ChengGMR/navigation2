...
class LifecycleServiceClient {
   public:
    ...
    explicit LifecycleServiceClient(const string& lifecycle_node_name, NodeT parent_node)
        : change_state_(lifecycle_node_name + "/change_state", parent_node, true /*creates and spins an internal executor*/)
        , get_state_(lifecycle_node_name + "/get_state", parent_node, true /*creates and spins an internal executor*/) {
        // Block until server is up
        rclcpp::Rate r(20);
        while (!get_state_.wait_for_service(2s)) {
            RCLCPP_INFO(parent_node->get_logger(), "Waiting for service %s...", get_state_.getServiceName().c_str());
            r.sleep();
        }
    }
     ...
     */
    bool change_state(const uint8_t transition, // takes a lifecycle_msgs::msg::Transition id
