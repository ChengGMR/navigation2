...
class OdomSmoother {
   public:
    ...
    explicit OdomSmoother(const NodeT& parent, double filter_duration = 0.3, const std::string& odom_topic = "odom")
        ...
        odom_sub_ = nav2::interfaces::create_subscription<nav_msgs::msg::Odometry>(
            parent, odom_topic, std::bind(&OdomSmoother::odomCallback, this, std::placeholders::_1));
    ...
    inline geometry_msgs::msg::Twist getTwist() {
        ...
        if (!received_odom_) {
            RCLCPP_ERROR(rclcpp::get_logger("OdomSmoother"), "OdomSmoother has not received any data yet, returning empty Twist");
