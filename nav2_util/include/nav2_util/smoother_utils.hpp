 ...
 */
inline std::vector<PathSegment> findDirectionalPathSegments(const nav_msgs::msg::Path& path, bool is_holonomic = false) {
 ...
 * @param is_holonomic Whether the motion model is holonomic (default is false)
 Only set as true when the input path is known to be generated from a holonomic planner like NavFn.
 */
inline void updateApproximatePathOrientations(nav_msgs::msg::Path& path, bool& reversing_segment, bool is_holonomic = false) {
