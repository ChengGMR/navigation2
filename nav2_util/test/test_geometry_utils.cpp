...
TEST(GeometryUtils, calculate_path_length) {
    ...
    ASSERT_NEAR(calculate_path_length(straight_line_path), (nb_path_points - 1) * distance_between_poses, 1e-5);

    ASSERT_NEAR(calculate_path_length(straight_line_path, straight_line_path.poses.size()), 0.0, 1e-5);
...
TEST(GeometryUtils, find_next_matching_goal_in_waypoint_statuses) {
    ...
    ASSERT_EQ(find_next_matching_goal_in_waypoint_statuses(waypoint_statuses, waypoint_statuses[matching_index].waypoint_pose), matching_index);
    ...
    ASSERT_EQ(find_next_matching_goal_in_waypoint_statuses(waypoint_statuses, waypoint_statuses[matching_index].waypoint_pose), -1);
}
