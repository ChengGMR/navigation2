...
TEST(TestBaseFootprintPublisher, TestBaseFootprintPublisher) {
    ...
    auto timer_interface = std::make_shared<tf2_ros::CreateTimerROS>(node->get_node_base_interface(), node->get_node_timers_interface());
    ...
    executor.spin_some();
    EXPECT_THROW(buffer->lookupTransform(base_link, base_footprint, tf2::TimePointZero), tf2::TransformException);
