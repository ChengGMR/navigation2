...
class TestParameterHandler : public nav2_util::ParameterHandler<DummyParams> {
   public:
    TestParameterHandler(const nav2::LifecycleNode::SharedPtr& node, rclcpp::Logger& logger) : ParameterHandler<DummyParams>(node, logger) {}
    ...
    void updateParametersCallback(const std::vector<rclcpp::Parameter>& /*parameters*/) override { updated = true; }
};
