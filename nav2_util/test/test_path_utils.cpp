...
void generateCirclePath(nav_msgs::msg::Path& path, double center_x, double center_y, double radius, int num_points, double start_angle = 0.0,
    ...
    for (int i = 0; i < num_points; ++i) {
        ...
        path.poses.push_back(createPoseStamped(center_x + radius * std::cos(angle), center_y + radius * std::sin(angle)));
    }
}

void generateCircleTrajectory(std::vector<geometry_msgs::msg::Pose>& path, double center_x, double center_y, double radius, int num_points,
                              double start_angle = 0.0, double end_angle = 2.0 * M_PI) {
    const double angle_step = (end_angle - start_angle) / (num_points - 1);
    for (int i = 0; i < num_points; ++i) {
        const double angle = start_angle + i * angle_step;
        path.push_back(createPose(center_x + radius * std::cos(angle), center_y + radius * std::sin(angle)));
    }
}
...
class ZigZagPathTest : public ::testing::Test {
   protected:
    void SetUp() override {
        ...
        robot_trajectory = {createPose(1.0, 0.2),  createPose(5.0, 0.2), createPose(9.0, 0.2), createPose(9.8, 1.0),
                            createPose(10.2, 2.5), createPose(9.8, 4.0), createPose(8.0, 5.2), createPose(5.0, 5.2),
                            createPose(2.0, 5.2),  createPose(0.2, 6.0), createPose(0.2, 8.0), createPose(0.2, 9.8)};
    }
    nav_msgs::msg::Path target_path;
    std::vector<geometry_msgs::msg::Pose> robot_trajectory;
};
...
class HairpinTurnTest : public ::testing::Test {
   protected:
    void SetUp() override {
        ...
        robot_trajectory = {createPose(1.0, 1.2),  createPose(3.0, 1.2),  createPose(5.0, 1.2),  createPose(7.0, 1.2),  createPose(8.5, 1.0),
                            createPose(9.2, 0.5),  createPose(9.8, 0.0),  createPose(9.8, -0.5), createPose(9.2, -1.0), createPose(8.0, -1.2),
                            createPose(6.0, -1.2), createPose(4.0, -1.2), createPose(2.0, -1.2)};
    }
    nav_msgs::msg::Path target_path;
    std::vector<geometry_msgs::msg::Pose> robot_trajectory;
};
...
class CuttingCornerTest : public ::testing::Test {
   protected:
    void SetUp() override {
        ...
        robot_trajectory = {createPose(0.0, 0.2), createPose(2.0, 0.2), createPose(4.0, 0.2), createPose(6.0, 0.2),
                            createPose(8.0, 0.2), createPose(9.0, 1.0), createPose(9.8, 2.0), createPose(9.8, 4.0),
                            createPose(9.8, 6.0), createPose(9.8, 8.0), createPose(9.8, 10.0)};
...
TEST_F(CuttingCornerWindowedTest, WindowedSearch) {
    ...
    for (size_t i = 0; i < robot_trajectory.size(); ++i) {
        const auto& robot_pose = robot_trajectory[i];
        auto result = nav2_util::distance_from_path(target_path, robot_pose, start_index, search_window);
...
TEST(PathUtilsTest, FourArgThrowsOnStartIndexOutOfBounds) {
    ...
    EXPECT_THROW(nav2_util::distance_from_path(path, robot_pose, 3, 5.0), // 3 >= 3
                 std::runtime_error);

    // start_index is way out of bounds
    EXPECT_THROW(nav2_util::distance_from_path(path, robot_pose, 100, 5.0), // 100 >= 3
                 std::runtime_error);
}
