...
class BaseFootprintPublisherListener : public tf2_ros::TransformListener {
   public:
    BaseFootprintPublisherListener(tf2::BufferCore& buffer, bool spin_thread, rclcpp::Node& node) : tf2_ros::TransformListener(buffer, spin_thread) {
        node.declare_parameter("base_link_frame", rclcpp::ParameterValue(std::string("base_link")));
        node.declare_parameter("base_footprint_frame", rclcpp::ParameterValue(std::string("base_footprint")));
...
class BaseFootprintPublisher : public rclcpp::Node {
   public:
     ...
     * @brief A constructor
     */
    explicit BaseFootprintPublisher(const rclcpp::NodeOptions& options = rclcpp::NodeOptions()) : Node("base_footprint_publisher", options) {
        ...
        listener_publisher_ = std::make_shared<BaseFootprintPublisherListener>(*tf_buffer_, true, *this);
    }
