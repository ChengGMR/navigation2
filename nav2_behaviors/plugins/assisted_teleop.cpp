...
AssistedTeleop::AssistedTeleop() : TimedBehavior<AssistedTeleopAction>(), feedback_(std::make_shared<AssistedTeleopAction::Feedback>()) {}
...
void AssistedTeleop::onConfigure() {
    ...
    std::string cmd_vel_teleop = node->declare_or_get_parameter("cmd_vel_teleop", std::string("cmd_vel_teleop"));

    vel_sub_ = std::make_unique<nav2_util::TwistSubscriber>(
        node, cmd_vel_teleop, [&](geometry_msgs::msg::Twist::SharedPtr msg) { teleop_twist_.twist = *msg; },
        [&](geometry_msgs::msg::TwistStamped::SharedPtr msg) { teleop_twist_ = *msg; });

    preempt_teleop_sub_ = node->create_subscription<std_msgs::msg::Empty>(
        "preempt_teleop", std::bind(&AssistedTeleop::preemptTeleopCallback, this, std::placeholders::_1));
}
...
ResultStatus AssistedTeleop::onCycleUpdate() {
    ...
    if (time_remaining.seconds() < 0.0 && command_time_allowance_.seconds() > 0.0) {
        stopRobot();
        std::string error_msg = "Exceeded time allowance before reaching the " + behavior_name_ + "goal - Exiting " + behavior_name_;
    ...
    for (double time = simulation_time_step_; time < projection_time_; time += simulation_time_step_) {
        ...
        if (!local_collision_checker_->isCollisionFree(projected_pose)) {
            if (time == simulation_time_step_) {
                RCLCPP_DEBUG_STREAM_THROTTLE(logger_, *clock_, 1000,
                ...
                RCLCPP_DEBUG_STREAM_THROTTLE(logger_, *clock_, 1000, behavior_name_.c_str() << " collision approaching in " << time << " seconds");
...
geometry_msgs::msg::Pose AssistedTeleop::projectPose(const geometry_msgs::msg::Pose& pose, const geometry_msgs::msg::Twist& twist,
                                                     double projection_time) {
    geometry_msgs::msg::Pose projected_pose = pose;

    double theta = tf2::getYaw(pose.orientation);

    projected_pose.position.x += projection_time * (twist.linear.x * cos(theta) + twist.linear.y * sin(theta));

    projected_pose.position.y += projection_time * (twist.linear.x * sin(theta) - twist.linear.y * cos(theta));
