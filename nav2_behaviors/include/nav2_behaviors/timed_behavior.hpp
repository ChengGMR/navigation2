...
class TimedBehavior : public nav2_core::Behavior {
   public:
    ...
    void configure(const nav2::LifecycleNode::WeakPtr& parent, const std::string& name, std::shared_ptr<tf2_ros::Buffer> tf,
                   std::shared_ptr<nav2_costmap_2d::CostmapTopicCollisionChecker> local_collision_checker,
                   std::shared_ptr<nav2_costmap_2d::CostmapTopicCollisionChecker> global_collision_checker) override {
        node_ = parent;
        ...
        action_server_ = node->create_action_server<ActionT>(behavior_name_, std::bind(&TimedBehavior::execute, this), nullptr,
   ...
   protected:
    ...
    void execute() {
        ...
        if (on_run_result.status != Status::SUCCEEDED) {
            ...
            RCLCPP_INFO(logger_, "Initial checks failed for %s - %s", behavior_name_.c_str(), on_run_result.error_msg.c_str());
