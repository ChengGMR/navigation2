...
class DriveOnHeading : public TimedBehavior<ActionT> {
   ...
   public:
    ...
    ResultStatus onRun(const std::shared_ptr<const typename ActionT::Goal> command) override {
        ...
        if (!nav2_util::getCurrentPose(initial_pose_, *this->tf_, this->local_frame_, this->robot_base_frame_, this->transform_tolerance_)) {
    ...
    ResultStatus onCycleUpdate() override {
        ...
        if (time_remaining.seconds() < 0.0 && command_time_allowance_.seconds() > 0.0) {
            this->stopRobot();
            std::string error_msg = "Exceeded time allowance before reaching the DriveOnHeading goal - Exiting DriveOnHeading";
            RCLCPP_WARN(this->logger_, error_msg.c_str());
            return ResultStatus{Status::FAILED, ActionT::Result::TIMEOUT, error_msg};
        }

        geometry_msgs::msg::PoseStamped current_pose;
        if (!nav2_util::getCurrentPose(current_pose, *this->tf_, this->local_frame_, this->robot_base_frame_, this->transform_tolerance_)) {
    ...
    void onActionCompletion(std::shared_ptr<typename ActionT::Result> /*result*/) override { last_vel_ = std::numeric_limits<double>::max(); }
   ...
   protected:
    ...
    void onConfigure() override {
        ...
        acceleration_limit_ = node->declare_or_get_parameter(this->behavior_name_ + ".acceleration_limit", 2.5);
        deceleration_limit_ = node->declare_or_get_parameter(this->behavior_name_ + ".deceleration_limit", -2.5);
        minimum_speed_ = node->declare_or_get_parameter(this->behavior_name_ + ".minimum_speed", 0.10);
