...
class BehaviorTest : public ::testing::Test {
   protected:
    ...
    void SetUp() override {
        node_lifecycle_ = std::make_shared<nav2::LifecycleNode>("LifecycleBehaviorTestNode", rclcpp::NodeOptions());

        std::string local_costmap_topic = node_lifecycle_->declare_or_get_parameter("local_costmap_topic", std::string("local_costmap/costmap_raw"));
        std::string local_footprint_topic =
            node_lifecycle_->declare_or_get_parameter("local_footprint_topic", std::string("local_costmap/published_footprint"));
        std::string global_costmap_topic =
            node_lifecycle_->declare_or_get_parameter("global_costmap_topic", std::string("global_costmap/costmap_raw"));
        std::string global_footprint_topic =
            node_lifecycle_->declare_or_get_parameter("global_footprint_topic", std::string("global_costmap/published_footprint"));
        ...
        std::shared_ptr<nav2_costmap_2d::CostmapSubscriber> local_costmap_sub_ =
            std::make_shared<nav2_costmap_2d::CostmapSubscriber>(node_lifecycle_, global_costmap_topic);
        ...
        std::shared_ptr<nav2_costmap_2d::FootprintSubscriber> local_footprint_sub_ =
            std::make_shared<nav2_costmap_2d::FootprintSubscriber>(node_lifecycle_, local_footprint_topic, *tf_buffer_);

        std::shared_ptr<nav2_costmap_2d::FootprintSubscriber> global_footprint_sub_ =
            std::make_shared<nav2_costmap_2d::FootprintSubscriber>(node_lifecycle_, global_footprint_topic, *tf_buffer_);

        std::shared_ptr<nav2_costmap_2d::CostmapTopicCollisionChecker> local_collision_checker_ =
            std::make_shared<nav2_costmap_2d::CostmapTopicCollisionChecker>(*local_costmap_sub_, *local_footprint_sub_, node_lifecycle_->get_name());

        std::shared_ptr<nav2_costmap_2d::CostmapTopicCollisionChecker> global_collision_checker_ =
            std::make_shared<nav2_costmap_2d::CostmapTopicCollisionChecker>(*global_costmap_sub_, *global_footprint_sub_,
                                                                            node_lifecycle_->get_name());

        behavior_ = std::make_shared<DummyBehavior>();
        behavior_->configure(node_lifecycle_, "Behavior", tf_buffer_, local_collision_checker_, global_collision_checker_);
        behavior_->activate();

        client_ = rclcpp_action::create_client<BehaviorAction>(
            node_lifecycle_->get_node_base_interface(), node_lifecycle_->get_node_graph_interface(), node_lifecycle_->get_node_logging_interface(),
    ...
    bool sendCommand(const std::string& command) {
        ...
        if (rclcpp::spin_until_future_complete(node_lifecycle_, future_goal) != rclcpp::FutureReturnCode::SUCCESS) {
