...
nav2::CallbackReturn VectorObjectServer::on_configure(const rclcpp_lifecycle::State& /*state*/) {
    ...
    map_pub_ = create_publisher<nav_msgs::msg::OccupancyGrid>(map_topic_, nav2::qos::LatchedPublisherQoS());

    add_shapes_service_ =
        create_service<nav2_msgs::srv::AddShapes>("~/add_shapes", std::bind(&VectorObjectServer::addShapesCallback, this, _1, _2, _3));

    get_shapes_service_ =
        create_service<nav2_msgs::srv::GetShapes>("~/get_shapes", std::bind(&VectorObjectServer::getShapesCallback, this, _1, _2, _3));

    remove_shapes_service_ =
        create_service<nav2_msgs::srv::RemoveShapes>("~/remove_shapes", std::bind(&VectorObjectServer::removeShapesCallback, this, _1, _2, _3));

    return nav2::CallbackReturn::SUCCESS;
}

nav2::CallbackReturn VectorObjectServer::on_activate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn VectorObjectServer::on_deactivate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn VectorObjectServer::on_cleanup(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn VectorObjectServer::on_shutdown(const rclcpp_lifecycle::State& /*state*/) {
...
bool VectorObjectServer::obtainParams() {
    ...
    default_value_ = nav2::declare_or_get_parameter(node, "default_value", static_cast<int>(nav2_util::OCC_GRID_UNKNOWN));
    overlay_type_ = static_cast<OverlayType>(nav2::declare_or_get_parameter(node, "overlay_type", static_cast<int>(OverlayType::OVERLAY_SEQ)));
    ...
    for (std::string shape_name : shape_names) {
        ...
        try {
            ...
            RCLCPP_ERROR(get_logger(), "Error while getting shape %s type: %s", shape_name.c_str(), ex.what());
            return false;
        }
        ...
        if (shape_type == "polygon") {
            ...
            RCLCPP_ERROR(get_logger(), "Please specify the correct type for shape %s. Supported types are 'polygon' and 'circle'",
                         shape_name.c_str());
            return false;
        }
    }
...
std::vector<std::shared_ptr<Shape>>::iterator VectorObjectServer::findShape(const unsigned char* uuid) {
    ...
    for (auto shape : shapes_) {
        if (shape->getFrameID() != global_frame_id_ && !shape->getFrameID().empty()) {
            ...
            if (!shape->toFrame(global_frame_id_, tf_buffer_, transform_tolerance_)) {
                RCLCPP_ERROR(get_logger(), "Can not transform vector object from %s to %s frame", shape->getFrameID().c_str(),
...
void VectorObjectServer::getMapBoundaries(double& min_x, double& min_y, double& max_x, double& max_y) const {
    ...
    if (min_x == std::numeric_limits<double>::max() || min_y == std::numeric_limits<double>::max() || max_x == std::numeric_limits<double>::lowest()
        || max_y == std::numeric_limits<double>::lowest()) {
        throw std::runtime_error("Can not obtain map boundaries");
    }
}

void VectorObjectServer::updateMap(const double& min_x, const double& min_y, const double& max_x, const double& max_y) {
    ...
    if (map_->info.width != static_cast<unsigned int>(size_x) || map_->info.height != static_cast<unsigned int>(size_y)) {
...
void VectorObjectServer::putVectorObjectsOnMap() {
    ...
    for (auto shape : shapes_) {
        if (shape->isFill()) {
            ...
            if (!nav2_util::worldToMap(map_, wx1, wy1, mx1, my1) || !nav2_util::worldToMap(map_, wx2, wy2, mx2, my2)) {
                RCLCPP_ERROR(get_logger(), "Error to get shape boundaries on map (UUID: %s)", shape->getUUID().c_str());
                return;
            }
...
void VectorObjectServer::switchMapUpdate() {
    for (auto shape : shapes_) {
        if (shape->getFrameID() != global_frame_id_ && !shape->getFrameID().empty()) {
            if (!map_timer_) {
                map_timer_ =
                    this->create_timer(std::chrono::duration<double>(1.0 / update_frequency_), std::bind(&VectorObjectServer::processMap, this));
            }
...
void VectorObjectServer::addShapesCallback(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
    ...
    for (auto req_poly : request->polygons) {
        nav2_msgs::msg::PolygonObject::SharedPtr new_params = std::make_shared<nav2_msgs::msg::PolygonObject>(req_poly);
        ...
        if (it != shapes_.end()) {
            ...
            if ((*it)->getType() != POLYGON) {
                RCLCPP_ERROR(get_logger(), "Shape (UUID: %s) is not a polygon type for a polygon update. Not adding shape.",
                             (*it)->getUUID().c_str());
            ...
            if (polygon->setParams(new_params)) {
                ...
                RCLCPP_ERROR(get_logger(), "Failed to create a new polygon object using the provided params.");
                response->success = false;
            }
        }
    }
    ...
    for (auto req_crcl : request->circles) {
        nav2_msgs::msg::CircleObject::SharedPtr new_params = std::make_shared<nav2_msgs::msg::CircleObject>(req_crcl);
        ...
        if (it != shapes_.end()) {
            ...
            if ((*it)->getType() != CIRCLE) {
                RCLCPP_ERROR(get_logger(), "Shape (UUID: %s) is not a circle type for a circle update. Not adding shape.", (*it)->getUUID().c_str());
                response->success = false;
                // Do not add this shape
                continue;
            }
            ...
            if (circle->setParams(new_params)) {
                ...
                RCLCPP_ERROR(get_logger(), "Failed to create a new circle object using the provided params.");
                response->success = false;
            }
        }
    }
...
void VectorObjectServer::getShapesCallback(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
...
void VectorObjectServer::removeShapesCallback(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
    ...
    if (request->all_objects) {
        ...
        for (auto req_uuid : request->uuids) {
            ...
            if (it != shapes_.end()) {
                ...
                RCLCPP_ERROR(get_logger(), "Can not find shape to remove with UUID: %s", unparseUUID(req_uuid.uuid.data()).c_str());
                response->success = false;
            }
        }
    }
