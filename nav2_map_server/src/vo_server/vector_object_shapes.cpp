...
Shape::Shape(const nav2::LifecycleNode::WeakPtr& node) : type_(UNKNOWN), node_(node) {}
...
bool Shape::obtainShapeUUID(const std::string& shape_name, unsigned char* out_uuid) {
    ...
    try {
        ...
        std::string uuid_str = nav2::declare_or_get_parameter<std::string>(node, shape_name + ".uuid");
        if (uuid_parse(uuid_str.c_str(), out_uuid) != 0) {
            RCLCPP_ERROR(node->get_logger(), "[%s] Can not parse UUID string for shape: %s", shape_name.c_str(), uuid_str.c_str());
            return false;
        }
        ...
        RCLCPP_INFO(node->get_logger(), "[%s] No UUID is specified for shape. Generating a new one: %s", shape_name.c_str(), uuid_str);
    }
...
Polygon::Polygon(const nav2::LifecycleNode::WeakPtr& node) : Shape::Shape(node) {
    type_ = POLYGON;
}
...
bool Polygon::obtainParams(const std::string& shape_name) {
    ...
    params_->header.frame_id = nav2::declare_or_get_parameter(node, shape_name + ".frame_id", std::string{"map"});
    params_->value = nav2::declare_or_get_parameter(node, shape_name + ".value", static_cast<int>(nav2_util::OCC_GRID_OCCUPIED));
    params_->closed = nav2::declare_or_get_parameter(node, shape_name + ".closed", true);

    std::vector<double> poly_row;
    try {
        poly_row = nav2::declare_or_get_parameter<std::vector<double>>(node, shape_name + ".points");
    } catch (const std::exception& ex) {
        RCLCPP_ERROR(node->get_logger(), "[%s] Error while getting polygon parameters: %s", shape_name.c_str(), ex.what());
        return false;
    }
    // Check for points format correctness
    if (poly_row.size() < 6 || poly_row.size() % 2 != 0) {
        RCLCPP_ERROR(node->get_logger(), "[%s] Polygon has incorrect points description", shape_name.c_str());
        return false;
    }
...
bool Polygon::toFrame(const std::string& to_frame, const std::shared_ptr<tf2_ros::Buffer> tf_buffer, const double transform_tolerance) {
    ...
    for (unsigned int i = 0; i < params_->points.size(); i++) {
        ...
        if (nav2_util::transformPoseInTargetFrame(from_pose, to_pose, *tf_buffer, to_frame, transform_tolerance)) {
...
void Polygon::putBorders(nav_msgs::msg::OccupancyGrid::SharedPtr map, const OverlayType overlay_type) {
    ...
    if (!nav2_util::worldToMap(map, polygon_->points[0].x, polygon_->points[0].y, mx1, my1)) {
        RCLCPP_ERROR(node->get_logger(), "[UUID: %s] Can not convert (%f, %f) point to map", getUUID().c_str(), polygon_->points[0].x,
                     polygon_->points[0].y);
        return;
    }
    ...
    for (unsigned int i = 1; i < polygon_->points.size(); i++) {
        ...
        if (!nav2_util::worldToMap(map, polygon_->points[i].x, polygon_->points[i].y, mx1, my1)) {
            RCLCPP_ERROR(node->get_logger(), "[UUID: %s] Can not convert (%f, %f) point to map", getUUID().c_str(), polygon_->points[i].x,
                         polygon_->points[i].y);
            return;
        }
...
bool Polygon::checkConsistency() {
    if (params_->points.size() < 3) {
        ...
        RCLCPP_ERROR(node->get_logger(), "[UUID: %s] Polygon has incorrect number of vertices: %li", getUUID().c_str(), params_->points.size());
        return false;
    }
...
Circle::Circle(const nav2::LifecycleNode::WeakPtr& node) : Shape::Shape(node) {
    type_ = CIRCLE;
}
...
bool Circle::obtainParams(const std::string& shape_name) {
    ...
    params_->header.frame_id = nav2::declare_or_get_parameter(node, shape_name + ".frame_id", std::string{"map"});
    params_->value = nav2::declare_or_get_parameter(node, shape_name + ".value", static_cast<int>(nav2_util::OCC_GRID_OCCUPIED));
    params_->fill = nav2::declare_or_get_parameter(node, shape_name + ".fill", true);

    std::vector<double> center_row;
    try {
        center_row = nav2::declare_or_get_parameter<std::vector<double>>(node, shape_name + ".center");
        params_->radius = nav2::declare_or_get_parameter<double>(node, shape_name + ".radius");
        if (params_->radius < 0) {
            RCLCPP_ERROR(node->get_logger(), "[%s] Circle has incorrect radius less than zero", shape_name.c_str());
            return false;
        }
    } catch (const std::exception& ex) {
        RCLCPP_ERROR(node->get_logger(), "[%s] Error while getting circle parameters: %s", shape_name.c_str(), ex.what());
        return false;
    }
    // Check for points format correctness
    if (center_row.size() != 2) {
        RCLCPP_ERROR(node->get_logger(), "[%s] Circle has incorrect center description", shape_name.c_str());
        return false;
    }
...
bool Circle::toFrame(const std::string& to_frame, const std::shared_ptr<tf2_ros::Buffer> tf_buffer, const double transform_tolerance) {
    geometry_msgs::msg::PoseStamped from_pose, to_pose;
    from_pose.header = params_->header;
...
bool Circle::isPointInside(const double px, const double py) const {
    return ((px - center_->x) * (px - center_->x) + (py - center_->y) * (py - center_->y)) <= params_->radius * params_->radius;
}

void Circle::putBorders(nav_msgs::msg::OccupancyGrid::SharedPtr map, const OverlayType overlay_type) {
...
bool Circle::checkConsistency() {
    if (params_->radius < 0.0) {
        ...
        RCLCPP_ERROR(node->get_logger(), "[UUID: %s] Circle has incorrect radius less than zero", getUUID().c_str());
        return false;
    }
    return true;
}

bool Circle::centerToMap(nav_msgs::msg::OccupancyGrid::ConstSharedPtr map, unsigned int& mcx, unsigned int& mcy) {
    auto node = node_.lock();
    if (!node) {
        throw std::runtime_error{"Failed to lock node"};
    }
    ...
    if (center_->x < map->info.origin.position.x || center_->y < map->info.origin.position.y) {
        RCLCPP_ERROR(node->get_logger(), "[UUID: %s] Can not convert (%f, %f) circle center to map", getUUID().c_str(), center_->x, center_->y);
        return false;
    }
    ...
    mcx = static_cast<unsigned int>(std::round((center_->x - map->info.origin.position.x) / map->info.resolution)) - 1;
    mcy = static_cast<unsigned int>(std::round((center_->y - map->info.origin.position.y) / map->info.resolution)) - 1;
    if (mcx >= map->info.width || mcy >= map->info.height) {
        RCLCPP_ERROR(node->get_logger(), "[UUID: %s] Can not convert (%f, %f) point to map", getUUID().c_str(), center_->x, center_->y);
        return false;
    }
...
inline void Circle::putPoint(unsigned int mx, unsigned int my, nav_msgs::msg::OccupancyGrid::SharedPtr map, const OverlayType overlay_type) {
