...
TEST(MapSaverCLI, CLITest) {
    ...
    auto publisher = node->create_publisher<nav_msgs::msg::OccupancyGrid>("/map", nav2::qos::LatchedPublisherQoS());
    ...
    command = std::string(
              ...
              + file_path + std::string(" --ros-args --remap __node:=map_saver_test_node");
    ...
    command = std::string("ros2 run nav2_map_server map_saver_cli --ros-args --remap __node:=map_saver_test_node");
    return_code = system(command.c_str());
    EXPECT_EQ(return_code, 0);
