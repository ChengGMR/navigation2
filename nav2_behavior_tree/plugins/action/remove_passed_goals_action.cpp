...
inline BT::NodeStatus RemovePassedGoals::tick() {
    ...
    if (!nav2_util::getCurrentPose(current_pose, *tf_, goal_poses.goals[0].header.frame_id, robot_base_frame_, transform_tolerance_)) {
    ...
    while (goal_poses.goals.size() > 1) {
        ...
        if (waypoint_statuses_get_res) {
            auto cur_waypoint_index = find_next_matching_goal_in_waypoint_statuses(waypoint_statuses, goal_poses.goals[0]);
            if (cur_waypoint_index == -1) {
                RCLCPP_ERROR_ONCE(node_->get_logger(), "Failed to find matching goal in waypoint_statuses");
                return BT::NodeStatus::FAILURE;
            }
            waypoint_statuses[cur_waypoint_index].waypoint_status = nav2_msgs::msg::WaypointStatus::COMPLETED;
        }
