...
PlannerSelector::PlannerSelector(const std::string& name, const BT::NodeConfiguration& conf) : BT::SyncActionNode(name, conf) {
    initialize();
...
void PlannerSelector::createROSInterfaces() {
    ...
    if (topic_new != topic_name_ || !planner_selector_sub_) {
        ...
        planner_selector_sub_ = node_->create_subscription<std_msgs::msg::String>(
            topic_name_, std::bind(&PlannerSelector::callbackPlannerSelect, this, _1), nav2::qos::LatchedSubscriptionQoS(), callback_group_);
    }
}
