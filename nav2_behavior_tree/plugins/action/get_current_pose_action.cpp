...
GetCurrentPoseAction::GetCurrentPoseAction(const std::string& name, const BT::NodeConfiguration& conf) : BT::ActionNodeBase(name, conf) {
...
inline BT::NodeStatus GetCurrentPoseAction::tick() {
    ...
    if (!nav2_util::getCurrentPose(current_pose, *tf_, global_frame_, robot_base_frame_, transform_tolerance_)) {
        RCLCPP_WARN(config().blackboard->get<nav2::LifecycleNode::SharedPtr>("node")->get_logger(), "Current robot pose is not available.");
