...
SmootherSelector::SmootherSelector(const std::string& name, const BT::NodeConfiguration& conf) : BT::SyncActionNode(name, conf) {
    initialize();
...
void SmootherSelector::createROSInterfaces() {
    ...
    if (topic_new != topic_name_ || !smoother_selector_sub_) {
        ...
        smoother_selector_sub_ = node_->create_subscription<std_msgs::msg::String>(
            topic_name_, std::bind(&SmootherSelector::callbackSmootherSelect, this, _1), nav2::qos::LatchedSubscriptionQoS(), callback_group_);
    }
}
