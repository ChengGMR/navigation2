...
GoalCheckerSelector::GoalCheckerSelector(const std::string& name, const BT::NodeConfiguration& conf) : BT::SyncActionNode(name, conf) {
    initialize();
...
void GoalCheckerSelector::createROSInterfaces() {
    ...
    if (topic_new != topic_name_ || !goal_checker_selector_sub_) {
        ...
        goal_checker_selector_sub_ = node_->create_subscription<std_msgs::msg::String>(
            topic_name_, std::bind(&GoalCheckerSelector::callbackGoalCheckerSelect, this, _1), nav2::qos::LatchedSubscriptionQoS(), callback_group_);
    }
}
