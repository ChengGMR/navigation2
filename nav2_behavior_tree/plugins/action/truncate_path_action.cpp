...
TruncatePath::TruncatePath(const std::string& name, const BT::NodeConfiguration& conf) : BT::ActionNodeBase(name, conf), distance_(1.0) {}
...
inline BT::NodeStatus TruncatePath::tick() {
    ...
    if (std::isnan(final_angle) || std::isinf(final_angle)) {
        RCLCPP_WARN(config().blackboard->get<nav2::LifecycleNode::SharedPtr>("node")->get_logger(),
