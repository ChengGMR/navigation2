...
ControllerSelector::ControllerSelector(const std::string& name, const BT::NodeConfiguration& conf) : BT::SyncActionNode(name, conf) {
    initialize();
...
void ControllerSelector::createROSInterfaces() {
    ...
    if (topic_new != topic_name_ || !controller_selector_sub_) {
        ...
        controller_selector_sub_ = node_->create_subscription<std_msgs::msg::String>(
            topic_name_, std::bind(&ControllerSelector::callbackControllerSelect, this, _1), nav2::qos::LatchedSubscriptionQoS(), callback_group_);
    }
}
