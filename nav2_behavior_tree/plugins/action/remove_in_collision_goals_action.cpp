...
RemoveInCollisionGoals::RemoveInCollisionGoals(const std::string& service_node_name, const BT::NodeConfiguration& conf)
    : BtServiceNode<nav2_msgs::srv::GetCosts>(service_node_name, conf, "/global_costmap/get_cost_global_costmap") {}
...
BT::NodeStatus RemoveInCollisionGoals::on_completion(std::shared_ptr<nav2_msgs::srv::GetCosts::Response> response) {
    if (!response->success) {
    ...
    for (size_t i = 0; i < response->costs.size(); ++i) {
        if ((response->costs[i] == 255 && !consider_unknown_as_obstacle_) || response->costs[i] < cost_threshold_) {
            ...
            auto cur_waypoint_index = find_next_matching_goal_in_waypoint_statuses(waypoint_statuses, input_goals_.goals[i]);
            if (cur_waypoint_index == -1) {
                RCLCPP_ERROR(node_->get_logger(), "Failed to find matching goal in waypoint_statuses");
                return BT::NodeStatus::FAILURE;
            }
            waypoint_statuses[cur_waypoint_index].waypoint_status = nav2_msgs::msg::WaypointStatus::SKIPPED;
        }
    }
    // Inform if all goals have been removed
    if (valid_goal_poses.goals.empty()) {
        RCLCPP_INFO(node_->get_logger(), "All goals are in collision and have been removed from the list");
    }
