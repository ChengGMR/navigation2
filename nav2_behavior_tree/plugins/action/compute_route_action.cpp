...
ComputeRouteAction::ComputeRouteAction(const std::string& xml_tag_name, const std::string& action_name, const BT::NodeConfiguration& conf)
...
BT_REGISTER_NODES(factory) {
    BT::NodeBuilder builder = [](const std::string& name, const BT::NodeConfiguration& config) {
        return std::make_unique<nav2_behavior_tree::ComputeRouteAction>(name, "compute_route", config);
    };

    factory.registerBuilder<nav2_behavior_tree::ComputeRouteAction>("ComputeRoute", builder);
}
