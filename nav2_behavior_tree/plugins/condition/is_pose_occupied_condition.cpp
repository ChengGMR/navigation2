...
IsPoseOccupiedCondition::IsPoseOccupiedCondition(const std::string& condition_name, const BT::NodeConfiguration& conf)
    : BT::ConditionNode(condition_name, conf), use_footprint_(true), consider_unknown_as_obstacle_(false), cost_threshold_(254) {
    initialize();
}
...
void IsPoseOccupiedCondition::createROSInterfaces() {
    ...
    if (service_new != service_name_ || !client_) {
        ...
        client_ = node_->create_client<nav2_msgs::srv::GetCosts>(service_name_, false /* Does not create and spin an internal executor*/);
    }
}
...
BT::NodeStatus IsPoseOccupiedCondition::tick() {
    ...
    if ((response->costs[0] == 255 && !consider_unknown_as_obstacle_) || response->costs[0] < cost_threshold_) {
        return BT::NodeStatus::FAILURE;
    } else {
