...
IsBatteryChargingCondition::IsBatteryChargingCondition(const std::string& condition_name, const BT::NodeConfiguration& conf)
...
void IsBatteryChargingCondition::createROSInterfaces() {
    ...
    if (battery_topic_new != battery_topic_ || !battery_sub_) {
        ...
        battery_sub_ = node->create_subscription<sensor_msgs::msg::BatteryState>(
            battery_topic_, std::bind(&IsBatteryChargingCondition::batteryCallback, this, std::placeholders::_1), nav2::qos::StandardTopicQoS(),
            callback_group_);
    }
}
