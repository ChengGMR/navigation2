...
PauseResumeController::PauseResumeController(const std::string& xml_tag_name, const BT::NodeConfiguration& conf)
    ...
    cb_group_ = node->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive, false);
    ...
    pause_srv_ =
        node->create_service<Trigger>(pause_service_name, std::bind(&PauseResumeController::pauseServiceCallback, this, _1, _2, _3), cb_group_);

    std::string resume_service_name;
    getInput("resume_service_name", resume_service_name);
    resume_srv_ =
        node->create_service<Trigger>(resume_service_name, std::bind(&PauseResumeController::resumeServiceCallback, this, _1, _2, _3), cb_group_);
}
...
BT::NodeStatus PauseResumeController::tick() {
    ...
    switch (child_status) {
        ...
        case BT::NodeStatus::FAILURE:
            RCLCPP_ERROR(logger_, "%s child returned FAILURE", state_names.at(state_).c_str());
...
void PauseResumeController::pauseServiceCallback(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
...
void PauseResumeController::resumeServiceCallback(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
                                                  const std::shared_ptr<Trigger::Request> /*request*/, std::shared_ptr<Trigger::Response> response) {
    if (state_ == PAUSED) {
...
BT_REGISTER_NODES(factory) {
    BT::NodeBuilder builder = [](const std::string& name, const BT::NodeConfiguration& config) {
        return std::make_unique<nav2_behavior_tree::PauseResumeController>(name, config);
    };
