...
class TestNode : public BT::SyncActionNode {
   public:
    TestNode(const std::string& name, const BT::NodeConfiguration& config) : SyncActionNode(name, config) {}
...
TEST(PoseStampedVectorPortTest, test_wrong_syntax) {
    ...
    BT::BehaviorTreeFactory factory;
    factory.registerNodeType<TestNode<std::vector<geometry_msgs::msg::PoseStamped>>>("PoseStampedVectorPortTest");
    EXPECT_THROW({ auto unused = factory.createTreeFromText(xml_txt); }, std::exception);
...
TEST(GoalsArrayPortTest, test_wrong_syntax) {
    ...
    BT::BehaviorTreeFactory factory;
    factory.registerNodeType<TestNode<nav_msgs::msg::Goals>>("GoalsArrayPortTest");
    EXPECT_THROW({ auto unused = factory.createTreeFromText(xml_txt); }, std::exception);
...
TEST(PathPortTest, test_wrong_syntax) {
    ...
    BT::BehaviorTreeFactory factory;
    factory.registerNodeType<TestNode<nav_msgs::msg::Path>>("PathPortTest");
    EXPECT_THROW({ auto unused = factory.createTreeFromText(xml_txt); }, std::exception);
...
TEST(WaypointStatusVectorPortTest, test_wrong_syntax) {
    ...
    BT::BehaviorTreeFactory factory;
    factory.registerNodeType<TestNode<std::vector<nav2_msgs::msg::WaypointStatus>>>("WaypointStatusVectorPort");
    EXPECT_THROW({ auto unused = factory.createTreeFromText(xml_txt); }, std::exception);
...
TEST(deconflictPortAndParamFrameTest, test_correct_syntax) {
    ...
    int value = BT::deconflictPortAndParamFrame<int>(node, "test_alternative", tree.rootNode());

    EXPECT_EQ(value, 3);

    value = BT::deconflictPortAndParamFrame<int>(node, "test", tree.rootNode());
