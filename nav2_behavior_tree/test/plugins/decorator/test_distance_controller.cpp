...
class DistanceControllerTestFixture : public nav2_behavior_tree::BehaviorTreeTestFixture {
   public:
    void SetUp() {
        ...
        bt_node_ = std::make_shared<nav2_behavior_tree::DistanceController>("distance_controller", *config_);
...
std::shared_ptr<nav2_behavior_tree::DistanceController> DistanceControllerTestFixture::bt_node_ = nullptr;
    ...
    for (int i = 1; i <= 20; i++) {
        ...
        while (traveled < i * 0.5) {
            if (nav2_util::getCurrentPose(pose, *transform_handler_->getBuffer())) {
                traveled = std::sqrt(pose.pose.position.x * pose.pose.position.x + pose.pose.position.y * pose.pose.position.y);
            }
        }
