...
class PauseResumeControllerTestFixture : public nav2_behavior_tree::BehaviorTreeTestFixture {
   public:
    ...
    static void CheckServiceResult(const nav2::ServiceClient<std_srvs::srv::Trigger>::SharedPtr& client,
                                   std::shared_future<std::shared_ptr<std_srvs::srv::Trigger::Response>> future, bool success = true) {
...
nav2::ServiceClient<std_srvs::srv::Trigger>::SharedPtr PauseResumeControllerTestFixture::pause_client_ = nullptr;
nav2::ServiceClient<std_srvs::srv::Trigger>::SharedPtr PauseResumeControllerTestFixture::resume_client_ = nullptr;
...
TEST_F(PauseResumeControllerTestFixture, test_incorrect_num_children) {
    ...
    EXPECT_THROW(auto tree = factory_->createTreeFromText(xml_txt, config_->blackboard), BT::RuntimeError);
}
...
TEST_F(PauseResumeControllerTestFixture, test_unused_children) {
    ...
    auto pause_bt_node = nav2_behavior_tree::get_node_from_tree<nav2_behavior_tree::PauseResumeController>(tree_);
    ASSERT_NE(pause_bt_node, nullptr);
    using state_t = nav2_behavior_tree::state_t;

    // get dummy nodes so we can change their status
    auto resumed_child = nav2_behavior_tree::get_node_from_tree<nav2_behavior_tree::DummyNode>(tree_, 0);
...
TEST_F(PauseResumeControllerTestFixture, test_behavior) {
    ...
    // get dummy nodes so we can change their status
    auto resumed_child = nav2_behavior_tree::get_node_from_tree<nav2_behavior_tree::DummyNode>(tree_, 0);
    auto paused_child = nav2_behavior_tree::get_node_from_tree<nav2_behavior_tree::DummyNode>(tree_, 1);
    auto on_pause_child = nav2_behavior_tree::get_node_from_tree<nav2_behavior_tree::DummyNode>(tree_, 2);
    auto on_resume_child = nav2_behavior_tree::get_node_from_tree<nav2_behavior_tree::DummyNode>(tree_, 3);
    ASSERT_NE(resumed_child, nullptr);
