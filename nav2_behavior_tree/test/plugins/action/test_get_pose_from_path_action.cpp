   ...
   public:
    static void SetUpTestCase() {
        ...
        BT::NodeBuilder builder = [](const std::string& name, const BT::NodeConfiguration& config) {
            return std::make_unique<nav2_behavior_tree::GetPoseFromPath>(name, config);
        };

        factory_->registerBuilder<nav2_behavior_tree::GetPoseFromPath>("GetPoseFromPath", builder);
    }
