...
class CancelAssistedTeleopServer : public TestActionServer<nav2_msgs::action::AssistedTeleop> {
   ...
   protected:
    void execute(const typename std::shared_ptr<rclcpp_action::ServerGoalHandle<nav2_msgs::action::AssistedTeleop>> goal_handle) {
...
class CancelAssistedTeleopActionTestFixture : public ::testing::Test {
   public:
    static void SetUpTestCase() {
        ...
        client_ = rclcpp_action::create_client<nav2_msgs::action::AssistedTeleop>(node_, "assisted_teleop");

        BT::NodeBuilder builder = [](const std::string& name, const BT::NodeConfiguration& config) {
            return std::make_unique<nav2_behavior_tree::AssistedTeleopCancel>(name, "assisted_teleop", config);
        };

        factory_->registerBuilder<nav2_behavior_tree::AssistedTeleopCancel>("CancelAssistedTeleop", builder);
    }
...
std::shared_ptr<CancelAssistedTeleopServer> CancelAssistedTeleopActionTestFixture::action_server_ = nullptr;
std::shared_ptr<nav2::ActionClient<nav2_msgs::action::AssistedTeleop>> CancelAssistedTeleopActionTestFixture::client_ = nullptr;
...
TEST_F(CancelAssistedTeleopActionTestFixture, test_ports) {
    ...
    auto send_goal_options = nav2::ActionClient<nav2_msgs::action::AssistedTeleop>::SendGoalOptions();
...
int main(int argc, char** argv) {
    ...
    CancelAssistedTeleopActionTestFixture::action_server_ = std::make_shared<CancelAssistedTeleopServer>();
    std::thread server_thread([]() { rclcpp::spin(CancelAssistedTeleopActionTestFixture::action_server_); });
