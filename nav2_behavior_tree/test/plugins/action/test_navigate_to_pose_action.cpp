   ...
   public:
    static void SetUpTestCase() {
        ...
        BT::NodeBuilder builder = [](const std::string& name, const BT::NodeConfiguration& config) {
            return std::make_unique<nav2_behavior_tree::NavigateToPoseAction>(name, "navigate_to_pose", config);
        };
...
std::shared_ptr<NavigateToPoseActionServer> NavigateToPoseActionTestFixture::action_server_ = nullptr;
...
int main(int argc, char** argv) {
    ...
    std::thread server_thread([]() { rclcpp::spin(NavigateToPoseActionTestFixture::action_server_); });
