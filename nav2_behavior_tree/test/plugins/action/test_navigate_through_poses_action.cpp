...
class NavigateThroughPosesActionServer : public TestActionServer<nav2_msgs::action::NavigateThroughPoses> {
   public:
    NavigateThroughPosesActionServer() : TestActionServer("navigate_through_poses") {}

   protected:
    void execute(const typename std::shared_ptr<rclcpp_action::ServerGoalHandle<nav2_msgs::action::NavigateThroughPoses>> goal_handle) override {
   ...
   public:
    static void SetUpTestCase() {
        ...
        BT::NodeBuilder builder = [](const std::string& name, const BT::NodeConfiguration& config) {
            return std::make_unique<nav2_behavior_tree::NavigateThroughPosesAction>(name, "navigate_through_poses", config);
        };

        factory_->registerBuilder<nav2_behavior_tree::NavigateThroughPosesAction>("NavigateThroughPoses", builder);
    }
...
std::shared_ptr<NavigateThroughPosesActionServer> NavigateThroughPosesActionTestFixture::action_server_ = nullptr;
...
int main(int argc, char** argv) {
    ...
    NavigateThroughPosesActionTestFixture::action_server_ = std::make_shared<NavigateThroughPosesActionServer>();

    std::thread server_thread([]() { rclcpp::spin(NavigateThroughPosesActionTestFixture::action_server_); });
