...
class AssistedTeleopActionServer : public TestActionServer<nav2_msgs::action::AssistedTeleop> {
   ...
   protected:
    void execute(const typename std::shared_ptr<rclcpp_action::ServerGoalHandle<nav2_msgs::action::AssistedTeleop>> goal_handle) override {
        nav2_msgs::action::AssistedTeleop::Result::SharedPtr result = std::make_shared<nav2_msgs::action::AssistedTeleop::Result>();
   ...
   public:
    static void SetUpTestCase() {
        ...
        BT::NodeBuilder builder = [](const std::string& name, const BT::NodeConfiguration& config) {
            return std::make_unique<nav2_behavior_tree::AssistedTeleopAction>(name, "assisted_teleop", config);
        };
...
std::shared_ptr<AssistedTeleopActionServer> AssistedTeleopActionTestFixture::action_server_ = nullptr;
...
int main(int argc, char** argv) {
    ...
    std::thread server_thread([]() { rclcpp::spin(AssistedTeleopActionTestFixture::action_server_); });
