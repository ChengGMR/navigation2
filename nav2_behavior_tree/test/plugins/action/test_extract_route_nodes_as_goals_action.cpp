   ...
   public:
    static void SetUpTestCase() {
        ...
        BT::NodeBuilder builder = [](const std::string& name, const BT::NodeConfiguration& config) {
            return std::make_unique<nav2_behavior_tree::ExtractRouteNodesAsGoals>(name, config);
        };

        factory_->registerBuilder<nav2_behavior_tree::ExtractRouteNodesAsGoals>("ExtractRouteNodesAsGoals", builder);
    }
...
std::shared_ptr<nav2_behavior_tree::TransformHandler> ExtractRouteNodesAsGoalsTestFixture::transform_handler_ = nullptr;
