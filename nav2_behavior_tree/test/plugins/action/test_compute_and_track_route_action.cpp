...
class ComputeAndTrackRouteActionServer : public TestActionServer<nav2_msgs::action::ComputeAndTrackRoute> {
   public:
    ComputeAndTrackRouteActionServer() : TestActionServer("compute_and_track_route") {}

   protected:
    void execute(const typename std::shared_ptr<rclcpp_action::ServerGoalHandle<nav2_msgs::action::ComputeAndTrackRoute>> goal_handle) override {
   ...
   public:
    static void SetUpTestCase() {
        ...
        BT::NodeBuilder builder = [](const std::string& name, const BT::NodeConfiguration& config) {
            return std::make_unique<nav2_behavior_tree::ComputeAndTrackRouteAction>(name, "compute_and_track_route", config);
        };

        factory_->registerBuilder<nav2_behavior_tree::ComputeAndTrackRouteAction>("ComputeAndTrackRoute", builder);
    }
...
std::shared_ptr<ComputeAndTrackRouteActionServer> ComputeAndTrackRouteActionTestFixture::action_server_ = nullptr;
...
TEST_F(ComputeAndTrackRouteActionTestFixture, test_tick_poses) {
    ...
    EXPECT_TRUE(config_->blackboard->get<builtin_interfaces::msg::Duration>("execution_duration", time1));
...
int main(int argc, char** argv) {
    ...
    ComputeAndTrackRouteActionTestFixture::action_server_ = std::make_shared<ComputeAndTrackRouteActionServer>();

    std::thread server_thread([]() { rclcpp::spin(ComputeAndTrackRouteActionTestFixture::action_server_); });
