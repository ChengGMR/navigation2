   ...
   public:
    static void SetUpTestCase() {
        ...
        BT::NodeBuilder builder = [](const std::string& name, const BT::NodeConfiguration& config) {
            return std::make_unique<nav2_behavior_tree::GetNextFewGoals>(name, config);
        };

        factory_->registerBuilder<nav2_behavior_tree::GetNextFewGoals>("GetNextFewGoals", builder);
    }
...
std::shared_ptr<nav2_behavior_tree::TransformHandler> GetNextFewGoalsTestFixture::transform_handler_ = nullptr;
