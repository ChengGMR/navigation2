...
class FollowObjectActionServer : public TestActionServer<nav2_msgs::action::FollowObject> {
   public:
    FollowObjectActionServer() : TestActionServer("follow_object") {}

   protected:
    void execute(const typename std::shared_ptr<rclcpp_action::ServerGoalHandle<nav2_msgs::action::FollowObject>> goal_handle) override {
   ...
   public:
    static void SetUpTestCase() {
        ...
        BT::NodeBuilder builder = [](const std::string& name, const BT::NodeConfiguration& config) {
            return std::make_unique<nav2_behavior_tree::FollowObjectAction>(name, "follow_object", config);
        };
...
std::shared_ptr<FollowObjectActionServer> FollowObjectActionTestFixture::action_server_ = nullptr;
...
int main(int argc, char** argv) {
    ...
    FollowObjectActionTestFixture::action_server_ = std::make_shared<FollowObjectActionServer>();

    std::thread server_thread([]() { rclcpp::spin(FollowObjectActionTestFixture::action_server_); });
