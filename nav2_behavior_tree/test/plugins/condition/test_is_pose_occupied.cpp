...
class IsPoseOccupiedSuccessService : public TestService<nav2_msgs::srv::GetCosts> {
   public:
    IsPoseOccupiedSuccessService() : TestService("/global_costmap/get_cost_global_costmap") {}
...
class IsPoseOccupiedFailureService : public TestService<nav2_msgs::srv::GetCosts> {
   public:
    IsPoseOccupiedFailureService() : TestService("/local_costmap/get_cost_local_costmap") {}
   ...
   public:
    static void SetUpTestCase() {
        ...
        BT::NodeBuilder builder = [](const std::string& name, const BT::NodeConfiguration& config) {
            return std::make_unique<nav2_behavior_tree::IsPoseOccupiedCondition>(name, config);
        };

        factory_->registerBuilder<nav2_behavior_tree::IsPoseOccupiedCondition>("IsPoseOccupied", builder);
    }
...
std::shared_ptr<IsPoseOccupiedSuccessService> IsPoseOccupiedTestFixture::success_server_ = nullptr;
std::shared_ptr<IsPoseOccupiedFailureService> IsPoseOccupiedTestFixture::failure_server_ = nullptr;
...
int main(int argc, char** argv) {
    ...
    IsPoseOccupiedTestFixture::success_server_ = std::make_shared<IsPoseOccupiedSuccessService>();
    std::thread success_server_thread([]() { rclcpp::spin(IsPoseOccupiedTestFixture::success_server_); });

    IsPoseOccupiedTestFixture::failure_server_ = std::make_shared<IsPoseOccupiedFailureService>();
    std::thread failure_server_thread([]() { rclcpp::spin(IsPoseOccupiedTestFixture::failure_server_); });
