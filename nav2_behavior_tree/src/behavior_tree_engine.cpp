...
BehaviorTreeEngine::BehaviorTreeEngine(const std::vector<std::string>& plugin_libraries, const nav2::LifecycleNode::SharedPtr node) {
...
BtStatus BehaviorTreeEngine::run(BT::Tree* tree, std::function<void()> onLoop, std::function<bool()> cancelRequested,
    ...
    try {
        while (rclcpp::ok() && result == BT::NodeStatus::RUNNING) {
            ...
            if (!loopRate.sleep()) {
                RCLCPP_DEBUG_THROTTLE(rclcpp::get_logger("BehaviorTreeEngine"), *clock_, 1000, "Behavior Tree tick rate %0.2f was exceeded!",
                                      1.0 / (loopRate.period().count() * 1.0e-9));
            }
        }
    } catch (const std::exception& ex) {
        RCLCPP_ERROR(rclcpp::get_logger("BehaviorTreeEngine"), "Behavior tree threw exception: %s. Exiting with failure.", ex.what());
...
std::string BehaviorTreeEngine::extractBehaviorTreeID(const std::string& bt_file) {
    if (bt_file.empty()) {
        RCLCPP_ERROR(rclcpp::get_logger("BehaviorTreeEngine"), "Error: Empty BT file passed to extractBehaviorTreeID");
        return "";
    }
    tinyxml2::XMLDocument doc;
    if (doc.LoadFile(bt_file.c_str()) != tinyxml2::XML_SUCCESS) {
        RCLCPP_ERROR(rclcpp::get_logger("BehaviorTreeEngine"), "Error: Could not open or parse file %s", bt_file.c_str());
        return "";
    }
    tinyxml2::XMLElement* rootElement = doc.RootElement();
    if (!rootElement) {
        RCLCPP_ERROR(rclcpp::get_logger("BehaviorTreeEngine"), "Error: Root element not found in %s", bt_file.c_str());
        return "";
    }
    tinyxml2::XMLElement* btElement = rootElement->FirstChildElement("BehaviorTree");
    if (!btElement) {
        RCLCPP_ERROR(rclcpp::get_logger("BehaviorTreeEngine"), "Error: <BehaviorTree> element not found in %s", bt_file.c_str());
        return "";
    }
    ...
    if (idValue) {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("BehaviorTreeEngine"), "Error: ID attribute not found on <BehaviorTree> element in %s", bt_file.c_str());
        return "";
    }
}

BT::Tree BehaviorTreeEngine::createTree(const std::string& tree_id, BT::Blackboard::Ptr blackboard) {
