...
class BtActionNode : public BT::ActionNodeBase {
   public:
    ...
    void createActionClient(const std::string& action_name) {
        ...
        if (!action_client_->wait_for_action_server(wait_for_service_timeout_)) {
            RCLCPP_ERROR(node_->get_logger(), "\"%s\" action server not available after waiting for %.2fs", action_name.c_str(),
        ...
        try {
            ...
            if (future_goal_handle_) {
                ...
                if (!is_future_goal_handle_complete(elapsed)) {
                    ...
                    RCLCPP_WARN(node_->get_logger(), "Timed out while waiting for action server to acknowledge goal request for %s",
                                action_name_.c_str());
    ...
    void halt() override {
        if (should_cancel_goal()) {
            ...
            if (callback_group_executor_.spin_until_future_complete(future_result, server_timeout_) != rclcpp::FutureReturnCode::SUCCESS) {
                RCLCPP_ERROR(node_->get_logger(), "Failed to get result for %s in node halt!", action_name_.c_str());
            }
   ...
   protected:
    ...
    void send_new_goal() {
        ...
        send_goal_options.feedback_callback = [this](typename rclcpp_action::ClientGoalHandle<ActionT>::SharedPtr,
                                                     const std::shared_ptr<const typename ActionT::Feedback> feedback) {
            feedback_ = feedback;
            emitWakeUpSignal();
        };

        future_goal_handle_ = std::make_shared<std::shared_future<typename rclcpp_action::ClientGoalHandle<ActionT>::SharedPtr>>(
