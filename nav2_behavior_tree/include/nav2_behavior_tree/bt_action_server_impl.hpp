...
BtActionServer<ActionT, NodeT>::BtActionServer(const typename NodeT::WeakPtr& parent, const std::string& action_name,
    ...
    std::vector<std::string> default_error_code_name_prefixes = {
        "assisted_teleop", "backup",         "compute_path", "dock_robot", "drive_on_heading", "follow_object",
        "follow_path",     "nav_thru_poses", "nav_to_pose",  "spin",       "undock_robot",     "wait",
    };
    ...
    error_code_name_prefixes_ = node->declare_or_get_parameter("error_code_name_prefixes", default_error_code_name_prefixes);
    ...
    if (error_code_name_prefixes_ == default_error_code_name_prefixes) {
        RCLCPP_WARN_STREAM(logger_, "error_code_name_prefixes parameters were not set. Using default values of:"
        ...
        RCLCPP_INFO_STREAM(logger_, "Error_code parameters were set to:" << error_code_name_prefixes_str);
    }
}
...
bool BtActionServer<ActionT, NodeT>::on_configure() {
    ...
    nav2::replaceOrAddArgument(new_arguments, "-r", "__node",
    ...
    action_server_ = nav2::interfaces::create_action_server<ActionT>(
        node, action_name_, std::bind(&BtActionServer<ActionT, NodeT>::executeCallback, this), nullptr, std::chrono::milliseconds(500), false);

    // Get parameters for BT timeouts
    bt_loop_duration_ = std::chrono::milliseconds(node->declare_or_get_parameter("bt_loop_duration", 10));

    default_server_timeout_ = std::chrono::milliseconds(node->declare_or_get_parameter("default_server_timeout", 20));

    wait_for_service_timeout_ = std::chrono::milliseconds(node->declare_or_get_parameter("wait_for_service_timeout", 1000));

    always_reload_bt_ = node->declare_or_get_parameter("always_reload_bt_xml", false);
    ...
    blackboard_->template set<std::chrono::milliseconds>("wait_for_service_timeout", wait_for_service_timeout_);
...
bool BtActionServer<ActionT, NodeT>::loadBehaviorTree(const std::string& bt_xml_filename_or_id) {
    ...
    auto file_or_id = bt_xml_filename_or_id.empty() ? default_bt_xml_filename_or_id_ : bt_xml_filename_or_id;
        ...
        for (const auto& directory : search_directories_) {
            for (const auto& entry : fs::directory_iterator(directory)) {
                ...
                if (registered_ids.count(id)) {
                    RCLCPP_WARN(logger_, "Skipping conflicting BT file %s (duplicate ID %s)", entry.path().c_str(), id.c_str());
                    continue;
                }
    ...
    try {
        ...
        setInternalError(ActionT::Result::FAILED_TO_LOAD_BEHAVIOR_TREE, "Exception registering behavior trees: " + std::string(e.what()));
        return false;
    }
    ...
    try {
        ...
        for (auto& subtree : tree_.subtrees) {
            ...
            blackboard->template set<std::chrono::milliseconds>("wait_for_service_timeout", wait_for_service_timeout_);
        }
    } catch (const std::exception& e) {
        setInternalError(ActionT::Result::FAILED_TO_LOAD_BEHAVIOR_TREE, std::string("Exception when creating BT tree from ID: ") + e.what());
        return false;
    }
    ...
    if (enable_groot_monitoring_) {
        ...
        RCLCPP_DEBUG(logger_, "Enabling Groot2 monitoring for %s: %d", action_name_.c_str(), groot_server_port_);
    }
...
void BtActionServer<ActionT, NodeT>::executeCallback() {
    ...
    switch (rc) {
        ...
        case nav2_behavior_tree::BtStatus::FAILED:
            action_server_->terminate_current(result);
            RCLCPP_ERROR(logger_, "Goal failed error_code:%d error_msg:'%s'", result->error_code, result->error_msg.c_str());
            break;
...
void BtActionServer<ActionT, NodeT>::setInternalError(uint16_t error_code, const std::string& error_msg) {
    ...
    RCLCPP_ERROR(logger_, "Setting internal error error_code:%d, error_msg:%s", internal_error_code_, internal_error_msg_.c_str());
}
...
template <class ActionT, class NodeT>
bool BtActionServer<ActionT, NodeT>::populateInternalError(typename std::shared_ptr<typename ActionT::Result> result) {
...
template <class ActionT, class NodeT>
void BtActionServer<ActionT, NodeT>::populateErrorCode(typename std::shared_ptr<typename ActionT::Result> result) {
    ...
    for (const auto& error_code_name_prefix : error_code_name_prefixes_) {
        try {
            ...
            RCLCPP_DEBUG(logger_, "Failed to get error code name: %s from blackboard", name.c_str());
        }
    }
