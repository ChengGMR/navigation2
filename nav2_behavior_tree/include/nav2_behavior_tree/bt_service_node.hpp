   ...
   public:
    ...
    BtServiceNode(const std::string& service_node_name, const BT::NodeConfiguration& conf, const std::string& service_name = "")
        ...
        if (!service_client_->wait_for_service(wait_for_service_timeout_)) {
            RCLCPP_ERROR(node_->get_logger(), "\"%s\" service server not available after waiting for %.2fs", service_name_.c_str(),
                         wait_for_service_timeout_.count() / 1000.0);
            throw std::runtime_error(std::string("Service server ") + service_name_ + std::string(" not available"));
        }
    ...
    void createROSInterfaces() {
        ...
        if (service_new != service_name_ || !service_client_) {
            ...
            service_client_ = node_->create_client<ServiceT>(service_name_, true /*creates and spins an internal executor*/);
        }
    }
     ...
     */
    virtual BT::NodeStatus on_completion(std::shared_ptr<typename ServiceT::Response> /*response*/) { return BT::NodeStatus::SUCCESS; }
