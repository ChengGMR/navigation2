...
const std::map<state_t, std::string> state_names = {
    {RESUMED, "RESUMED"},    {PAUSED, "PAUSED"}, {PAUSE_REQUESTED, "PAUSE_REQUESTED"}, {ON_PAUSE, "ON_PAUSE"}, {RESUME_REQUESTED, "RESUME_REQUESTED"},
    {ON_RESUME, "ON_RESUME"}};
...
class PauseResumeController : public BT::ControlNode {
   ...
   private:
    ...
    void pauseServiceCallback(const std::shared_ptr<rmw_request_id_t> /*request_header*/, const std::shared_ptr<Trigger::Request> request,
                              std::shared_ptr<Trigger::Response> response);

    //! @brief Set state to RESUME_REQUESTED
    void resumeServiceCallback(const std::shared_ptr<rmw_request_id_t> /*request_header*/, const std::shared_ptr<Trigger::Request> request,
                               std::shared_ptr<Trigger::Response> response);
