...
class BtActionServer {
   public:
     ...
     */
    explicit BtActionServer(const typename NodeT::WeakPtr& parent, const std::string& action_name, const std::vector<std::string>& plugin_lib_names,
     ...
     */
    std::string getCurrentBTFilenameOrID() const { return current_bt_file_or_id_; }

    /**
     * @brief Getter function for default BT XML filename or ID
     * @return string Containing default BT XML filename or ID
     */
    std::string getDefaultBTFilenameOrID() const { return default_bt_xml_filename_or_id_; }
     ...
     */
    const std::shared_ptr<const typename ActionT::Goal> acceptPendingGoal() { return action_server_->accept_pending_goal(); }
     ...
     */
    const std::shared_ptr<const typename ActionT::Goal> getCurrentGoal() const { return action_server_->get_current_goal(); }

    /**
     * @brief Wrapper function to get pending goal
     * @return Shared pointer to pending action goal
     */
    const std::shared_ptr<const typename ActionT::Goal> getPendingGoal() const { return action_server_->get_pending_goal(); }

    /**
     * @brief Wrapper function to publish action feedback
     */
    void publishFeedback(typename std::shared_ptr<typename ActionT::Feedback> feedback) { action_server_->publish_feedback(feedback); }
