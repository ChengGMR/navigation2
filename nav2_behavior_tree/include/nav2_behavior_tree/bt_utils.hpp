...
inline nav2_msgs::msg::WaypointStatus convertFromString(const StringView key) {
    ...
    if (parts.size() != 13) {
        ...
        waypoint_status.waypoint_pose.header.stamp = rclcpp::Time(BT::convertFromString<int64_t>(parts[2]));
...
inline std::vector<nav2_msgs::msg::WaypointStatus> convertFromString(const StringView key) {
    ...
    if (parts.size() % 13 != 0) {
        ...
        for (size_t i = 0; i < parts.size(); i += 13) {
            ...
            wp_status.waypoint_pose.header.stamp = rclcpp::Time(BT::convertFromString<int64_t>(parts[i + 2]));
...
T1 deconflictPortAndParamFrame(nav2::LifecycleNode::SharedPtr node, std::string param_name, const T2* behavior_tree_node) {
    T1 param_value;
    ...
    if (!param_from_input) {
        ...
        RCLCPP_DEBUG(node->get_logger(), "Parameter '%s' provided by behavior tree xml file", param_name.c_str());
        return param_value;
    }
}
 ...
 */
template <typename T>
inline bool getInputPortOrBlackboard(const BT::TreeNode& bt_node, const BT::Blackboard& blackboard, const std::string& param_name, T& value) {
