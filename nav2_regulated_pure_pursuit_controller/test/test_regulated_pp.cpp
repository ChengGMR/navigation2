...
class BasicAPIRPP : public nav2_regulated_pure_pursuit_controller::RegulatedPurePursuitController {
   public:
    ...
    double getLookAheadDistanceWrapper(const geometry_msgs::msg::Twist& twist) { return getLookAheadDistance(twist); }
    ...
    bool shouldRotateToGoalHeadingWrapper(const geometry_msgs::msg::PoseStamped& carrot_pose) { return shouldRotateToGoalHeading(carrot_pose); }
    ...
    void applyConstraintsWrapper(const double& curvature, const geometry_msgs::msg::Twist& curr_speed, const double& pose_cost,
    ...
    double findVelocitySignChangeWrapper(const nav_msgs::msg::Path& transformed_plan) { return findVelocitySignChange(transformed_plan); }
...
TEST(RegulatedPurePursuitTest, rotateTests) {
    ...
    auto ctrl = std::make_shared<BasicAPIRPP>();
    auto node = std::make_shared<nav2::LifecycleNode>("testRPP");
    nav2::declare_parameter_if_not_declared(node, "PathFollower.stateful", rclcpp::ParameterValue(false));
    ...
    // -----------------------
    node->set_parameter(rclcpp::Parameter("PathFollower.stateful", true));
...
TEST(RegulatedPurePursuitTest, applyConstraints) {
    ...
    nav2::declare_parameter_if_not_declared(node, name + ".approach_velocity_scaling_dist", rclcpp::ParameterValue(approach_velocity_scaling_dist));

    ctrl->configure(node, name, tf, costmap);

    auto no_approach_path = path_utils::generate_path(geometry_msgs::msg::PoseStamped(), 0.1,
...
TEST(RegulatedPurePursuitTest, testDynamicParameter) {
    ...
    auto rec_param = std::make_shared<rclcpp::AsyncParametersClient>(node->get_node_base_interface(), node->get_node_topics_interface(),
                                                                     node->get_node_graph_interface(), node->get_node_services_interface());

    auto results = rec_param->set_parameters_atomically({rclcpp::Parameter("test.desired_linear_vel", 1.0),
    ...
    // Should fail
    auto results2 = rec_param->set_parameters_atomically({rclcpp::Parameter("test.inflation_cost_scaling_factor", -1.0)});

    rclcpp::spin_until_future_complete(node->get_node_base_interface(), results2);

    auto results3 = rec_param->set_parameters_atomically({rclcpp::Parameter("test.use_rotate_to_heading", true)});

    rclcpp::spin_until_future_complete(node->get_node_base_interface(), results3);

    auto results4 =
        rec_param->set_parameters_atomically({rclcpp::Parameter("test.allow_reversing", false), rclcpp::Parameter("test.use_rotate_to_heading", true),
                                              rclcpp::Parameter("test.allow_reversing", true)});

    rclcpp::spin_until_future_complete(node->get_node_base_interface(), results4);
}
...
class TransformGlobalPlanTest : public ::testing::Test {
   protected:
    ...
    void configure_costmap(uint16_t width, double resolution) {
        ...
        auto results = costmap_->set_parameters({rclcpp::Parameter("global_frame", costmap_frame), rclcpp::Parameter("robot_base_frame", robot_frame),
                                                 rclcpp::Parameter("width", width), rclcpp::Parameter("height", width),
    ...
    void configure_controller(double max_robot_pose_search_dist) {
        ...
        nav2::declare_parameter_if_not_declared(node_, plugin_name + ".max_robot_pose_search_dist",
...
TEST_F(TransformGlobalPlanTest, prune_after_leaving_costmap) {
    ...
    auto global_plan =
        path_utils::generate_path(start_of_path, spacing,
                                  {std::make_unique<path_utils::Straight>(path_length), std::make_unique<path_utils::LeftTurnAround>(1.0),
