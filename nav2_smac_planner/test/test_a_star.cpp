...
TEST(AStarTest, test_a_star_2d) {
    ...
    nav2_smac_planner::AStarAlgorithm<nav2_smac_planner::Node2D> a_star(nav2_smac_planner::MotionModel::TWOD, info);
    ...
    a_star.initialize(false, max_iterations, it_on_approach, terminal_checking_interval, max_planning_time, 0.0, 1);
    ...
    *costmap = *costmapA;
    ...
    nav2_smac_planner::AStarAlgorithm<nav2_smac_planner::Node2D> a_star_2(nav2_smac_planner::MotionModel::TWOD, info);
    a_star_2.initialize(false, max_iterations, it_on_approach, terminal_checking_interval, max_planning_time, 0, 1);
    num_it = 0;
    EXPECT_THROW(a_star_2.createPath(path, num_it, tolerance, dummy_cancel_checker), std::runtime_error);
    ...
    nav2_smac_planner::AnalyticExpansion<nav2_smac_planner::Node2D> expander(nav2_smac_planner::MotionModel::TWOD, info, false, 1);

    auto analytic_expansion_nodes = nav2_smac_planner::AnalyticExpansion<nav2_smac_planner::Node2D>::AnalyticExpansionNodes();
    EXPECT_EQ(expander.setAnalyticPath(nullptr, nullptr, analytic_expansion_nodes), nullptr);
    int dummy_int1 = 0;
    int dummy_int2 = 0;
    EXPECT_EQ(expander.tryAnalyticExpansion(nullptr, {}, {}, {}, nullptr, dummy_int1, dummy_int2), nullptr);

    nav2_smac_planner::Node2D* start = nullptr;
    EXPECT_EQ(expander.refineAnalyticPath(start, nullptr, nullptr, analytic_expansion_nodes), std::numeric_limits<float>::max());
    nav2_smac_planner::AnalyticExpansion<nav2_smac_planner::Node2D>::AnalyticExpansionNodes expected_nodes =
        expander.getAnalyticPath(nullptr, nullptr, nullptr, nullptr);
...
TEST(AStarTest, test_a_star_se2) {
    ...
    nav2_smac_planner::AStarAlgorithm<nav2_smac_planner::NodeHybrid> a_star(nav2_smac_planner::MotionModel::DUBIN, info);
    int max_iterations = 10000;
    ...
    a_star.initialize(false, max_iterations, it_on_approach, terminal_checking_interval, max_planning_time, 401, size_theta);
...
TEST(AStarTest, test_a_star_analytic_expansion) {
    ...
    unsigned int size_theta = 72;
    info.cost_penalty = 1.7;
    nav2_smac_planner::AStarAlgorithm<nav2_smac_planner::NodeHybrid> a_star(nav2_smac_planner::MotionModel::REEDS_SHEPP, info);
    int max_iterations = 10000;
    float tolerance = 10.0;
    int it_on_approach = 10;
    int terminal_checking_interval = 5000;
...
TEST(AStarTest, test_a_star_lattice) {
    ...
    nav2_smac_planner::AStarAlgorithm<nav2_smac_planner::NodeLattice> a_star(nav2_smac_planner::MotionModel::STATE_LATTICE, info);
    int max_iterations = 10000;
    float tolerance = 10.0;
    int terminal_checking_interval = 5000;
    double max_planning_time = 120.0;
    ...
    a_star.initialize(false, max_iterations, std::numeric_limits<int>::max(), terminal_checking_interval, max_planning_time, 401, size_theta);

    nav2_costmap_2d::Costmap2D* costmapA = new nav2_costmap_2d::Costmap2D(100, 100, 0.05, 0.0, 0.0, 0);
    // island in the middle of lethal cost to cross
...
TEST(AStarTest, test_se2_single_pose_path) {
    ...
    *costmap = *costmapA;
    ...
    EXPECT_THROW(a_star.createPath(path, num_it, tolerance, dummy_cancel_checker), std::runtime_error);
...
TEST(AStarTest, test_goal_heading_mode) {
    ...
    *costmap = *costmapA;
    ...
    EXPECT_THROW(a_star.setGoal(80u, 80u, 40u, nav2_smac_planner::GoalHeadingMode::BIDIRECTIONAL), std::runtime_error);
    a_star.setStart(10u, 10u, 0u);
    ...
    EXPECT_EQ(a_star.getGoalManager().getGoalsState().size(), a_star.getGoalManager().getGoalsCoordinates().size());
    ...
    EXPECT_TRUE(a_star.createPath(path, num_it, tolerance, dummy_cancel_checker, expansions.get()));
    EXPECT_TRUE(a_star.getGoalManager().getGoalsState().size() == a_star.getGoalManager().getGoalsCoordinates().size());

    // UNKNOWN goal heading mode
    a_star.setCollisionChecker(checker.get());
    a_star.setStart(10u, 10u, 0u);

    EXPECT_THROW(a_star.setGoal(80u, 80u, 10u, nav2_smac_planner::GoalHeadingMode::UNKNOWN), std::runtime_error);
    delete costmapA;
    nav2_smac_planner::NodeHybrid::destroyStaticAssets();
}
...
TEST(AStarTest, test_constants) {
    ...
    EXPECT_EQ(nav2_smac_planner::fromString("REEDS_SHEPP"), nav2_smac_planner::MotionModel::REEDS_SHEPP);
    EXPECT_EQ(nav2_smac_planner::fromString("NONE"), nav2_smac_planner::MotionModel::UNKNOWN);

    EXPECT_EQ(nav2_smac_planner::fromStringToGH("DEFAULT"), nav2_smac_planner::GoalHeadingMode::DEFAULT);
    EXPECT_EQ(nav2_smac_planner::fromStringToGH("BIDIRECTIONAL"), nav2_smac_planner::GoalHeadingMode::BIDIRECTIONAL);
    EXPECT_EQ(nav2_smac_planner::fromStringToGH("ALL_DIRECTION"), nav2_smac_planner::GoalHeadingMode::ALL_DIRECTION);
