...
class UnconstrainedSmootherCostFunction : public ceres::FirstOrderFunction {
   public:
     ...
     */
    UnconstrainedSmootherCostFunction(std::vector<Eigen::Vector2d>* original_path, nav2_costmap_2d::Costmap2D* costmap, const SmootherParams& params)
        : _original_path(original_path), _num_params(2 * original_path->size()), _costmap(costmap), _params(params) {
   ...
   protected:
     ...
     */
    inline void addSmoothingResidual(const double& weight, const Eigen::Vector2d& pt, const Eigen::Vector2d& pt_p, const Eigen::Vector2d& pt_m,
                                     double& r) const {
        r += weight
             * (pt_p.dot(pt_p) - 4 * pt_p.dot(pt) + 2 * pt_p.dot(pt_m) + 4 * pt.dot(pt) - 4 * pt.dot(pt_m)
     ...
     */
    inline void addSmoothingJacobian(const double& weight, const Eigen::Vector2d& pt, const Eigen::Vector2d& pt_p, const Eigen::Vector2d& pt_m,
     ...
     * @param r Residual (cost) of term
     */
    inline void addCurvatureResidual(const double& weight, const Eigen::Vector2d& pt, const Eigen::Vector2d& pt_p, const Eigen::Vector2d& pt_m,
                                     CurvatureComputations& curvature_params, double& r) const {
        ...
        if (curvature_params.delta_xi_norm < EPSILON || curvature_params.delta_xi_p_norm < EPSILON || std::isnan(curvature_params.delta_xi_p_norm)
            || std::isnan(curvature_params.delta_xi_norm) || std::isinf(curvature_params.delta_xi_p_norm)
            || std::isinf(curvature_params.delta_xi_norm)) {
     ...
     * @param j1 Gradient of Y term
     */
    inline void addCurvatureJacobian(const double& weight, const Eigen::Vector2d& pt, const Eigen::Vector2d& pt_p, const Eigen::Vector2d& /*pt_m*/,
                                     CurvatureComputations& curvature_params, double& j0, double& j1) const {
     ...
     * @param r Residual (cost) of term
     */
    inline void addDistanceResidual(const double& weight, const Eigen::Vector2d& xi, const Eigen::Vector2d& xi_original, double& r) const {
     ...
     * @param j1 Gradient of Y term
     */
    inline void addDistanceJacobian(const double& weight, const Eigen::Vector2d& xi, const Eigen::Vector2d& xi_original, double& j0,
                                    double& j1) const {
     ...
     * @param r Residual (cost) of term
     */
    inline void addCostResidual(const double& weight, const double& value, double& r, Eigen::Vector2d& xi) const {
        if (value == FREE_COST) {
            return;
        }
     ...
     * @param j1 Gradient of Y term
     */
    inline void addCostJacobian(const double& weight, const unsigned int& mx, const unsigned int& my, const double& value, double& j0,
                                double& j1) const {
        if (value == FREE_COST) {
            return;
        }
     ...
     */
    inline Eigen::Vector2d normalizedOrthogonalComplement(const Eigen::Vector2d& a, const Eigen::Vector2d& b, const double& a_norm,
