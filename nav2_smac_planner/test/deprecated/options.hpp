...
struct SmootherParams {
    ...
    void get(nav2::LifecycleNode* node, const std::string& name) {
        ...
        // Smoother params
        nav2::declare_parameter_if_not_declared(node, local_name + "w_curve", rclcpp::ParameterValue(1.5));
        node->get_parameter(local_name + "w_curve", curvature_weight);
        nav2::declare_parameter_if_not_declared(node, local_name + "w_cost", rclcpp::ParameterValue(0.0));
        node->get_parameter(local_name + "w_cost", costmap_weight);
        nav2::declare_parameter_if_not_declared(node, local_name + "w_dist", rclcpp::ParameterValue(0.0));
        node->get_parameter(local_name + "w_dist", distance_weight);
        nav2::declare_parameter_if_not_declared(node, local_name + "w_smooth", rclcpp::ParameterValue(15000.0));
        node->get_parameter(local_name + "w_smooth", smooth_weight);
        nav2::declare_parameter_if_not_declared(node, local_name + "cost_scaling_factor", rclcpp::ParameterValue(10.0));
...
struct OptimizerParams {
    OptimizerParams() : debug(false), max_iterations(50), max_time(1e4), param_tol(1e-8), fn_tol(1e-6), gradient_tol(1e-10) {}
    ...
    struct AdvancedParams {
        ...
        void get(nav2::LifecycleNode* node, const std::string& name) {
            ...
            nav2::declare_parameter_if_not_declared(node, local_name + "min_line_search_step_size", rclcpp::ParameterValue(1e-20));
            node->get_parameter(local_name + "min_line_search_step_size", min_line_search_step_size);
            nav2::declare_parameter_if_not_declared(node, local_name + "max_num_line_search_step_size_iterations", rclcpp::ParameterValue(50));
            node->get_parameter(local_name + "max_num_line_search_step_size_iterations", max_num_line_search_step_size_iterations);
            nav2::declare_parameter_if_not_declared(node, local_name + "line_search_sufficient_function_decrease", rclcpp::ParameterValue(1e-20));
            node->get_parameter(local_name + "line_search_sufficient_function_decrease", line_search_sufficient_function_decrease);
            nav2::declare_parameter_if_not_declared(node, local_name + "max_num_line_search_direction_restarts", rclcpp::ParameterValue(10));
            node->get_parameter(local_name + "max_num_line_search_direction_restarts", max_num_line_search_direction_restarts);
            nav2::declare_parameter_if_not_declared(node, local_name + "max_line_search_step_expansion", rclcpp::ParameterValue(50));
            node->get_parameter(local_name + "max_line_search_step_expansion", max_line_search_step_expansion);
        }
    ...
    void get(nav2::LifecycleNode* node, const std::string& name) {
        ...
        // Optimizer params
        nav2::declare_parameter_if_not_declared(node, local_name + "param_tol", rclcpp::ParameterValue(1e-15));
        node->get_parameter(local_name + "param_tol", param_tol);
        nav2::declare_parameter_if_not_declared(node, local_name + "fn_tol", rclcpp::ParameterValue(1e-7));
        node->get_parameter(local_name + "fn_tol", fn_tol);
        nav2::declare_parameter_if_not_declared(node, local_name + "gradient_tol", rclcpp::ParameterValue(1e-10));
        node->get_parameter(local_name + "gradient_tol", gradient_tol);
        nav2::declare_parameter_if_not_declared(node, local_name + "max_iterations", rclcpp::ParameterValue(500));
        node->get_parameter(local_name + "max_iterations", max_iterations);
        nav2::declare_parameter_if_not_declared(node, local_name + "max_time", rclcpp::ParameterValue(0.100));
        node->get_parameter(local_name + "max_time", max_time);
        nav2::declare_parameter_if_not_declared(node, local_name + "debug_optimizer", rclcpp::ParameterValue(false));
