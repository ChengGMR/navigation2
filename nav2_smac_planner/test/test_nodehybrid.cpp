...
TEST(NodeHybridTest, test_node_hybrid) {
    ...
    nav2_smac_planner::NodeHybrid::initMotionModel(nav2_smac_planner::MotionModel::DUBIN, size_x, size_y, size_theta, info);
...
TEST(NodeHybridTest, test_obstacle_heuristic) {
    ...
    *costmap = *costmapA;
    ...
    nav2_smac_planner::NodeHybrid::resetObstacleHeuristic(costmap_ros, testA.pose.x, testA.pose.y, testB.pose.x, testB.pose.y);
...
TEST(NodeHybridTest, test_node_reeds_neighbors) {
    ...
    unsigned int size_y = 100;
    unsigned int size_theta = 72;
    nav2_smac_planner::NodeHybrid::initMotionModel(nav2_smac_planner::MotionModel::REEDS_SHEPP, size_x, size_y, size_theta, info);
    ...
    *costmap = costmapA;
    ...
    std::function<bool(const uint64_t&, nav2_smac_planner::NodeHybrid*&)> neighborGetter = [](const uint64_t&,
                                                                                              nav2_smac_planner::NodeHybrid*&) -> bool {
