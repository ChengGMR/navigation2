...
TEST(GoalManagerTest, test_goal_manager) {
    ...
    *costmap = *costmapA;
    ...
    auto checker = std::make_unique<nav2_smac_planner::GridCollisionChecker>(costmap_ros, 72, node);
    ...
    EXPECT_THROW(goal_manager.removeInvalidGoals(tolerance, checker.get(), allow_unknow), std::runtime_error);
