...
TEST(Node2DTest, test_node_2d) {
    ...
    *costmap = costmapA;
    ...
    nav2_smac_planner::Node2D::initMotionModel(nav2_smac_planner::MotionModel::TWOD, size, size, size, info);
...
TEST(Node2DTest, test_node_2d_neighbors) {
    ...
    size_x = 100u;
    nav2_smac_planner::Node2D::initMotionModel(nav2_smac_planner::MotionModel::TWOD, size_x, size_y, quant, info);
    ...
    *costmap = costmapA;
    ...
    node->setCost(cost);
    std::function<bool(const uint64_t&, nav2_smac_planner::Node2D*&)> neighborGetter = [](const uint64_t&, nav2_smac_planner::Node2D*&) -> bool {
        return false;
    };
