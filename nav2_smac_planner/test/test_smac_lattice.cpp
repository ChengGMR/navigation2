...
class LatticeWrap : public nav2_smac_planner::SmacPlannerLattice {
   public:
    void callDynamicParams(std::vector<rclcpp::Parameter> parameters) { dynamicParametersCallback(parameters); }

    int getCoarseSearchResolution() { return _coarse_search_resolution; }

    int getMaxIterations() { return _max_iterations; }

    int getMaxOnApproachIterations() { return _max_on_approach_iterations; }

    nav2_smac_planner::GoalHeadingMode getGoalHeadingMode() { return _goal_heading_mode; }
};
...
TEST(SmacTest, test_smac_lattice) {
    nav2::LifecycleNode::SharedPtr nodeLattice = std::make_shared<nav2::LifecycleNode>("SmacLatticeTest");
    nodeLattice->declare_parameter("test.debug_visualizations", rclcpp::ParameterValue(true));

    std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros = std::make_shared<nav2_costmap_2d::Costmap2DROS>("global_costmap");
...
TEST(SmacTest, test_smac_lattice_reconfigure) {
    ...
    auto rec_param =
        std::make_shared<rclcpp::AsyncParametersClient>(nodeLattice->get_node_base_interface(), nodeLattice->get_node_topics_interface(),
    ...
    auto results = rec_param->set_parameters_atomically(
        {rclcpp::Parameter("test.allow_unknown", false), rclcpp::Parameter("test.max_iterations", -1),
         rclcpp::Parameter("test.cache_obstacle_heuristic", true), rclcpp::Parameter("test.reverse_penalty", 5.0),
         rclcpp::Parameter("test.change_penalty", 1.0), rclcpp::Parameter("test.non_straight_penalty", 2.0),
         rclcpp::Parameter("test.cost_penalty", 2.0), rclcpp::Parameter("test.retrospective_penalty", 0.2),
         rclcpp::Parameter("test.analytic_expansion_ratio", 4.0), rclcpp::Parameter("test.max_planning_time", 10.0),
         rclcpp::Parameter("test.lookup_table_size", 30.0), rclcpp::Parameter("test.smooth_path", false),
         rclcpp::Parameter("test.analytic_expansion_max_length", 42.0), rclcpp::Parameter("test.tolerance", 42.0),
         rclcpp::Parameter("test.rotation_penalty", 42.0), rclcpp::Parameter("test.max_on_approach_iterations", 42),
         rclcpp::Parameter("test.terminal_checking_interval", 42), rclcpp::Parameter("test.allow_reverse_expansion", true)});
    ...
    parameters.push_back(rclcpp::Parameter(
        "test.lattice_filepath", ament_index_cpp::get_package_share_directory("nav2_smac_planner") + "/sample_primitives/test/output.json"));
    EXPECT_NO_THROW(planner->callDynamicParams(parameters));
    EXPECT_EQ(planner->getCoarseSearchResolution(), 1);
