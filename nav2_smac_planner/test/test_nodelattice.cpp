...
TEST(NodeLatticeTest, parser_test) {
    ...
    std::string filePath = pkg_share_dir + "/sample_primitives/5cm_resolution/0.5m_turning_radius/ackermann" + "/output.json";
...
TEST(NodeLatticeTest, test_node_lattice_neighbors_and_parsing) {
    ...
    nav2_smac_planner::NodeLattice::initMotionModel(nav2_smac_planner::MotionModel::STATE_LATTICE, x, y, angle_quantization, info);
    ...
    nav2_smac_planner::MotionPrimitivePtrs projections =
        nav2_smac_planner::NodeLattice::motion_table.getMotionPrimitives(&aNode, direction_change_index);
    ...
    EXPECT_NEAR(nav2_smac_planner::NodeLattice::motion_table.getLatticeMetadata(filePath).grid_resolution, 0.05, 0.005);
}
...
TEST(NodeLatticeTest, test_get_neighbors) {
    ...
    *costmap = *costmapA;
    ...
    std::function<bool(const uint64_t&, nav2_smac_planner::NodeLattice*&)> neighborGetter = [](const uint64_t&,
                                                                                               nav2_smac_planner::NodeLattice*&) -> bool {
