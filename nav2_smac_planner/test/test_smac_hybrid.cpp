...
TEST(SmacTest, test_smac_se2) {
    nav2::LifecycleNode::SharedPtr nodeSE2 = std::make_shared<nav2::LifecycleNode>("SmacSE2Test");
...
TEST(SmacTest, test_smac_se2_reconfigure) {
    ...
    auto rec_param = std::make_shared<rclcpp::AsyncParametersClient>(nodeSE2->get_node_base_interface(), nodeSE2->get_node_topics_interface(),
                                                                     nodeSE2->get_node_graph_interface(), nodeSE2->get_node_services_interface());

    auto results = rec_param->set_parameters_atomically({rclcpp::Parameter("test.downsample_costmap", true),
    ...
    EXPECT_EQ(nodeSE2->get_parameter("test.motion_model_for_search").as_string(), std::string("REEDS_SHEPP"));
    EXPECT_EQ(nodeSE2->get_parameter("test.goal_heading_mode").as_string(), std::string("BIDIRECTIONAL"));

    auto results2 = rec_param->set_parameters_atomically({rclcpp::Parameter("resolution", 0.2)});
    rclcpp::spin_until_future_complete(nodeSE2->get_node_base_interface(), results2);
