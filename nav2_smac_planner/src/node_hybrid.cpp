...
void HybridMotionTable::initDubin(unsigned int& size_x_in, unsigned int& /*size_y_in*/, unsigned int& num_angle_quantization_in,
    ...
    if (num_angle_quantization_in == num_angle_quantization && min_turning_radius == search_info.minimum_turning_radius
    ...
    if (search_info.allow_primitive_interpolation && increments > 1.0f) {
        ...
        for (unsigned int i = 1; i < static_cast<unsigned int>(increments); i++) {
            ...
            projections.emplace_back(delta_x_n, delta_y_n, static_cast<float>(i), TurnDirection::LEFT); // Left
            projections.emplace_back(delta_x_n, -delta_y_n, -static_cast<float>(i), TurnDirection::RIGHT); // Right
        }
    }
...
void HybridMotionTable::initReedsShepp(unsigned int& size_x_in, unsigned int& /*size_y_in*/, unsigned int& num_angle_quantization_in,
                                       SearchInfo& search_info) {
    size_x = size_x_in;
    change_penalty = search_info.change_penalty;
    non_straight_penalty = search_info.non_straight_penalty;
    ...
    if (search_info.allow_primitive_interpolation && increments > 1.0f) {
        ...
        for (unsigned int i = 1; i < static_cast<unsigned int>(increments); i++) {
            ...
            projections.emplace_back(-delta_x_n, delta_y_n, -static_cast<float>(i),
...
bool NodeHybrid::isNodeValid(const bool& traverse_unknown, GridCollisionChecker* collision_checker) {
    ...
    _is_node_valid = !collision_checker->inCollision(this->pose.x, this->pose.y, this->pose.theta /*bin number*/, traverse_unknown);
...
float NodeHybrid::getTraversalCost(const NodePtr& child) {
    ...
    if (motion_table.use_quadratic_cost_penalty) {
        travel_cost_raw *= (motion_table.travel_distance_reward + (motion_table.cost_penalty * normalized_cost * normalized_cost));
    } else {
    ...
    if (child_turn_dir == TurnDirection::REV_RIGHT || child_turn_dir == TurnDirection::REV_LEFT || child_turn_dir == TurnDirection::REVERSE) {
        // reverse direction
...
float NodeHybrid::getHeuristicCost(const Coordinates& node_coords, const CoordinateVector& goals_coords) {
    ...
    for (unsigned int i = 0; i < goals_coords.size(); i++) {
        distance_heuristic = std::min(distance_heuristic, getDistanceHeuristic(node_coords, goals_coords[i], obstacle_heuristic));
    }
    return std::max(obstacle_heuristic, distance_heuristic);
}

void NodeHybrid::initMotionModel(const MotionModel& motion_model, unsigned int& size_x, unsigned int& size_y, unsigned int& num_angle_quantization,
                                 SearchInfo& search_info) {
...
inline float distanceHeuristic2D(const uint64_t idx, const unsigned int size_x, const unsigned int target_x, const unsigned int target_y) {
...
void NodeHybrid::resetObstacleHeuristic(std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros_i, const unsigned int& start_x,
    ...
    if (obstacle_heuristic_lookup_table.size() == size) {
        ...
        std::fill(obstacle_heuristic_lookup_table.begin(), obstacle_heuristic_lookup_table.end(), 0.0f);
    } else {
        ...
        std::fill_n(obstacle_heuristic_lookup_table.begin(), obstacle_size, 0.0f);
    }
...
float NodeHybrid::getObstacleHeuristic(const Coordinates& node_coords, const Coordinates&, const float& cost_penalty) {
...
void NodeHybrid::precomputeDistanceHeuristic(const float& lookup_table_dim, const MotionModel& motion_model, const unsigned int& dim_3_size,
                                             const SearchInfo& search_info) {
    // Dubin or Reeds-Shepp shortest distances
    if (motion_model == MotionModel::DUBIN) {
        motion_table.state_space = std::make_shared<ompl::base::DubinsStateSpace>(search_info.minimum_turning_radius);
...
void NodeHybrid::getNeighbors(std::function<bool(const uint64_t&, nav2_smac_planner::NodeHybrid*&)>& NeighborGetter,
    ...
    for (unsigned int i = 0; i != motion_projections.size(); i++) {
        index =
            NodeHybrid::getIndex(static_cast<unsigned int>(motion_projections[i]._x), static_cast<unsigned int>(motion_projections[i]._y),
                                 static_cast<unsigned int>(motion_projections[i]._theta), motion_table.size_x, motion_table.num_angle_quantization);
