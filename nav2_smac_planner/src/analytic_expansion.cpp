...
AnalyticExpansion<NodeT>::AnalyticExpansion(const MotionModel& motion_model, const SearchInfo& search_info, const bool& traverse_unknown,
...
typename AnalyticExpansion<NodeT>::NodePtr AnalyticExpansion<NodeT>::tryAnalyticExpansion(
    const NodePtr& current_node, const NodeVector& coarse_check_goals, const NodeVector& fine_check_goals, const CoordinateVector& goals_coords,
    const NodeGetter& getter, int& analytic_iterations, int& closest_distance) {
    ...
    if (_motion_model == MotionModel::DUBIN || _motion_model == MotionModel::REEDS_SHEPP || _motion_model == MotionModel::STATE_LATTICE) {
        ...
        const Coordinates node_coords = NodeT::getCoords(current_node->getIndex(), _collision_checker->getCostmap()->getSizeInCellsX(), _dim_3_size);
        ...
        closest_distance = std::min(closest_distance, static_cast<int>(NodeT::getHeuristicCost(node_coords, goals_coords)));
        ...
        if (analytic_iterations <= 0) {
            ...
            for (auto& current_goal_node : coarse_check_goals) {
                AnalyticExpansionNodes analytic_nodes =
                    getAnalyticPath(current_node, current_goal_node, getter, current_node->motion_table.state_space);
        ...
        if (!current_best_analytic_nodes.nodes.empty()) {
            return setAnalyticPath(current_best_node, current_best_goal, current_best_analytic_nodes);
        }
        analytic_iterations--;
    }
...
template <typename NodeT>
int AnalyticExpansion<NodeT>::countDirectionChanges(const ompl::base::ReedsSheppStateSpace::ReedsSheppPath& path) {
...
template <typename NodeT>
typename AnalyticExpansion<NodeT>::AnalyticExpansionNodes AnalyticExpansion<NodeT>::getAnalyticPath(const NodePtr& node, const NodePtr& goal,
    ...
    if (!failure) {
        ...
        if (max_cost_it != node_costs.end() && *max_cost_it > max_cost) {
            ...
            if (failure) {
                if (d < 2.0f * M_PI * goal->motion_table.min_turning_radius && _search_info.analytic_expansion_max_cost_override) {
                    failure = false;
                }
            }
        }
...
template <typename NodeT>
float AnalyticExpansion<NodeT>::refineAnalyticPath(NodePtr& node, const NodePtr& goal_node, const NodeGetter& getter,
                                                   AnalyticExpansionNodes& analytic_nodes) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (test_node->parent && test_node->parent->parent && test_node->parent->parent->parent && test_node->parent->parent->parent->parent
            && test_node->parent->parent->parent->parent->parent) {
            test_node = test_node->parent->parent->parent->parent->parent;
            // print the goals pose
            refined_analytic_nodes = getAnalyticPath(test_node, goal_node, getter, test_node->motion_table.state_space);
    ...
    while (min_turn_rad < max_min_turn_rad) {
        ...
        if (score <= best_score && refined_analytic_nodes.direction_changes <= analytic_nodes.direction_changes) {
            analytic_nodes = refined_analytic_nodes;
            best_score = score;
            continue;
        }
        ...
        if (score <= original_score && refined_analytic_nodes.direction_changes < analytic_nodes.direction_changes) {
            analytic_nodes = refined_analytic_nodes;
            best_score = score;
        }
    }
...
template <typename NodeT>
typename AnalyticExpansion<NodeT>::NodePtr AnalyticExpansion<NodeT>::setAnalyticPath(const NodePtr& node, const NodePtr& goal_node,
...
template <>
typename AnalyticExpansion<Node2D>::AnalyticExpansionNodes AnalyticExpansion<Node2D>::getAnalyticPath(const NodePtr&, const NodePtr&,
                                                                                                      const NodeGetter&,
...
template <>
float AnalyticExpansion<Node2D>::refineAnalyticPath(NodePtr&, const NodePtr&, const NodeGetter&, AnalyticExpansionNodes&) {
    return std::numeric_limits<float>::max();
}

template <>
typename AnalyticExpansion<Node2D>::NodePtr AnalyticExpansion<Node2D>::setAnalyticPath(const NodePtr&, const NodePtr&,
                                                                                       const AnalyticExpansionNodes&) {
    return NodePtr(nullptr);
}

template <>
typename AnalyticExpansion<Node2D>::NodePtr AnalyticExpansion<Node2D>::tryAnalyticExpansion(const NodePtr&, const NodeVector&, const NodeVector&,
                                                                                            const CoordinateVector&, const NodeGetter&, int&, int&) {
    return NodePtr(nullptr);
}
