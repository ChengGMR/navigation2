...
template <typename NodeT>
void AStarAlgorithm<NodeT>::initialize(const bool& allow_unknown, int& max_iterations, const int& max_on_approach_iterations,
                                       const int& terminal_checking_interval, const double& max_planning_time, const float& lookup_table_size,
...
template <>
void AStarAlgorithm<Node2D>::initialize(const bool& allow_unknown, int& max_iterations, const int& max_on_approach_iterations,
                                        const int& terminal_checking_interval, const double& max_planning_time, const float& /*lookup_table_size*/,
                                        const unsigned int& dim_3_size) {
    _traverse_unknown = allow_unknown;
    _max_iterations = max_iterations;
...
template <typename NodeT>
typename AStarAlgorithm<NodeT>::NodePtr AStarAlgorithm<NodeT>::addToGraph(const uint64_t& index) {
...
template <>
void AStarAlgorithm<Node2D>::setStart(const float& mx, const float& my, const unsigned int& dim_3) {
    if (dim_3 != 0) {
        throw std::runtime_error("Node type Node2D cannot be given non-zero starting dim 3.");
    }
    _start = addToGraph(Node2D::getIndex(static_cast<unsigned int>(mx), static_cast<unsigned int>(my), getSizeX()));
}

template <typename NodeT>
void AStarAlgorithm<NodeT>::setStart(const float& mx, const float& my, const unsigned int& dim_3) {
    _start = addToGraph(NodeT::getIndex(static_cast<unsigned int>(mx), static_cast<unsigned int>(my), dim_3));
    _start->setPose(Coordinates(mx, my, dim_3));
}

template <>
void AStarAlgorithm<Node2D>::populateExpansionsLog(const NodePtr& node, std::vector<std::tuple<float, float, float>>* expansions_log) {
    Node2D::Coordinates coords = node->getCoords(node->getIndex());
    expansions_log->emplace_back(_costmap->getOriginX() + ((coords.x + 0.5) * _costmap->getResolution()),
                                 _costmap->getOriginY() + ((coords.y + 0.5) * _costmap->getResolution()), 0.0);
}

template <typename NodeT>
void AStarAlgorithm<NodeT>::populateExpansionsLog(const NodePtr& node, std::vector<std::tuple<float, float, float>>* expansions_log) {
...
template <>
void AStarAlgorithm<Node2D>::setGoal(const float& mx, const float& my, const unsigned int& dim_3, const GoalHeadingMode& /*goal_heading_mode*/,
                                     const int& /*coarse_search_resolution*/) {
    if (dim_3 != 0) {
        throw std::runtime_error("Node type Node2D cannot be given non-zero goal dim 3.");
    }
    _goal_manager.clear();
    auto goal = addToGraph(Node2D::getIndex(static_cast<unsigned int>(mx), static_cast<unsigned int>(my), getSizeX()));
...
template <typename NodeT>
void AStarAlgorithm<NodeT>::setGoal(const float& mx, const float& my, const unsigned int& dim_3, const GoalHeadingMode& goal_heading_mode,
                                    const int& coarse_search_resolution) {
    ...
    if (!_search_info.cache_obstacle_heuristic || _goal_manager.hasGoalChanged(ref_goal_coord)) {
        if (!_start) {
            throw std::runtime_error("Start must be set before goal.");
        }

        NodeT::resetObstacleHeuristic(_collision_checker->getCostmapROS(), _start->pose.x, _start->pose.y, mx, my);
    }
    ...
    switch (goal_heading_mode) {
        case GoalHeadingMode::DEFAULT: {
            ...
            auto goal = addToGraph(NodeT::getIndex(static_cast<unsigned int>(mx), static_cast<unsigned int>(my), dim_3));
        ...
        case GoalHeadingMode::BIDIRECTIONAL: {
            ...
            auto opposite_goal = addToGraph(NodeT::getIndex(static_cast<unsigned int>(mx), static_cast<unsigned int>(my), opposite_heading));
        ...
        case GoalHeadingMode::ALL_DIRECTION: {
            ...
            for (unsigned int i = 0; i < num_bins; ++i) {
                auto goal = addToGraph(NodeT::getIndex(static_cast<unsigned int>(mx), static_cast<unsigned int>(my), i));
...
template <typename NodeT>
bool AStarAlgorithm<NodeT>::createPath(CoordinateVector& path, int& iterations, const float& tolerance, std::function<bool()> cancel_checker,
    ...
    _goal_manager.prepareGoalsForAnalyticExpansion(coarse_check_goals, fine_check_goals, _coarse_search_resolution);
    ...
    const uint64_t max_index = static_cast<uint64_t>(getSizeX()) * static_cast<uint64_t>(getSizeY()) * static_cast<uint64_t>(getSizeDim3());
    ...
    while (iterations < getMaxIterations() && !_queue.empty()) {
        ...
        expansion_result = _expander->tryAnalyticExpansion(current_node, coarse_check_goals, fine_check_goals, _goal_manager.getGoalsCoordinates(),
