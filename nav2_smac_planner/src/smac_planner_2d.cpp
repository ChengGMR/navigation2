...
SmacPlanner2D::SmacPlanner2D()
    : _a_star(nullptr), _collision_checker(nullptr, 1, nullptr), _smoother(nullptr), _costmap(nullptr), _costmap_downsampler(nullptr) {}

SmacPlanner2D::~SmacPlanner2D() {
    RCLCPP_INFO(_logger, "Destroying plugin %s of type SmacPlanner2D", _name.c_str());
}

void SmacPlanner2D::configure(const nav2::LifecycleNode::WeakPtr& parent, std::string name, std::shared_ptr<tf2_ros::Buffer> /*tf*/,
    ...
    nav2::declare_parameter_if_not_declared(node, name + ".tolerance", rclcpp::ParameterValue(0.125));
    ...
    nav2::declare_parameter_if_not_declared(node, name + ".cost_travel_multiplier", rclcpp::ParameterValue(1.0));
    ...
    nav2::declare_parameter_if_not_declared(node, name + ".use_final_approach_orientation", rclcpp::ParameterValue(false));
    node->get_parameter(name + ".use_final_approach_orientation", _use_final_approach_orientation);

    nav2::declare_parameter_if_not_declared(node, name + ".max_planning_time", rclcpp::ParameterValue(2.0));
    ...
    _collision_checker.setFootprint(costmap_ros->getRobotFootprint(), true /*for 2D, most use radius*/,
...
void SmacPlanner2D::activate() {
    ...
    _dyn_params_handler = node->add_on_set_parameters_callback(std::bind(&SmacPlanner2D::dynamicParametersCallback, this, _1));
}
...
nav_msgs::msg::Path SmacPlanner2D::createPlan(const geometry_msgs::msg::PoseStamped& start, const geometry_msgs::msg::PoseStamped& goal,
    ...
    if (!_a_star->createPath(path, num_iterations, _tolerance / static_cast<float>(costmap->getResolution()), cancel_checker)) {
...
rcl_interfaces::msg::SetParametersResult SmacPlanner2D::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters) {
