...
void LatticeMotionTable::initMotionModel(unsigned int& size_x_in, SearchInfo& search_info) {
    ...
    if (!state_space) {
        if (!allow_reverse_expansion) {
            state_space = std::make_shared<ompl::base::DubinsStateSpace>(lattice_metadata.min_turning_radius);
            motion_model = MotionModel::DUBIN;
        } else {
            state_space = std::make_shared<ompl::base::ReedsSheppStateSpace>(lattice_metadata.min_turning_radius);
    ...
    for (unsigned int i = 0; i < lattice_metadata.heading_angles.size(); ++i) {
        trig_values.emplace_back(cos(lattice_metadata.heading_angles[i]), sin(lattice_metadata.heading_angles[i]));
    }
}

MotionPrimitivePtrs LatticeMotionTable::getMotionPrimitives(const NodeLattice* node, unsigned int& direction_change_index) {
...
bool NodeLattice::isNodeValid(const bool& traverse_unknown, GridCollisionChecker* collision_checker, MotionPrimitive* motion_primitive,
                              bool is_backwards) {
    ...
    if (collision_checker->inCollision(this->pose.x, this->pose.y, angle /*bin in collision checker*/, traverse_unknown)) {
        _is_node_valid = false;
    ...
    if (motion_primitive) {
        ...
        for (auto it = motion_primitive->poses.begin(); it != motion_primitive->poses.end(); ++it) {
            ...
            if (pose_dist._x * pose_dist._x + pose_dist._y * pose_dist._y > resolution_diag_sq) {
                ...
                if (collision_checker->inCollision(prim_pose._x, prim_pose._y, prim_pose._theta / bin_size /*bin in collision checker*/,
                                                   traverse_unknown)) {
                    _is_node_valid = false;
...
float NodeLattice::getTraversalCost(const NodePtr& child) {
    ...
    const float prim_length = transition_prim->trajectory_length / motion_table.lattice_metadata.grid_resolution;
    if (prim == nullptr) {
        return prim_length;
    }
    ...
    float travel_cost_raw = prim_length * (motion_table.travel_distance_reward + motion_table.cost_penalty * normalized_cost);
...
float NodeLattice::getHeuristicCost(const Coordinates& node_coords, const CoordinateVector& goals_coords) {
...
void NodeLattice::initMotionModel(const MotionModel& motion_model, unsigned int& size_x, unsigned int& /*size_y*/,
...
float NodeLattice::getDistanceHeuristic(const Coordinates& node_coords, const Coordinates& goal_coords, const float& obstacle_heuristic) {
...
void NodeLattice::precomputeDistanceHeuristic(const float& lookup_table_dim, const MotionModel& /*motion_model*/, const unsigned int& dim_3_size,
...
void NodeLattice::getNeighbors(std::function<bool(const uint64_t&, nav2_smac_planner::NodeLattice*&)>& NeighborGetter,
    ...
    MotionPrimitivePtrs motion_primitives = motion_table.getMotionPrimitives(this, direction_change_index);
    const float& grid_resolution = motion_table.lattice_metadata.grid_resolution;
    ...
    for (unsigned int i = 0; i != motion_primitives.size(); i++) {
        ...
        if (i >= direction_change_index) {
            backwards = true;
            float opposite_heading_theta = motion_projection.theta - (motion_table.num_angle_quantization / 2);
        ...
        index = NodeLattice::getIndex(static_cast<unsigned int>(motion_projection.x), static_cast<unsigned int>(motion_projection.y),
        ...
        if (NeighborGetter(index, neighbor) && !neighbor->wasVisited()) {
            ...
            neighbor->setPose(Coordinates(motion_projection.x, motion_projection.y, motion_projection.theta));
            ...
            if (neighbor->isNodeValid(traverse_unknown, collision_checker, motion_primitives[i], backwards)) {
...
void NodeLattice::addNodeToPath(NodeLattice::NodePtr current_node, NodeLattice::CoordinateVector& path) {
