...
class NodeHybrid {
   public:
    ...
    struct Coordinates {
        ...
        inline bool operator==(const Coordinates& rhs) const { return this->x == rhs.x && this->y == rhs.y && this->theta == rhs.theta; }
     ...
     */
    inline TurnDirection& getTurnDirection() { return _turn_dir; }
     ...
     * @return Index
     */
    static inline uint64_t getIndex(const unsigned int& x, const unsigned int& y, const unsigned int& angle, const unsigned int& width,
                                    const unsigned int& angle_quantization) {
        return static_cast<uint64_t>(angle) + static_cast<uint64_t>(x) * static_cast<uint64_t>(angle_quantization)
               + static_cast<uint64_t>(y) * static_cast<uint64_t>(width) * static_cast<uint64_t>(angle_quantization);
    }
     ...
     */
    static void initMotionModel(const MotionModel& motion_model, unsigned int& size_x, unsigned int& size_y, unsigned int& angle_quantization,
     ...
     */
    static void precomputeDistanceHeuristic(const float& lookup_table_dim, const MotionModel& motion_model, const unsigned int& dim_3_size,
     ...
     */
    static void resetObstacleHeuristic(std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros, const unsigned int& start_x,
     ...
     */
    void getNeighbors(std::function<bool(const uint64_t&, nav2_smac_planner::NodeHybrid*&)>& validity_checker,
     ...
     */
    static void destroyStaticAssets() { costmap_ros.reset(); }
