...
inline geometry_msgs::msg::Pose getWorldCoords(const float& mx, const float& my, const nav2_costmap_2d::Costmap2D* costmap) {
    ...
    msg.position.x = static_cast<float>(costmap->getOriginX()) + mx * costmap->getResolution();
    msg.position.y = static_cast<float>(costmap->getOriginY()) + my * costmap->getResolution();
    return msg;
}
 ...
 */
inline std::vector<geometry_msgs::msg::Point> transformFootprintToEdges(const geometry_msgs::msg::Pose& pose,
 ...
 */
inline visualization_msgs::msg::Marker createMarker(const std::vector<geometry_msgs::msg::Point> edge, unsigned int i, const std::string& frame_id,
