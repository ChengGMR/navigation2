...
class NodeLattice {
   public:
     ...
     */
    bool isNodeValid(const bool& traverse_unknown, GridCollisionChecker* collision_checker, MotionPrimitive* primitive = nullptr,
    ...
    static inline uint64_t getIndex(const unsigned int& x, const unsigned int& y, const unsigned int& angle) {
        ...
        return NodeHybrid::getIndex(x, y, angle, motion_table.size_x, motion_table.num_angle_quantization);
    }
    ...
    static inline Coordinates getCoords(const uint64_t& index, const unsigned int& width, const unsigned int& angle_quantization) {
        // Hybrid-A* and State Lattice share a coordinate system
        return NodeHybrid::Coordinates((index / angle_quantization) % width, // x
     ...
     */
    void getNeighbors(std::function<bool(const uint64_t&, nav2_smac_planner::NodeLattice*&)>& validity_checker,
