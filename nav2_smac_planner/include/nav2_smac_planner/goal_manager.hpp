...
class GoalManager {
   public:
     ...
     */
    GoalManager() : _goals_set(NodeSet()), _goals_state(GoalStateVector()), _goals_coordinate(CoordinateVector()), _ref_goal_coord(Coordinates()) {}
     ...
     */
    bool goalsIsEmpty() { return _goals_state.empty(); }

    /**
     * @brief Adds goal to the goal vector
     *@param goal Reference to the NodePtr
     */
    void addGoal(NodePtr& goal) { _goals_state.push_back({goal, true}); }
    ...
    bool isZoneValid(const NodePtr node, const float& radius, GridCollisionChecker* collision_checker, const bool& traverse_unknown) const {
        ...
        const float max_x = std::min(static_cast<float>(size_x - 1), std::ceil(center_point.x + radius));
        const float max_y = std::min(static_cast<float>(size_y - 1), std::ceil(center_point.y + radius));
