...
class AStarAlgorithm {
   public:
     ...
     */
    void initialize(const bool& allow_unknown, int& max_iterations, const int& max_on_approach_iterations, const int& terminal_checking_interval,
                    const double& max_planning_time, const float& lookup_table_size, const unsigned int& dim_3_size);
     ...
     */
    bool createPath(CoordinateVector& path, int& num_iterations, const float& tolerance, std::function<bool()> cancel_checker,
     ...
     */
    void setGoal(const float& mx, const float& my, const unsigned int& dim_3, const GoalHeadingMode& goal_heading_mode = GoalHeadingMode::DEFAULT,
