...
class TestLifecycleNode : public nav2::LifecycleNode {
   public:
    explicit TestLifecycleNode(const string& name) : nav2::LifecycleNode(name) {}

    nav2::CallbackReturn on_configure(const rclcpp_lifecycle::State&) { return nav2::CallbackReturn::SUCCESS; }

    nav2::CallbackReturn on_activate(const rclcpp_lifecycle::State&) { return nav2::CallbackReturn::SUCCESS; }

    nav2::CallbackReturn on_deactivate(const rclcpp_lifecycle::State&) { return nav2::CallbackReturn::SUCCESS; }

    nav2::CallbackReturn on_cleanup(const rclcpp_lifecycle::State&) { return nav2::CallbackReturn::SUCCESS; }

    nav2::CallbackReturn onShutdown(const rclcpp_lifecycle::State&) { return nav2::CallbackReturn::SUCCESS; }

    nav2::CallbackReturn onError(const rclcpp_lifecycle::State&) { return nav2::CallbackReturn::SUCCESS; }
};
...
class TestNode : public ::testing::Test {
   public:
    ...
    std::vector<geometry_msgs::msg::Point> setRadii(nav2_costmap_2d::LayeredCostmap& layers, double length, double width) {
...
TEST_F(TestNode, testObstacleAndStaticLayers) {
    ...
    node_->declare_parameter("pclayer_a.static.map_topic", rclcpp::ParameterValue(std::string("map")));
...
TEST_F(TestNode, testDifferentInflationLayers) {
    ...
    node_->declare_parameter("pclayer_b.inflation.cost_scaling_factor", rclcpp::ParameterValue(1.0));

    node_->declare_parameter("pclayer_b.inflation.inflation_radius", rclcpp::ParameterValue(1.0));
...
TEST_F(TestNode, testDifferentInflationLayers2) {
    ...
    node_->declare_parameter("pclayer_a.inflation.cost_scaling_factor", rclcpp::ParameterValue(1.0));

    node_->declare_parameter("pclayer_a.inflation.inflation_radius", rclcpp::ParameterValue(1.0));
...
TEST_F(TestNode, testClearable) {
    ...
    std::shared_ptr<nav2_costmap_2d::StaticLayer> slayer_b = std::make_shared<nav2_costmap_2d::StaticLayer>();
...
TEST_F(TestNode, testDynParamsSetPluginContainerLayer) {
    ...
    costmap->declare_parameter("plugin_container_layer_a.plugin", rclcpp::ParameterValue(std::string("nav2_costmap_2d::PluginContainerLayer")));
    costmap->declare_parameter("plugin_container_layer_b.plugin", rclcpp::ParameterValue(std::string("nav2_costmap_2d::PluginContainerLayer")));

    std::vector<std::string> plugins_str_a;
    plugins_str_a.push_back("obstacle_layer");
    plugins_str_a.push_back("inflation_layer");
    costmap->declare_parameter("plugin_container_layer_a.plugins", rclcpp::ParameterValue(plugins_str_a));
    ...
    costmap->declare_parameter("plugin_container_layer_b.plugins", rclcpp::ParameterValue(plugins_str_b));

    costmap->declare_parameter("plugin_container_layer_b.static_layer.plugin", rclcpp::ParameterValue(std::string("nav2_costmap_2d::StaticLayer")));
    ...
    auto parameter_client =
        std::make_shared<rclcpp::AsyncParametersClient>(costmap->get_node_base_interface(), costmap->get_node_topics_interface(),
    ...
    auto results = parameter_client->set_parameters_atomically(
        {rclcpp::Parameter("plugin_container_layer_a.combination_method", 2), rclcpp::Parameter("plugin_container_layer_b.combination_method", 1),
         rclcpp::Parameter("plugin_container_layer_a.enabled", false), rclcpp::Parameter("plugin_container_layer_b.enabled", true)});
