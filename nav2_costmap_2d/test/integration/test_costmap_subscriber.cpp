...
class TestCostmapSubscriberShould : public ::testing::Test {
   public:
    TestCostmapSubscriberShould() : topicName("/costmap"), node(std::make_shared<nav2::LifecycleNode>("test_subscriber")) {
        dummyCostmapMsgSubscriber = node->create_subscription<nav_msgs::msg::OccupancyGrid>(
            topicName, std::bind(&TestCostmapSubscriberShould::costmapCallback, this, std::placeholders::_1));

        dummyCostmapRawMsgSubscriber = node->create_subscription<nav2_msgs::msg::Costmap>(
            topicName + "_raw", std::bind(&TestCostmapSubscriberShould::costmapRawCallback, this, std::placeholders::_1));

        dummyCostmapUpdateMsgSubscriber = node->create_subscription<map_msgs::msg::OccupancyGridUpdate>(
            topicName + "_updates", std::bind(&TestCostmapSubscriberShould::costmapUpdateCallback, this, std::placeholders::_1));

        dummyCostmapRawUpdateMsgSubscriber = node->create_subscription<nav2_msgs::msg::CostmapUpdate>(
            topicName + "_raw_updates", std::bind(&TestCostmapSubscriberShould::costmapRawUpdateCallback, this, std::placeholders::_1));
    }
    ...
    void SetUp() override {
        ...
        costmapSubscriber = std::make_unique<nav2_costmap_2d::CostmapSubscriber>(node, topicName + "_raw");
    ...
    void costmapRawCallback(const nav2_msgs::msg::Costmap::SharedPtr) { this->fullCostmapRawMsgCount++; }
    ...
    void costmapRawUpdateCallback(const nav2_msgs::msg::CostmapUpdate::SharedPtr) { this->updateCostmapRawMsgCount++; }
   ...
   protected:
    std::vector<uint8_t> getCurrentCharMapFromSubscriber() {
        ...
        return std::vector<uint8_t>(
            ...
            currentSubscriberCostmap->getCharMap() + currentSubscriberCostmap->getSizeInCellsX() * currentSubscriberCostmap->getSizeInCellsY());
    }

    std::vector<uint8_t> getUpdatedCharMapFromSubscriber(std::uint32_t x0, std::uint32_t xn, std::uint32_t y0, std::uint32_t yn) {
        auto currentSubscriberCostmap = costmapSubscriber->getCostmap();
    ...
    std::vector<uint8_t> getCurrentCharMapToSend() {
        return std::vector<uint8_t>(costmapToSend->getCharMap(),
                                    costmapToSend->getCharMap() + costmapToSend->getSizeInCellsX() * costmapToSend->getSizeInCellsY());
    }

    std::vector<uint8_t> getUpdatedCostmapToSend(std::uint32_t x0, std::uint32_t xn, std::uint32_t y0, std::uint32_t yn) {
        std::vector<uint8_t> updatedCostmap;
        for (std::uint32_t y = y0; y < yn; y++) {
            for (std::uint32_t x = x0; x < xn; x++) {
    ...
    nav2::Subscription<map_msgs::msg::OccupancyGridUpdate>::SharedPtr dummyCostmapUpdateMsgSubscriber;
...
TEST_F(TestCostmapSubscriberShould, handleFullCostmapMsgs) {
    ...
    auto costmapPublisher = std::make_shared<nav2_costmap_2d::Costmap2DPublisher>(node, costmapToSend.get(), "", topicName, always_send_full_costmap);
...
TEST_F(TestCostmapSubscriberShould, throwExceptionIfGetCostmapMethodIsCalledBeforeAnyCostmapMsgReceived) {
