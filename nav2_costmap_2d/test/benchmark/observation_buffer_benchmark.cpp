...
class TestObstacleLayer : public nav2_costmap_2d::ObstacleLayer {
   public:
    ...
    static void addObstacleLayer(nav2_costmap_2d::LayeredCostmap& layers, tf2_ros::Buffer& tf, nav2::LifecycleNode::SharedPtr node,
                                 std::shared_ptr<TestObstacleLayer>& olayer, rclcpp::CallbackGroup::SharedPtr callback_group = nullptr) {
...
class ObservationBufferFixture : public benchmark::Fixture {
   public:
    void SetUp(benchmark::State& /*state*/) {
        node_ = std::make_shared<nav2::LifecycleNode>("observation_buffer_test_node");
        tf_buffer_ = std::make_shared<tf2_ros::Buffer>(node_->get_clock());
        buffer_ =
            std::make_shared<nav2_costmap_2d::ObservationBuffer>(node_, "/observations", 10.0, 0.1, 0.0, std::numeric_limits<double>::max(), 10.0,
                                                                 1.0, 5.0, 2.0, *tf_buffer_, global_frame, sensor_frame, tf2::Duration{0});
    ...
    static sensor_msgs::msg::PointCloud2 generateRandomPointCloud(size_t num_points, const rclcpp::Time& stamp) {
