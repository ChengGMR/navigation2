...
class ObservationBufferFixture : public ::testing::Test {
   protected:
    ...
    ~ObservationBufferFixture() { node_->shutdown(); }
    ...
    static sensor_msgs::msg::PointCloud2 generatePointCloud(double x, double y, double z, const rclcpp::Time& stamp) {
    ...
    nav2::LifecycleNode::SharedPtr node_{std::make_shared<nav2::LifecycleNode>("observation_buffer_test_node")};
    tf2_ros::Buffer tf_buffer_{node_->get_clock()};
    nav2_costmap_2d::ObservationBuffer buffer_{
        node_,         "/observations", 10.0, 0.1, 0.0, std::numeric_limits<double>::max(), 10.0, 1.0, 5.0, 2.0, tf_buffer_, global_frame_,
