 ...
 */
template <class T>
Image<T> imageFromString(const std::string& s, std::vector<T>& buffer, const std::map<char, T>& codes = {{'.', 0}, {'x', 255}}) {
    ...
    image.forEach([&](T& pixel) {
        try {
            ...
            throw std::logic_error("Test data error: parseBinaryMatrix: Unexpected symbol: " + std::string(1, *iter));
        }
    });
    return image;
}
