...
void DenoiseLayer::updateBounds(double /*robot_x*/, double /*robot_y*/, double /*robot_yaw*/, double* /*min_x*/, double* /*min_y*/, double* /*max_x*/,
                                double* /*max_y*/) {}
...
void DenoiseLayer::removeSinglePixels(Image<uint8_t>& image) const {
    ...
    if (!no_information_is_obstacle_) {
        ...
        auto max = [&](const std::initializer_list<uint8_t> lst) {
            std::array<uint8_t, 3> rbuf = {replace_to_free(*lst.begin()), replace_to_free(*(lst.begin() + 1)), replace_to_free(*(lst.begin() + 2))};
