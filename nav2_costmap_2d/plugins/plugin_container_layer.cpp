...
void PluginContainerLayer::onInitialize() {
    ...
    nav2::declare_parameter_if_not_declared(node, name_ + "." + "plugins", rclcpp::ParameterValue(std::vector<std::string>{}));
    ...
    dyn_params_handler_ =
        node->add_on_set_parameters_callback(std::bind(&PluginContainerLayer::dynamicParametersCallback, this, std::placeholders::_1));
...
void PluginContainerLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double* min_x, double* min_y, double* max_x,
                                        double* max_y) {
...
void PluginContainerLayer::updateCosts(nav2_costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j) {
...
rcl_interfaces::msg::SetParametersResult PluginContainerLayer::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters) {
    std::lock_guard<Costmap2D::mutex_t> guard(*getMutex());
