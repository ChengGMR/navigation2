...
void morphologyOperation(const Image<uint8_t>& input, Image<uint8_t>& output, const Image<uint8_t>& shape, AggregateFn aggregate);
 ...
 */
template <class Max>
inline void dilate(const Image<uint8_t>& input, Image<uint8_t>& output, ConnectivityType connectivity, Max&& max_function) {
...
T* MemoryBuffer::get(std::size_t count) {
    ...
    static_assert(alignof(std::max_align_t) >= alignof(T), "T alignment is more than the fundamental alignment of the platform");
...
class Window {
   public:
     ...
     */
    inline Window(T* up_row, T* down_row, Border<T> border = {}) : up_row_{up_row}, down_row_{down_row}, border_{border} {}
 ...
 */
template <class T>
Window<T, out_of_bounds_policy::ReplaceToZero> makeSafeWindow(const T* up_row, const T* down_row, size_t columns, size_t offset = 0) {
    return {dropConst(up_row) + offset, dropConst(down_row) + offset, out_of_bounds_policy::ReplaceToZero<T>{up_row, down_row, columns}};
}
...
void probeRows(const Image<uint8_t>& input, size_t first_input_row, Image<uint8_t>& output, size_t first_output_row, const uint8_t* shape,
               Apply touch_fn) {
...
void morphologyOperation(const Image<uint8_t>& input, Image<uint8_t>& output, const Image<uint8_t>& shape, AggregateFn aggregate) {
    if (input.rows() != output.rows() || input.columns() != output.columns()) {
        throw std::logic_error("morphologyOperation: the sizes of the input and output images are different");
    }
 ...
 */
template <ConnectivityType connectivity, class Label, class IsBg>
Label connectedComponentsImpl(const Image<uint8_t>& image, Image<Label>& labels, imgproc_impl::EquivalenceLabelTrees<Label>& label_trees,
...
class GroupsRemover {
   public:
     ...
     */
    template <class IsBg>
    void removeGroups(Image<uint8_t>& image, MemoryBuffer& buffer, ConnectivityType group_connectivity_type, size_t minimal_group_size,
   ...
   private:
    ...
    void removeGroupsPickLabelType(Image<uint8_t>& image, MemoryBuffer& buffer, size_t minimal_group_size, const IsBg& is_background) const {
        bool success{};
        auto label_trees16 = dynamic_cast<imgproc_impl::EquivalenceLabelTrees<uint16_t>*>(label_trees_.get());

        if (label_trees16) {
            success = tryRemoveGroupsWithLabelType<connectivity>(image, buffer, minimal_group_size, *label_trees16, is_background, false);
        }
        ...
        if (!success) {
            ...
            tryRemoveGroupsWithLabelType<connectivity>(image, buffer, minimal_group_size, *label_trees32, is_background, true);
        }
    }
    /**
    ...
    template <ConnectivityType connectivity, class Label, class IsBg>
    void removeGroupsImpl(Image<uint8_t>& image, MemoryBuffer& buffer, imgproc_impl::EquivalenceLabelTrees<Label>& label_trees,
...
template <ConnectivityType connectivity, class Label, class IsBg>
Image<Label> connectedComponents(const Image<uint8_t>& image, MemoryBuffer& buffer, imgproc_impl::EquivalenceLabelTrees<Label>& label_trees,
