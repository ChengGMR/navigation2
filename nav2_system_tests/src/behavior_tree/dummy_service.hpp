...
class DummyService {
   public:
    explicit DummyService(const rclcpp::Node::SharedPtr& node, std::string service_name)
        ...
        service_ = node->create_service<ServiceT>(service_name, std::bind(&DummyService::handle_service, this, _1, _2, _3));
    }
    ...
    void enable() {
        if (disabled_) {
            service_ = node_->create_service<ServiceT>(service_name_, std::bind(&DummyService::handle_service, this, _1, _2, _3));
            disabled_ = false;
        }
    }
   ...
   protected:
    virtual void fillResponse(const std::shared_ptr<typename ServiceT::Request> /*request*/,
                              const std::shared_ptr<typename ServiceT::Response> /*response*/) {}

    void handle_service(const std::shared_ptr<rmw_request_id_t> /*request_header*/, const std::shared_ptr<typename ServiceT::Request> request,
                        const std::shared_ptr<typename ServiceT::Response> response) {
        request_count_++;
