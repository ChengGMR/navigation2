...
class BehaviorTreeHandler {
   public:
    BehaviorTreeHandler() {
        ...
        auto timer_interface = std::make_shared<tf2_ros::CreateTimerROS>(node_->get_node_base_interface(), node_->get_node_timers_interface());
    ...
    std::string extractBehaviorTreeID(const std::string& file_or_id) { return bt_engine_->extractBehaviorTreeID(file_or_id); }

    bool loadBehaviorTree(const std::string& file_or_id, const std::vector<std::string>& search_directories) {
        namespace fs = std::filesystem;
        const std::string kXmlExtension = ".xml";
        const bool is_bt_id = (file_or_id.length() < kXmlExtension.size())
                              || (file_or_id.compare(file_or_id.length() - kXmlExtension.size(), kXmlExtension.size(), kXmlExtension) != 0);
            ...
            for (const auto& directory : search_directories) {
                for (const auto& entry : fs::directory_iterator(directory)) {
                    ...
                    if (registered_ids.count(id)) {
                        std::cerr << "Skipping conflicting BT file " << entry.path() << " (duplicate ID " << id << ")" << "\n";
                        continue;
                    }
...
TEST_F(BehaviorTreeTestFixture, TestBTXMLFiles) {
    ...
    const auto root_dir = std::filesystem::path(ament_index_cpp::get_package_share_directory("nav2_bt_navigator")) / "behavior_trees";
    ...
    for (auto const& entry : std::filesystem::recursive_directory_iterator(root_dir)) {
        if (entry.is_regular_file() && entry.path().extension() == ".xml") {
            ...
            EXPECT_TRUE(bt_handler->loadBehaviorTree(main_bt, search_directories)) << "Failed to load: " << main_bt;
        }
    }
}
...
TEST_F(BehaviorTreeTestFixture, TestDuplicateIDsWithFileSpecified) {
    ...
    bool found_conflict = log_output.find("Skipping conflicting BT file \"" + dup2_file + "\" (duplicate ID DuplicateTree)") != std::string::npos;
    EXPECT_TRUE(found_conflict);

    EXPECT_NE(log_output.find("Registering Tree from File"), std::string::npos);
    EXPECT_NE(log_output.find("Skipping conflicting BT file"), std::string::npos) << "Should warn about duplicate ID";
...
TEST_F(BehaviorTreeTestFixture, TestDuplicateIDsWithIDSpecified) {
    ...
    EXPECT_NE(log_output.find("Registering Tree from File"), std::string::npos) << "Should have registered at least one BT file";
    EXPECT_NE(log_output.find("Skipping conflicting BT file"), std::string::npos) << "Should warn about duplicate IDs";
    EXPECT_NE(log_output.find("Created BT from ID: DuplicateTree"), std::string::npos) << "Should have created BT from the given ID";

    bool registered_dup1 = log_output.find("Registering Tree from File: " + dup1_file) != std::string::npos;
    bool registered_dup2 = log_output.find("Registering Tree from File: " + dup2_file) != std::string::npos;

    EXPECT_TRUE(registered_dup1 || registered_dup2) << "At least one duplicate file should have been registered";
    EXPECT_FALSE(registered_dup1 && registered_dup2) << "Only one of the duplicate files should be registered as the main tree";
