...
class TestAmclPose : public ::testing::Test {
   public:
    TestAmclPose() {
        ...
        initial_pose_pub_ = node->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>("initialpose", rclcpp::SystemDefaultsQoS());
...
bool TestAmclPose::defaultAmclTest() {
    ...
    if (std::abs(amcl_pose_x - testPose_.pose.pose.position.x) < tol_ && std::abs(amcl_pose_y - testPose_.pose.pose.position.y) < tol_) {
        return true;
    } else {
        return false;
    }
