...
AssistedTeleopBehaviorTester::AssistedTeleopBehaviorTester() : is_active_(false), initial_pose_received_(false) {
    ...
    client_ptr_ =
        rclcpp_action::create_client<AssistedTeleop>(node_->get_node_base_interface(), node_->get_node_graph_interface(),
    ...
    cmd_vel_pub_ = node_->create_publisher<geometry_msgs::msg::TwistStamped>("cmd_vel_teleop", 10);
    ...
    filtered_vel_sub_ = node_->create_subscription<geometry_msgs::msg::TwistStamped>(
        "cmd_vel", rclcpp::SystemDefaultsQoS(), std::bind(&AssistedTeleopBehaviorTester::filteredVelCallback, this, std::placeholders::_1));
    ...
    footprint_sub_ = std::make_shared<nav2_costmap_2d::FootprintSubscriber>(node_, footprint_topic, *tf_buffer_);

    collision_checker_ = std::make_unique<nav2_costmap_2d::CostmapTopicCollisionChecker>(*costmap_sub_, *footprint_sub_);
...
bool AssistedTeleopBehaviorTester::defaultAssistedTeleopTest(const float lin_vel, const float ang_vel) {
    if (!is_active_) {
    ...
    if (executor_.spin_until_future_complete(goal_handle_future) != rclcpp::FutureReturnCode::SUCCESS) {
    ...
    if (executor_.spin_until_future_complete(result_future) != rclcpp::FutureReturnCode::SUCCESS) {
...
void AssistedTeleopBehaviorTester::amclPoseCallback(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr) {
    initial_pose_received_ = true;
}

void AssistedTeleopBehaviorTester::filteredVelCallback(geometry_msgs::msg::TwistStamped::SharedPtr msg) {
