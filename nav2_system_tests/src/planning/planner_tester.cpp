...
PlannerTester::PlannerTester()
    : nav2::LifecycleNode("PlannerTester")
    , is_active_(false)
    , map_set_(false)
    , costmap_set_(false)
...
void PlannerTester::activate() {
    ...
    planner_tester_->set_parameter(rclcpp::Parameter(std::string("GridBased.use_astar"), rclcpp::ParameterValue(true)));
    planner_tester_->set_parameter(rclcpp::Parameter(std::string("expected_planner_frequency"), rclcpp::ParameterValue(-1.0)));
...
void PlannerTester::startRobotTransform() {
    ...
    transform_timer_ = create_wall_timer(100ms, std::bind(&PlannerTester::publishRobotTransform, this));
}
...
void PlannerTester::setCostmap() {
    ...
    costmap_ = std::make_unique<Costmap>(this, trinary_costmap_, track_unknown_space_, lethal_threshold_, unknown_cost_value_);
...
bool PlannerTester::defaultPlannerTest(ComputePathToPoseResult& path, const double /*deviation_tolerance*/) {
    if (!costmap_set_) {
...
bool PlannerTester::defaultPlannerRandomTests(const unsigned int number_tests, const float acceptable_fail_ratio = 0.1) {
    if (!costmap_set_) {
        RCLCPP_ERROR(this->get_logger(), "Costmap must be set before requesting a plan");
        return false;
    }
    ...
    if (using_fake_costmap_) {
        RCLCPP_ERROR(this->get_logger(), "Randomized testing with hardcoded costmaps not implemented yet");
        return false;
    }
    ...
    for (unsigned int test_num = 0; test_num < number_tests; ++test_num) {
        ...
        if (!plannerTest(robot_position, goal, path)) {
            RCLCPP_WARN(this->get_logger(), "Failed with start at %0.2f, %0.2f and goal at %0.2f, %0.2f", robot_position.x, robot_position.y,
    ...
    RCLCPP_INFO(this->get_logger(), "Tested with %u tests. Planner failed on %u. Test time %ld ms", number_tests, num_fail, elapsed.count());
...
bool PlannerTester::plannerTest(const geometry_msgs::msg::Point& robot_position, const ComputePathToPoseCommand& goal,
                                ComputePathToPoseResult& path) {
...
TaskStatus PlannerTester::createPlan(const ComputePathToPoseCommand& goal, ComputePathToPoseResult& path) {
...
bool PlannerTester::isPathValid(nav_msgs::msg::Path& path, unsigned int max_cost, bool consider_unknown_as_obstacle) {
    planner_tester_->setCostmap(costmap_.get());
    ...
    if (rclcpp::spin_until_future_complete(this->planner_tester_, result, std::chrono::milliseconds(100)) == rclcpp::FutureReturnCode::SUCCESS) {
...
bool PlannerTester::isCollisionFree(const ComputePathToPoseResult& path) {
    ...
    for (auto pose : path.poses) {
        collisionFree = costmap_->is_free(static_cast<unsigned int>(std::round(pose.pose.position.x)),
                                          static_cast<unsigned int>(std::round(pose.pose.position.y)));

        if (!collisionFree) {
            RCLCPP_WARN(this->get_logger(), "Path has collision at (%.2f, %.2f)", pose.pose.position.x, pose.pose.position.y);
            printPath(path);
            return false;
        }
    }
...
bool PlannerTester::isWithinTolerance(const geometry_msgs::msg::Point& robot_position, const ComputePathToPoseCommand& goal,
...
bool PlannerTester::isWithinTolerance(const geometry_msgs::msg::Point& robot_position, const ComputePathToPoseCommand& goal,
    ...
    if (path_start.pose.position.x == robot_position.x && path_start.pose.position.y == robot_position.y
        && path_end.pose.position.x == goal.pose.position.x && path_end.pose.position.y == goal.pose.position.y) {
    ...
    RCLCPP_DEBUG(this->get_logger(), "Requested path starts at (%.2f, %.2f) and ends at (%.2f, %.2f)", robot_position.x, robot_position.y,
                 goal.pose.position.x, goal.pose.position.y);

    RCLCPP_DEBUG(this->get_logger(), "Computed path starts at (%.2f, %.2f) and ends at (%.2f, %.2f)", path_start.pose.position.x,
                 path_start.pose.position.y, path_end.pose.position.x, path_end.pose.position.y);
...
void PlannerTester::printPath(const ComputePathToPoseResult& path) const {
    ...
    for (auto pose : path.poses) {
        ss << "   point #" << index << " with" << " x: " << std::setprecision(3) << pose.pose.position.x << " y: " << std::setprecision(3)
