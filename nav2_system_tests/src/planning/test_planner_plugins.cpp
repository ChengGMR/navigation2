...
void testSmallPathValidityAndNoOrientation(std::string plugin, double length) {
    ...
    if (path_size == 1) {
        EXPECT_NEAR(tf2::getYaw(path.poses.back().pose.orientation), tf2::getYaw(start.pose.orientation), 0.01);
    } else {
...
void testCancel(std::string plugin) {
    ...
    EXPECT_THROW(obj->getPlan(start, goal, "GridBased", always_cancelled), nav2_core::PlannerCancelled);
    obj->onCleanup(state);
    obj.reset();
}
...
TEST(testPluginMap, Failures) {
    ...
    obj->onConfigure(state);
    obj->create_subscription<nav_msgs::msg::Path>("plan", callback);
