...
void ConstrainedSmoother::configure(const nav2::LifecycleNode::WeakPtr& parent, std::string name, std::shared_ptr<tf2_ros::Buffer> tf,
...
bool ConstrainedSmoother::smooth(nav_msgs::msg::Path& path, const rclcpp::Duration& max_time) {
    ...
    if (!smoother_->smooth(path_world, start_dir, end_dir, costmap.get(), smoother_params_)) {
        ...
        throw nav2_core::FailedToSmoothPath("Failed to smooth plan, Ceres could not find a usable solution");
    }
