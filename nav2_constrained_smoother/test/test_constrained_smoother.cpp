...
class DummyCostmapSubscriber : public nav2_costmap_2d::CostmapSubscriber {
   public:
    DummyCostmapSubscriber(nav2::LifecycleNode::SharedPtr node, const std::string& topic_name) : CostmapSubscriber(node, topic_name) {
    ...
    void setCostmap(nav2_msgs::msg::Costmap::SharedPtr msg) {
        costmap_msg_ = msg;
        costmap_ = std::make_shared<nav2_costmap_2d::Costmap2D>(msg->metadata.size_x, msg->metadata.size_y, msg->metadata.resolution,
...
class SmootherTest : public ::testing::Test {
   protected:
    ...
    void SetUp() override {
        node_lifecycle_ = std::make_shared<nav2::LifecycleNode>("ConstrainedSmootherTestNode");

        tf_buffer_ = std::make_shared<tf2_ros::Buffer>(node_lifecycle_->get_clock());
        auto timer_interface =
            std::make_shared<tf2_ros::CreateTimerROS>(node_lifecycle_->get_node_base_interface(), node_lifecycle_->get_node_timers_interface());
        ...
        costmap_pub_ =
            std::make_shared<nav2_costmap_2d::Costmap2DPublisher>(node_lifecycle_, costmap_sub_->getCostmap().get(), "map", "/costmap", true);
     ...
     */
    double assessPathImprovement(const std::vector<Eigen::Vector3d>& input, const std::vector<Eigen::Vector3d>& output,
     ...
     * @return Percentage of improvement (relative to input path quality)
     */
    double assessWorstPoseImprovement(const std::vector<Eigen::Vector3d>& input, const std::vector<Eigen::Vector3d>& output,
    ...
    nav2::Publisher<geometry_msgs::msg::PoseArray>::SharedPtr path_poses_pub_orig_;
    nav2::Publisher<geometry_msgs::msg::PoseArray>::SharedPtr path_poses_pub_;
    nav2::Publisher<geometry_msgs::msg::PoseArray>::SharedPtr path_poses_pub_cmp_;
    std::shared_ptr<nav2_costmap_2d::Costmap2DPublisher> costmap_pub_;

    int cusp_i_ = -1;
    QualityCriterion3 mvmt_smoothness_criterion_ = [this](int i, const Eigen::Vector3d& prev_p, const Eigen::Vector3d& p,
...
TEST_F(SmootherTest, testingSmoothness) {
    ...
    std::vector<Eigen::Vector3d> sharp_turn_90 = {
        {0, 0, 0}, {0.1, 0, 0}, {0.2, 0, 0}, {0.3, 0, M_PI / 4}, {0.3, 0.1, M_PI / 2}, {0.3, 0.2, M_PI / 2}, {0.3, 0.3, M_PI / 2}};
    ...
    auto orientation_smoothness_criterion = [](int, const Eigen::Vector3d& prev_p, const Eigen::Vector3d& p) {
    ...
    // path with a cusp
    std::vector<Eigen::Vector3d> sharp_turn_90_then_reverse = {{0, 0, 0},
                                                               {0.1, 0, 0},
                                                               {0.2, 0, 0},
                                                               {0.3, 0, 0},
                                                               {0.4, 0, 0},
...
TEST_F(SmootherTest, testingMaxCurvature) {
    ...
    std::vector<Eigen::Vector3d> radius_0_3_turn_90 = {{0, 0, 0},
                                                       {0.1, 0, 0},
                                                       {0.2, 0, 0},
...
TEST_F(SmootherTest, testingObstacleAvoidance) {
    ...
    std::vector<Eigen::Vector3d> straight_near_obstacle = {{0.05, 0.05, 0}, {0.45, 0.05, 0}, {0.85, 0.05, 0}, {1.25, 0.05, 0},
...
TEST_F(SmootherTest, testingObstacleAvoidanceNearCusps) {
    ...
    // path with a cusp
    std::vector<Eigen::Vector3d> cusp_near_obstacle = {{0.05, 0.05, 0},
                                                       {0.15, 0.05, 0},
                                                       {0.25, 0.05, 0},
                                                       {0.35, 0.05, 0},
                                                       {0.45, 0.05, 0},
    ...
    RCLCPP_INFO(rclcpp::get_logger("ceres_smoother"), "Cost avoidance improvement (cusp, simple): %lf, %lf", cost_avoidance_improvement_simple,
    ...
    node_lifecycle_->set_parameter(rclcpp::Parameter(
        "SmoothPath.cost_check_points", std::vector<double>({-0.05, 0.0, 0.5, -0.45, 0.0, 0.5}) // x1, y1, weight1, x2, y2, weight2
        ));
    reloadParams();
    ...
    node_lifecycle_->set_parameter(rclcpp::Parameter(
        "SmoothPath.cost_check_points", std::vector<double>({-0.05, 0.0, 1.0, -0.45, 0.0, 1.0}) // x1, y1, weight1, x2, y2, weight2
        ));
    reloadParams();
...
TEST_F(SmootherTest, testingDownsamplingUpsampling) {
    ...
    QualityCriterion3 mvmt_smoothness_criterion_out = [&cusp_i_out](int i, const Eigen::Vector3d& prev_p, const Eigen::Vector3d& p,
                                                                    const Eigen::Vector3d& next_p) {
        Eigen::Vector2d prev_mvmt = p.block<2, 1>(0, 0) - prev_p.block<2, 1>(0, 0);
        Eigen::Vector2d next_mvmt = next_p.block<2, 1>(0, 0) - p.block<2, 1>(0, 0);
        if (i == cusp_i_out) {
    ...
    double smoothness_improvement =
        assessPathImprovement(smoothed_path_downsampled, smoothed_path, mvmt_smoothness_criterion_, &mvmt_smoothness_criterion_out);
...
TEST_F(SmootherTest, testingCostCheckPointsParamValidity) {
    ...
    node_lifecycle_->set_parameter(
        rclcpp::Parameter("SmoothPath.cost_check_points", std::vector<double>({0, 0, 0, 0, 0, 0, 0, 0, 0}))); // multiple of 3 is ok
    reloadParams();
