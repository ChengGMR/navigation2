...
struct SmootherParams {
    ...
    void get(nav2::LifecycleNode* node, const std::string& name) {
        ...
        // Smoother params
        double minimum_turning_radius = node->declare_or_get_parameter(name + ".minimum_turning_radius", 0.4);
        ...
        double cost_cusp_multiplier = node->declare_or_get_parameter(local_name + "w_cost_cusp_multiplier", 3.0);
        ...
        cost_check_points = node->declare_or_get_parameter(local_name + "cost_check_points", std::vector<double>());
        if (cost_check_points.size() % 3 != 0) {
            RCLCPP_ERROR(rclcpp::get_logger("constrained_smoother"),
        ...
        path_downsampling_factor = node->declare_or_get_parameter(local_name + "path_downsampling_factor", 1);
        path_upsampling_factor = node->declare_or_get_parameter(local_name + "path_upsampling_factor", 1);
        reversing_enabled = node->declare_or_get_parameter(local_name + "reversing_enabled", true);
        keep_goal_orientation = node->declare_or_get_parameter(local_name + "keep_goal_orientation", true);
        keep_start_orientation = node->declare_or_get_parameter(local_name + "keep_start_orientation", true);
    }
...
struct OptimizerParams {
    ...
    void get(nav2::LifecycleNode* node, const std::string& name) {
        ...
        // Optimizer params
        linear_solver_type = node->declare_or_get_parameter(local_name + "linear_solver_type", std::string("SPARSE_NORMAL_CHOLESKY"));
        if (solver_types.find(linear_solver_type) == solver_types.end()) {
            ...
            RCLCPP_ERROR(rclcpp::get_logger("constrained_smoother"), "Invalid linear_solver_type. Valid values are %s",
    ...
    const std::map<std::string, ceres::LinearSolverType> solver_types = {{"DENSE_QR", ceres::DENSE_QR},
