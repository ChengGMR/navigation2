...
nav2::CallbackReturn CollisionMonitor::on_configure(const rclcpp_lifecycle::State& state) {
    ...
    cmd_vel_in_sub_ = std::make_unique<nav2_util::TwistSubscriber>(
        shared_from_this(), cmd_vel_in_topic, std::bind(&CollisionMonitor::cmdVelInCallbackUnstamped, this, std::placeholders::_1),
    ...
    if (!state_topic.empty()) {
        state_pub_ = this->create_publisher<nav2_msgs::msg::CollisionMonitorState>(state_topic);
    }

    collision_points_marker_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("~/collision_points_marker");

    // Toggle service initialization
    toggle_cm_service_ = create_service<nav2_msgs::srv::Toggle>("~/toggle", std::bind(&CollisionMonitor::toggleCMServiceCallback, this, _1, _2, _3));
...
nav2::CallbackReturn CollisionMonitor::on_activate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn CollisionMonitor::on_deactivate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn CollisionMonitor::on_cleanup(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn CollisionMonitor::on_shutdown(const rclcpp_lifecycle::State& /*state*/) {
...
void CollisionMonitor::publishVelocity(const Action& robot_action, const std_msgs::msg::Header& header) {
...
bool CollisionMonitor::getParameters(std::string& cmd_vel_in_topic, std::string& cmd_vel_out_topic, std::string& state_topic) {
    ...
    cmd_vel_in_topic = node->declare_or_get_parameter("cmd_vel_in_topic", std::string("cmd_vel_smoothed"));
    cmd_vel_out_topic = node->declare_or_get_parameter("cmd_vel_out_topic", std::string("cmd_vel"));
    ...
    transform_tolerance = tf2::durationFromSec(node->declare_or_get_parameter("transform_tolerance", 0.1));
    source_timeout = rclcpp::Duration::from_seconds(node->declare_or_get_parameter("source_timeout", 2.0));
    const bool base_shift_correction = node->declare_or_get_parameter("base_shift_correction", true);

    stop_pub_timeout_ = rclcpp::Duration::from_seconds(node->declare_or_get_parameter("stop_pub_timeout", 1.0));
...
bool CollisionMonitor::configurePolygons(const std::string& base_frame_id, const tf2::Duration& transform_tolerance) {
    try {
        ...
        std::vector<std::string> polygon_names = node->declare_or_get_parameter<std::vector<std::string>>("polygons");
        for (std::string polygon_name : polygon_names) {
            // Leave it not initialized: the will cause an error if it will not set
            const std::string polygon_type = node->declare_or_get_parameter<std::string>(polygon_name + ".type");
            ...
            if (polygon_type == "polygon") {
                ...
                polygons_.push_back(std::make_shared<VelocityPolygon>(node, polygon_name, tf_buffer_, base_frame_id, transform_tolerance));
...
bool CollisionMonitor::configureSources(const std::string& base_frame_id, const std::string& odom_frame_id, const tf2::Duration& transform_tolerance,
                                        const rclcpp::Duration& source_timeout, const bool base_shift_correction) {
    try {
        auto node = shared_from_this();

        // Leave it to be not initialized: to intentionally cause an error if it will not set
        std::vector<std::string> source_names = node->declare_or_get_parameter<std::vector<std::string>>("observation_sources");
        for (std::string source_name : source_names) {
            const std::string source_type = node->declare_or_get_parameter(source_name + ".type", std::string("scan")); // Laser scanner by default

            if (source_type == "scan") {
                std::shared_ptr<Scan> s = std::make_shared<Scan>(node, source_name, tf_buffer_, base_frame_id, odom_frame_id, transform_tolerance,
                ...
                std::shared_ptr<PointCloud> p = std::make_shared<PointCloud>(node, source_name, tf_buffer_, base_frame_id, odom_frame_id,
                ...
                std::shared_ptr<Range> r = std::make_shared<Range>(node, source_name, tf_buffer_, base_frame_id, odom_frame_id, transform_tolerance,
                                                                   source_timeout, base_shift_correction);
                ...
                std::shared_ptr<PolygonSource> ps = std::make_shared<PolygonSource>(node, source_name, tf_buffer_, base_frame_id, odom_frame_id,
                                                                                    transform_tolerance, source_timeout, base_shift_correction);
                ps->configure();

                sources_.push_back(ps);
            } else if (source_type == "costmap") {
                auto src = std::make_shared<CostmapSource>(node, source_name, tf_buffer_, base_frame_id, odom_frame_id, transform_tolerance,
                                                           source_timeout, base_shift_correction);
...
void CollisionMonitor::process(const Velocity& cmd_vel_in, const std_msgs::msg::Header& header) {
    ...
    for (std::shared_ptr<Source> source : sources_) {
        auto iter = sources_collision_points_map.insert({source->getSourceName(), std::vector<Point>()});

        if (source->getEnabled()) {
            if (!source->getData(curr_time, iter.first->second) && source->getSourceTimeout().seconds() != 0.0) {
    ...
    for (std::shared_ptr<Polygon> polygon : polygons_) {
        ...
        if (at == STOP || at == SLOWDOWN || at == LIMIT) {
            ...
            if (processStopSlowdownLimit(polygon, sources_collision_points_map, cmd_vel_in, robot_action)) {
...
void CollisionMonitor::notifyActionState(const Action& robot_action, const std::shared_ptr<Polygon> action_polygon) const {
    if (robot_action.action_type == STOP) {
        ...
        RCLCPP_INFO(get_logger(), "Robot to slowdown for %f percents due to %s polygon", action_polygon->getSlowdownRatio() * 100,
                    action_polygon->getName().c_str());
    } else if (robot_action.action_type == LIMIT) {
        RCLCPP_INFO(get_logger(), "Robot to limit speed due to %s polygon", action_polygon->getName().c_str());
    ...
    if (state_pub_) {
        std::unique_ptr<nav2_msgs::msg::CollisionMonitorState> state_msg = std::make_unique<nav2_msgs::msg::CollisionMonitorState>();
...
void CollisionMonitor::toggleCMServiceCallback(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
