...
Source::Source(const nav2::LifecycleNode::WeakPtr& node, const std::string& source_name, const std::shared_ptr<tf2_ros::Buffer> tf_buffer,
...
void Source::getCommonParameters(std::string& source_topic) {
    ...
    source_topic = node->declare_or_get_parameter(source_name_ + ".topic", std::string("scan")); // Set default topic for laser scanner

    enabled_ = node->declare_or_get_parameter(source_name_ + ".enabled", true);

    source_timeout_ = rclcpp::Duration::from_seconds(node->declare_or_get_parameter(source_name_ + ".source_timeout",
...
bool Source::getTransform(const rclcpp::Time& curr_time, const std_msgs::msg::Header& data_header, tf2::Transform& tf_transform) const {
    if (base_shift_correction_) {
        if (!nav2_util::getTransform(data_header.frame_id, data_header.stamp, base_frame_id_, curr_time, global_frame_id_, transform_tolerance_,
                                     tf_buffer_, tf_transform)) {
            return false;
        }
    } else {
        if (!nav2_util::getTransform(data_header.frame_id, base_frame_id_, transform_tolerance_, tf_buffer_, tf_transform)) {
            return false;
        }
    }
    return true;
