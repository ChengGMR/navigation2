...
Circle::Circle(const nav2::LifecycleNode::WeakPtr& node, const std::string& polygon_name, const std::shared_ptr<tf2_ros::Buffer> tf_buffer,
...
bool Circle::getParameters(std::string& polygon_sub_topic, std::string& polygon_pub_topic, std::string& footprint_topic) {
    ...
    try {
        ...
        RCLCPP_INFO(logger_, "[%s]: Polygon circle radius is not defined. Using dynamic subscription instead.", polygon_name_.c_str());
    }
    ...
    if (!getCommonParameters(polygon_sub_topic, polygon_pub_topic, footprint_topic, use_dynamic_sub)) {
        if (use_dynamic_sub && polygon_sub_topic.empty()) {
            RCLCPP_ERROR(logger_, "[%s]: Error while getting circle parameters: static radius and sub topic both not defined", polygon_name_.c_str());
        }
        ret = false;
    }
...
void Circle::createSubscription(std::string& polygon_sub_topic) {
    ...
    if (!polygon_sub_topic.empty()) {
        ...
        radius_sub_ = node->create_subscription<std_msgs::msg::Float32>(polygon_sub_topic,
                                                                        std::bind(&Circle::radiusCallback, this, std::placeholders::_1), polygon_qos);
    }
}
...
void Circle::radiusCallback(std_msgs::msg::Float32::ConstSharedPtr msg) {
    RCLCPP_INFO(logger_, "[%s]: Polygon circle radius update has been arrived", polygon_name_.c_str());
