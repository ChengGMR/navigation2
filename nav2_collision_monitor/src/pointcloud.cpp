...
PointCloud::PointCloud(const nav2::LifecycleNode::WeakPtr& node, const std::string& source_name, const std::shared_ptr<tf2_ros::Buffer> tf_buffer,
    ...
    : Source(node, source_name, tf_buffer, base_frame_id, global_frame_id, transform_tolerance, source_timeout, base_shift_correction)
    , data_(nullptr) {
...
void PointCloud::configure() {
    ...
    data_sub_ =
        pct_->subscribe(source_topic, nav2::qos::SensorDataQoS(), std::bind(&PointCloud::dataCallback, this, std::placeholders::_1), {}, &hint);
#else
    data_sub_ = node->create_subscription<sensor_msgs::msg::PointCloud2>(
        source_topic, std::bind(&PointCloud::dataCallback, this, std::placeholders::_1), nav2::qos::SensorDataQoS());
#endif

    // Add callback for dynamic parameters
    dyn_params_handler_ = node->add_on_set_parameters_callback(std::bind(&PointCloud::dynamicParametersCallback, this, std::placeholders::_1));
}
...
void PointCloud::getParameters(std::string& source_topic) {
    ...
    use_global_height_ = node->declare_or_get_parameter(source_name_ + ".use_global_height", false);
    transport_type_ = node->declare_or_get_parameter(source_name_ + ".transport_type", std::string("raw"));
}
...
rcl_interfaces::msg::SetParametersResult PointCloud::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters) {
