...
Polygon::Polygon(const nav2::LifecycleNode::WeakPtr& node, const std::string& polygon_name, const std::shared_ptr<tf2_ros::Buffer> tf_buffer,
...
bool Polygon::configure() {
    ...
    if (!footprint_topic.empty()) {
        RCLCPP_INFO(logger_, "[%s]: Making footprint subscriber on %s topic", polygon_name_.c_str(), footprint_topic.c_str());
        footprint_sub_ = std::make_unique<nav2_costmap_2d::FootprintSubscriber>(node, footprint_topic, *tf_buffer_, base_frame_id_,
    ...
    if (visualize_) {
        ...
        polygon_pub_ = node->create_publisher<geometry_msgs::msg::PolygonStamped>(polygon_pub_topic);
    }
...
void Polygon::updatePolygon(const Velocity& /*cmd_vel_in*/) {
    if (footprint_sub_ != nullptr) {
        ...
        if (!nav2_util::getTransform(polygon_.header.frame_id, base_frame_id_, transform_tolerance_, tf_buffer_, tf_transform)) {
            return;
        }
...
int Polygon::getPointsInside(const std::unordered_map<std::string, std::vector<Point>>& sources_collision_points_map) const {
    int num = 0;
...
double Polygon::getCollisionTime(const std::unordered_map<std::string, std::vector<Point>>& sources_collision_points_map,
...
bool Polygon::getCommonParameters(std::string& polygon_sub_topic, std::string& polygon_pub_topic, std::string& footprint_topic,
    ...
    try {
        ...
        const std::string at_str = node->declare_or_get_parameter<std::string>(polygon_name_ + ".action_type");
        if (at_str == "stop") {
            action_type_ = STOP;
        ...
        if (action_type_ == APPROACH) {
            time_before_collision_ = node->declare_or_get_parameter(polygon_name_ + ".time_before_collision", 2.0);
            simulation_time_step_ = node->declare_or_get_parameter(polygon_name_ + ".simulation_time_step", 0.1);
        }
        ...
        if (visualize_) {
            ...
            polygon_pub_topic = node->declare_or_get_parameter(polygon_name_ + ".polygon_pub_topic", polygon_name_);
        }

        polygon_subscribe_transient_local_ = node->declare_or_get_parameter(polygon_name_ + ".polygon_subscribe_transient_local", false);

        if (use_dynamic_sub_topic) {
            if (action_type_ != APPROACH) {
                // Get polygon sub topic
                polygon_sub_topic = node->declare_or_get_parameter<std::string>(polygon_name_ + ".polygon_sub_topic");
            } else {
                // Obtain the footprint topic to make a footprint subscription for approach polygon
                footprint_topic =
                    node->declare_or_get_parameter(polygon_name_ + ".footprint_topic", std::string("local_costmap/published_footprint"));
            }
        }

        // By default, use all observation sources for polygon
        const std::vector<std::string> observation_sources = node->declare_or_get_parameter<std::vector<std::string>>("observation_sources");
        sources_names_ = node->declare_or_get_parameter(polygon_name_ + ".sources_names", observation_sources);

        // Check the observation sources configured for polygon are defined
        for (auto source_name : sources_names_) {
            if (std::find(observation_sources.begin(), observation_sources.end(), source_name) == observation_sources.end()) {
                RCLCPP_ERROR_STREAM(logger_, "Observation source [" << source_name << "] configured for polygon [" << getName()
...
bool Polygon::getParameters(std::string& polygon_sub_topic, std::string& polygon_pub_topic, std::string& footprint_topic) {
    auto node = node_.lock();
    if (!node) {
        throw std::runtime_error{"Failed to lock node"};
    }
    ...
    try {
        ...
        RCLCPP_INFO(logger_, "[%s]: Polygon points are not defined. Using dynamic subscription instead.", polygon_name_.c_str());
    }

    if (!getCommonParameters(polygon_sub_topic, polygon_pub_topic, footprint_topic, use_dynamic_sub)) {
...
void Polygon::createSubscription(std::string& polygon_sub_topic) {
    ...
    if (!polygon_sub_topic.empty()) {
        RCLCPP_INFO(logger_, "[%s]: Subscribing on %s topic for polygon", polygon_name_.c_str(), polygon_sub_topic.c_str());
        ...
        polygon_sub_ = node->create_subscription<geometry_msgs::msg::PolygonStamped>(
            polygon_sub_topic, std::bind(&Polygon::polygonCallback, this, std::placeholders::_1), polygon_qos);
    }
}
...
void Polygon::updatePolygon(geometry_msgs::msg::PolygonStamped::ConstSharedPtr msg) {
    ...
    // Get the transform from PolygonStamped frame to base_frame_id_
    tf2::Stamped<tf2::Transform> tf_transform;
    if (!nav2_util::getTransform(msg->header.frame_id, base_frame_id_, transform_tolerance_, tf_buffer_, tf_transform)) {
        return;
    }
...
void Polygon::polygonCallback(geometry_msgs::msg::PolygonStamped::ConstSharedPtr msg) {
    RCLCPP_INFO_THROTTLE(logger_, *node_clock_, 2000, "[%s]: Polygon shape update has arrived", polygon_name_.c_str());
    updatePolygon(msg);
}
...
bool Polygon::getPolygonFromString(std::string& poly_string, std::vector<Point>& polygon) {
    ...
    if (error != "") {
        RCLCPP_ERROR(logger_, "Error parsing polygon parameter %s: '%s'", poly_string.c_str(), error.c_str());
        return false;
    }
