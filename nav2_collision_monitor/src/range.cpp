...
Range::Range(const nav2::LifecycleNode::WeakPtr& node, const std::string& source_name, const std::shared_ptr<tf2_ros::Buffer> tf_buffer,
...
void Range::configure() {
    ...
    data_sub_ = node->create_subscription<sensor_msgs::msg::Range>(source_topic, std::bind(&Range::dataCallback, this, std::placeholders::_1),
...
bool Range::getData(const rclcpp::Time& curr_time, std::vector<Point>& data) {
    ...
    if (data_->range < data_->min_range || data_->range > data_->max_range) {
        RCLCPP_DEBUG(logger_, "[%s]: Data range %fm is out of {%f..%f} sensor span. Ignoring...", source_name_.c_str(), data_->range,
                     data_->min_range, data_->max_range);
        return false;
    }
...
void Range::getParameters(std::string& source_topic) {
    ...
    obstacles_angle_ = node->declare_or_get_parameter(source_name_ + ".obstacles_angle", M_PI / 180);
}
