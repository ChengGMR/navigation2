...
CollisionDetector::CollisionDetector(const rclcpp::NodeOptions& options) : nav2::LifecycleNode("collision_detector", options) {}
...
nav2::CallbackReturn CollisionDetector::on_configure(const rclcpp_lifecycle::State& state) {
    ...
    state_pub_ = this->create_publisher<nav2_msgs::msg::CollisionDetectorState>("collision_detector_state");
...
nav2::CallbackReturn CollisionDetector::on_activate(const rclcpp_lifecycle::State& /*state*/) {
    ...
    // Creating timer
    timer_ = this->create_timer(std::chrono::duration<double>{1.0 / frequency_}, std::bind(&CollisionDetector::process, this));
...
nav2::CallbackReturn CollisionDetector::on_deactivate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn CollisionDetector::on_cleanup(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn CollisionDetector::on_shutdown(const rclcpp_lifecycle::State& /*state*/) {
...
bool CollisionDetector::configurePolygons(const std::string& base_frame_id, const tf2::Duration& transform_tolerance) {
    try {
        auto node = shared_from_this();
...
bool CollisionDetector::configureSources(const std::string& base_frame_id, const std::string& odom_frame_id, const tf2::Duration& transform_tolerance,
...
void CollisionDetector::process() {
    ...
    std::unique_ptr<nav2_msgs::msg::CollisionDetectorState> state_msg = std::make_unique<nav2_msgs::msg::CollisionDetectorState>();

    // Fill collision_points array from different data sources
    for (std::shared_ptr<Source> source : sources_) {
        if (source->getEnabled()) {
            if (!source->getData(curr_time, collision_points) && source->getSourceTimeout().seconds() != 0.0) {
