...
CostmapSource::CostmapSource(const nav2::LifecycleNode::WeakPtr& node, const std::string& source_name,
                             const std::shared_ptr<tf2_ros::Buffer> tf_buffer, const std::string& base_frame_id, const std::string& global_frame_id,
                             const tf2::Duration& transform_tolerance, const rclcpp::Duration& source_timeout, const bool base_shift_correction)
...
void CostmapSource::configure() {
    ...
    data_sub_ = node->create_subscription<nav2_msgs::msg::Costmap>(source_topic, std::bind(&CostmapSource::dataCallback, this, std::placeholders::_1),
                                                                   nav2::qos::StandardTopicQoS());
}

bool CostmapSource::getData(const rclcpp::Time& curr_time, std::vector<Point>& data) {
    auto node = node_.lock();
    if (data_ == nullptr) {
        return false;
    }
    ...
    if (src != base_frame_id_) {
        if (!getTransform(curr_time, data_->header, tf_transform)) {
            RCLCPP_WARN(logger_, "[%s] TF %s->%s unavailable at t=%.3f", source_name_.c_str(), src.c_str(), base_frame_id_.c_str(),
                        curr_time.seconds());
            return false;
        }
    }
    ...
    for (unsigned int y = 0; y < meta.size_y; ++y) {
        for (unsigned int x = 0; x < meta.size_x; ++x) {
            ...
            const bool is_obstacle = (cell_cost >= cost_threshold_ && cell_cost < nav2_costmap_2d::NO_INFORMATION)
            ...
            if (is_obstacle) {
                ...
                RCLCPP_DEBUG_THROTTLE(logger_, *node->get_clock(), 2000 /*ms*/, "[%s] Found obstacles in costmap", source_name_.c_str());
            }
        }
    }
