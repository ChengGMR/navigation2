...
class CollisionMonitor : public nav2::LifecycleNode {
   ...
   protected:
     ...
     */
    bool configureSources(const std::string& base_frame_id, const std::string& odom_frame_id, const tf2::Duration& transform_tolerance,
    ...
    bool processStopSlowdownLimit(const std::shared_ptr<Polygon> polygon,
                                  const std::unordered_map<std::string, std::vector<Point>>& sources_collision_points_map, const Velocity& velocity,
     ...
     */
    void toggleCMServiceCallback(const std::shared_ptr<rmw_request_id_t> request_header,
