...
class TestNode : public nav2::LifecycleNode {
   public:
    TestNode() : nav2::LifecycleNode("test_node") {}
    ...
    void publishPointCloud(const rclcpp::Time& stamp) {
        pointcloud_pub_ =
            this->create_publisher<sensor_msgs::msg::PointCloud2>(POINTCLOUD_TOPIC, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
        ...
        modifier.setPointCloud2Fields(3, "x", 1, sensor_msgs::msg::PointField::FLOAT32, "y", 1, sensor_msgs::msg::PointField::FLOAT32, "z", 1,
    ...
    void publishPolygon(const rclcpp::Time& stamp) {
        ...
        std::unique_ptr<geometry_msgs::msg::PolygonInstanceStamped> msg = std::make_unique<geometry_msgs::msg::PolygonInstanceStamped>();
    ...
    void publishCostmap(const rclcpp::Time& stamp, const std::string& frame_id) {
        costmap_pub_ = this->create_publisher<nav2_msgs::msg::Costmap>(COSTMAP_TOPIC, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
        costmap_pub_->on_activate();

        std::unique_ptr<nav2_msgs::msg::Costmap> msg = std::make_unique<nav2_msgs::msg::Costmap>();
   ...
   private:
    ...
    nav2::Publisher<geometry_msgs::msg::PolygonInstanceStamped>::SharedPtr polygon_pub_;
    nav2::Publisher<nav2_msgs::msg::Costmap>::SharedPtr costmap_pub_;
}; // TestNode

class ScanWrapper : public nav2_collision_monitor::Scan {
   public:
    ScanWrapper(const nav2::LifecycleNode::WeakPtr& node, const std::string& source_name, const std::shared_ptr<tf2_ros::Buffer> tf_buffer,
                const std::string& base_frame_id, const std::string& global_frame_id, const tf2::Duration& transform_tolerance,
                const rclcpp::Duration& data_timeout, const bool base_shift_correction)
        : nav2_collision_monitor::Scan(node, source_name, tf_buffer, base_frame_id, global_frame_id, transform_tolerance, data_timeout,
...
class PointCloudWrapper : public nav2_collision_monitor::PointCloud {
   public:
    PointCloudWrapper(const nav2::LifecycleNode::WeakPtr& node, const std::string& source_name, const std::shared_ptr<tf2_ros::Buffer> tf_buffer,
                      const std::string& base_frame_id, const std::string& global_frame_id, const tf2::Duration& transform_tolerance,
                      const rclcpp::Duration& data_timeout, const bool base_shift_correction)
        : nav2_collision_monitor::PointCloud(node, source_name, tf_buffer, base_frame_id, global_frame_id, transform_tolerance, data_timeout,
                                             base_shift_correction) {}
...
class RangeWrapper : public nav2_collision_monitor::Range {
   public:
    RangeWrapper(const nav2::LifecycleNode::WeakPtr& node, const std::string& source_name, const std::shared_ptr<tf2_ros::Buffer> tf_buffer,
                 const std::string& base_frame_id, const std::string& global_frame_id, const tf2::Duration& transform_tolerance,
                 const rclcpp::Duration& data_timeout, const bool base_shift_correction)
        : nav2_collision_monitor::Range(node, source_name, tf_buffer, base_frame_id, global_frame_id, transform_tolerance, data_timeout,
                                        base_shift_correction) {}
...
class PolygonWrapper : public nav2_collision_monitor::PolygonSource {
   public:
    PolygonWrapper(const nav2::LifecycleNode::WeakPtr& node, const std::string& source_name, const std::shared_ptr<tf2_ros::Buffer> tf_buffer,
                   const std::string& base_frame_id, const std::string& global_frame_id, const tf2::Duration& transform_tolerance,
                   const rclcpp::Duration& data_timeout, const bool base_shift_correction)
        : nav2_collision_monitor::PolygonSource(node, source_name, tf_buffer, base_frame_id, global_frame_id, transform_tolerance, data_timeout,
                                                base_shift_correction) {}
...
class CostmapWrapper : public nav2_collision_monitor::CostmapSource {
   public:
    CostmapWrapper(const nav2::LifecycleNode::WeakPtr& node, const std::string& source_name, const std::shared_ptr<tf2_ros::Buffer> tf_buffer,
                   const std::string& base_frame_id, const std::string& global_frame_id, const tf2::Duration& transform_tolerance,
                   const rclcpp::Duration& data_timeout, const bool base_shift_correction)
        : nav2_collision_monitor::CostmapSource(node, source_name, tf_buffer, base_frame_id, global_frame_id, transform_tolerance, data_timeout,
                                                base_shift_correction) {}

    bool dataReceived() const { return hasData(); }
}; // CostmapWrapper
...
void Tester::createSources(const bool base_shift_correction) {
    ...
    scan_ = std::make_shared<ScanWrapper>(test_node_, SCAN_NAME, tf_buffer_, BASE_FRAME_ID, GLOBAL_FRAME_ID, TRANSFORM_TOLERANCE, DATA_TIMEOUT,
                                          base_shift_correction);
    scan_->configure();

    // Create PointCloud object
    test_node_->declare_parameter(std::string(POINTCLOUD_NAME) + ".topic", rclcpp::ParameterValue(POINTCLOUD_TOPIC));
    test_node_->declare_parameter(std::string(POINTCLOUD_NAME) + ".min_height", rclcpp::ParameterValue(0.1));
    test_node_->declare_parameter(std::string(POINTCLOUD_NAME) + ".max_height", rclcpp::ParameterValue(1.0));

    pointcloud_ = std::make_shared<PointCloudWrapper>(test_node_, POINTCLOUD_NAME, tf_buffer_, BASE_FRAME_ID, GLOBAL_FRAME_ID, TRANSFORM_TOLERANCE,
                                                      DATA_TIMEOUT, base_shift_correction);
    pointcloud_->configure();

    // Create Range object
    test_node_->declare_parameter(std::string(RANGE_NAME) + ".topic", rclcpp::ParameterValue(RANGE_TOPIC));

    test_node_->declare_parameter(std::string(RANGE_NAME) + ".obstacles_angle", rclcpp::ParameterValue(M_PI / 199));

    range_ = std::make_shared<RangeWrapper>(test_node_, RANGE_NAME, tf_buffer_, BASE_FRAME_ID, GLOBAL_FRAME_ID, TRANSFORM_TOLERANCE, DATA_TIMEOUT,
                                            base_shift_correction);
    range_->configure();

    // Create Polygon object
    test_node_->declare_parameter(std::string(POLYGON_NAME) + ".topic", rclcpp::ParameterValue(POLYGON_TOPIC));

    test_node_->declare_parameter(std::string(POLYGON_NAME) + ".sampling_distance", rclcpp::ParameterValue(0.1));

    polygon_ = std::make_shared<PolygonWrapper>(test_node_, POLYGON_NAME, tf_buffer_, BASE_FRAME_ID, GLOBAL_FRAME_ID, TRANSFORM_TOLERANCE,
                                                DATA_TIMEOUT, base_shift_correction);
    polygon_->configure();

    // Create Costmap object
    test_node_->declare_parameter(std::string(COSTMAP_NAME) + ".topic", rclcpp::ParameterValue(COSTMAP_TOPIC));
    test_node_->declare_parameter(std::string(COSTMAP_NAME) + ".cost_threshold", rclcpp::ParameterValue(253));
    test_node_->declare_parameter(std::string(COSTMAP_NAME) + ".treat_unknown_as_obstacle", rclcpp::ParameterValue(true));

    costmap_ = std::make_shared<CostmapWrapper>(test_node_, COSTMAP_NAME, tf_buffer_, BASE_FRAME_ID, GLOBAL_FRAME_ID, TRANSFORM_TOLERANCE,
                                                DATA_TIMEOUT, base_shift_correction);
    costmap_->configure();
}
...
TEST_F(Tester, testPointCloudMinRange) {
    ...
    test_node_->declare_parameter(std::string(POINTCLOUD_NAME) + ".min_height", rclcpp::ParameterValue(0.1));
    test_node_->declare_parameter(std::string(POINTCLOUD_NAME) + ".max_height", rclcpp::ParameterValue(1.0));
    test_node_->declare_parameter(std::string(POINTCLOUD_NAME) + ".min_range", rclcpp::ParameterValue(0.16));
