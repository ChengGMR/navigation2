...
Tester::Tester() {
    ...
    scan_pub_ = cd_->create_publisher<sensor_msgs::msg::LaserScan>(SCAN_NAME, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
    scan_pub_->on_activate();
    pointcloud_pub_ =
        cd_->create_publisher<sensor_msgs::msg::PointCloud2>(POINTCLOUD_NAME, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
    pointcloud_pub_->on_activate();
    range_pub_ = cd_->create_publisher<sensor_msgs::msg::Range>(RANGE_NAME, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
    ...
    costmap_pub_ = cd_->create_publisher<nav2_msgs::msg::Costmap>(COSTMAP_NAME, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
    costmap_pub_->on_activate();

    state_sub_ =
        cd_->create_subscription<nav2_msgs::msg::CollisionDetectorState>(STATE_TOPIC, std::bind(&Tester::stateCallback, this, std::placeholders::_1));
...
void Tester::addPolygon(const std::string& polygon_name, const PolygonType type, const double size, const std::string& at) {
    ...
    cd_->declare_parameter(polygon_name + ".simulation_time_step", rclcpp::ParameterValue(SIMULATION_TIME_STEP));
...
void Tester::addSource(const std::string& source_name, const SourceType type) {
    if (type == SCAN) {
        ...
        cd_->declare_parameter(source_name + ".polygon_similarity_threshold", rclcpp::ParameterValue(2.0));
    } else if (type == COSTMAP) {
        cd_->declare_parameter(source_name + ".type", rclcpp::ParameterValue("costmap"));

        cd_->declare_parameter(source_name + ".cost_threshold", rclcpp::ParameterValue(253));
        cd_->declare_parameter(source_name + ".treat_unknown_as_obstacle", rclcpp::ParameterValue(true));
...
void Tester::sendTransforms(const rclcpp::Time& stamp) {
    std::shared_ptr<tf2_ros::TransformBroadcaster> tf_broadcaster = std::make_shared<tf2_ros::TransformBroadcaster>(cd_);
