...
class VelocityPolygonWrapper : public nav2_collision_monitor::VelocityPolygon {
   public:
    VelocityPolygonWrapper(const nav2::LifecycleNode::WeakPtr& node, const std::string& polygon_name,
                           const std::shared_ptr<tf2_ros::Buffer> tf_buffer, const std::string& base_frame_id,
                           const tf2::Duration& transform_tolerance)
        : nav2_collision_monitor::VelocityPolygon(node, polygon_name, tf_buffer, base_frame_id, transform_tolerance) {}
...
class Tester : public ::testing::Test {
   ...
   protected:
    ...
    void addPolygonVelocitySubPolygon(const std::string& sub_polygon_name, const double linear_min, const double linear_max, const double theta_min,
                                      const double theta_max, const double direction_end_angle, const double direction_start_angle,
...
void Tester::setCommonParameters(const std::string& polygon_name, const std::string& action_type) {
    ...
    test_node_->declare_parameter("observation_sources", rclcpp::ParameterValue(default_observation_sources));
}

void Tester::setVelocityPolygonParameters(const bool is_holonomic) {
    test_node_->declare_parameter(std::string(POLYGON_NAME) + ".holonomic", rclcpp::ParameterValue(is_holonomic));

    std::vector<std::string> velocity_polygons = {SUB_POLYGON_FORWARD_NAME, SUB_POLYGON_BACKWARD_NAME};
    ...
    if (is_holonomic) {
        ...
        //       -0.75pi   (right)    -0.25pi
        //                    |
        addPolygonVelocitySubPolygon(SUB_POLYGON_FORWARD_NAME, 0.0, 0.5, -1.0, 1.0, -M_PI_4, M_PI_4, FORWARD_POLYGON_STR, is_holonomic);
        addPolygonVelocitySubPolygon(SUB_POLYGON_BACKWARD_NAME, -0.5, 0.0, -1.0, 1.0, 0.75 * M_PI, -0.75 * M_PI, BACKWARD_POLYGON_STR, is_holonomic);
        addPolygonVelocitySubPolygon(SUB_POLYGON_LEFT_NAME, -0.5, 0.5, -1.0, 1.0, M_PI_4, 0.75 * M_PI, LEFT_POLYGON_STR, is_holonomic);
        addPolygonVelocitySubPolygon(SUB_POLYGON_RIGHT_NAME, -0.5, 0.5, -1.0, 1.0, -0.75 * M_PI, -M_PI_4, RIGHT_POLYGON_STR, is_holonomic);

        velocity_polygons = {SUB_POLYGON_FORWARD_NAME, SUB_POLYGON_BACKWARD_NAME, SUB_POLYGON_LEFT_NAME, SUB_POLYGON_RIGHT_NAME};
    } else {
        // draw forward and backward polygon
        addPolygonVelocitySubPolygon(SUB_POLYGON_FORWARD_NAME, 0.0, 0.5, -1.0, 1.0, 0.0, 0.0, FORWARD_POLYGON_STR, is_holonomic);
        addPolygonVelocitySubPolygon(SUB_POLYGON_BACKWARD_NAME, -0.5, 0.0, -1.0, 1.0, 0.0, 0.0, BACKWARD_POLYGON_STR, is_holonomic);
        velocity_polygons = {SUB_POLYGON_FORWARD_NAME, SUB_POLYGON_BACKWARD_NAME};
    }

    test_node_->declare_parameter(std::string(POLYGON_NAME) + ".velocity_polygons", rclcpp::ParameterValue(velocity_polygons));
}

void Tester::addPolygonVelocitySubPolygon(const std::string& sub_polygon_name, const double linear_min, const double linear_max,
    ...
    test_node_->declare_parameter(std::string(POLYGON_NAME) + "." + sub_polygon_name + ".points", rclcpp::ParameterValue(polygon_points));

    test_node_->declare_parameter(std::string(POLYGON_NAME) + "." + sub_polygon_name + ".linear_min", rclcpp::ParameterValue(linear_min));

    test_node_->declare_parameter(std::string(POLYGON_NAME) + "." + sub_polygon_name + ".linear_max", rclcpp::ParameterValue(linear_max));

    test_node_->declare_parameter(std::string(POLYGON_NAME) + "." + sub_polygon_name + ".theta_min", rclcpp::ParameterValue(theta_min));

    test_node_->declare_parameter(std::string(POLYGON_NAME) + "." + sub_polygon_name + ".theta_max", rclcpp::ParameterValue(theta_max));

    if (is_holonomic) {
        test_node_->declare_parameter(std::string(POLYGON_NAME) + "." + sub_polygon_name + ".direction_end_angle",
                                      rclcpp::ParameterValue(direction_end_angle));

        test_node_->declare_parameter(std::string(POLYGON_NAME) + "." + sub_polygon_name + ".direction_start_angle",
...
void Tester::createVelocityPolygon(const std::string& action_type, const bool is_holonomic) {
    ...
    velocity_polygon_ =
        std::make_shared<VelocityPolygonWrapper>(test_node_->weak_from_this(), POLYGON_NAME, tf_buffer_, BASE_FRAME_ID, TRANSFORM_TOLERANCE);
