...
class CollisionMonitorWrapper : public nav2_collision_monitor::CollisionMonitor {
   public:
    ...
    void configure() { ASSERT_EQ(on_configure(get_current_state()), nav2::CallbackReturn::SUCCESS); }

    void cant_configure() { ASSERT_EQ(on_configure(get_current_state()), nav2::CallbackReturn::FAILURE); }
...
class Tester : public ::testing::Test {
   public:
    ...
    void addPolygonVelocitySubPolygon(const std::string& polygon_name, const std::string& sub_polygon_name, const double linear_min,
    ...
    void setPolygonVelocityVectors(const std::string& polygon_name, const std::vector<std::string>& polygons);
    ...
    void publishPointCloudWithHeight(const double dist, const double height, const rclcpp::Time& stamp);
    ...
    bool waitToggle(rclcpp::Client<nav2_msgs::srv::Toggle>::SharedFuture result_future, const std::chrono::nanoseconds& timeout);
   ...
   protected:
    ...
    nav2::Publisher<geometry_msgs::msg::PolygonInstanceStamped>::SharedPtr polygon_source_pub_;
    ...
    nav2::Subscription<visualization_msgs::msg::MarkerArray>::SharedPtr collision_points_marker_sub_;
...
Tester::Tester() {
    ...
    footprint_pub_ =
        cm_->create_publisher<geometry_msgs::msg::PolygonStamped>(FOOTPRINT_TOPIC, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
    ...
    pointcloud_pub_ =
        cm_->create_publisher<sensor_msgs::msg::PointCloud2>(POINTCLOUD_NAME, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
    ...
    cmd_vel_in_pub_ =
        cm_->create_publisher<geometry_msgs::msg::Twist>(CMD_VEL_IN_TOPIC, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
    cmd_vel_in_pub_->on_activate();
    cmd_vel_out_sub_ =
        cm_->create_subscription<geometry_msgs::msg::Twist>(CMD_VEL_OUT_TOPIC, std::bind(&Tester::cmdVelOutCallback, this, std::placeholders::_1));

    action_state_sub_ = cm_->create_subscription<nav2_msgs::msg::CollisionMonitorState>(
        STATE_TOPIC, std::bind(&Tester::actionStateCallback, this, std::placeholders::_1));
    collision_points_marker_sub_ = cm_->create_subscription<visualization_msgs::msg::MarkerArray>(
        COLLISION_POINTS_MARKERS_TOPIC, std::bind(&Tester::collisionPointsMarkerCallback, this, std::placeholders::_1));
...
void Tester::addPolygon(const std::string& polygon_name, const PolygonType type, const double size, const std::string& at,
    ...
    if (type == POLYGON) {
        ...
        if (at != "approach") {
            const std::string points = "[[" + std::to_string(size) + ", " + std::to_string(size) + "], [" + std::to_string(size) + ", "
...
void Tester::addPolygonVelocitySubPolygon(const std::string& polygon_name, const std::string& sub_polygon_name, const double linear_min,
                                          const double linear_max, const double theta_min, const double theta_max, const double size) {
    const std::string points = "[[" + std::to_string(size) + ", " + std::to_string(size) + "], [" + std::to_string(size) + ", "
                               + std::to_string(-size) + "], [" + std::to_string(-size) + ", " + std::to_string(-size) + "], ["
                               + std::to_string(-size) + ", " + std::to_string(size) + "]]";
    cm_->declare_parameter(polygon_name + "." + sub_polygon_name + ".points", rclcpp::ParameterValue(points));

    cm_->declare_parameter(polygon_name + "." + sub_polygon_name + ".linear_min", rclcpp::ParameterValue(linear_min));

    cm_->declare_parameter(polygon_name + "." + sub_polygon_name + ".linear_max", rclcpp::ParameterValue(linear_max));

    cm_->declare_parameter(polygon_name + "." + sub_polygon_name + ".theta_min", rclcpp::ParameterValue(theta_min));

    cm_->declare_parameter(polygon_name + "." + sub_polygon_name + ".theta_max", rclcpp::ParameterValue(theta_max));
}
...
void Tester::addSource(const std::string& source_name, const SourceType type) {
    if (type == SCAN) {
        ...
        cm_->declare_parameter(source_name + ".polygon_similarity_threshold", rclcpp::ParameterValue(2.0));
...
void Tester::setPolygonVelocityVectors(const std::string& polygon_name, const std::vector<std::string>& polygons) {
...
void Tester::setGlobalHeightParams(const std::string& source_name, const double min_height) {
    cm_->declare_or_get_parameter(source_name + ".use_global_height", true);
    cm_->set_parameter(rclcpp::Parameter(source_name + ".use_global_height", true));

    cm_->declare_or_get_parameter(source_name + ".min_height", min_height);
...
void Tester::publishPointCloudWithHeight(const double dist, const double height, const rclcpp::Time& stamp) {
    std::unique_ptr<sensor_msgs::msg::PointCloud2> msg = std::make_unique<sensor_msgs::msg::PointCloud2>();
    sensor_msgs::PointCloud2Modifier modifier(*msg);
    ...
    modifier.setPointCloud2Fields(4, "x", 1, sensor_msgs::msg::PointField::FLOAT32, "y", 1, sensor_msgs::msg::PointField::FLOAT32, "z", 1,
                                  sensor_msgs::msg::PointField::FLOAT32, "height", 1, sensor_msgs::msg::PointField::FLOAT32);
    modifier.resize(2);
...
bool Tester::waitToggle(rclcpp::Client<nav2_msgs::srv::Toggle>::SharedFuture result_future, const std::chrono::nanoseconds& timeout) {
    rclcpp::Time start_time = cm_->now();
    while (rclcpp::ok() && cm_->now() - start_time <= rclcpp::Duration(timeout)) {
        std::future_status status = result_future.wait_for(10ms);
        if (status == std::future_status::ready) {
