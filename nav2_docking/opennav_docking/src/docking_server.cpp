...
DockingServer::DockingServer(const rclcpp::NodeOptions& options) : nav2::LifecycleNode("docking_server", "", options) {
...
nav2::CallbackReturn DockingServer::on_configure(const rclcpp_lifecycle::State& state) {
    ...
    docking_action_server_ = node->create_action_server<DockRobot>("dock_robot", std::bind(&DockingServer::dockRobot, this), nullptr,
                                                                   std::chrono::milliseconds(500), true);

    undocking_action_server_ = node->create_action_server<UndockRobot>("undock_robot", std::bind(&DockingServer::undockRobot, this), nullptr,
                                                                       std::chrono::milliseconds(500), true);
...
nav2::CallbackReturn DockingServer::on_activate(const rclcpp_lifecycle::State& /*state*/) {
    ...
    dyn_params_handler_ = node->add_on_set_parameters_callback(std::bind(&DockingServer::dynamicParametersCallback, this, _1));
...
nav2::CallbackReturn DockingServer::on_deactivate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn DockingServer::on_cleanup(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn DockingServer::on_shutdown(const rclcpp_lifecycle::State&) {
    RCLCPP_INFO(get_logger(), "Shutting down %s", get_name());
    return nav2::CallbackReturn::SUCCESS;
}

template <typename ActionT>
void DockingServer::getPreemptedGoalIfRequested(typename std::shared_ptr<const typename ActionT::Goal> goal,
...
template <typename ActionT>
bool DockingServer::checkAndWarnIfCancelled(typename nav2::SimpleActionServer<ActionT>::SharedPtr& action_server, const std::string& name) {
...
template <typename ActionT>
bool DockingServer::checkAndWarnIfPreempted(typename nav2::SimpleActionServer<ActionT>::SharedPtr& action_server, const std::string& name) {
    if (action_server->is_preempt_requested()) {
...
void DockingServer::dockRobot() {
    ...
    try {
        ...
        if (goal->use_dock_id) {
            RCLCPP_INFO(get_logger(), "Attempting to dock robot at %s.", goal->dock_id.c_str());
            dock = dock_db_->findDock(goal->dock_id);
        } else {
            RCLCPP_INFO(get_logger(), "Attempting to dock robot at position (%0.2f, %0.2f).", goal->dock_pose.pose.position.x,
                        goal->dock_pose.pose.position.y);
        ...
        if (dock->plugin->isCharger() && (dock->plugin->isDocked() || dock->plugin->isCharging())) {
            RCLCPP_INFO(get_logger(), "Robot is already docked and/or charging (if applicable), no need to dock");
            result->success = true;
        ...
        if (!goal->navigate_to_staging_pose || utils::l2Norm(robot_pose.pose, initial_staging_pose.pose) < dock_prestaging_tolerance_) {
            ...
            navigator_->goToPose(initial_staging_pose, rclcpp::Duration::from_seconds(goal->max_staging_time), isPreempted);
        ...
        bool dock_backward = dock_backwards_.has_value() ? dock_backwards_.value()
        ...
        if (dock->plugin->shouldRotateToDock()) {
            staging_pose.pose.orientation = nav2_util::geometry_utils::orientationAroundZAxis(tf2::getYaw(staging_pose.pose.orientation) + M_PI);
        }
        ...
        while (rclcpp::ok()) {
            try {
                ...
                if (approachDock(dock, dock_pose, dock_backward)) {
                    ...
                    RCLCPP_INFO(get_logger(), "Made contact with dock, waiting for charge to start (if applicable).");
                    publishZeroVelocity();
...
void DockingServer::doInitialPerception(Dock* dock, geometry_msgs::msg::PoseStamped& dock_pose) {
    ...
    while (!dock->plugin->getRefinedPose(dock_pose, dock->id)) {
        if (this->now() - start > timeout) {
            throw opennav_docking_core::FailedToDetectDock("Failed initial dock detection: Timeout exceeded");
        }
...
void DockingServer::rotateToDock(const geometry_msgs::msg::PoseStamped& dock_pose) {
    ...
    target_pose.pose.orientation = nav2_util::geometry_utils::orientationAroundZAxis(tf2::getYaw(target_pose.pose.orientation) + M_PI);
    ...
    while (rclcpp::ok()) {
        ...
        auto angular_distance_to_heading =
            angles::shortest_angular_distance(tf2::getYaw(robot_pose.pose.orientation), tf2::getYaw(target_pose.pose.orientation));
        ...
        command->twist = controller_->computeRotateToHeadingCommand(angular_distance_to_heading, current_vel->twist, dt);
...
bool DockingServer::approachDock(Dock* dock, geometry_msgs::msg::PoseStamped& dock_pose, bool backward) {
    rclcpp::Rate loop_rate(controller_frequency_);
    auto start = this->now();
    ...
    while (rclcpp::ok()) {
        ...
        if (this->now() - start > timeout) {
            throw opennav_docking_core::FailedToControl("Timed out approaching dock; dock nor charging (if applicable) detected");
        }
...
bool DockingServer::resetApproach(const geometry_msgs::msg::PoseStamped& staging_pose, bool backward) {
    rclcpp::Rate loop_rate(controller_frequency_);
    auto start = this->now();
    auto timeout = rclcpp::Duration::from_seconds(dock_approach_timeout_);
    while (rclcpp::ok()) {
        ...
        auto command = std::make_unique<geometry_msgs::msg::TwistStamped>();
        command->header.stamp = now();
        if (getCommandToPose(command->twist, staging_pose, undock_linear_tolerance_, undock_angular_tolerance_, false, !backward)) {
            return true;
        }
        vel_publisher_->publish(std::move(command));
...
bool DockingServer::getCommandToPose(geometry_msgs::msg::Twist& cmd, const geometry_msgs::msg::PoseStamped& pose, double linear_tolerance,
                                     double angular_tolerance, bool is_docking, bool backward) {
    ...
    const double dist = std::hypot(robot_pose.pose.position.x - pose.pose.position.x, robot_pose.pose.position.y - pose.pose.position.y);
    const double yaw = angles::shortest_angular_distance(tf2::getYaw(robot_pose.pose.orientation), tf2::getYaw(pose.pose.orientation));
...
void DockingServer::undockRobot() {
    ...
    try {
        ...
        RCLCPP_INFO(get_logger(), "Attempting to undock robot of dock type %s.", dock->getName().c_str());
        ...
        bool dock_backward =
            dock_backwards_.has_value() ? dock_backwards_.value() : (dock->getDockDirection() == opennav_docking_core::DockDirection::BACKWARD);
        ...
        if (dock_backward) {
            dock_pose.pose.orientation = nav2_util::geometry_utils::orientationAroundZAxis(tf2::getYaw(dock_pose.pose.orientation) + M_PI);
        }
        ...
        while (rclcpp::ok()) {
            ...
            if (getCommandToPose(command->twist, staging_pose, undock_linear_tolerance_, undock_angular_tolerance_, false, !dock_backward)) {
...
rcl_interfaces::msg::SetParametersResult DockingServer::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters) {
    std::lock_guard<std::mutex> lock(*mutex_);
