...
Navigator::Navigator(const nav2::LifecycleNode::WeakPtr& parent) : node_(parent) {
    auto node = node_.lock();
    nav2::declare_parameter_if_not_declared(node, "navigator_bt_xml", rclcpp::ParameterValue(std::string("")));
...
void Navigator::activate() {
    ...
    nav_to_pose_client_ = node->create_action_client<Nav2Pose>("navigate_to_pose", callback_group_);
}
...
void Navigator::goToPose(const geometry_msgs::msg::PoseStamped& pose, rclcpp::Duration remaining_staging_duration, std::function<bool()> isPreempted,
                         bool recursed) {
    auto node = node_.lock();
    ...
    if (executor_.spin_until_future_complete(future_goal_handle, 2s) == rclcpp::FutureReturnCode::SUCCESS) {
        ...
        while (rclcpp::ok()) {
            ...
            if (executor_.spin_until_future_complete(future_result, 10ms) == rclcpp::FutureReturnCode::SUCCESS) {
