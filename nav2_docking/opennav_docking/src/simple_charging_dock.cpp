...
void SimpleChargingDock::configure(const nav2::LifecycleNode::WeakPtr& parent, const std::string& name, std::shared_ptr<tf2_ros::Buffer> tf) {
    name_ = name;
    tf2_buffer_ = tf;
    is_charging_ = false;
    node_ = parent.lock();
    ...
    nav2::declare_parameter_if_not_declared(node_, name + ".use_battery_status", rclcpp::ParameterValue(true));

    // Parameters for optional detector control
    nav2::declare_parameter_if_not_declared(node_, name + ".detector_service_name", rclcpp::ParameterValue(""));
    nav2::declare_parameter_if_not_declared(node_, name + ".detector_service_timeout", rclcpp::ParameterValue(5.0));
    nav2::declare_parameter_if_not_declared(node_, name + ".subscribe_toggle", rclcpp::ParameterValue(false));

    // Parameters for optional external detection of dock pose
    nav2::declare_parameter_if_not_declared(node_, name + ".use_external_detection_pose", rclcpp::ParameterValue(false));
    nav2::declare_parameter_if_not_declared(node_, name + ".external_detection_timeout", rclcpp::ParameterValue(1.0));
    nav2::declare_parameter_if_not_declared(node_, name + ".external_detection_translation_x", rclcpp::ParameterValue(-0.20));
    nav2::declare_parameter_if_not_declared(node_, name + ".external_detection_translation_y", rclcpp::ParameterValue(0.0));
    nav2::declare_parameter_if_not_declared(node_, name + ".external_detection_rotation_yaw", rclcpp::ParameterValue(0.0));
    nav2::declare_parameter_if_not_declared(node_, name + ".external_detection_rotation_pitch", rclcpp::ParameterValue(1.57));
    nav2::declare_parameter_if_not_declared(node_, name + ".external_detection_rotation_roll", rclcpp::ParameterValue(-1.57));
    nav2::declare_parameter_if_not_declared(node_, name + ".filter_coef", rclcpp::ParameterValue(0.1));

    // Charging threshold from BatteryState message
    nav2::declare_parameter_if_not_declared(node_, name + ".charging_threshold", rclcpp::ParameterValue(0.5));

    // Optionally determine if docked via stall detection using joint_states
    nav2::declare_parameter_if_not_declared(node_, name + ".use_stall_detection", rclcpp::ParameterValue(false));
    nav2::declare_parameter_if_not_declared(node_, name + ".stall_joint_names", rclcpp::PARAMETER_STRING_ARRAY);
    nav2::declare_parameter_if_not_declared(node_, name + ".stall_velocity_threshold", rclcpp::ParameterValue(1.0));
    nav2::declare_parameter_if_not_declared(node_, name + ".stall_effort_threshold", rclcpp::ParameterValue(1.0));

    // If not using stall detection, this is how close robot should get to pose
    nav2::declare_parameter_if_not_declared(node_, name + ".docking_threshold", rclcpp::ParameterValue(0.05));

    // Staging pose configuration
    nav2::declare_parameter_if_not_declared(node_, name + ".staging_x_offset", rclcpp::ParameterValue(-0.7));
    nav2::declare_parameter_if_not_declared(node_, name + ".staging_yaw_offset", rclcpp::ParameterValue(0.0));

    // Direction of docking and if we should rotate to dock
    nav2::declare_parameter_if_not_declared(node_, name + ".dock_direction", rclcpp::ParameterValue(std::string("forward")));
    nav2::declare_parameter_if_not_declared(node_, name + ".rotate_to_dock", rclcpp::ParameterValue(false));
    ...
    if (!detector_service_name_.empty()) {
        detector_client_ = node_->create_client<std_srvs::srv::Trigger>(detector_service_name_, false);
    }

    if (use_battery_status_) {
        battery_sub_ = node_->create_subscription<sensor_msgs::msg::BatteryState>(
            "battery_state", [this](const sensor_msgs::msg::BatteryState::SharedPtr state) { is_charging_ = state->current > charging_threshold_; });
    }
    ...
    if (use_stall_detection) {
        ...
        joint_state_sub_ = node_->create_subscription<sensor_msgs::msg::JointState>(
            "joint_states", std::bind(&SimpleChargingDock::jointStateCallback, this, std::placeholders::_1), nav2::qos::StandardTopicQoS());
    }

    dock_pose_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>("dock_pose");
    filtered_dock_pose_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>("filtered_dock_pose");
    staging_pose_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>("staging_pose");
}

geometry_msgs::msg::PoseStamped SimpleChargingDock::getStagingPose(const geometry_msgs::msg::Pose& pose, const std::string& frame) {
...
bool SimpleChargingDock::getRefinedPose(geometry_msgs::msg::PoseStamped& pose, std::string /*id*/) {
    ...
    if (detected.header.frame_id != pose.header.frame_id) {
        try {
            if (!tf2_buffer_->canTransform(pose.header.frame_id, detected.header.frame_id, detected.header.stamp,
...
bool SimpleChargingDock::isDocked() {
    ...
    double d = std::hypot(base_pose.pose.position.x - dock_pose_.pose.position.x, base_pose.pose.position.y - dock_pose_.pose.position.y);
    ...
    if (detector_client_) {
        ...
        try {
            auto future = detector_client_->invoke(
                req, std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::duration<double>(detector_service_timeout_)));

            if (!future || !future->success) {
                RCLCPP_ERROR(node_->get_logger(), "Detector service '%s' failed to start.", detector_service_name_.c_str());
                return false;
            }
        } catch (const std::exception& e) {
            RCLCPP_ERROR(node_->get_logger(), "Calling detector service '%s' failed: %s", detector_service_name_.c_str(), e.what());
            return false;
        }
    }
...
bool SimpleChargingDock::stopDetectionProcess() {
    ...
    if (detector_client_) {
        ...
        try {
            ...
            if (!future || !future->success) {
                RCLCPP_ERROR(node_->get_logger(), "Detector service '%s' failed to stop.", detector_service_name_.c_str());
                return false;
            }
        } catch (const std::exception& e) {
            RCLCPP_ERROR(node_->get_logger(), "Calling detector service '%s' failed: %s", detector_service_name_.c_str(), e.what());
