...
void SimpleNonChargingDock::configure(const nav2::LifecycleNode::WeakPtr& parent, const std::string& name, std::shared_ptr<tf2_ros::Buffer> tf) {
    name_ = name;
    tf2_buffer_ = tf;
    node_ = parent.lock();
    if (!node_) {
    ...
    if (use_stall_detection) {
        ...
        joint_state_sub_ = node_->create_subscription<sensor_msgs::msg::JointState>(
            "joint_states", std::bind(&SimpleNonChargingDock::jointStateCallback, this, std::placeholders::_1), nav2::qos::StandardTopicQoS());
    }
...
geometry_msgs::msg::PoseStamped SimpleNonChargingDock::getStagingPose(const geometry_msgs::msg::Pose& pose, const std::string& frame) {
