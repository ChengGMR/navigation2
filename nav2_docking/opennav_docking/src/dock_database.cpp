...
DockDatabase::DockDatabase(std::shared_ptr<std::mutex> mutex)
    : mutex_(mutex), dock_loader_("opennav_docking_core", "opennav_docking_core::ChargingDock") {}
...
bool DockDatabase::initialize(const nav2::LifecycleNode::WeakPtr& parent, std::shared_ptr<tf2_ros::Buffer> tf) {
    node_ = parent;
    auto node = node_.lock();

    if (!getDockPlugins(node, tf)) {
        RCLCPP_ERROR(node->get_logger(), "An error occurred while getting the dock plugins!");
        return false;
    }

    if (!getDockInstances(node)) {
        RCLCPP_ERROR(node->get_logger(), "An error occurred while getting the dock instances!");
        return false;
    }

    RCLCPP_INFO(node->get_logger(), "Docking Server has %u dock types and %u dock instances available.", this->plugin_size(), this->instance_size());

    reload_db_service_ = node->create_service<nav2_msgs::srv::ReloadDockDatabase>(
        "~/reload_database", std::bind(&DockDatabase::reloadDbCb, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
void DockDatabase::reloadDbCb(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
    ...
    if (utils::parseDockFile(request->filepath, node, dock_instances)) {
        ...
        RCLCPP_INFO(node->get_logger(), "Dock database reloaded from file %s.", request->filepath.c_str());
        mutex_->unlock();
        return;
    }
    response->success = false;
...
bool DockDatabase::getDockPlugins(const nav2::LifecycleNode::SharedPtr& node, std::shared_ptr<tf2_ros::Buffer> tf) {
    ...
    for (size_t i = 0; i != docks_plugins.size(); i++) {
        try {
            std::string plugin_type = nav2::get_plugin_type_param(node, docks_plugins[i]);
            opennav_docking_core::ChargingDock::Ptr dock = dock_loader_.createUniqueInstance(plugin_type);
            RCLCPP_INFO(node->get_logger(), "Created charging dock plugin %s of type %s", docks_plugins[i].c_str(), plugin_type.c_str());
            dock->configure(node, docks_plugins[i], tf);
            dock_plugins_.insert({docks_plugins[i], dock});
        } catch (const std::exception& ex) {
            RCLCPP_FATAL(node->get_logger(), "Failed to create Charging Dock plugin. Exception: %s", ex.what());
            return false;
        }
    }
...
bool DockDatabase::getDockInstances(const nav2::LifecycleNode::SharedPtr& node) {
    ...
    if (node->get_parameter("dock_database", dock_filepath)) {
        RCLCPP_INFO(node->get_logger(), "Loading dock from database file  %s.", dock_filepath.c_str());
        try {
            return utils::parseDockFile(dock_filepath, node, dock_instances_);
        } catch (YAML::BadConversion& e) {
            RCLCPP_ERROR(node->get_logger(), "Dock database (%s) is malformed: %s.", dock_filepath.c_str(), e.what());
            return false;
        }
        return true;
    }
