...
Controller::Controller(const nav2::LifecycleNode::SharedPtr& node, std::shared_ptr<tf2_ros::Buffer> tf, std::string fixed_frame,
                       std::string base_frame)
    : tf2_buffer_(tf), fixed_frame_(fixed_frame), base_frame_(base_frame) {
    logger_ = node->get_logger();
    clock_ = node->get_clock();

    nav2::declare_parameter_if_not_declared(node, "controller.k_phi", rclcpp::ParameterValue(3.0));
    nav2::declare_parameter_if_not_declared(node, "controller.k_delta", rclcpp::ParameterValue(2.0));
    nav2::declare_parameter_if_not_declared(node, "controller.beta", rclcpp::ParameterValue(0.4));
    nav2::declare_parameter_if_not_declared(node, "controller.lambda", rclcpp::ParameterValue(2.0));
    nav2::declare_parameter_if_not_declared(node, "controller.v_linear_min", rclcpp::ParameterValue(0.1));
    nav2::declare_parameter_if_not_declared(node, "controller.v_linear_max", rclcpp::ParameterValue(0.25));
    nav2::declare_parameter_if_not_declared(node, "controller.v_angular_max", rclcpp::ParameterValue(0.75));
    nav2::declare_parameter_if_not_declared(node, "controller.slowdown_radius", rclcpp::ParameterValue(0.25));
    nav2::declare_parameter_if_not_declared(node, "controller.rotate_to_heading_angular_vel", rclcpp::ParameterValue(1.0));
    nav2::declare_parameter_if_not_declared(node, "controller.rotate_to_heading_max_angular_accel", rclcpp::ParameterValue(3.2));
    nav2::declare_parameter_if_not_declared(node, "controller.use_collision_detection", rclcpp::ParameterValue(true));
    nav2::declare_parameter_if_not_declared(node, "controller.costmap_topic", rclcpp::ParameterValue(std::string("local_costmap/costmap_raw")));
    nav2::declare_parameter_if_not_declared(node, "controller.footprint_topic",
                                            rclcpp::ParameterValue(std::string("local_costmap/published_footprint")));
    nav2::declare_parameter_if_not_declared(node, "controller.transform_tolerance", rclcpp::ParameterValue(0.1));
    nav2::declare_parameter_if_not_declared(node, "controller.projection_time", rclcpp::ParameterValue(5.0));
    nav2::declare_parameter_if_not_declared(node, "controller.simulation_time_step", rclcpp::ParameterValue(0.1));
    nav2::declare_parameter_if_not_declared(node, "controller.dock_collision_threshold", rclcpp::ParameterValue(0.3));
    ...
    control_law_ = std::make_unique<nav2_graceful_controller::SmoothControlLaw>(k_phi_, k_delta_, beta_, lambda_, slowdown_radius_, v_linear_min_,
                                                                                v_linear_max_, v_angular_max_);

    // Add callback for dynamic parameters
    dyn_params_handler_ = node->add_on_set_parameters_callback(std::bind(&Controller::dynamicParametersCallback, this, std::placeholders::_1));
...
bool Controller::computeVelocityCommand(const geometry_msgs::msg::Pose& pose, geometry_msgs::msg::Twist& cmd, bool is_docking, bool backward) {
...
geometry_msgs::msg::Twist Controller::computeRotateToHeadingCommand(const double& angular_distance_to_heading,
                                                                    const geometry_msgs::msg::Twist& current_velocity, const double& dt) {
    ...
    cmd_vel.angular.z = std::clamp(angular_vel, min_feasible_angular_speed, max_feasible_angular_speed);

    // Check if we need to slow down to avoid overshooting
    double max_vel_to_stop = std::sqrt(2 * rotate_to_heading_max_angular_accel_ * fabs(angular_distance_to_heading));
...
bool Controller::isTrajectoryCollisionFree(const geometry_msgs::msg::Pose& target_pose, bool is_docking, bool backward) {
    ...
    try {
        base_to_fixed_transform =
            tf2_buffer_->lookupTransform(fixed_frame_, base_frame_, trajectory.header.stamp, tf2::durationFromSec(transform_tolerance_));
    } catch (tf2::TransformException& ex) {
        RCLCPP_ERROR(logger_, "Could not get transform from %s to %s: %s", base_frame_.c_str(), fixed_frame_.c_str(), ex.what());
        return false;
    }
    ...
    do {
        ...
        next_pose.pose = control_law_->calculateNextPose(simulation_time_step_, target_pose, next_pose.pose, backward);
        ...
        double dock_collision_distance = is_docking ? nav2_util::geometry_utils::euclidean_distance(target_pose, next_pose.pose)
        ...
        if (use_collision_detection_ && dock_collision_distance > dock_collision_threshold_
            && !collision_checker_->isCollisionFree(local_pose.pose)) {
            RCLCPP_WARN(logger_, "Collision detected at pose: (%.2f, %.2f, %.2f) in frame %s", local_pose.pose.position.x, local_pose.pose.position.y,
                        local_pose.pose.position.z, local_pose.header.frame_id.c_str());
...
void Controller::configureCollisionChecker(const nav2::LifecycleNode::SharedPtr& node, std::string costmap_topic, std::string footprint_topic,
                                           double transform_tolerance) {
    costmap_sub_ = std::make_unique<nav2_costmap_2d::CostmapSubscriber>(node, costmap_topic);
    footprint_sub_ = std::make_unique<nav2_costmap_2d::FootprintSubscriber>(node, footprint_topic, *tf2_buffer_, base_frame_, transform_tolerance);
    collision_checker_ = std::make_shared<nav2_costmap_2d::CostmapTopicCollisionChecker>(*costmap_sub_, *footprint_sub_, node->get_name());
}

rcl_interfaces::msg::SetParametersResult Controller::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters) {
    std::lock_guard<std::mutex> lock(dynamic_params_lock_);
