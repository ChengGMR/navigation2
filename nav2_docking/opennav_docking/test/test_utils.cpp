...
TEST(UtilsTests, parseDockFile) {
    ...
    DockMap db;
    std::string filepath = ament_index_cpp::get_package_share_directory("opennav_docking") + "/dock_files/test_dock_file.yaml";
...
TEST(UtilsTests, parseDockFile2) {
    ...
    std::string filepath = ament_index_cpp::get_package_share_directory("opennav_docking") + "/dock_files/test_no_docks_file.yaml";
    EXPECT_FALSE(utils::parseDockFile(filepath, node, db));

    // Test with a file that has no type
    filepath = ament_index_cpp::get_package_share_directory("opennav_docking") + "/dock_files/test_dock_no_type_file.yaml";
    EXPECT_FALSE(utils::parseDockFile(filepath, node, db));

    // Test with a file that has no pose
    filepath = ament_index_cpp::get_package_share_directory("opennav_docking") + "/dock_files/test_dock_no_pose_file.yaml";
    EXPECT_FALSE(utils::parseDockFile(filepath, node, db));

    // Test with a file that has wring pose array size
    filepath = ament_index_cpp::get_package_share_directory("opennav_docking") + "/dock_files/test_dock_bad_pose_file.yaml";
    EXPECT_FALSE(utils::parseDockFile(filepath, node, db));
}
