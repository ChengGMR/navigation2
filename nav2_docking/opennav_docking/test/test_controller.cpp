...
class ControllerFixture : public opennav_docking::Controller {
   public:
    ControllerFixture(const nav2::LifecycleNode::SharedPtr& node, std::shared_ptr<tf2_ros::Buffer> tf, std::string fixed_frame,
    ...
    bool isTrajectoryCollisionFree(const geometry_msgs::msg::Pose& target_pose, bool is_docking, bool backward = false) {
        return opennav_docking::Controller::isTrajectoryCollisionFree(target_pose, is_docking, backward);
    }

    void setCollisionTolerance(double tolerance) { dock_collision_threshold_ = tolerance; }
};

class TestCollisionChecker : public nav2::LifecycleNode {
   public:
    explicit TestCollisionChecker(std::string name) : LifecycleNode(name) {}
    ...
    nav2::CallbackReturn on_configure(const rclcpp_lifecycle::State& /*state*/) {
        ...
        footprint_pub_ =
            create_publisher<geometry_msgs::msg::PolygonStamped>("test_footprint", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
        footprint_pub_->on_activate();
        costmap_pub_ =
            std::make_shared<nav2_costmap_2d::Costmap2DPublisher>(shared_from_this(), costmap_.get(), "test_base_frame", "test_costmap", true);
    ...
    void publishFootprint(const double radius, const double center_x, const double center_y, std::string base_frame, const rclcpp::Time& stamp) {
    ...
    void publishCostmap() { costmap_pub_->publishCostmap(); }
    ...
    void setRectangle(double width, double height, double center_x, double center_y, unsigned char cost) {
        unsigned int mx, my;
...
TEST(ControllerTests, ObjectLifecycle) {
    ...
    nav2::declare_parameter_if_not_declared(node, "controller.use_collision_detection", rclcpp::ParameterValue(false));

    auto controller = std::make_unique<opennav_docking::Controller>(node, tf, "test_base_frame", "test_base_frame");
...
TEST(ControllerTests, DynamicParameters) {
    auto node = std::make_shared<nav2::LifecycleNode>("test");
    auto controller = std::make_unique<opennav_docking::Controller>(node, nullptr, "test_base_frame", "test_base_frame");
    ...
    auto results = params->set_parameters_atomically(
        {rclcpp::Parameter("controller.k_phi", 1.0), rclcpp::Parameter("controller.k_delta", 2.0), rclcpp::Parameter("controller.beta", 3.0),
         rclcpp::Parameter("controller.lambda", 4.0), rclcpp::Parameter("controller.v_linear_min", 5.0),
         rclcpp::Parameter("controller.v_linear_max", 6.0), rclcpp::Parameter("controller.v_angular_max", 7.0),
         rclcpp::Parameter("controller.slowdown_radius", 8.0), rclcpp::Parameter("controller.projection_time", 9.0),
         rclcpp::Parameter("controller.simulation_time_step", 10.0), rclcpp::Parameter("controller.dock_collision_threshold", 11.0),
    ...
    EXPECT_EQ(node->get_parameter("controller.rotate_to_heading_max_angular_accel").as_double(), 13.0);
}
...
TEST(ControllerTests, TFException) {
    ...
    auto controller = std::make_unique<opennav_docking::ControllerFixture>(node, tf, "test_fixed_frame", "test_base_frame");
...
TEST(ControllerTests, CollisionCheckerDockForward) {
    ...
    nav2::declare_parameter_if_not_declared(node, "controller.footprint_topic", rclcpp::ParameterValue("test_footprint"));
    nav2::declare_parameter_if_not_declared(node, "controller.costmap_topic", rclcpp::ParameterValue("test_costmap_raw"));
    nav2::declare_parameter_if_not_declared(node, "controller.projection_time", rclcpp::ParameterValue(10.0));
    ...
    auto controller = std::make_unique<opennav_docking::ControllerFixture>(node, tf, "test_base_frame", "test_base_frame");
...
TEST(ControllerTests, RotateToHeading) {
    ...
    nav2::declare_parameter_if_not_declared(node, "controller.rotate_to_heading_angular_vel", rclcpp::ParameterValue(rotate_to_heading_angular_vel));
    nav2::declare_parameter_if_not_declared(node, "controller.rotate_to_heading_max_angular_accel",
    ...
    auto cmd_vel = controller->computeRotateToHeadingCommand(angular_distance_to_heading, current_velocity, dt);
    ...
    angular_distance_to_heading = -0.5;
    current_velocity.angular.z = -0.1;
    cmd_vel = controller->computeRotateToHeadingCommand(angular_distance_to_heading, current_velocity, dt);
    EXPECT_DOUBLE_EQ(cmd_vel.linear.x, 0.0);
    ...
    cmd_vel = controller->computeRotateToHeadingCommand(angular_distance_to_heading, current_velocity, dt);
    EXPECT_DOUBLE_EQ(cmd_vel.linear.x, 0.0);
    EXPECT_DOUBLE_EQ(cmd_vel.angular.z, current_velocity.angular.z + rotate_to_heading_max_angular_accel * dt);
