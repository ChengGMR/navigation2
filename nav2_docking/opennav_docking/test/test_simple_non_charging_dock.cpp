...
class SimpleNonChargingDockShim : public opennav_docking::SimpleNonChargingDock {
   public:
    SimpleNonChargingDockShim() : opennav_docking::SimpleNonChargingDock() {}

    std::vector<std::string> getStallJointNames() { return stall_joint_names_; }
};
...
TEST(SimpleNonChargingDockTests, StallDetection) {
    auto node = std::make_shared<nav2::LifecycleNode>("test");
    auto pub = node->create_publisher<sensor_msgs::msg::JointState>("joint_states", rclcpp::QoS(1));
    pub->on_activate();
    ...
    node->set_parameter(rclcpp::Parameter("my_dock.stall_joint_names", rclcpp::ParameterValue(names)));
    dock->configure(node, "my_dock", nullptr);
...
TEST(SimpleNonChargingDockTests, RefinedPoseTest) {
    auto node = std::make_shared<nav2::LifecycleNode>("test");
    node->declare_parameter("my_dock.use_external_detection_pose", rclcpp::ParameterValue(true));
    auto pub = node->create_publisher<geometry_msgs::msg::PoseStamped>("detected_dock_pose", rclcpp::QoS(1));
    pub->on_activate();
    auto dock = std::make_unique<opennav_docking::SimpleNonChargingDock>();
...
TEST(SimpleNonChargingDockTests, GetDockDirection) {
    ...
    // Now set to BACKWARD
    node->set_parameter(rclcpp::Parameter("my_dock.dock_direction", rclcpp::ParameterValue("backward")));
    EXPECT_NO_THROW(dock->configure(node, "my_dock", nullptr));
    EXPECT_EQ(dock->getDockDirection(), opennav_docking_core::DockDirection::BACKWARD);
    dock->cleanup();

    // Now set to UNKNOWN
    node->set_parameter(rclcpp::Parameter("my_dock.dock_direction", rclcpp::ParameterValue("other")));
...
TEST(SimpleChargingDockTests, ShouldRotateToDock) {
    ...
    node->set_parameter(rclcpp::Parameter("my_dock.rotate_to_dock", rclcpp::ParameterValue(false)));
    EXPECT_NO_THROW(dock->configure(node, "my_dock", nullptr));
    EXPECT_EQ(dock->shouldRotateToDock(), false);

    // Case 3: Direction to FORWARD and rotate_to_dock to true
    node->set_parameter(rclcpp::Parameter("my_dock.dock_direction", rclcpp::ParameterValue("forward")));
    node->set_parameter(rclcpp::Parameter("my_dock.rotate_to_dock", rclcpp::ParameterValue(true)));
    EXPECT_THROW(dock->configure(node, "my_dock", nullptr), std::runtime_error);
    EXPECT_EQ(dock->shouldRotateToDock(), true);
    dock->cleanup();
...
TEST(SimpleNonChargingDockTests, DetectorLifecycle) {
    ...
    node->declare_parameter("my_dock.use_external_detection_pose", rclcpp::ParameterValue(true));
    node->declare_parameter("my_dock.detector_service_name", rclcpp::ParameterValue("test_detector_service"));
    ...
    auto service = node->create_service<std_srvs::srv::Trigger>(
        "test_detector_service", [&service_called](std::shared_ptr<rmw_request_id_t>, std::shared_ptr<std_srvs::srv::Trigger::Request>,
    ...
    while (!service_called && std::chrono::steady_clock::now() - start_time < std::chrono::seconds(2)) {
        executor.spin_some();
...
TEST(SimpleNonChargingDockTests, SubscriptionCallback) {
    auto node = std::make_shared<nav2::LifecycleNode>("test_subscription_reliable_non_charging");
    ...
    auto publisher = node->create_publisher<geometry_msgs::msg::PoseStamped>("detected_dock_pose", rclcpp::QoS(1));
...
TEST(SimpleNonChargingDockTests, DetectorServiceTimeout) {
    ...
    auto mock_service = node->create_service<std_srvs::srv::Trigger>(
        "slow_service", [](std::shared_ptr<rmw_request_id_t>, std::shared_ptr<std_srvs::srv::Trigger::Request>,
                           std::shared_ptr<std_srvs::srv::Trigger::Response>) { std::this_thread::sleep_for(200ms); });
...
TEST(SimpleNonChargingDockTests, DetectorServiceFailure) {
    ...
    rclcpp::NodeOptions options;
    options.parameter_overrides({{"my_dock.use_external_detection_pose", true},
    ...
    auto node = std::make_shared<nav2::LifecycleNode>("test_detector_failure_non_charging", options);
    ...
    auto slow_service = node->create_service<std_srvs::srv::Trigger>(
        service_name, [](std::shared_ptr<rmw_request_id_t>, std::shared_ptr<std_srvs::srv::Trigger::Request>,
                         std::shared_ptr<std_srvs::srv::Trigger::Response>) { std::this_thread::sleep_for(200ms); });
...
TEST(SimpleNonChargingDockTests, SubscriptionPersistent) {
    rclcpp::NodeOptions options;
    options.parameter_overrides({
        {"my_dock.use_external_detection_pose", true}, {"my_dock.subscribe_toggle", false} // The key parameter to test.
    });
    auto node = std::make_shared<nav2::LifecycleNode>("test_sub_persistent_non_charging", options);
