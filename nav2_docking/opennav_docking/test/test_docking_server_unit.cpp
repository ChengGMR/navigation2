...
class DockingServerShim : public DockingServer {
   public:
    DockingServerShim() : DockingServer() {}

    // Bypass TF
    virtual geometry_msgs::msg::PoseStamped getRobotPoseInFrame(const std::string&) { return geometry_msgs::msg::PoseStamped(); }

    std::optional<bool> getDockBackward() { return dock_backwards_; }
};

// Test shim to expose the TF2 buffer for injection
class DockingServerTFShim : public DockingServerShim {
   public:
    DockingServerTFShim() : DockingServerShim() {}
...
TEST(DockingServerTests, testErrorExceptions) {
    ...
    node->declare_parameter("docks", rclcpp::ParameterValue(std::vector<std::string>{"test_dock"}));
    node->declare_parameter("test_dock.type", rclcpp::ParameterValue(std::string{"dock_plugin"}));
    node->declare_parameter("test_dock.pose", rclcpp::ParameterValue(std::vector<double>{0.0, 0.0, 0.0}));
    node->declare_parameter("dock_plugins", rclcpp::ParameterValue(std::vector<std::string>{"dock_plugin"}));
    node->declare_parameter("dock_plugin.plugin", rclcpp::ParameterValue(std::string{"opennav_docking::TestFailureDock"}));
    ...
    // Error codes docking
    std::vector<std::string> error_ids{"TransformException", "DockNotInDB",    "DockNotValid",     "FailedToStage", "FailedToDetectDock",
                                       "FailedToControl",    "FailedToCharge", "DockingException", "exception"};
    ...
    for (unsigned int i = 0; i != error_ids.size(); i++) {
        node->set_parameter(rclcpp::Parameter("exception_to_throw", rclcpp::ParameterValue(error_ids[i])));
        ...
        if (rclcpp::spin_until_future_complete(node2, future_goal_handle, 2s) == rclcpp::FutureReturnCode::SUCCESS) {
            auto future_result = client->async_get_result(future_goal_handle.get());
            if (rclcpp::spin_until_future_complete(node2, future_result, 5s) == rclcpp::FutureReturnCode::SUCCESS) {
    ...
    // Now for undocking
    std::vector<std::string> error_ids_undocking{"TransformException", "DockNotValid", "FailedToControl", "DockingException", "exception"};
    ...
    for (unsigned int i = 0; i != error_ids_undocking.size(); i++) {
        node->set_parameter(rclcpp::Parameter("exception_to_throw", rclcpp::ParameterValue(error_ids_undocking[i])));
...
TEST(DockingServerTests, testDynamicParams) {
    ...
    auto results =
        rec_param->set_parameters_atomically({rclcpp::Parameter("controller_frequency", 0.2), rclcpp::Parameter("initial_perception_timeout", 1.0),
                                              rclcpp::Parameter("wait_charge_timeout", 1.2), rclcpp::Parameter("undock_linear_tolerance", 0.25),
                                              rclcpp::Parameter("undock_angular_tolerance", 0.125),
                                              rclcpp::Parameter("base_frame", std::string("hi")), rclcpp::Parameter("fixed_frame", std::string("hi")),
                                              rclcpp::Parameter("max_retries", 7), rclcpp::Parameter("rotation_angular_tolerance", 0.42)});
...
TEST(DockingServerTests, StopDetectionOnSuccess) {
    ...
    auto future_result = client->async_get_result(goal_handle);
    ASSERT_EQ(rclcpp::spin_until_future_complete(client_node, future_result, 5s), rclcpp::FutureReturnCode::SUCCESS);
