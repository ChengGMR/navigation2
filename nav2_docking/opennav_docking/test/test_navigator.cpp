...
class DummyNavigationServer : rclcpp::Node {
   public:
    ...
    DummyNavigationServer() : Node("dummy_navigator") {
        action_server_ =
            std::make_shared<ActionServer>(get_node_base_interface(), get_node_clock_interface(), get_node_logging_interface(),
                                           get_node_waitables_interface(), get_node_parameters_interface(), "navigate_to_pose",
                                           std::bind(&DummyNavigationServer::executeCallback, this), nullptr, std::chrono::milliseconds(500), true);
    ...
    void setReturn(const bool rtn) { return_state_ = rtn; }

    void setToggle() { toggle_ = true; }
...
TEST(NavigatorTests, TestNavigator) {
    ...
    EXPECT_NO_THROW(navigator->goToPose(geometry_msgs::msg::PoseStamped(), rclcpp::Duration(10.0, 10.0), is_preempted_false));

    // Should fail, timeout exceeded
    EXPECT_THROW(navigator->goToPose(geometry_msgs::msg::PoseStamped(), rclcpp::Duration(0.0, 0.0), is_preempted_false),
                 opennav_docking_core::FailedToStage);

    // Should fail, action server set to succeed
    dummy_navigator_node->setReturn(false);
    EXPECT_THROW(navigator->goToPose(geometry_msgs::msg::PoseStamped(), rclcpp::Duration(10.0, 10.0), is_preempted_false),
                 opennav_docking_core::FailedToStage);
    ...
    dummy_navigator_node->setReturn(true);
    EXPECT_THROW(navigator->goToPose(geometry_msgs::msg::PoseStamped(), rclcpp::Duration(10.0, 10.0), is_preempted_true),
                 opennav_docking_core::FailedToStage);
