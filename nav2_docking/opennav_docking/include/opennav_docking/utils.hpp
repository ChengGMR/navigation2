 ...
 */
inline bool parseDockFile(const std::string& yaml_filepath, const nav2::LifecycleNode::SharedPtr& node, DockMap& dock_db) {
    YAML::Node yaml_file;
    try {
    ...
    if (!yaml_file["docks"]) {
        RCLCPP_ERROR(node->get_logger(), "Dock database (%s) does not contain 'docks'.", yaml_filepath.c_str());
        return false;
    }
    ...
    for (const auto& yaml_dock : yaml_docks) {
        ...
        if (!dock_attribs["type"]) {
            RCLCPP_ERROR(node->get_logger(), "Dock database (%s) entries do not contain 'type'.", yaml_filepath.c_str());
            return false;
        }
        curr_dock.type = dock_attribs["type"].as<std::string>();

        if (!dock_attribs["pose"]) {
            RCLCPP_ERROR(node->get_logger(), "Dock database (%s) entries do not contain 'pose'.", yaml_filepath.c_str());
            return false;
        }
        std::vector<double> pose_arr = dock_attribs["pose"].as<std::vector<double>>();
        if (pose_arr.size() != 3u) {
            RCLCPP_ERROR(node->get_logger(), "Dock database (%s) entries do not contain pose of size 3.", yaml_filepath.c_str());
            return false;
        }
 ...
 * @param dock_db Database to populate
 */
inline bool parseDockParams(const std::vector<std::string>& docks_param, const nav2::LifecycleNode::SharedPtr& node, DockMap& dock_db) {
    Dock curr_dock;
 ...
 */
inline geometry_msgs::msg::PoseStamped getDockPoseStamped(const Dock* dock, const rclcpp::Time& t) {
...
inline DockDirection getDockDirectionFromString(const std::string& direction) {
    ...
    std::transform(upper_direction.begin(), upper_direction.end(), upper_direction.begin(), ::toupper);
