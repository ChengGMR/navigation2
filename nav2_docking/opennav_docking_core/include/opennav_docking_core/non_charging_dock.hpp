...
class NonChargingDock : public ChargingDock {
   public:
     ...
     * the docking controller loop.
     */
    bool isCharging() final { throw std::runtime_error("This dock is not a charging dock!"); }
     ...
     * the docking controller loop.
     */
    bool disableCharging() final { throw std::runtime_error("This dock is not a charging dock!"); }

    /**
     * @brief Similar to isCharging() but called when undocking.
     */
    bool hasStoppedCharging() final { throw std::runtime_error("This dock is not a charging dock!"); }
