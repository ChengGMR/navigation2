...
class DockRobotActionServer : public TestActionServer<nav2_msgs::action::DockRobot> {
   public:
    DockRobotActionServer() : TestActionServer("dock_robot") {}

   protected:
    void execute(const typename std::shared_ptr<rclcpp_action::ServerGoalHandle<nav2_msgs::action::DockRobot>> goal_handle) override {
...
class DockRobotActionTestFixture : public ::testing::Test {
   public:
    static void SetUpTestCase() {
        ...
        factory_->registerBuilder<opennav_docking_bt::DockRobotAction>("DockRobot", builder);
    }
...
std::shared_ptr<DockRobotActionServer> DockRobotActionTestFixture::action_server_ = nullptr;
...
int main(int argc, char** argv) {
    ...
    DockRobotActionTestFixture::action_server_ = std::make_shared<DockRobotActionServer>();

    std::thread server_thread([]() { rclcpp::spin(DockRobotActionTestFixture::action_server_); });
