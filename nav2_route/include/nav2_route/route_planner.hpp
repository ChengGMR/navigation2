...
class RoutePlanner {
   public:
     ...
     */
    void configure(nav2::LifecycleNode::SharedPtr node, const std::shared_ptr<tf2_ros::Buffer> tf_buffer,
     ...
     */
    virtual Route findRoute(Graph& graph, unsigned int start_index, unsigned int goal_index, const std::vector<unsigned int>& blocked_ids,
   ...
   protected:
     ...
     * @param blocked_ids A set of blocked node and edge IDs not to traverse
     */
    void findShortestGraphTraversal(Graph& graph, const NodePtr start_node, const NodePtr goal_node, const std::vector<unsigned int>& blocked_ids,
                                    const RouteRequest& route_request);
