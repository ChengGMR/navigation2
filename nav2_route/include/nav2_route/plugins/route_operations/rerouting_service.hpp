...
class ReroutingService : public RouteOperation {
   public:
     ...
     * @brief Configure
     */
    void configure(const nav2::LifecycleNode::SharedPtr node, std::shared_ptr<nav2_costmap_2d::CostmapSubscriber> costmap_subscriber,
     ...
     */
    OperationResult perform(NodePtr node, EdgePtr edge_entered, EdgePtr edge_exited, const Route& route,
                            const geometry_msgs::msg::PoseStamped& curr_pose, const Metadata* mdata = nullptr) override;
     ...
     */
    void serviceCb(const std::shared_ptr<rmw_request_id_t> request_header, const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
