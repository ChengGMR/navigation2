...
class BreadthFirstSearch {
   public:
     ...
     */
    explicit BreadthFirstSearch(std::shared_ptr<nav2_costmap_2d::Costmap2D> costmap) : costmap_(costmap) {}
     ...
     */
    bool search(const geometry_msgs::msg::PoseStamped& reference_node, const std::vector<geometry_msgs::msg::PoseStamped>& candidate_nodes,
                const int max_iterations = std::numeric_limits<int>::max()) {
        closest_node_idx_ = 0u;

        // Convert target to costmap space
        unsigned int goal_x, goal_y;
        if (!costmap_->worldToMap(reference_node.pose.position.x, reference_node.pose.position.y, goal_x, goal_y)) {
            return false;
        }
    ...
    std::vector<unsigned int> getNeighbors(const unsigned int current_id) {
        ...
        const std::vector<int> offsets = {-1, 1, -size_x, size_x, -size_x - 1, -size_x + 1, size_x - 1, size_x + 1};
     ...
     */
    unsigned int getClosestNodeIdx() { return closest_node_idx_; }
...
class LoSCollisionChecker {
   public:
     ...
     * @param costmap Costmap to check
     */
    explicit LoSCollisionChecker(std::shared_ptr<nav2_costmap_2d::Costmap2D> costmap) : costmap_(costmap) {}
    ...
    bool worldToMap(const geometry_msgs::msg::Point& start, const geometry_msgs::msg::Point& end) {
        if (!costmap_->worldToMap(start.x, start.y, x0_, y0_) || !costmap_->worldToMap(end.x, end.y, x1_, y1_)) {
            return false;
        }
        return true;
    }
