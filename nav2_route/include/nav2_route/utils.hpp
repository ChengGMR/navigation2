 ...
 */
inline visualization_msgs::msg::MarkerArray toMsg(const nav2_route::Graph& graph, const std::string& frame, const rclcpp::Time& now) {
    ...
    for (const auto& node : graph) {
        ...
        for (const auto& neighbor : node.neighbors) {
            ...
            edge_id_marker.id++;
            edge_id_marker.pose.position.x = node.coords.x + ((neighbor.end->coords.x - node.coords.x) / 2.0) + x_offset;
            edge_id_marker.pose.position.y = node.coords.y + ((neighbor.end->coords.y - node.coords.y) / 2.0) + y_offset;
 ...
 */
inline nav2_msgs::msg::Route toMsg(const nav2_route::Route& route, const std::string& frame, const rclcpp::Time& now) {
 ...
 */
inline float normalizedDot(const float v1x, const float v1y, const float v2x, const float v2y) {
 ...
 */
inline Coordinates findClosestPoint(const geometry_msgs::msg::PoseStamped& pose, const Coordinates& start, const Coordinates& end) {
    Coordinates pt;
