...
class OperationsManager {
   public:
     ...
     */
    explicit OperationsManager(nav2::LifecycleNode::SharedPtr node, std::shared_ptr<nav2_costmap_2d::CostmapSubscriber> costmap_subscriber);
     ...
     */
    OperationsResult process(const bool status_change, const RouteTrackingState& state, const Route& route,
   ...
   protected:
     ...
     */
    void processOperationsPluginVector(const std::vector<RouteOperation::Ptr>& operations, OperationsResult& result, const NodePtr node,
                                       const EdgePtr edge_entered, const EdgePtr edge_exited, const Route& route,
