...
class CornerArc {
   public:
    ...
    CornerArc(const Coordinates& start, const Coordinates& corner, const Coordinates& end, float minimum_radius) {
        ...
        if (start_edge_length_ == 0.0 || end_edge_length_ == 0.0) {
            return;
        }
        ...
        if (tangent_length < start_edge_length_ && tangent_length < end_edge_length_) {
            std::vector<float> start_edge_unit_tangent = {(start.x - corner.x) / start_edge_length_, (start.y - corner.y) / start_edge_length_};
            std::vector<float> end_edge_unit_tangent = {(end.x - corner.x) / end_edge_length_, (end.y - corner.y) / end_edge_length_};
            ...
            std::vector<float> unit_bisector = {bisector_x / bisector_magnitude, bisector_y / bisector_magnitude};
    ...
    void interpolateArc(const float& max_angle_resolution, std::vector<geometry_msgs::msg::PoseStamped>& poses) {
        std::vector<float> r_start{start_coordinate_.x - circle_center_coordinate_.x, start_coordinate_.y - circle_center_coordinate_.y};
        std::vector<float> r_end{end_coordinate_.x - circle_center_coordinate_.x, end_coordinate_.y - circle_center_coordinate_.y};
   ...
   protected:
    ...
    float getAngleBetweenEdges(const Coordinates& start, const Coordinates& corner, const Coordinates& end) {
        ...
        float angle = acos((start_dx * end_dx + start_dy * end_dy) / (start_edge_length_ * end_edge_length_));
