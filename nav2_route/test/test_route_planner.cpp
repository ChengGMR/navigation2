...
TEST(RoutePlannerTest, test_route_planner_positive) {
    ...
    start = 15;
    goal = 0;
    EXPECT_THROW(planner.findRoute(graph, start, goal, blocked_ids, route_request), nav2_core::NoValidRouteCouldBeFound);
...
TEST(RoutePlannerTest, test_route_planner_negative) {
    ...
    EXPECT_THROW(planner.findRoute(graph, start, goal, blocked_ids, route_request), nav2_core::NoValidGraph);
