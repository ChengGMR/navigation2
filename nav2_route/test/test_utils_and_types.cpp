...
TEST(UtilsTest, test_to_visualization_msg_conversion) {
    ...
    constexpr size_t expected_total_markers =
        expected_edge_markers + expected_node_markers + expected_edge_id_text_markers + expected_node_id_text_markers;
    ...
    for (auto& marker : graph_msg.markers) {
        if (marker.ns == "route_graph_ids") {
            ...
            EXPECT_TRUE((marker.type == visualization_msgs::msg::Marker::LINE_LIST) || (marker.type == visualization_msgs::msg::Marker::SPHERE));
        }
    }
}
