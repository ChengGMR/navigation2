...
TEST(OperationsManagerTest, test_processing_speed_on_status) {
    ...
    auto sub = node->create_subscription<nav2_msgs::msg::SpeedLimit>("speed_limit", [&, this](nav2_msgs::msg::SpeedLimit msg) {
        got_msg = true;
        my_msg = msg;
    });
...
TEST(OperationsManagerTest, test_rerouting_service_on_query) {
    ...
    node->declare_parameter("operations", rclcpp::ParameterValue(std::vector<std::string>{"ReroutingService"}));
    nav2::declare_parameter_if_not_declared(node, "ReroutingService.plugin", rclcpp::ParameterValue(std::string{"nav2_route::ReroutingService"}));
    std::shared_ptr<nav2_costmap_2d::CostmapSubscriber> costmap_subscriber;
    OperationsManager manager(node, costmap_subscriber);
...
TEST(OperationsManagerTest, test_trigger_event_on_graph) {
    ...
    node->declare_parameter("operations", rclcpp::ParameterValue(std::vector<std::string>{"OpenDoor"}));
    nav2::declare_parameter_if_not_declared(node, "OpenDoor.plugin", rclcpp::ParameterValue(std::string{"nav2_route::TriggerEvent"}));
    std::shared_ptr<nav2_costmap_2d::CostmapSubscriber> costmap_subscriber;
    OperationsManager manager(node, costmap_subscriber);
    ...
    bool got_srv = false;
    auto callback = [&](const std::shared_ptr<rmw_request_id_t> /*request_header*/,
...
TEST(OperationsManagerTest, test_trigger_event_on_graph_global_service) {
    ...
    node->declare_parameter("operations", rclcpp::ParameterValue(std::vector<std::string>{"OpenDoor"}));
    nav2::declare_parameter_if_not_declared(node, "OpenDoor.plugin", rclcpp::ParameterValue(std::string{"nav2_route::TriggerEvent"}));
    nav2::declare_parameter_if_not_declared(node, "OpenDoor.service_name", rclcpp::ParameterValue(std::string{"hello_world"}));
    std::shared_ptr<nav2_costmap_2d::CostmapSubscriber> costmap_subscriber;
    OperationsManager manager(node, costmap_subscriber);
    ...
    bool got_srv2 = false;
    auto callback2 = [&](const std::shared_ptr<rmw_request_id_t> /*request_header*/,
                         const std::shared_ptr<std_srvs::srv::Trigger::Request> /*request*/,
                         std::shared_ptr<std_srvs::srv::Trigger::Response> response) -> void {
        got_srv2 = true;
        response->success = true;
...
TEST(OperationsManagerTest, test_time_marker) {
    auto node = std::make_shared<nav2::LifecycleNode>("operations_manager_test");
    auto node_thread = std::make_unique<nav2::NodeThread>(node);
    node->declare_parameter("operations", rclcpp::ParameterValue(std::vector<std::string>{"TimeMarker"}));
    nav2::declare_parameter_if_not_declared(node, "TimeMarker.plugin", rclcpp::ParameterValue(std::string{"nav2_route::TimeMarker"}));
    std::shared_ptr<nav2_costmap_2d::CostmapSubscriber> costmap_subscriber;
    OperationsManager manager(node, costmap_subscriber);
...
class TestRouteOperations : public nav2_route::RouteOperation {
   public:
    void configure(const nav2::LifecycleNode::SharedPtr, std::shared_ptr<nav2_costmap_2d::CostmapSubscriber>, const std::string&) override {}

    std::string getName() override { return "TestRouteOperation"; }
    OperationResult perform(NodePtr, EdgePtr, EdgePtr, const Route&, const geometry_msgs::msg::PoseStamped&, const Metadata*) override {
