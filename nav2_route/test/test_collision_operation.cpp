...
class CollisionMonitorWrapper : public CollisionMonitor {
   public:
    ...
    Coordinates backoutValidEndPointWrapper(const Coordinates& start, const Coordinates& end, const float dist) {
        return backoutValidEndPoint(start, end, dist);
    }

    bool lineToMapWrapper(const Coordinates& start, const Coordinates& end, LineSegment& line) { return lineToMap(start, end, line); }

    bool backoutValidEndPointWrapper(const Coordinates& start, LineSegment& line) { return backoutValidEndPoint(start, line); }

    bool isInCollisionWrapper(const LineSegment& line) { return isInCollision(line); }

    void getCostmapWrapper() { getCostmap(); }
};
...
TEST(TestCollisionMonitor, test_costmap_apis) {
    ...
    nav2_costmap_2d::Costmap2DPublisher publisher(node, costmap, "map", "local_costmap/costmap", true);
    ...
    OperationResult result = monitor.perform(&node1 /*unused*/, curr_edge, curr_edge /*unused*/, route, pose, nullptr);
    ...
    result = monitor.perform(&node1 /*unused*/, curr_edge, curr_edge /*unused*/, route, pose, nullptr);
    EXPECT_FALSE(result.reroute);
    rclcpp::Rate r2(0.8);
    r2.sleep();
