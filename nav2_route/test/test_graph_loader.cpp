...
TEST(GraphLoader, test_invalid_plugin) {
    ...
    nav2::declare_parameter_if_not_declared(
        node, "graph_filepath", rclcpp::ParameterValue(ament_index_cpp::get_package_share_directory("nav2_route") + "/graphs/aws_graph.geojson"));

    // Set dummy parameter
    std::string default_plugin = "nav2_route::Dummy";
    nav2::declare_parameter_if_not_declared(node, "graph_file_loader", rclcpp::ParameterValue(default_plugin));
...
TEST(GraphLoader, test_transformation_api) {
    ...
    GraphToIDMap graph_to_id_map;
    std::string filepath;
    filepath = ament_index_cpp::get_package_share_directory("nav2_route") + "/graphs/aws_graph.geojson";
...
TEST(GraphLoader, test_transformation_api2) {
    ...
    nav2::declare_parameter_if_not_declared(
        node, "graph_filepath",
        rclcpp::ParameterValue(ament_index_cpp::get_package_share_directory("nav2_route") + "/test/test_graphs/no_frame.json"));
    ...
    std::string filepath = ament_index_cpp::get_package_share_directory("nav2_route") + "/test/test_graphs/no_frame.json";
    EXPECT_FALSE(graph_loader.loadGraphFromFile(graph, graph_to_id_map, filepath));
}
