...
TEST(PathConverterTest, test_path_converter_api) {
    ...
    auto sub = node->create_subscription<nav_msgs::msg::Path>("plan", [&, this](nav_msgs::msg::Path msg) { path_msg = msg; });
...
TEST(PathConverterTest, test_path_converter_interpolation) {
    ...
    for (unsigned int i = 0; i != poses.size() - 1; i++) {
        ...
        EXPECT_LT(hypotf(poses[i].pose.position.x - poses[i + 1].pose.position.x, poses[i].pose.position.y - poses[i + 1].pose.position.y), 0.05);
    }
}
