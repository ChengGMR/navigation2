...
class RoutePlannerErrorTester : public RoutePlanner {
   public:
    ...
    Route findRoute(Graph& graph, unsigned int start_index, unsigned int goal_index, const std::vector<unsigned int>& blocked_ids,
...
class RouteServerWrapper : public RouteServer {
   public:
    explicit RouteServerWrapper(const rclcpp::NodeOptions& options) : RouteServer(options) {}
    ...
    void configure() { on_configure(rclcpp_lifecycle::State()); }

    void activate() { on_activate(rclcpp_lifecycle::State()); }
    ...
    void testPrint(const std::shared_ptr<const nav2_msgs::action::ComputeRoute::Goal> goal, const std::exception& ex) { exceptionWarning(goal, ex); }

    rclcpp::Duration findPlanningDurationWrapper(const rclcpp::Time& start) { return findPlanningDuration(start); }

    void populateActionResultWrapper(std::shared_ptr<nav2_msgs::action::ComputeRoute::Result> result, const Route& route,
                                     const nav_msgs::msg::Path& path, const rclcpp::Duration& planning_duration) {
        return populateActionResult(result, route, path, planning_duration);
    }

    bool isRequestValidWrapper() { return isRequestValid<ComputeAndTrackRoute>(compute_and_track_route_server_); }

    void setNontrivialGraph() { graph_.resize(1); }

    void useErrorCodePlanner() { route_planner_ = std::make_shared<RoutePlannerErrorTester>(); }
};
...
TEST(RouteServerTest, test_set_srv) {
    ...
    server->startup();
    auto srv_client = nav2::ServiceClient<nav2_msgs::srv::SetRouteGraph>("route_server/set_route_graph", node2);
    ...
    req3->graph_filepath = ament_index_cpp::get_package_share_directory("nav2_route") + "/test/test_graphs/invalid.json";
...
TEST(RouteServerTest, test_complete_action_api) {
    ...
    auto node2 = std::make_shared<rclcpp::Node>("my_node2");
    auto compute_client = rclcpp_action::create_client<nav2_msgs::action::ComputeRoute>(node2, "compute_route");
    ...
    auto track_client = rclcpp_action::create_client<nav2_msgs::action::ComputeAndTrackRoute>(node2, "compute_and_track_route");
    ...
    auto srv_client = nav2::ServiceClient<std_srvs::srv::Trigger>("route_server/ReroutingService/reroute", node_int);
...
TEST(RouteServerTest, test_error_codes) {
    ...
    auto node2 = std::make_shared<rclcpp::Node>("my_node2");
    auto compute_client = rclcpp_action::create_client<ComputeAndTrackRoute>(node2, "compute_and_track_route");
