...
void RoutePlanner::configure(nav2::LifecycleNode::SharedPtr node, const std::shared_ptr<tf2_ros::Buffer> tf_buffer,
                             const std::shared_ptr<nav2_costmap_2d::CostmapSubscriber> costmap_subscriber) {
    nav2::declare_parameter_if_not_declared(node, "max_iterations", rclcpp::ParameterValue(0));
...
Route RoutePlanner::findRoute(Graph& graph, unsigned int start_index, unsigned int goal_index, const std::vector<unsigned int>& blocked_ids,
...
void RoutePlanner::findShortestGraphTraversal(Graph& graph, const NodePtr start_node, const NodePtr goal_node,
                                              const std::vector<unsigned int>& blocked_ids, const RouteRequest& route_request) {
...
bool RoutePlanner::getTraversalCost(const EdgePtr edge, float& score, const std::vector<unsigned int>& blocked_ids,
                                    const RouteRequest& route_request) {
    // If edge or node is in the blocked list, don't expand
    auto is_blocked =
        std::find_if(blocked_ids.begin(), blocked_ids.end(), [&](unsigned int id) { return id == edge->edgeid || id == edge->end->nodeid; });
    ...
    if (!edge->edge_cost.overridable || edge_scorer_->numPlugins() == 0) {
        if (edge->edge_cost.cost <= 0.0) {
            throw nav2_core::NoValidGraph("Edge " + std::to_string(edge->edgeid) + " doesn't contain and cannot compute a valid edge cost!");
        }
