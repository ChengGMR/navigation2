...
void TimeScorer::configure(const nav2::LifecycleNode::SharedPtr node, const std::shared_ptr<tf2_ros::Buffer> /* tf_buffer */,
    ...
    nav2::declare_parameter_if_not_declared(node, getName() + ".speed_tag", rclcpp::ParameterValue("abs_speed_limit"));
    speed_tag_ = node->get_parameter(getName() + ".speed_tag").as_string();

    nav2::declare_parameter_if_not_declared(node, getName() + ".time_tag", rclcpp::ParameterValue("abs_time_taken"));
    ...
    nav2::declare_parameter_if_not_declared(node, getName() + ".max_vel", rclcpp::ParameterValue(0.5));
    max_vel_ = static_cast<float>(node->get_parameter(getName() + ".max_vel").as_double());
}

bool TimeScorer::score(const EdgePtr edge, const RouteRequest& /* route_request */, const EdgeType& /* edge_type */, float& cost) {
    ...
    cost = weight_ * hypotf(edge->end->coords.x - edge->start->coords.x, edge->end->coords.y - edge->start->coords.y) / velocity;
    return true;
}
