...
void SemanticScorer::configure(const nav2::LifecycleNode::SharedPtr node, const std::shared_ptr<tf2_ros::Buffer> /* tf_buffer */,
    ...
    nav2::declare_parameter_if_not_declared(node, getName() + ".semantic_classes", rclcpp::ParameterValue(std::vector<std::string>{}));
    std::vector<std::string> classes = node->get_parameter(getName() + ".semantic_classes").as_string_array();
    for (auto& cl : classes) {
        nav2::declare_parameter_if_not_declared(node, getName() + "." + cl, rclcpp::ParameterType::PARAMETER_DOUBLE);
    ...
    nav2::declare_parameter_if_not_declared(node, getName() + ".semantic_key", rclcpp::ParameterValue(std::string("class")));
...
bool SemanticScorer::score(const EdgePtr edge, const RouteRequest& /* route_request */, const EdgeType& /* edge_type */, float& cost) {
    float score = 0.0;
