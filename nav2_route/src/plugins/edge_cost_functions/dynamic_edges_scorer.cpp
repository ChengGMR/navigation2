...
void DynamicEdgesScorer::configure(const nav2::LifecycleNode::SharedPtr node, const std::shared_ptr<tf2_ros::Buffer> /* tf_buffer */,
    ...
    service_ = node->create_service<nav2_msgs::srv::DynamicEdges>(
        std::string(node->get_name()) + "/" + getName() + "/adjust_edges",
        std::bind(&DynamicEdgesScorer::closedEdgesCb, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));

    dynamic_penalties_.clear();
    closed_edges_.clear();
}

void DynamicEdgesScorer::closedEdgesCb(const std::shared_ptr<rmw_request_id_t>, const std::shared_ptr<nav2_msgs::srv::DynamicEdges::Request> request,
...
bool DynamicEdgesScorer::score(const EdgePtr edge, const RouteRequest& /* route_request */, const EdgeType& /* edge_type */, float& cost) {
