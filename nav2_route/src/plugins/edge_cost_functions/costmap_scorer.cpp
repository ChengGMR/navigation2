...
void CostmapScorer::configure(const nav2::LifecycleNode::SharedPtr node, const std::shared_ptr<tf2_ros::Buffer> /* tf_buffer */,
                              std::shared_ptr<nav2_costmap_2d::CostmapSubscriber> costmap_subscriber, const std::string& name) {
    ...
    nav2::declare_parameter_if_not_declared(node, getName() + ".use_maximum", rclcpp::ParameterValue(true));
    use_max_ = static_cast<float>(node->get_parameter(getName() + ".use_maximum").as_bool());

    // Edge is invalid if its in collision
    nav2::declare_parameter_if_not_declared(node, getName() + ".invalid_on_collision", rclcpp::ParameterValue(true));
    invalid_on_collision_ = static_cast<float>(node->get_parameter(getName() + ".invalid_on_collision").as_bool());

    // Edge is invalid if edge is off the costmap
    nav2::declare_parameter_if_not_declared(node, getName() + ".invalid_off_map", rclcpp::ParameterValue(true));
    invalid_off_map_ = static_cast<float>(node->get_parameter(getName() + ".invalid_off_map").as_bool());
    ...
    nav2::declare_parameter_if_not_declared(node, getName() + ".check_resolution", rclcpp::ParameterValue(2));
    ...
    nav2::declare_parameter_if_not_declared(node, getName() + ".costmap_topic", rclcpp::ParameterValue(std::string("global_costmap/costmap_raw")));
    ...
    if (costmap_topic != server_costmap_topic) {
        ...
        RCLCPP_INFO(node->get_logger(), "Using costmap topic: %s instead of server costmap topic: %s for CostmapScorer.", costmap_topic.c_str(),
    ...
    nav2::declare_parameter_if_not_declared(node, getName() + ".weight", rclcpp::ParameterValue(1.0));
...
bool CostmapScorer::score(const EdgePtr edge, const RouteRequest& /* route_request */, const EdgeType& /* edge_type */, float& cost) {
    if (!costmap_) {
