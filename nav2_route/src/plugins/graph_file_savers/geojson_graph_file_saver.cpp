...
void GeoJsonGraphFileSaver::configure(const nav2::LifecycleNode::SharedPtr node) {
    RCLCPP_INFO(node->get_logger(), "Configuring geojson graph file saver");
    logger_ = node->get_logger();
}

bool GeoJsonGraphFileSaver::saveGraphToFile(Graph& graph, std::string filepath) {
    if (filepath.empty()) {
...
void GeoJsonGraphFileSaver::loadNodesFromGraph(Graph& graph, std::vector<Json>& json_features) {
...
void GeoJsonGraphFileSaver::loadEdgesFromGraph(Graph& graph, std::vector<Json>& json_edges) {
    for (const auto& node : graph) {
...
void GeoJsonGraphFileSaver::convertMetaDataToJson(const Metadata& metadata, Json& json_metadata) {
...
void GeoJsonGraphFileSaver::convertOperationsToJson(const Operations& operations, Json& json_operations) {
