...
void GeoJsonGraphFileLoader::configure(const nav2::LifecycleNode::SharedPtr node) {
    RCLCPP_INFO(node->get_logger(), "Configuring geojson graph file loader");
    logger_ = node->get_logger();
}

bool GeoJsonGraphFileLoader::loadGraphFromFile(Graph& graph, GraphToIDMap& graph_to_id_map, std::string filepath) {
    ...
    if (nodes.empty() || edges.empty()) {
        RCLCPP_ERROR(logger_, "The graph is malformed. Is does not contain nodes or edges. Please check %s", filepath.c_str());
        return false;
    }
...
void GeoJsonGraphFileLoader::getGraphElements(const Json& features, std::vector<Json>& nodes, std::vector<Json>& edges) {
    for (const auto& feature : features) {
        if (feature["geometry"]["type"] == "Point") {
            nodes.emplace_back(feature);
        } else if ( // NOLINT
            feature["geometry"]["type"] == "MultiLineString" || feature["geometry"]["type"] == "LineString") {
            edges.emplace_back(feature);
        }
    }
}

void GeoJsonGraphFileLoader::addNodesToGraph(Graph& graph, GraphToIDMap& graph_to_id_map, std::vector<Json>& nodes) {
    int idx = 0;
...
void GeoJsonGraphFileLoader::addEdgesToGraph(Graph& graph, GraphToIDMap& graph_to_id_map, std::vector<Json>& edges) {
    for (const auto& edge : edges) {
        ...
        graph[graph_to_id_map[start_id]].addEdge(edge_cost, &graph[graph_to_id_map[end_id]], id, metadata, operations);
    }
}
...
Metadata GeoJsonGraphFileLoader::convertMetaDataFromJson(const Json& properties, const std::string& key) {
    Metadata metadata;
    ...
    for (const auto& data : properties[key].items()) {
        ...
        const auto setPrimitiveType = [&](const auto& value) -> std::any {
            ...
            RCLCPP_ERROR(logger_, "Failed to convert the key: %s to a value", data.key().c_str());
