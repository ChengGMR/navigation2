...
void CollisionMonitor::configure(const nav2::LifecycleNode::SharedPtr node, std::shared_ptr<nav2_costmap_2d::CostmapSubscriber> costmap_subscriber,
    ...
    nav2::declare_parameter_if_not_declared(node, getName() + ".costmap_topic", rclcpp::ParameterValue("local_costmap/costmap_raw"));
    std::string costmap_topic = node->get_parameter(getName() + ".costmap_topic").as_string();
    if (costmap_topic != server_costmap_topic) {
        RCLCPP_INFO(node->get_logger(), "Using costmap topic: %s instead of server costmap topic: %s for CollisionMonitor.", costmap_topic.c_str(),
                    server_costmap_topic.c_str());
    ...
    nav2::declare_parameter_if_not_declared(node, getName() + ".rate", rclcpp::ParameterValue(1.0));
    ...
    nav2::declare_parameter_if_not_declared(node, getName() + ".reroute_on_collision", rclcpp::ParameterValue(true));
    reroute_on_collision_ = node->get_parameter(getName() + ".reroute_on_collision").as_bool();

    nav2::declare_parameter_if_not_declared(node, getName() + ".max_cost", rclcpp::ParameterValue(253.0));
    ...
    nav2::declare_parameter_if_not_declared(node, getName() + ".check_resolution", rclcpp::ParameterValue(1));
    check_resolution_ = static_cast<unsigned int>(node->get_parameter(getName() + ".check_resolution").as_int());

    nav2::declare_parameter_if_not_declared(node, getName() + ".max_collision_dist", rclcpp::ParameterValue(5.0));
    max_collision_dist_ = static_cast<float>(node->get_parameter(getName() + ".max_collision_dist").as_double());
    if (max_collision_dist_ <= 0.0) {
        RCLCPP_INFO(logger_, "Max collision distance to evaluate is zero or negative, checking the full route.");
...
void CollisionMonitor::getCostmap() {
    try {
        ...
        throw nav2_core::OperationFailed("Collision Monitor could not obtain a costmap from topic: " + topic_);
    }
}

OperationResult CollisionMonitor::perform(NodePtr /*node*/, EdgePtr curr_edge, EdgePtr /*edge_exited*/, const Route& route,
                                          const geometry_msgs::msg::PoseStamped& curr_pose, const Metadata* /*mdata*/) {
    ...
    Coordinates start = utils::findClosestPoint(curr_pose, curr_edge->start->coords, end);
    ...
    while (!final_edge) {
        ...
        if (isInCollision(line)) {
            RCLCPP_INFO(logger_, "Collision has been detected within %0.2fm of robot pose!", max_collision_dist_);
            ...
            throw nav2_core::OperationFailed("Collision detected, but rerouting is not enabled, canceling tracking task.");
        }
...
Coordinates CollisionMonitor::backoutValidEndPoint(const Coordinates& start, const Coordinates& end, const float dist) {
    Coordinates new_end;
...
bool CollisionMonitor::backoutValidEndPoint(const Coordinates& start, LineSegment& line) {
...
bool CollisionMonitor::lineToMap(const Coordinates& start, const Coordinates& end, LineSegment& line) {
    if (!costmap_->worldToMap(start.x, start.y, line.x0, line.y0) || !costmap_->worldToMap(end.x, end.y, line.x1, line.y1)) {
        return false;
    }
    return true;
}
