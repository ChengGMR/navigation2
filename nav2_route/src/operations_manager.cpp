...
OperationsManager::OperationsManager(nav2::LifecycleNode::SharedPtr node, std::shared_ptr<nav2_costmap_2d::CostmapSubscriber> costmap_subscriber)
    : plugin_loader_("nav2_route", "nav2_route::RouteOperation") {
    logger_ = node->get_logger();

    // Have some default operations
    const std::vector<std::string> default_plugin_ids({"AdjustSpeedLimit", "ReroutingService"});
    const std::vector<std::string> default_plugin_types({"nav2_route::AdjustSpeedLimit", "nav2_route::ReroutingService"});

    nav2::declare_parameter_if_not_declared(node, "operations", rclcpp::ParameterValue(default_plugin_ids));
    ...
    if (operation_ids == default_plugin_ids) {
        for (unsigned int i = 0; i != operation_ids.size(); i++) {
            nav2::declare_parameter_if_not_declared(node, default_plugin_ids[i] + ".plugin", rclcpp::ParameterValue(default_plugin_types[i]));
        }
    }
    ...
    for (size_t i = 0; i != operation_ids.size(); i++) {
        try {
            ...
            RCLCPP_INFO(node->get_logger(), "Created route operation %s of type %s", operation_ids[i].c_str(), type.c_str());
...
template <typename T>
void OperationsManager::findGraphOperationsToProcess(T& obj, const OperationTrigger& trigger, OperationPtrs& operations) {
    if (!obj) {
        return;
    }
...
OperationPtrs OperationsManager::findGraphOperations(const NodePtr node, const EdgePtr edge_enter, const EdgePtr edge_exit) {
    OperationPtrs ops;
...
void OperationsManager::updateResult(const std::string& name, const OperationResult& op_result, OperationsResult& result) {
    result.reroute = result.reroute || op_result.reroute;
    result.blocked_ids.insert(result.blocked_ids.end(), op_result.blocked_ids.begin(), op_result.blocked_ids.end());
    result.operations_triggered.push_back(name);
}

void OperationsManager::processOperationsPluginVector(const std::vector<RouteOperation::Ptr>& route_operations, OperationsResult& result,
                                                      const NodePtr node, const EdgePtr edge_entered, const EdgePtr edge_exited, const Route& route,
...
OperationsResult OperationsManager::process(const bool status_change, const RouteTrackingState& state, const Route& route,
                                            const geometry_msgs::msg::PoseStamped& pose, const ReroutingState& rerouting_info) {
    ...
    EdgePtr edge_exited = state.route_edges_idx > 0 ? route.edges[state.route_edges_idx - 1] : nullptr;
    ...
    if (status_change) {
        ...
        for (unsigned int i = 0; i != operations.size(); i++) {
            ...
            if (op != graph_operations_.end()) {
                OperationResult op_result = op->second->perform(node, edge_entered, edge_exited, route, pose, &operations[i]->metadata);
                updateResult(op->second->getName(), op_result, result);
            } else {
                throw nav2_core::OperationFailed("Operation " + operations[i]->type + " does not exist in route operations loaded!");
            }
        }

        // Process operations which trigger on any status changes
        processOperationsPluginVector(change_operations_, result, node, edge_entered, edge_exited, route, pose);
    }

    // Process operations which trigger regardless of status change or nodes / edges
    processOperationsPluginVector(query_operations_, result, node, edge_entered /*edge_curr*/, edge_exited, route, pose);
    return result;
}
