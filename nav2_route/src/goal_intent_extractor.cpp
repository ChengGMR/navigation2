...
void GoalIntentExtractor::configure(nav2::LifecycleNode::SharedPtr node, Graph& graph, GraphToIDMap* id_to_graph_map,
                                    std::shared_ptr<tf2_ros::Buffer> tf, std::shared_ptr<nav2_costmap_2d::CostmapSubscriber> costmap_subscriber,
                                    const std::string& route_frame, const std::string& base_frame) {
    ...
    nav2::declare_parameter_if_not_declared(node, "prune_goal", rclcpp::ParameterValue(true));
    prune_goal_ = node->get_parameter("prune_goal").as_bool();

    nav2::declare_parameter_if_not_declared(node, "max_prune_dist_from_edge", rclcpp::ParameterValue(8.0));
    max_dist_from_edge_ = static_cast<float>(node->get_parameter("max_prune_dist_from_edge").as_double());
    nav2::declare_parameter_if_not_declared(node, "min_prune_dist_from_goal", rclcpp::ParameterValue(0.15));
    min_dist_from_goal_ = static_cast<float>(node->get_parameter("min_prune_dist_from_goal").as_double());
    nav2::declare_parameter_if_not_declared(node, "min_prune_dist_from_start", rclcpp::ParameterValue(0.10));
    min_dist_from_start_ = static_cast<float>(node->get_parameter("min_prune_dist_from_start").as_double());

    nav2::declare_parameter_if_not_declared(node, "enable_nn_search", rclcpp::ParameterValue(true));
    enable_search_ = node->get_parameter("enable_nn_search").as_bool();
    nav2::declare_parameter_if_not_declared(node, "max_nn_search_iterations", rclcpp::ParameterValue(10000));
    max_nn_search_iterations_ = node->get_parameter("max_nn_search_iterations").as_int();

    nav2::declare_parameter_if_not_declared(node, "num_nearest_nodes", rclcpp::ParameterValue(5));
...
geometry_msgs::msg::PoseStamped GoalIntentExtractor::transformPose(geometry_msgs::msg::PoseStamped& pose, const std::string& target_frame) {
    if (pose.header.frame_id != target_frame) {
        RCLCPP_INFO(logger_, "Request pose in %s frame. Converting to route server frame: %s.", pose.header.frame_id.c_str(), target_frame.c_str());
...
NodeExtents GoalIntentExtractor::findStartandGoal(const std::shared_ptr<const GoalT> goal) {
    ...
    if (!node_spatial_tree_->findNearestGraphNodesToPose(start_, start_route) || !node_spatial_tree_->findNearestGraphNodesToPose(goal_, end_route)) {
        throw nav2_core::IndeterminantNodesOnGraph("Could not determine node closest to start or goal pose requested!");
    }
    ...
    if (enable_search) {
        try {
            ...
            RCLCPP_WARN(logger_,
                        ...
                        "Falling back to closest euclidean route node instead.",
                        ex.what());
        }
    }
    ...
    if (enable_search && start_route.size() > 1u) {
        ...
        if (los_checker.worldToMap(candidate_nodes.front().pose.position, transformed_start.pose.position)) {
    ...
    if (enable_search && end_route.size() > 1u) {
        ...
        GoalIntentSearch::LoSCollisionChecker los_checker(costmap);
        if (los_checker.worldToMap(candidate_nodes.front().pose.position, transformed_end.pose.position)) {
            if (los_checker.isInCollision()) {
                GoalIntentSearch::BreadthFirstSearch bfs(costmap);
...
template <typename GoalT>
Route GoalIntentExtractor::pruneStartandGoal(const Route& input_route, const std::shared_ptr<const GoalT> goal, ReroutingState& rerouting_info) {
...
template Route GoalIntentExtractor::pruneStartandGoal<nav2_msgs::action::ComputeRoute::Goal>(
    const Route& input_route, const std::shared_ptr<const nav2_msgs::action::ComputeRoute::Goal> goal, ReroutingState& rerouting_info);
template Route GoalIntentExtractor::pruneStartandGoal<nav2_msgs::action::ComputeAndTrackRoute::Goal>(
    const Route& input_route, const std::shared_ptr<const nav2_msgs::action::ComputeAndTrackRoute::Goal> goal, ReroutingState& rerouting_info);
...
template NodeExtents GoalIntentExtractor::findStartandGoal<nav2_msgs::action::ComputeAndTrackRoute::Goal>(
