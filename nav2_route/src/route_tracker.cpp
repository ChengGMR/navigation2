...
void RouteTracker::configure(nav2::LifecycleNode::SharedPtr node, std::shared_ptr<tf2_ros::Buffer> tf_buffer,
                             std::shared_ptr<nav2_costmap_2d::CostmapSubscriber> costmap_subscriber, std::shared_ptr<ActionServerTrack> action_server,
    ...
    nav2::declare_parameter_if_not_declared(node, "radius_to_achieve_node", rclcpp::ParameterValue(2.0));
    radius_threshold_ = node->get_parameter("radius_to_achieve_node").as_double();
    nav2::declare_parameter_if_not_declared(node, "boundary_radius_to_achieve_node", rclcpp::ParameterValue(1.0));
    boundary_radius_threshold_ = node->get_parameter("boundary_radius_to_achieve_node").as_double();
    nav2::declare_parameter_if_not_declared(node, "tracker_update_rate", rclcpp::ParameterValue(50.0));
    tracker_update_rate_ = node->get_parameter("tracker_update_rate").as_double();
    nav2::declare_parameter_if_not_declared(node, "aggregate_blocked_ids", rclcpp::ParameterValue(false));
...
bool RouteTracker::nodeAchieved(const geometry_msgs::msg::PoseStamped& pose, RouteTrackingState& state, const Route& route) {
    ...
    const bool in_radius = (dist_mag <= (is_boundary_node ? boundary_radius_threshold_ : radius_threshold_));
...
bool RouteTracker::isStartOrEndNode(RouteTrackingState& state, const Route& route) {
    ...
    return (state.route_edges_idx == static_cast<int>(route.edges.size() - 1)) || (state.route_edges_idx == -1 && !state.current_edge);
}

void RouteTracker::publishFeedback(const bool rereouted, const unsigned int next_node_id, const unsigned int last_node_id, const unsigned int edge_id,
...
TrackerResult RouteTracker::trackRoute(const Route& route, const nav_msgs::msg::Path& path, ReroutingState& rerouting_info) {
    ...
    if (rerouting_info.curr_edge) {
        ...
        publishFeedback(true, route.start_node->nodeid, state.last_node->nodeid, state.current_edge->edgeid, {});
    } else {
    ...
    while (rclcpp::ok()) {
        ...
        OperationsResult ops_result = operations_manager_->process(status_change, state, route, robot_pose, rerouting_info);
        ...
        if ((status_change || !ops_result.operations_triggered.empty()) && state.current_edge) {
            publishFeedback(false, // No rerouting occurred
                            state.next_node->nodeid, state.last_node->nodeid, state.current_edge->edgeid, ops_result.operations_triggered);
        }
        ...
        if (ops_result.reroute) {
            if (!aggregate_blocked_ids_) {
                ...
                rerouting_info.blocked_ids.insert(rerouting_info.blocked_ids.end(), ops_result.blocked_ids.begin(), ops_result.blocked_ids.end());
            }
