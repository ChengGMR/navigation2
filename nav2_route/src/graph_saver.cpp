...
GraphSaver::GraphSaver(nav2::LifecycleNode::SharedPtr node, std::shared_ptr<tf2_ros::Buffer> tf, const std::string frame)
    : plugin_loader_("nav2_route", "nav2_route::GraphFileSaver"), default_plugin_id_("GeoJsonGraphFileSaver") {
    ...
    nav2::declare_parameter_if_not_declared(node, "graph_file_saver", rclcpp::ParameterValue(default_plugin_id_));
    auto graph_file_saver_id = node->get_parameter("graph_file_saver").as_string();
    if (graph_file_saver_id == default_plugin_id_) {
        nav2::declare_parameter_if_not_declared(node, default_plugin_id_ + ".plugin", rclcpp::ParameterValue(default_plugin_type));
    }
    ...
    try {
        ...
        RCLCPP_INFO(logger_, "Created GraphFileSaver %s of type %s", graph_file_saver_id.c_str(), plugin_type_.c_str());
        graph_file_saver_->configure(node);
    } catch (pluginlib::PluginlibException& ex) {
        RCLCPP_FATAL(logger_, "Failed to create GraphFileSaver. Exception: %s", ex.what());
        throw ex;
    }
}

bool GraphSaver::saveGraphToFile(Graph& graph, std::string filepath) {
    ...
    RCLCPP_INFO(logger_, "Saving graph file from %s, by parser %s", filepath.c_str(), plugin_type_.c_str());
...
bool GraphSaver::transformGraph(Graph& graph) {
    ...
    for (auto& node : graph) {
        ...
        if (cached_transforms.find(node_frame) == cached_transforms.end()) {
            ...
            if (!got_transform) {
                RCLCPP_WARN(logger_, "Could not get transform from node frame %s to route frame %s", node_frame.c_str(), route_frame_.c_str());
                return false;
            }
