...
void PathConverter::configure(nav2::LifecycleNode::SharedPtr node) {
    ...
    nav2::declare_parameter_if_not_declared(node, "path_density", rclcpp::ParameterValue(0.05));
    density_ = static_cast<float>(node->get_parameter("path_density").as_double());
    nav2::declare_parameter_if_not_declared(node, "smoothing_radius", rclcpp::ParameterValue(1.0));
    smoothing_radius_ = static_cast<float>(node->get_parameter("smoothing_radius").as_double());
    nav2::declare_parameter_if_not_declared(node, "smooth_corners", rclcpp::ParameterValue(false));
...
nav_msgs::msg::Path PathConverter::densify(const Route& route, const ReroutingState& rerouting_info, const std::string& frame,
    ...
    if (!route.edges.empty()) {
        ...
        for (unsigned int i = 0; i < route.edges.size() - 1; i++) {
            ...
            if (corner_arc.isCornerValid() && smooth_corners_) {
                ...
                if (smooth_corners_) {
                    RCLCPP_WARN(logger_, "Unable to smooth corner between edge %i and edge %i", edge->edgeid, next_edge->edgeid);
                }
                interpolateEdge(start.x, start.y, end.x, end.y, path.poses);
                start = end;
            }
    ...
    if (route.edges.empty()) {
        ...
        path.poses.push_back(utils::toMsg(route.edges.back()->end->coords.x, route.edges.back()->end->coords.y));
    }
    ...
    if (!route.edges.empty()) {
        ...
        path.poses.back().pose.orientation = nav2_util::geometry_utils::orientationAroundZAxis(atan2(dy, dx));
    }
...
void PathConverter::interpolateEdge(float x0, float y0, float x1, float y1, std::vector<geometry_msgs::msg::PoseStamped>& poses) {
