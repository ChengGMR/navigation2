...
RouteServer::RouteServer(const rclcpp::NodeOptions& options) : nav2::LifecycleNode("route_server", "", options) {}

nav2::CallbackReturn RouteServer::on_configure(const rclcpp_lifecycle::State& /*state*/) {
    ...
    graph_vis_publisher_ = node->create_publisher<visualization_msgs::msg::MarkerArray>("route_graph", nav2::qos::LatchedPublisherQoS());

    compute_route_server_ = create_action_server<ComputeRoute>("compute_route", std::bind(&RouteServer::computeRoute, this), nullptr,
                                                               std::chrono::milliseconds(500), true);

    compute_and_track_route_server_ = create_action_server<ComputeAndTrackRoute>(
        "compute_and_track_route", std::bind(&RouteServer::computeAndTrackRoute, this), nullptr, std::chrono::milliseconds(500), true);

    set_graph_service_ = node->create_service<nav2_msgs::srv::SetRouteGraph>(
        std::string(node->get_name()) + "/set_route_graph",
        std::bind(&RouteServer::setRouteGraph, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));

    declare_parameter_if_not_declared(node, "route_frame", rclcpp::ParameterValue(std::string("map")));
    declare_parameter_if_not_declared(node, "base_frame", rclcpp::ParameterValue(std::string("base_link")));
    declare_parameter_if_not_declared(node, "max_planning_time", rclcpp::ParameterValue(2.0));
    ...
    nav2::declare_parameter_if_not_declared(node, "costmap_topic", rclcpp::ParameterValue(std::string("global_costmap/costmap_raw")));
    ...
    try {
        ...
        goal_intent_extractor_->configure(node, graph_, &id_to_graph_map_, tf_, costmap_subscriber_, route_frame_, base_frame_);
        ...
        route_tracker_->configure(node, tf_, costmap_subscriber_, compute_and_track_route_server_, route_frame_, base_frame_);
...
nav2::CallbackReturn RouteServer::on_activate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn RouteServer::on_deactivate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn RouteServer::on_cleanup(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn RouteServer::on_shutdown(const rclcpp_lifecycle::State&) {
    RCLCPP_INFO(get_logger(), "Shutting down");
    return nav2::CallbackReturn::SUCCESS;
}

rclcpp::Duration RouteServer::findPlanningDuration(const rclcpp::Time& start_time) {
    auto cycle_duration = this->now() - start_time;
    if (max_planning_time_ > 0.0 && cycle_duration.seconds() > max_planning_time_) {
        RCLCPP_WARN(get_logger(), "Route planner missed its desired rate of %.4f Hz. Current loop rate is %.4f Hz", 1 / max_planning_time_,
...
bool RouteServer::isRequestValid(typename nav2::SimpleActionServer<ActionT>::SharedPtr& action_server) {
...
void RouteServer::populateActionResult(std::shared_ptr<ComputeRoute::Result> result, const Route& route, const nav_msgs::msg::Path& path,
...
void RouteServer::populateActionResult(std::shared_ptr<ComputeAndTrackRoute::Result> result, const Route&, const nav_msgs::msg::Path&,
                                       const rclcpp::Duration& execution_duration) {
    result->execution_duration = execution_duration;
}

template <typename GoalT>
Route RouteServer::findRoute(const std::shared_ptr<const GoalT> goal, ReroutingState& rerouting_info) {
    ...
    if (start_route == end_route) {
        ...
        route = route_planner_->findRoute(graph_, start_route, end_route, rerouting_info.blocked_ids, route_request);
    }
...
template <typename ActionT>
void RouteServer::processRouteRequest(typename nav2::SimpleActionServer<ActionT>::SharedPtr& action_server) {
    ...
    try {
        while (rclcpp::ok()) {
            ...
            RCLCPP_INFO(get_logger(), "Route found with %zu nodes and %zu edges", route.edges.size() + 1u, route.edges.size());
...
void RouteServer::setRouteGraph(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
    ...
    RCLCPP_WARN(get_logger(), "Failed to set new route graph: %s!", request->graph_filepath.c_str());
    response->success = false;
}

template <typename GoalT>
void RouteServer::exceptionWarning(const std::shared_ptr<const GoalT> goal, const std::exception& ex) {
    RCLCPP_WARN(get_logger(),
                "Route server failed on request: Start: [(%0.2f, %0.2f) / %i] Goal: [(%0.2f, %0.2f) / %i]:"
                " \"%s\"",
                goal->start.pose.position.x, goal->start.pose.position.y, goal->start_id, goal->goal.pose.position.x, goal->goal.pose.position.y,
                goal->goal_id, ex.what());
}
