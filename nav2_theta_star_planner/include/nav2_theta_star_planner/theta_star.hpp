...
class ThetaStar {
   ...
   protected:
     ...
     * @param cost denotes the total straight line traversal cost; it adds the traversal cost for the node (cx, cy) at every instance; it is also
     * being returned
     ...
     */
    inline double getCost(const int& cx, const int& cy) const { return 26 + 0.9 * costmap_->getCost(cx, cy); }
     ...
     */
    inline double getEuclideanCost(const int& ax, const int& ay, const int& bx, const int& by) { return w_euc_cost_ * std::hypot(ax - bx, ay - by); }
     ...
     */
    inline double getHCost(const int& cx, const int& cy) { return w_heuristic_cost_ * std::hypot(cx - dst_.x, cy - dst_.y); }

    /**
     * @brief checks if the given coordinates(cx, cy) lies within the map
     * @return the result of the check
     */
    inline bool withinLimits(const int& cx, const int& cy) const { return cx >= 0 && cx < size_x_ && cy >= 0 && cy < size_y_; }
     ...
     */
    inline void addIndex(const int& cx, const int& cy, tree_node* node_this) { node_position_[size_x_ * cy + cx] = node_this; }
