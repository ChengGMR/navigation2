...
class test_theta_star : public theta_star::ThetaStar {
   public:
    ...
    bool ulosCheck(const int& x0, const int& y0, const int& x1, const int& y1, double& sl_cost) { return losCheck(x0, y0, x1, y1, sl_cost); }
    ...
    bool runAlgo(std::vector<coordsW>& path, std::function<bool()> cancel_checker = []() { return false; }) {
        if (!isUnsafeToPlan()) {
...
TEST(ThetaStarPlanner, test_theta_star_planner) {
    nav2::LifecycleNode::SharedPtr life_node = std::make_shared<nav2::LifecycleNode>("ThetaStarPlannerTest");
...
TEST(ThetaStarPlanner, test_theta_star_reconfigure) {
    ...
    auto rec_param = std::make_shared<rclcpp::AsyncParametersClient>(life_node->get_node_base_interface(), life_node->get_node_topics_interface(),
                                                                     life_node->get_node_graph_interface(), life_node->get_node_services_interface());

    auto results = rec_param->set_parameters_atomically(
        {rclcpp::Parameter("test.how_many_corners", 8), rclcpp::Parameter("test.w_euc_cost", 1.0), rclcpp::Parameter("test.w_traversal_cost", 2.0),
         rclcpp::Parameter("test.use_final_approach_orientation", false), rclcpp::Parameter("test.allow_unknown", false),
