...
class TestServiceClient : public ServiceClient<std_srvs::srv::Empty> {
   public:
    TestServiceClient(const std::string& name, const rclcpp::Node::SharedPtr& provided_node = rclcpp::Node::SharedPtr(),
...
TEST(ServiceClient, can_ServiceClient_use_passed_in_node) {
    std::vector<std::string> introspection_modes = {"disabled", "metadata", "contents"};
...
TEST(ServiceClient, can_ServiceClient_invoke_in_callback) {
    ...
    auto service = service_node->create_service<std_srvs::srv::Empty>(
        "empty_srv", [&a](std_srvs::srv::Empty::Request::SharedPtr, std_srvs::srv::Empty::Response::SharedPtr) { a = 1; });
    auto srv_thread = std::thread([&]() { rclcpp::spin(service_node); });

    auto pub_node = rclcpp::Node::make_shared("pub_node");
    auto pub = pub_node->create_publisher<std_msgs::msg::Empty>("empty_topic", rclcpp::QoS(1).transient_local());
    ...
    auto sub = sub_node->create_subscription<std_msgs::msg::Empty>("empty_topic", rclcpp::QoS(1), [&client](std_msgs::msg::Empty::SharedPtr) {
...
TEST(ServiceClient, can_ServiceClient_async_call) {
    ...
    auto req = std::make_shared<std_srvs::srv::Empty::Request>();
    auto callback = [&callback_called](rclcpp::Client<std_srvs::srv::Empty>::SharedFuture /*future*/) { callback_called = true; };
    // Test async_call
