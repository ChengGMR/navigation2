...
class FibonacciServerNode : public rclcpp::Node {
   public:
    ...
    void on_init() {
        action_server_ =
            std::make_shared<nav2::SimpleActionServer<Fibonacci>>(shared_from_this(), "fibonacci", std::bind(&FibonacciServerNode::execute, this));

        deactivate_subs_ = create_subscription<std_msgs::msg::Empty>("deactivate_server", 1, [this](std_msgs::msg::Empty::UniquePtr /*msg*/) {
            RCLCPP_INFO(this->get_logger(), "Deactivating");
            action_server_->deactivate();
        });

        activate_subs_ = create_subscription<std_msgs::msg::Empty>("activate_server", 1, [this](std_msgs::msg::Empty::UniquePtr /*msg*/) {
            RCLCPP_INFO(this->get_logger(), "Activating");
            action_server_->activate();
        });

        omit_preempt_subs_ = create_subscription<std_msgs::msg::Empty>("omit_preemption", 1, [this](std_msgs::msg::Empty::UniquePtr /*msg*/) {
...
class RclCppFixture {
   public:
    ...
    void Setup() { server_thread_ = std::make_shared<std::thread>(std::bind(&RclCppFixture::server_thread_func, this)); }
...
class ActionTestNode : public rclcpp::Node {
   public:
    ActionTestNode() : rclcpp::Node(nav2::generate_internal_node_name("action_test_node")) {}
...
TEST_F(ActionTest, test_simple_action) {
    ...
    EXPECT_EQ(rclcpp::spin_until_future_complete(node_, future_goal_handle), rclcpp::FutureReturnCode::SUCCESS);
    ...
    EXPECT_EQ(rclcpp::spin_until_future_complete(node_, future_result), rclcpp::FutureReturnCode::SUCCESS);
...
TEST_F(ActionTest, test_simple_action_with_feedback) {
    ...
    auto feedback_callback = [&feedback_sum](rclcpp_action::ClientGoalHandle<Fibonacci>::SharedPtr /*goal_handle*/,
...
TEST_F(ActionTest, test_simple_action_preemption_after_succeeded) {
    ...
    // Wait for the result of initial goal
    future_result = node_->action_client_->async_get_result(goal_handle);
    ASSERT_EQ(rclcpp::spin_until_future_complete(node_, future_result), rclcpp::FutureReturnCode::SUCCESS);
...
TEST_F(ActionTest, test_handle_cancel) {
    ...
    EXPECT_EQ(rclcpp::spin_until_future_complete(node_, cancel_response), rclcpp::FutureReturnCode::SUCCESS);
