...
class TestServiceServer : public ServiceServer<std_srvs::srv::Empty> {
   public:
    TestServiceServer(const std::string& name, const rclcpp::Node::SharedPtr& provided_node, CallbackType callback)
...
TEST(ServiceServer, can_handle_all_introspection_modes) {
    ...
    for (const auto& mode : introspection_modes) {
        ...
        auto callback = [&a](const std::shared_ptr<rmw_request_id_t>, const std::shared_ptr<std_srvs::srv::Empty::Request>,
                             std::shared_ptr<std_srvs::srv::Empty::Response>) { a = 1; };
