...
class ServiceClient {
   public:
    ...
    explicit ServiceClient(const std::string& service_name, const NodeT& provided_node, bool use_internal_executor = false)
        ...
        if (use_internal_executor) {
            callback_group_ = provided_node->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive, false);
            callback_group_executor_ = std::make_shared<rclcpp::executors::SingleThreadedExecutor>();
            callback_group_executor_->add_callback_group(callback_group_, provided_node->get_node_base_interface());
        }
        // When a nullptr is passed, the client will use the default callback group
        client_ = rclcpp::create_client<ServiceT>(provided_node->get_node_base_interface(), provided_node->get_node_graph_interface(),
                                                  provided_node->get_node_services_interface(), service_name,
                                                  rclcpp::ServicesQoS(), // Use consistent QoS settings
                                                  callback_group_);

        nav2::setIntrospectionMode(this->client_, provided_node->get_node_parameters_interface(), clock_);
    }
    ...
    typename ResponseType::SharedPtr invoke(typename RequestType::SharedPtr& request,
        ...
        while (!client_->wait_for_service(std::chrono::seconds(1))) {
            ...
            RCLCPP_INFO(logger_, "%s service client: waiting for service to appear...", service_name_.c_str());

            if (clock_->now() - now > wait_for_service_timeout) {
                throw std::runtime_error(service_name_ + " service client: timed out waiting for service");
            }
        }

        RCLCPP_DEBUG(logger_, "%s service client: send async request", service_name_.c_str());
     ...
     */
    bool wait_for_service(const std::chrono::nanoseconds timeout = std::chrono::nanoseconds::max()) { return client_->wait_for_service(timeout); }
