...
class LifecycleNode : public rclcpp_lifecycle::LifecycleNode {
   public:
    ...
    LifecycleNode(const std::string& node_name, const std::string& ns, const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
        ...
        this->set_parameter(rclcpp::Parameter(bond::msg::Constants::DISABLE_HEARTBEAT_TIMEOUT_PARAM, true));
    ...
    inline ParameterT declare_or_get_parameter(const std::string& parameter_name,
                                               const ParameterDescriptor& parameter_descriptor = ParameterDescriptor()) {
        return nav2::declare_or_get_parameter<ParameterT>(this, parameter_name, parameter_descriptor);
    }
    ...
    inline ParamType declare_or_get_parameter(const std::string& parameter_name, const ParamType& default_value,
                                              const ParameterDescriptor& parameter_descriptor = ParameterDescriptor()) {
        return nav2::declare_or_get_parameter(this, parameter_name, default_value, parameter_descriptor);
    }
    ...
    typename nav2::Subscription<MessageT>::SharedPtr create_subscription(const std::string& topic_name, CallbackT&& callback,
                                                                         const rclcpp::QoS& qos = nav2::qos::StandardTopicQoS(),
                                                                         const rclcpp::CallbackGroup::SharedPtr& callback_group = nullptr) {
        return nav2::interfaces::create_subscription<MessageT>(shared_from_this(), topic_name, std::forward<CallbackT>(callback), qos,
                                                               callback_group);
    }
    ...
    typename nav2::Publisher<MessageT>::SharedPtr create_publisher(const std::string& topic_name,
                                                                   const rclcpp::QoS& qos = nav2::qos::StandardTopicQoS(),
                                                                   const rclcpp::CallbackGroup::SharedPtr& callback_group = nullptr) {
        auto pub = nav2::interfaces::create_publisher<MessageT>(shared_from_this(), topic_name, qos, callback_group);
    ...
    typename nav2::ServiceClient<ServiceT>::SharedPtr create_client(const std::string& service_name, bool use_internal_executor = false) {
        return nav2::interfaces::create_client<ServiceT>(shared_from_this(), service_name, use_internal_executor);
    }
     ...
     */
    template <typename ServiceT>
    typename nav2::ServiceServer<ServiceT>::SharedPtr create_service(const std::string& service_name,
                                                                     typename nav2::ServiceServer<ServiceT>::CallbackType cb,
                                                                     rclcpp::CallbackGroup::SharedPtr callback_group = nullptr) {
        return nav2::interfaces::create_service<ServiceT>(shared_from_this(), service_name, cb, callback_group);
    }
    ...
    typename nav2::SimpleActionServer<ActionT>::SharedPtr create_action_server(
        const std::string& action_name, typename nav2::SimpleActionServer<ActionT>::ExecuteCallback execute_callback,
        ...
        return nav2::interfaces::create_action_server<ActionT>(shared_from_this(), action_name, execute_callback, compl_cb, server_timeout,
                                                               spin_thread, realtime);
    }
     ...
     */
    template <typename ActionT>
    typename nav2::ActionClient<ActionT>::SharedPtr create_action_client(const std::string& action_name,
                                                                         rclcpp::CallbackGroup::SharedPtr callback_group = nullptr) {
        return nav2::interfaces::create_action_client<ActionT>(shared_from_this(), action_name, callback_group);
    }
    ...
    nav2::LifecycleNode::SharedPtr shared_from_this() {
        return std::static_pointer_cast<nav2::LifecycleNode>(rclcpp_lifecycle::LifecycleNode::shared_from_this());
    }
