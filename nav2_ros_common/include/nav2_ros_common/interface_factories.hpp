...
inline rclcpp::SubscriptionOptions createSubscriptionOptions(
    const std::string& topic_name, const bool allow_parameter_qos_overrides = true,
    const rclcpp::CallbackGroup::SharedPtr callback_group_ptr = nullptr, rclcpp::QOSMessageLostCallbackType qos_message_lost_callback = nullptr,
    ...
    if (allow_parameter_qos_overrides) {
        options.qos_overriding_options = rclcpp::QosOverridingOptions(
            {rclcpp::QosPolicyKind::Depth, rclcpp::QosPolicyKind::Durability, rclcpp::QosPolicyKind::Reliability, rclcpp::QosPolicyKind::History});
    }
    ...
    if (qos_message_lost_callback) {
        ...
        options.event_callbacks.message_lost_callback = [topic_name](rclcpp::QOSMessageLostInfo& info) {
            RCLCPP_WARN(rclcpp::get_logger("nav2::interfaces"), "[%lu] Message was dropped on topic [%s] due to queue size or network constraints.",
                        info.total_count_change, topic_name.c_str());
        };
    }
    ...
    if (subscription_matched_callback) {
        ...
        options.event_callbacks.matched_callback = [topic_name](rclcpp::MatchedInfo& status) {
            if (status.current_count_change > 0) {
                RCLCPP_DEBUG(rclcpp::get_logger("nav2::interfaces"), "Connected: %d new publisher(s) to [%s]. Total active: %zu.",
                             status.current_count_change, topic_name.c_str(), status.current_count);
            } else if (status.current_count_change < 0) {
                RCLCPP_DEBUG(rclcpp::get_logger("nav2::interfaces"), "Disconnected: %d publisher(s) from [%s]. Total active: %zu.",
                             -status.current_count_change, topic_name.c_str(), status.current_count);
            }
        };
    }
 ...
 */
inline rclcpp::PublisherOptions createPublisherOptions(const std::string& topic_name, const bool allow_parameter_qos_overrides = true,
    ...
    if (publisher_matched_callback) {
        ...
        options.event_callbacks.matched_callback = [topic_name](rclcpp::MatchedInfo& status) {
            if (status.current_count_change > 0) {
                RCLCPP_DEBUG(rclcpp::get_logger("nav2::interfaces"), "Connected: %d new subscriber(s) to [%s]. Total active: %zu.",
                             status.current_count_change, topic_name.c_str(), status.current_count);
            } else if (status.current_count_change < 0) {
                RCLCPP_DEBUG(rclcpp::get_logger("nav2::interfaces"), "Disconnected: %d subscriber(s) from [%s]. Total active: %zu.",
                             -status.current_count_change, topic_name.c_str(), status.current_count);
            }
        };
    }
...
typename nav2::Subscription<MessageT>::SharedPtr create_subscription(const NodeT& node, const std::string& topic_name, CallbackT&& callback,
                                                                     const rclcpp::QoS& qos = nav2::qos::StandardTopicQoS(),
                                                                     const rclcpp::CallbackGroup::SharedPtr& callback_group = nullptr) {
    bool allow_parameter_qos_overrides = nav2::declare_or_get_parameter(node, "allow_parameter_qos_overrides", true);

    auto params_interface = node->get_node_parameters_interface();
    auto topics_interface = node->get_node_topics_interface();
    return rclcpp::create_subscription<MessageT, CallbackT>(params_interface, topics_interface, topic_name, qos, std::forward<CallbackT>(callback),
...
typename nav2::Publisher<MessageT>::SharedPtr create_publisher(const NodeT& node, const std::string& topic_name,
                                                               const rclcpp::QoS& qos = nav2::qos::StandardTopicQoS(),
                                                               const rclcpp::CallbackGroup::SharedPtr& callback_group = nullptr) {
    bool allow_parameter_qos_overrides = nav2::declare_or_get_parameter(node, "allow_parameter_qos_overrides", true);
    using PublisherT = nav2::Publisher<MessageT>;
    auto pub = rclcpp::create_publisher<MessageT, std::allocator<void>, PublisherT>(
        *node, topic_name, qos, createPublisherOptions(topic_name, allow_parameter_qos_overrides, callback_group));
    return pub;
}
...
typename nav2::ServiceClient<SrvT>::SharedPtr create_client(const NodeT& node, const std::string& service_name, bool use_internal_executor = false) {
    return std::make_shared<nav2::ServiceClient<SrvT>>(service_name, node, use_internal_executor);
}
...
typename nav2::ServiceServer<SrvT>::SharedPtr create_service(const NodeT& node, const std::string& service_name, CallbackT&& callback,
                                                             rclcpp::CallbackGroup::SharedPtr callback_group = nullptr) {
    using Request = typename SrvT::Request;
    using Response = typename SrvT::Response;
    using CallbackFn = std::function<void(const std::shared_ptr<rmw_request_id_t>, const std::shared_ptr<Request>, std::shared_ptr<Response>)>;
    CallbackFn cb = std::forward<CallbackT>(callback);

    return std::make_shared<nav2::ServiceServer<SrvT>>(service_name, node, cb, callback_group);
}
...
typename nav2::SimpleActionServer<ActionT>::SharedPtr create_action_server(
    const NodeT& node, const std::string& action_name, typename nav2::SimpleActionServer<ActionT>::ExecuteCallback execute_callback,
    typename nav2::SimpleActionServer<ActionT>::CompletionCallback complete_cb = nullptr,
    std::chrono::milliseconds server_timeout = std::chrono::milliseconds(500), bool spin_thread = false, const bool realtime = false) {
    return std::make_shared<nav2::SimpleActionServer<ActionT>>(node, action_name, execute_callback, complete_cb, server_timeout, spin_thread,
                                                               realtime);
}
 ...
 */
template <typename ActionT, typename NodeT>
typename nav2::ActionClient<ActionT>::SharedPtr create_action_client(const NodeT& node, const std::string& action_name,
                                                                     rclcpp::CallbackGroup::SharedPtr callback_group = nullptr) {
    auto client = rclcpp_action::create_client<ActionT>(node, action_name, callback_group);
    nav2::setIntrospectionMode(client, node->get_node_parameters_interface(), node->get_clock());
    return client;
}
