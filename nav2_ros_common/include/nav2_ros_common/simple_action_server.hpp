...
class SimpleActionServer {
   public:
    ...
    explicit SimpleActionServer(NodeT node, const std::string& action_name, ExecuteCallback execute_callback,
                                ...
                                std::chrono::milliseconds server_timeout = std::chrono::milliseconds(500), bool spin_thread = false,
                                const bool realtime = false)
        : SimpleActionServer(node->get_node_base_interface(), node->get_node_clock_interface(), node->get_node_logging_interface(),
                             node->get_node_waitables_interface(), node->get_node_parameters_interface(), action_name, execute_callback,
    ...
    explicit SimpleActionServer(rclcpp::node_interfaces::NodeBaseInterface::SharedPtr node_base_interface,
                                ...
                                rclcpp::node_interfaces::NodeParametersInterface::SharedPtr node_parameters_interface, const std::string& action_name,
        ...
        action_server_ = rclcpp_action::create_server<ActionT>(
            node_base_interface_, node_clock_interface_, node_logging_interface_, node_waitables_interface_, action_name_,
            std::bind(&SimpleActionServer::handle_goal, this, _1, _2), std::bind(&SimpleActionServer::handle_cancel, this, _1),
            std::bind(&SimpleActionServer::handle_accepted, this, _1),
            rcl_action_server_get_default_options(), // Use consistent QoS settings
            callback_group_);

        nav2::setIntrospectionMode(this->action_server_, node_parameters_interface_, node_clock_interface_->get_clock());
    ...
    void work() {
        while (rclcpp::ok() && !stop_execution_ && is_active(current_handle_)) {
            ...
            try {
                ...
                RCLCPP_ERROR(node_logging_interface_->get_logger(), "Action server failed while executing action callback: \"%s\"", ex.what());
                terminate_all();
     ...
     */
    bool is_running() {
        return execution_future_.valid() && (execution_future_.wait_for(std::chrono::milliseconds(0)) == std::future_status::timeout);
    }
   ...
   protected:
    ...
    void log_error_details_if_available(const T& result) {
        if constexpr (has_error_code<typename ActionT::Result>::value && has_error_msg<typename ActionT::Result>::value) {
            warn_msg("Aborting handle. error_code:" + std::to_string(result->error_code) + ", error_msg:'" + result->error_msg + "'.");
    ...
    void debug_msg(const std::string& msg) const {
        RCLCPP_DEBUG(node_logging_interface_->get_logger(), "[%s] [ActionServer] %s", action_name_.c_str(), msg.c_str());
    }
    ...
    void error_msg(const std::string& msg) const {
        RCLCPP_ERROR(node_logging_interface_->get_logger(), "[%s] [ActionServer] %s", action_name_.c_str(), msg.c_str());
    }
