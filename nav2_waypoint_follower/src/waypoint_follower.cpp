...
WaypointFollower::WaypointFollower(const rclcpp::NodeOptions& options)
    ...
    nav2::declare_parameter_if_not_declared(this, std::string("waypoint_task_executor_plugin"),
                                            rclcpp::ParameterValue(std::string("wait_at_waypoint")));
    nav2::declare_parameter_if_not_declared(this, std::string("wait_at_waypoint.plugin"),
                                            rclcpp::ParameterValue(std::string("nav2_waypoint_follower::WaitAtWaypoint")));
}

WaypointFollower::~WaypointFollower() {}

nav2::CallbackReturn WaypointFollower::on_configure(const rclcpp_lifecycle::State& state) {
    ...
    nav_to_pose_client_ = create_action_client<ClientT>("navigate_to_pose", callback_group_);

    xyz_action_server_ = create_action_server<ActionT>("follow_waypoints", std::bind(&WaypointFollower::followWaypointsCallback, this), nullptr,
                                                       std::chrono::milliseconds(500), false);

    from_ll_to_map_client_ = node->create_client<robot_localization::srv::FromLL>("/fromLL", true /*creates and spins an internal executor*/);

    gps_action_server_ = create_action_server<ActionTGPS>("follow_gps_waypoints", std::bind(&WaypointFollower::followGPSWaypointsCallback, this),
                                                          nullptr, std::chrono::milliseconds(500), false);

    try {
        waypoint_task_executor_type_ = nav2::get_plugin_type_param(this, waypoint_task_executor_id_);
        ...
        RCLCPP_FATAL(get_logger(), "Failed to create waypoint_task_executor. Exception: %s", e.what());
        on_cleanup(state);
    }
...
nav2::CallbackReturn WaypointFollower::on_activate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn WaypointFollower::on_deactivate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn WaypointFollower::on_cleanup(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn WaypointFollower::on_shutdown(const rclcpp_lifecycle::State& /*state*/) {
    RCLCPP_INFO(get_logger(), "Shutting down");
    return nav2::CallbackReturn::SUCCESS;
}

template <typename T>
std::vector<geometry_msgs::msg::PoseStamped> WaypointFollower::getLatestGoalPoses(const T& action_server) {
    ...
    if constexpr (std::is_same<T, ActionServer::SharedPtr>::value) {
        ...
        poses = convertGPSPosesToMapPoses(current_goal->gps_poses);
    }
    return poses;
}

template <typename T, typename V, typename Z>
void WaypointFollower::followWaypointsHandler(const T& action_server, const V& feedback, const Z& result) {
    ...
    if (poses.empty()) {
        result->error_code = nav2_msgs::action::FollowWaypoints::Result::NO_VALID_WAYPOINTS;
        result->error_msg =
    ...
    while (rclcpp::ok()) {
        ...
        if (current_goal_status_.status == ActionStatus::FAILED || current_goal_status_.status == ActionStatus::UNKNOWN) {
            ...
            if (stop_on_failure_) {
                result->error_code = nav2_msgs::action::FollowWaypoints::Result::STOP_ON_MISSED_WAYPOINT;
                result->error_msg =
          ...
          " Terminating action.";
            ...
            bool is_task_executed = waypoint_task_executor_->processAtWaypoint(poses[goal_index], goal_index);
            ...
            if (!is_task_executed && stop_on_failure_) {
                result->error_code = nav2_msgs::action::FollowWaypoints::Result::TASK_EXECUTOR_FAILED;
                result->error_msg =
                    "Failed to execute task at waypoint " + std::to_string(goal_index) + " stop on failure is enabled. Terminating action.";
                RCLCPP_WARN(get_logger(), result->error_msg.c_str());
                action_server->terminate_current(result);
                current_goal_status_.error_code = 0;
                current_goal_status_.error_msg = "";
        ...
        if (current_goal_status_.status != ActionStatus::PROCESSING) {
            ...
            if (goal_index >= poses.size()) {
                if (current_loop_no == no_of_loops) {
                    RCLCPP_INFO(get_logger(), "Completed all %zu waypoints requested.", poses.size());
                    action_server->succeeded_current(result);
                    current_goal_status_.error_code = 0;
                    current_goal_status_.error_msg = "";
                    return;
                }
                RCLCPP_INFO(get_logger(), "Starting a new loop, current loop count is %i", current_loop_no);
                goal_index = 0;
                current_loop_no++;
            }
        }
...
void WaypointFollower::followWaypointsCallback() {
    ...
    followWaypointsHandler<typename ActionServer::SharedPtr, ActionT::Feedback::SharedPtr, ActionT::Result::SharedPtr>(xyz_action_server_, feedback,
                                                                                                                       result);
}
...
void WaypointFollower::followGPSWaypointsCallback() {
    ...
    followWaypointsHandler<typename ActionServerGPS::SharedPtr, ActionTGPS::Feedback::SharedPtr, ActionTGPS::Result::SharedPtr>(gps_action_server_,
                                                                                                                                feedback, result);
}
...
std::vector<geometry_msgs::msg::PoseStamped> WaypointFollower::convertGPSPosesToMapPoses(
    ...
    RCLCPP_INFO(this->get_logger(), "Converted all %i GPS waypoint to %s frame", static_cast<int>(poses_in_map_frame_vector.size()),
