...
void PhotoAtWaypoint::initialize(const nav2::LifecycleNode::WeakPtr& parent, const std::string& plugin_name) {
    auto node = parent.lock();

    curr_frame_msg_ = std::make_shared<sensor_msgs::msg::Image>();

    nav2::declare_parameter_if_not_declared(node, plugin_name + ".enabled", rclcpp::ParameterValue(true));
    nav2::declare_parameter_if_not_declared(node, plugin_name + ".image_topic", rclcpp::ParameterValue("/camera/color/image_raw"));
    nav2::declare_parameter_if_not_declared(node, plugin_name + ".save_dir", rclcpp::ParameterValue("/tmp/waypoint_images"));
    nav2::declare_parameter_if_not_declared(node, plugin_name + ".image_format", rclcpp::ParameterValue("png"));
    ...
    if (!is_enabled_) {
        ...
        RCLCPP_INFO(logger_, "Initializing photo at waypoint plugin, subscribing to camera topic named; %s", image_topic_.c_str());
        camera_image_subscriber_ =
            node->create_subscription<sensor_msgs::msg::Image>(image_topic_, std::bind(&PhotoAtWaypoint::imageCallback, this, std::placeholders::_1));
    }
}

bool PhotoAtWaypoint::processAtWaypoint(const geometry_msgs::msg::PoseStamped& curr_pose, const int& curr_waypoint_index) {
    if (!is_enabled_) {
    ...
    try {
        ...
        RCLCPP_INFO(logger_, "Photo has been taken successfully at waypoint %i", curr_waypoint_index);
    } catch (const std::exception& e) {
        RCLCPP_ERROR(logger_,
