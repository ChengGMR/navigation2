...
class LifecycleNodeTest : public nav2::LifecycleNode {
   public:
    LifecycleNodeTest() : nav2::LifecycleNode("lifecycle_node_test") {}
...
TEST(LifecycleClientTest, BasicTest) {
    ...
    EXPECT_EQ(nav2_lifecycle_manager::SystemStatus::TIMEOUT, client.is_active(std::chrono::nanoseconds(1000)));
    EXPECT_TRUE(client.startup());
    EXPECT_EQ(nav2_lifecycle_manager::SystemStatus::ACTIVE, client.is_active(std::chrono::nanoseconds(1000000000)));
    EXPECT_EQ(nav2_lifecycle_manager::SystemStatus::ACTIVE, client.is_active());
    EXPECT_TRUE(client.pause());
    EXPECT_EQ(nav2_lifecycle_manager::SystemStatus::INACTIVE, client.is_active(std::chrono::nanoseconds(1000000000)));
