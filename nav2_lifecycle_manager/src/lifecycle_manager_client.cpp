...
SystemStatus LifecycleManagerClient::is_active(const std::chrono::nanoseconds timeout) {
    ...
    RCLCPP_DEBUG(logger_, "Waiting for the %s service...", active_service_name_.c_str());

    if (!is_active_client_->wait_for_service(std::chrono::seconds(1))) {
        return SystemStatus::TIMEOUT;
    }

    RCLCPP_DEBUG(logger_, "Sending %s request", active_service_name_.c_str());
...
bool LifecycleManagerClient::callService(uint8_t command, const std::chrono::nanoseconds timeout) {
    ...
    RCLCPP_DEBUG(logger_, "Waiting for the %s service...", manage_service_name_.c_str());
    ...
    RCLCPP_DEBUG(logger_, "Sending %s request", manage_service_name_.c_str());
    try {
