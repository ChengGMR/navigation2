...
LifecycleManager::LifecycleManager(const rclcpp::NodeOptions& options) : Node("lifecycle_manager", options), diagnostics_updater_(this) {
    ...
    service_timeout_ = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::duration<double>(service_timeout_s));
...
void LifecycleManager::setState(const NodeState& state) {
    managed_nodes_state_ = state;
    publishIsActiveState();
}

inline bool LifecycleManager::isActive() {
    return managed_nodes_state_ == NodeState::ACTIVE;
}

void LifecycleManager::publishIsActiveState() {
...
void LifecycleManager::createLifecycleServiceServers() {
    ...
    manager_srv_ =
        nav2::interfaces::create_service<ManageLifecycleNodes>(shared_from_this(), get_name() + std::string("/manage_nodes"),
                                                               std::bind(&LifecycleManager::managerCallback, this, _1, _2, _3), callback_group_);

    is_active_srv_ =
        nav2::interfaces::create_service<std_srvs::srv::Trigger>(shared_from_this(), get_name() + std::string("/is_active"),
                                                                 std::bind(&LifecycleManager::isActiveCallback, this, _1, _2, _3), callback_group_);
}

void LifecycleManager::createLifecyclePublishers() {
    message("Creating and initializing lifecycle publishers");

    is_active_pub_ = nav2::interfaces::create_publisher<std_msgs::msg::Bool>(shared_from_this(), get_name() + std::string("/managed_nodes_activated"),
                                                                             nav2::qos::LatchedPublisherQoS(), callback_group_);
...
void LifecycleManager::destroyLifecyclePublishers() {
...
void LifecycleManager::createBondTimer() {
    ...
    bond_timer_ = this->create_wall_timer(200ms, std::bind(&LifecycleManager::checkBondConnections, this), callback_group_);
}
    ...
    for (auto& node_name : node_names_) {
        ...
        if (bond_map_[node_name]->isBroken()) {
            ...
            if (attempt_respawn_reconnection_) {
                bond_respawn_timer_ = this->create_wall_timer(1s, std::bind(&LifecycleManager::checkBondRespawnConnection, this), callback_group_);
            }
            return;
        }
    }
