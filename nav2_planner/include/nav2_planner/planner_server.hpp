...
class PlannerServer : public nav2::LifecycleNode {
   ...
   protected:
     ...
     */
    template <typename T>
    void getPreemptedGoalIfRequested(typename nav2::SimpleActionServer<T>::SharedPtr& action_server,
     ...
     */
    void isPathValid(const std::shared_ptr<rmw_request_id_t> request_header, const std::shared_ptr<nav2_msgs::srv::IsPathValid::Request> request,
    ...
    void exceptionWarning(const geometry_msgs::msg::PoseStamped& start, const geometry_msgs::msg::PoseStamped& goal, const std::string& planner_id,
                          const std::exception& ex, std::string& msg);
