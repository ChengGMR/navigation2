...
TEST(WPTest, test_dynamic_parameters) {
    ...
    auto rec_param = std::make_shared<rclcpp::AsyncParametersClient>(planner->get_node_base_interface(), planner->get_node_topics_interface(),
                                                                     planner->get_node_graph_interface(), planner->get_node_services_interface());

    auto results = rec_param->set_parameters_atomically(
        {rclcpp::Parameter("expected_planner_frequency", 100.0), rclcpp::Parameter("allow_partial_planning", true)});
    ...
    results = rec_param->set_parameters_atomically(
        {rclcpp::Parameter("expected_planner_frequency", -1.0), rclcpp::Parameter("allow_partial_planning", false)});
