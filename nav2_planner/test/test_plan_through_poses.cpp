...
class PlannerServerWrapper : public nav2_planner::PlannerServer {
   public:
    PlannerServerWrapper() : nav2_planner::PlannerServer(rclcpp::NodeOptions()) {}
    ...
    nav2_costmap_2d::Costmap2DROS::SharedPtr getCostmapROS() { return costmap_ros_; }

    nav2_costmap_2d::Costmap2D* getCostmap() { return costmap_ros_->getCostmap(); }
};
...
class Tester : public ::testing::Test {
   public:
    ...
    void setGoalFromPoses(const std::vector<geometry_msgs::msg::Pose>& poses, Action::Goal& goal_out);

    void waitForResult(ActionGHFuture& gh_future, Tester::ActionGoalHandle::WrappedResult& result_out, const std::chrono::milliseconds& timeout);
...
Tester::Tester() {
    ...
    map_publisher_ = tester_node_->create_publisher<nav_msgs::msg::OccupancyGrid>("/map", nav2::qos::LatchedPublisherQoS());

    broadcast_timer_ = tester_node_->create_wall_timer(200ms, std::bind(&Tester::broadcastRobotPose, this));
}
...
void Tester::setParameters() {
    ...
    planner_->set_parameter(rclcpp::Parameter(PLANNER_PLUGINS.front() + ".plugin", PLANNER_PLUGIN_NAME));

    planner_->declare_parameter(PLANNER_PLUGINS.front() + ".tolerance", rclcpp::ParameterValue(PLANNER_TOLERANCE));
    ...
    costmap->declare_parameter("static_layer.map_subscribe_transient_local", rclcpp::ParameterValue(true));
...
void Tester::setGoalFromPoses(const std::vector<geometry_msgs::msg::Pose>& poses, Action::Goal& goal_out) {
...
void Tester::waitForResult(ActionGHFuture& gh_future, Tester::ActionGoalHandle::WrappedResult& result_out, const std::chrono::milliseconds& timeout) {
...
void Tester::addObstacleCells(unsigned int x, unsigned int y, unsigned int width, unsigned int height) {
    const unsigned int size_x = map_.info.width;
    const unsigned int size_y = map_.info.height;

    RCLCPP_DEBUG_STREAM(tester_node_->get_logger(), "Adding obstacle at: " << x << ", " << y << ", " << "with size " << width << ", " << height);
    ...
    RCLCPP_DEBUG_STREAM(tester_node_->get_logger(), "Bounding values for reference:" << min_x << ", " << max_x << ", " << min_y << ", " << max_y);
