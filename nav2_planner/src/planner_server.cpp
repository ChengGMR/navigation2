...
PlannerServer::PlannerServer(const rclcpp::NodeOptions& options)
    ...
    costmap_ros_ = std::make_shared<nav2_costmap_2d::Costmap2DROS>("global_costmap", std::string{get_namespace()},
...
nav2::CallbackReturn PlannerServer::on_configure(const rclcpp_lifecycle::State& state) {
    ...
    for (size_t i = 0; i != planner_ids_.size(); i++) {
        try {
            planner_types_[i] = nav2::get_plugin_type_param(node, planner_ids_[i]);
            nav2_core::GlobalPlanner::Ptr planner = gp_loader_.createUniqueInstance(planner_types_[i]);
            RCLCPP_INFO(get_logger(), "Created global planner plugin %s of type %s", planner_ids_[i].c_str(), planner_types_[i].c_str());
    ...
    if (expected_planner_frequency > 0) {
        ...
        RCLCPP_WARN(get_logger(),
                    ...
                    " than 0.0 to turn on duration overrun warning messages",
    ...
    action_server_pose_ = create_action_server<ActionToPose>("compute_path_to_pose", std::bind(&PlannerServer::computePlan, this), nullptr,
                                                             std::chrono::milliseconds(500), true);

    action_server_poses_ = create_action_server<ActionThroughPoses>(
        "compute_path_through_poses", std::bind(&PlannerServer::computePlanThroughPoses, this), nullptr, std::chrono::milliseconds(500), true);

    return nav2::CallbackReturn::SUCCESS;
}

nav2::CallbackReturn PlannerServer::on_activate(const rclcpp_lifecycle::State& /*state*/) {
    ...
    is_path_valid_service_ = create_service<nav2_msgs::srv::IsPathValid>(
        "is_path_valid", std::bind(&PlannerServer::isPathValid, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));

    // Add callback for dynamic parameters
    dyn_params_handler_ = add_on_set_parameters_callback(std::bind(&PlannerServer::dynamicParametersCallback, this, _1));
...
nav2::CallbackReturn PlannerServer::on_deactivate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn PlannerServer::on_cleanup(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn PlannerServer::on_shutdown(const rclcpp_lifecycle::State&) {
    RCLCPP_INFO(get_logger(), "Shutting down");
    return nav2::CallbackReturn::SUCCESS;
}

template <typename T>
bool PlannerServer::isServerInactive(typename nav2::SimpleActionServer<T>::SharedPtr& action_server) {
...
template <typename T>
bool PlannerServer::isCancelRequested(typename nav2::SimpleActionServer<T>::SharedPtr& action_server) {
...
template <typename T>
void PlannerServer::getPreemptedGoalIfRequested(typename nav2::SimpleActionServer<T>::SharedPtr& action_server,
...
template <typename T>
bool PlannerServer::getStartPose(typename std::shared_ptr<const typename T::Goal> goal, geometry_msgs::msg::PoseStamped& start) {
    if (goal->use_start) {
        start = goal->start;
...
bool PlannerServer::transformPosesToGlobalFrame(geometry_msgs::msg::PoseStamped& curr_start, geometry_msgs::msg::PoseStamped& curr_goal) {
    if (!costmap_ros_->transformPoseToGlobalFrame(curr_start, curr_start) || !costmap_ros_->transformPoseToGlobalFrame(curr_goal, curr_goal)) {
        return false;
    }
...
template <typename T>
bool PlannerServer::validatePath(const geometry_msgs::msg::PoseStamped& goal, const nav_msgs::msg::Path& path, const std::string& planner_id) {
    if (path.poses.empty()) {
        RCLCPP_WARN(get_logger(),
...
void PlannerServer::computePlanThroughPoses() {
    ...
    try {
        if (isServerInactive<ActionThroughPoses>(action_server_poses_) || isCancelRequested<ActionThroughPoses>(action_server_poses_)) {
            return;
        }
        ...
        auto cancel_checker = [this]() { return action_server_poses_->is_cancel_requested(); };
        ...
        for (unsigned int i = 0; i != goal->goals.goals.size(); i++) {
            ...
            try {
                ...
                RCLCPP_WARN(get_logger(), "Planner server failed to compute full path. Outputting partial path instead.");
                break;
            }

            if (!validatePath<ActionThroughPoses>(curr_goal, curr_path, goal->planner_id)) {
                auto exception = nav2_core::NoValidPathCouldBeFound(goal->planner_id + " generated a empty path");
            ...
            if (i == 0) {
                ...
                concat_path.poses.insert(concat_path.poses.end(), curr_path.poses.begin() + 1, curr_path.poses.end());
            }
        ...
        if (max_planner_duration_ && cycle_duration.seconds() > max_planner_duration_) {
            RCLCPP_WARN(get_logger(), "Planner loop missed its desired rate of %.4f Hz. Current loop rate is %.4f Hz", 1 / max_planner_duration_,
...
void PlannerServer::computePlan() {
    ...
    try {
        if (isServerInactive<ActionToPose>(action_server_pose_) || isCancelRequested<ActionToPose>(action_server_pose_)) {
            return;
        }
...
nav_msgs::msg::Path PlannerServer::getPlan(const geometry_msgs::msg::PoseStamped& start, const geometry_msgs::msg::PoseStamped& goal,
...
void PlannerServer::isPathValid(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
    ...
    if (costmap_ros_->getRobotPose(current_pose)) {
        ...
        for (unsigned int i = closest_point_index; i < request->path.poses.size(); ++i) {
            ...
            if (use_radius
                && (cost >= request->max_cost || cost == nav2_costmap_2d::LETHAL_OBSTACLE || cost == nav2_costmap_2d::INSCRIBED_INFLATED_OBSTACLE)) {
                response->is_valid = false;
                break;
...
void PlannerServer::exceptionWarning(const geometry_msgs::msg::PoseStamped& start, const geometry_msgs::msg::PoseStamped& goal,
                                     const std::string& planner_id, const std::exception& ex, std::string& error_msg) {
    std::stringstream ss;
    ss << std::fixed << std::setprecision(2) << planner_id << "plugin failed to plan from (" << start.pose.position.x << ", " << start.pose.position.y
