...
class ParametersHandler {
   public:
     ...
     */
    explicit ParametersHandler(const nav2::LifecycleNode::WeakPtr& parent, std::string& name);
...
inline auto ParametersHandler::getParamGetter(const std::string& ns) {
    return [this, ns](auto& setting, const std::string& name, auto default_value, ParameterType param_type = ParameterType::Dynamic) {
...
template <typename SettingT, typename ParamT>
void ParametersHandler::getParam(SettingT& setting, const std::string& name, ParamT default_value, ParameterType param_type) {
    auto node = node_.lock();

    nav2::declare_parameter_if_not_declared(node, name, rclcpp::ParameterValue(default_value));
...
template <typename ParamT, typename SettingT>
void ParametersHandler::setParam(SettingT& setting, const std::string& name, nav2::LifecycleNode::SharedPtr node) const {
    ParamT param_in{};
...
template <typename T>
void ParametersHandler::setParamCallback(T& setting, const std::string& name, ParameterType param_type) {
    if (get_param_callbacks_.find(name) != get_param_callbacks_.end()) {
        return;
    }

    auto dynamic_callback = [this, &setting, name](const rclcpp::Parameter& param, rcl_interfaces::msg::SetParametersResult& /*result*/) {
        setting = as<T>(param);
        if (verbose_) {
    ...
    auto static_callback = [this, &setting, name](const rclcpp::Parameter& param, rcl_interfaces::msg::SetParametersResult& result) {
