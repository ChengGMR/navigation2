 ...
 */
inline visualization_msgs::msg::Marker createMarker(int id, const geometry_msgs::msg::Pose& pose, const geometry_msgs::msg::Vector3& scale,
 ...
 * @param frame Reference frame to use
 */
inline geometry_msgs::msg::TwistStamped toTwistStamped(float vx, float wz, const builtin_interfaces::msg::Time& stamp, const std::string& frame) {
 ...
 * @param frame Reference frame to use
 */
inline geometry_msgs::msg::TwistStamped toTwistStamped(float vx, float vy, float wz, const builtin_interfaces::msg::Time& stamp,
...
inline std::unique_ptr<nav2_msgs::msg::Trajectory> toTrajectoryMsg(const Eigen::ArrayXXf& trajectory, const models::ControlSequence& control_sequence,
                                                                   const double& model_dt, const std_msgs::msg::Header& header) {
 ...
 * @param data Data to use
 */
inline void findPathCosts(CriticData& data, std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros) {
 ...
 * @param data Data to use
 */
inline void setPathCostsIfNotSet(CriticData& data, std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros) {
 ...
 */
inline float posePointAngle(const geometry_msgs::msg::Pose& pose, double point_x, double point_y, bool forward_preference) {
    ...
    if (!forward_preference) {
        return std::min(fabs(angles::shortest_angular_distance(yaw, pose_yaw)),
 ...
 * @return Angle between two points
 */
inline float posePointAngle(const geometry_msgs::msg::Pose& pose, double point_x, double point_y, double point_yaw) {
 ...
 */
inline void savitskyGolayFilter(models::ControlSequence& control_sequence, std::array<mppi::models::Control, 4>& control_history,
    ...
    auto applyFilterOverAxis = [&](Eigen::ArrayXf& sequence, const Eigen::ArrayXf& initial_sequence, const float hist_0, const float hist_1,
    ...
    applyFilterOverAxis(control_sequence.vx, initial_control_sequence.vx, control_history[0].vx, control_history[1].vx, control_history[2].vx,
                        control_history[3].vx);
    applyFilterOverAxis(control_sequence.vy, initial_control_sequence.vy, control_history[0].vy, control_history[1].vy, control_history[2].vy,
                        control_history[3].vy);
    applyFilterOverAxis(control_sequence.wz, initial_control_sequence.wz, control_history[0].wz, control_history[1].wz, control_history[2].wz,
...
inline auto normalize_yaws_between_points(const Eigen::Ref<const Eigen::ArrayXf>& last_yaws,
    ...
    Eigen::ArrayXf yaws = utils::shortest_angular_distance(last_yaws, yaw_between_points).abs();
    int size = yaws.size();
    Eigen::ArrayXf yaws_between_points_corrected(size);
    for (int i = 0; i != size; i++) {
        const float& yaw_between_point = yaw_between_points[i];
        yaws_between_points_corrected[i] = yaws[i] < M_PIF_2 ? yaw_between_point : angles::normalize_angle(yaw_between_point + M_PIF);
    }
 ...
 * @return Normalized yaw between points
 */
inline auto normalize_yaws_between_points(const float goal_yaw, const Eigen::Ref<const Eigen::ArrayXf>& yaw_between_points) {
    int size = yaw_between_points.size();
    Eigen::ArrayXf yaws_between_points_corrected(size);
    for (int i = 0; i != size; i++) {
        const float& yaw_between_point = yaw_between_points[i];
        yaws_between_points_corrected[i] = fabs(angles::normalize_angle(yaw_between_point - goal_yaw)) < M_PIF_2
                                               ? yaw_between_point
                                               : angles::normalize_angle(yaw_between_point + M_PIF);
    }
    return yaws_between_points_corrected;
}
