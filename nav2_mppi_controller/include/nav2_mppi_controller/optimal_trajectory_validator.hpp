...
class OptimalTrajectoryValidator {
   public:
     ...
     */
    virtual void initialize(const nav2::LifecycleNode::WeakPtr& parent, const std::string& name,
                            const std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap, ParametersHandler* param_handler,
                            const std::shared_ptr<tf2_ros::Buffer> tf_buffer, const models::OptimizerSettings& settings) {
        ...
        if (consider_footprint_) {
            collision_checker_ =
                std::make_unique<nav2_costmap_2d::FootprintCollisionChecker<nav2_costmap_2d::Costmap2D*>>(costmap_ros_->getCostmap());
        }
    }
     ...
     */
    virtual ValidationResult validateTrajectory(const Eigen::ArrayXXf& optimal_trajectory, const models::ControlSequence& /*control_sequence*/,
        ...
        for (size_t i = 0; i < traj_samples_to_evaluate_; ++i) {
            ...
            if (consider_footprint_) {
                ...
                double footprint_cost = collision_checker_->footprintCostAtPose(x, y, theta, costmap_ros_->getRobotFootprint());
