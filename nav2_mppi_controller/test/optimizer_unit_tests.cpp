...
class OptimizerTester : public Optimizer {
   public:
    ...
    void testPrepare(const geometry_msgs::msg::PoseStamped& robot_pose, const geometry_msgs::msg::Twist& robot_speed, const nav_msgs::msg::Path& plan,
    ...
    models::ControlConstraints& getControlConstraints() { return settings_.constraints; }
    ...
    void integrateStateVelocitiesWrapper(models::Trajectories& traj, const models::State& state) { return integrateStateVelocities(traj, state); }
};
...
TEST(OptimizerTests, BasicInitializedFunctions) {
    ...
    auto costmap_ros = std::make_shared<nav2_costmap_2d::Costmap2DROS>("dummy_costmap", "", true);
...
TEST(OptimizerTests, Omni_openLoopMppiTest) {
    ...
    auto [cmd1, optimal_trajectory1] = optimizer_tester.evalControl(pose, robot_speed, path, goal, nullptr);

    EXPECT_LE(std::abs(cmd1.twist.linear.x), optimizer_tester.getSettings().model_dt * optimizer_tester.getControlConstraints().ax_max);
    EXPECT_LE(std::abs(cmd1.twist.angular.z), optimizer_tester.getSettings().model_dt * optimizer_tester.getControlConstraints().az_max);
    EXPECT_LE(std::abs(cmd1.twist.linear.y), optimizer_tester.getSettings().model_dt * optimizer_tester.getControlConstraints().ay_max);

    auto [cmd2, optimal_trajectory2] = optimizer_tester.evalControl(pose, robot_speed, path, goal, nullptr);
    ...
    EXPECT_LE(vx_delta, optimizer_tester.getSettings().model_dt * optimizer_tester.getControlConstraints().ax_max);
    EXPECT_LE(wz_delta, optimizer_tester.getSettings().model_dt * optimizer_tester.getControlConstraints().az_max);
    EXPECT_LE(vy_delta, optimizer_tester.getSettings().model_dt * optimizer_tester.getControlConstraints().ay_max);
}
