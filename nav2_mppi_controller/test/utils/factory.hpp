 ...
 */
void setUpOptimizerParams(const TestOptimizerSettings& s, const std::vector<std::string>& critics, std::vector<rclcpp::Parameter>& params_,
...
void setUpControllerParams(bool visualize, std::vector<rclcpp::Parameter>& params_, std::string node_name = std::string("dummy")) {
...
std::shared_ptr<nav2_costmap_2d::Costmap2D> getDummyCostmap(TestCostmapSettings s) {
    auto costmap = std::make_shared<nav2_costmap_2d::Costmap2D>(s.cells_x, s.cells_y, s.resolution, s.origin_x, s.origin_y, s.cost_map_default_value);
...
nav2::LifecycleNode::SharedPtr getDummyNode(TestOptimizerSettings s, std::vector<std::string> critics, std::string node_name = std::string("dummy")) {
    auto node = std::make_shared<nav2::LifecycleNode>(node_name, getOptimizerOptions(s, critics));
    return node;
}

nav2::LifecycleNode::SharedPtr getDummyNode(rclcpp::NodeOptions options, std::string node_name = std::string("dummy")) {
...
std::shared_ptr<mppi::Optimizer> getDummyOptimizer(TNode node, TCostMap costmap_ros, TFBuffer tf_buffer, TParamHandler* params_handler) {
...
template <typename TNode, typename TCostMap, typename TFBuffer, typename TParamHandler>
mppi::PathHandler getDummyPathHandler(TNode node, TCostMap costmap_ros, TFBuffer tf_buffer, TParamHandler* params_handler) {
...
std::shared_ptr<nav2_mppi_controller::MPPIController> getDummyController(TNode node, TFBuffer tf_buffer, TCostMap costmap_ros) {
...
geometry_msgs::msg::PoseStamped getDummyPointStamped(TNode& node, std::string frame = std::string("odom")) {
