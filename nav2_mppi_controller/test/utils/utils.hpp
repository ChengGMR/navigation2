...
void sendTf(std::string_view source, std::string_view dest, std::shared_ptr<tf2_ros::TransformBroadcaster> tf_broadcaster,
...
void printMapWithTrajectoryAndGoal(nav2_costmap_2d::Costmap2D& costmap, const TTrajectory& trajectory, const geometry_msgs::msg::PoseStamped& goal) {
    ...
    // create new costmap
    nav2_costmap_2d::Costmap2D costmap2d(costmap.getSizeInCellsX(), costmap.getSizeInCellsY(), costmap.getResolution(), costmap.getOriginX(),
                                         costmap.getOriginY(), costmap.getDefaultValue());
 ...
 */
void addObstacle(nav2_costmap_2d::Costmap2D* costmap, unsigned int upper_left_corner_x, unsigned int upper_left_corner_y, unsigned int size,
                 unsigned char cost) {
...
void printInfo(TestOptimizerSettings os, TestPathSettings ps, const std::vector<std::string>& critics) {
    std::stringstream ss;
...
template <typename TTrajectory>
bool isGoalReached(const TTrajectory& trajectory, const nav2_costmap_2d::Costmap2D& costmap, const geometry_msgs::msg::PoseStamped& goal) {
    ...
    auto match_near = [&](unsigned int i, unsigned int j) {
        if (match(i, j, goal_i, goal_j) || match(i, j, goal_i + 1, goal_j) || match(i, j, goal_i - 1, goal_j) || match(i, j, goal_i, goal_j + 1)
            || match(i, j, goal_i, goal_j - 1) || match(i, j, goal_i + 1, goal_j + 1) || match(i, j, goal_i + 1, goal_j - 1)
