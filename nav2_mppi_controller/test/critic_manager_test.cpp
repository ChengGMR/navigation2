...
class CriticManagerWrapper : public CriticManager {
   public:
    ...
    virtual void loadCritics() {
        ...
        critics_.back()->on_configure(parent_, name_, name_ + "." + "DummyCritic", costmap_ros_, parameters_handler_);
    }
...
TEST(CriticManagerTests, BasicCriticOperations) {
    ...
    Eigen::ArrayXf costs;
    float model_dt = 0.1;
    CriticData data = {state, generated_trajectories, path, goal, costs, model_dt, false, nullptr, nullptr, std::nullopt, std::nullopt};
...
TEST(CriticManagerTests, CriticLoadingTest) {
    auto node = std::make_shared<nav2::LifecycleNode>("my_node");
    node->declare_parameter("critic_manager.critics", rclcpp::ParameterValue(std::vector<std::string>{"ConstraintCritic", "PreferForwardCritic"}));
    auto costmap_ros = std::make_shared<nav2_costmap_2d::Costmap2DROS>("dummy_costmap", "", true);
    std::string name = "test";
    ParametersHandler param_handler(node, name);
