...
class TestGoalChecker : public nav2_core::GoalChecker {
   public:
    ...
    virtual bool getTolerances(geometry_msgs::msg::Pose& pose_tolerance, geometry_msgs::msg::Twist& /*vel_tolerance*/) {
...
TEST(UtilsTests, FurthestAndClosestReachedPoint) {
    ...
    CriticData data = {state,   generated_trajectories, path,        goal, costs, model_dt, false, nullptr,
...
TEST(UtilsTests, SmootherTest) {
    ...
    auto noises = Eigen::ArrayXf::NullaryExpr(30, [&]() { return normal_dist(engine); });
...
TEST(UtilsTests, NormalizeYawsBetweenPointsTest) {
    ...
    Eigen::ArrayXf yaws_between_points_corrected = utils::normalize_yaws_between_points(last_yaws, yaw_between_points);
    ...
    yaws_between_points_corrected = utils::normalize_yaws_between_points(last_yaws, yaw_between_points);
    EXPECT_TRUE(yaws_between_points_corrected.isApprox(yaw_between_points));
    ...
    float goal_angle = 0;
    yaws_between_points_corrected = utils::normalize_yaws_between_points(goal_angle, yaw_between_points);
    EXPECT_NEAR(yaws_between_points_corrected[1], -0.8 * M_PIF_2, 1e-3);
}

TEST(UtilsTests, toTrajectoryMsgTest) {
    Eigen::ArrayXXf trajectory(5, 3);
    trajectory << 0.0, 0.0, 0.0, 1.0, 1.0, 1.0, 2.0, 2.0, 2.0, 3.0, 3.0, 3.0, 4.0, 4.0, 4.0;
