...
class ParametersHandlerWrapper : public ParametersHandler {
   public:
    ...
    explicit ParametersHandlerWrapper(const nav2::LifecycleNode::WeakPtr& parent, std::string name) : ParametersHandler(parent, name) {}
...
TEST(ParameterHandlerTest, asTypeConversionTest) {
    ...
    rclcpp::Parameter doublev_p("test.doublev_parameter", rclcpp::ParameterValue(std::vector<double>{10.0}));
    rclcpp::Parameter boolv_p("test.boolv_parameter", rclcpp::ParameterValue(std::vector<bool>{false}));
    rclcpp::Parameter stringv_p("test.stringv_parameter", rclcpp::ParameterValue(std::vector<std::string>{std::string("hello")}));
...
TEST(ParameterHandlerTest, PrePostDynamicCallbackTest) {
    ...
    auto dynamicCb = [&](const rclcpp::Parameter& /*param*/, rcl_interfaces::msg::SetParametersResult& /*result*/) { dynamic_triggered = true; };
...
TEST(ParameterHandlerTest, DynamicAndStaticParametersTest) {
    ...
    std::shared_future<rcl_interfaces::msg::SetParametersResult> result_future = rec_param->set_parameters_atomically(
        {rclcpp::Parameter("my_node.verbose", true), rclcpp::Parameter("test.dynamic_int", 10), rclcpp::Parameter("test.static_int", 10)});

    auto rc = rclcpp::spin_until_future_complete(node->get_node_base_interface(), result_future);
    ...
    EXPECT_EQ(result.reason,
              std::string("Rejected change to static parameter: ") + "{\"name\": \"test.static_int\", \"type\": \"integer\", \"value\": \"10\"}");
...
TEST(ParameterHandlerTest, DynamicAndStaticParametersNotVerboseTest) {
    ...
    std::shared_future<rcl_interfaces::msg::SetParametersResult> result_future =
        rec_param->set_parameters_atomically({// Don't set default param rclcpp::Parameter("my_node.verbose", false),
                                              rclcpp::Parameter("test.dynamic_int", 10), rclcpp::Parameter("test.static_int", 10)});
...
TEST(ParameterHandlerTest, DynamicAndStaticParametersNotDeclaredTest) {
    ...
    rc = rclcpp::spin_until_future_complete(node->get_node_base_interface(), result_future);
    ASSERT_EQ(rclcpp::FutureReturnCode::SUCCESS, rc);
