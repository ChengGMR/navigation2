...
class PathAngleCriticWrapper : public PathAngleCritic {
   public:
    PathAngleCriticWrapper() : PathAngleCritic() {}

    void setMode(int mode) { mode_ = static_cast<PathAngleMode>(mode); }
};
...
TEST(CriticTests, ObstacleCriticMisalignedParams) {
    ...
    EXPECT_THROW(critic.on_configure(node, "mppi", "critic", costmap_ros, &param_handler), nav2_core::ControllerException);
}
