...
void Optimizer::initialize(nav2::LifecycleNode::WeakPtr parent, const std::string& name, std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros,
                           std::shared_ptr<tf2_ros::Buffer> tf_buffer, ParametersHandler* param_handler) {
    parent_ = parent;
    name_ = name;
    ...
    nav2::declare_parameter_if_not_declared(node, name_ + ".TrajectoryValidator.plugin",
                                            rclcpp::ParameterValue("mppi::DefaultOptimalTrajectoryValidator"));
    std::string validator_plugin_type = nav2::get_plugin_type_param(node, name_ + ".TrajectoryValidator");
    validator_loader_ =
        std::make_unique<pluginlib::ClassLoader<OptimalTrajectoryValidator>>("nav2_mppi_controller", "mppi::OptimalTrajectoryValidator");
    trajectory_validator_ = validator_loader_->createUniqueInstance(validator_plugin_type);
    trajectory_validator_->initialize(parent_, name_ + ".TrajectoryValidator", costmap_ros_, parameters_handler_, tf_buffer, settings_);
...
void Optimizer::getParams() {
    ...
    if (s.base_constraints.ax_min > 0.0) {
        ...
        RCLCPP_WARN(logger_, "Sign of the parameter ax_min is incorrect, consider setting it negative.");
    }

    if (s.base_constraints.ay_min > 0.0) {
        s.base_constraints.ay_min = -1.0 * s.base_constraints.ay_min;
        RCLCPP_WARN(logger_, "Sign of the parameter ay_min is incorrect, consider setting it negative.");
    }
...
void Optimizer::setOffset(double controller_frequency) {
    ...
    if ((controller_period + eps) < settings_.model_dt) {
        ...
        throw nav2_core::ControllerException("Controller period more then model dt, set it equal to model dt");
    }
}
...
void Optimizer::reset(bool reset_dynamic_speed_limits) {
    ...
    trajectory_validator_->initialize(parent_, name_ + ".TrajectoryValidator", costmap_ros_, parameters_handler_, tf_buffer_, settings_);
...
std::tuple<geometry_msgs::msg::TwistStamped, Eigen::ArrayXXf> Optimizer::evalControl(const geometry_msgs::msg::PoseStamped& robot_pose,
    ...
    do {
        ...
        switch (trajectory_validator_->validateTrajectory(optimal_trajectory, control_sequence_, robot_pose, robot_speed, plan, goal)) {
            ...
            case mppi::ValidationResult::FAILURE:
                throw nav2_core::NoValidControl("Trajectory validator failed to validate trajectory, hard reset triggered.");
...
void Optimizer::prepare(const geometry_msgs::msg::PoseStamped& robot_pose, const geometry_msgs::msg::Twist& robot_speed,
                        const nav_msgs::msg::Path& plan, const geometry_msgs::msg::Pose& goal, nav2_core::GoalChecker* goal_checker) {
...
void Optimizer::integrateStateVelocities(Eigen::Array<float, Eigen::Dynamic, 3>& trajectory, const Eigen::ArrayXXf& sequence) const {
...
void Optimizer::setMotionModel(const std::string& model) {
    if (model == "DiffDrive") {
        ...
        throw nav2_core::ControllerException(std::string("Model " + model
