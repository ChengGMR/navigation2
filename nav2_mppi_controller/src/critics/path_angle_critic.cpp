...
void PathAngleCritic::initialize() {
    ...
    getParam(max_angle_to_furthest_, "max_angle_to_furthest", 0.785398f);
    ...
    RCLCPP_INFO(logger_, "PathAngleCritic instantiated with %d power and %f weight. Mode set to: %s", power_, weight_, modeToStr(mode_).c_str());
}
...
void PathAngleCritic::score(CriticData& data) {
    ...
    auto offsetted_idx = std::min(*data.furthest_reached_path_point + offset_from_furthest_, static_cast<size_t>(data.path.x.size()) - 1);
    ...
    auto yaws_between_points = diff_y.binaryExpr(diff_x, [&](const float& y, const float& x) { return atan2f(y, x); }).eval();

    switch (mode_) {
        case PathAngleMode::FORWARD_PREFERENCE: {
            auto last_yaws = data.trajectories.yaws.col(last_idx);
            auto yaws = utils::shortest_angular_distance(last_yaws, yaws_between_points).abs();
            if (power_ > 1u) {
        ...
        case PathAngleMode::NO_DIRECTIONAL_PREFERENCE: {
            auto last_yaws = data.trajectories.yaws.col(last_idx);
            auto yaws_between_points_corrected = utils::normalize_yaws_between_points(last_yaws, yaws_between_points);
            auto corrected_yaws = utils::shortest_angular_distance(last_yaws, yaws_between_points_corrected).abs();
            if (power_ > 1u) {
        ...
        case PathAngleMode::CONSIDER_FEASIBLE_PATH_ORIENTATIONS: {
            auto last_yaws = data.trajectories.yaws.col(last_idx);
            auto yaws_between_points_corrected = utils::normalize_yaws_between_points(goal_yaw, yaws_between_points);
            auto corrected_yaws = utils::shortest_angular_distance(last_yaws, yaws_between_points_corrected).abs();
            if (power_ > 1u) {
                data.costs += (corrected_yaws * weight_).pow(power_);
            } else {
