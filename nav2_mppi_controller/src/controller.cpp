...
void MPPIController::configure(const nav2::LifecycleNode::WeakPtr& parent, std::string name, const std::shared_ptr<tf2_ros::Buffer> tf,
    ...
    if (publish_optimal_trajectory_) {
        opt_traj_pub_ = node->create_publisher<nav2_msgs::msg::Trajectory>("~/optimal_trajectory");
    }
...
geometry_msgs::msg::TwistStamped MPPIController::computeVelocityCommands(const geometry_msgs::msg::PoseStamped& robot_pose,
    ...
    auto [cmd, optimal_trajectory] = optimizer_.evalControl(robot_pose, robot_speed, transformed_plan, goal, goal_checker);
    ...
    if (publish_optimal_trajectory_ && opt_traj_pub_->get_subscription_count() > 0) {
        ...
        auto trajectory_msg =
            utils::toTrajectoryMsg(optimal_trajectory, optimizer_.getOptimalControlSequence(), optimizer_.getSettings().model_dt, trajectory_header);
