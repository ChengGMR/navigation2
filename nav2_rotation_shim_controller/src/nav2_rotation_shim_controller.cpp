...
RotationShimController::RotationShimController()
    : lp_loader_("nav2_core", "nav2_core::Controller"), primary_controller_(nullptr), path_updated_(false), in_rotation_(false) {}

void RotationShimController::configure(const nav2::LifecycleNode::WeakPtr& parent, std::string name, std::shared_ptr<tf2_ros::Buffer> tf,
    ...
    try {
        ...
        RCLCPP_INFO(logger_, "Created internal controller for rotation shimming: %s of type %s", plugin_name_.c_str(),
                    params_->primary_controller.c_str());
    } catch (const pluginlib::PluginlibException& ex) {
        RCLCPP_FATAL(logger_, "Failed to create internal controller for rotation shimming. Exception: %s", ex.what());
        return;
    }
...
geometry_msgs::msg::TwistStamped RotationShimController::computeVelocityCommands(const geometry_msgs::msg::PoseStamped& pose,
        ...
        try {
            ...
            if (params_->use_path_orientations) {
                angular_distance_to_heading =
                    angles::shortest_angular_distance(tf2::getYaw(pose.pose.orientation), tf2::getYaw(sampled_pt.pose.orientation));
            } else {
                geometry_msgs::msg::Pose sampled_pt_base = transformPoseToBaseFrame(sampled_pt);
                angular_distance_to_heading = std::atan2(sampled_pt_base.position.y, sampled_pt_base.position.x);
            }

            double angular_thresh = in_rotation_ ? params_->angular_disengage_threshold : params_->angular_dist_threshold;
            if (abs(angular_distance_to_heading) > angular_thresh) {
                RCLCPP_DEBUG(logger_, "Robot is not within the new path's rough heading, rotating to heading...");
                in_rotation_ = true;
                auto cmd_vel = computeRotateToHeadingCommand(angular_distance_to_heading, pose, velocity);
                last_angular_vel_ = cmd_vel.twist.angular.z;
                return cmd_vel;
...
geometry_msgs::msg::PoseStamped RotationShimController::getSampledPathPt() {
    if (current_path_.poses.size() < 2) {
        throw nav2_core::ControllerException("Path is too short to find a valid sampled path point for rotation.");
    }
...
geometry_msgs::msg::Pose RotationShimController::transformPoseToBaseFrame(const geometry_msgs::msg::PoseStamped& pt) {
...
geometry_msgs::msg::TwistStamped RotationShimController::computeRotateToHeadingCommand(const double& angular_distance_to_heading,
    ...
    double max_vel_to_stop = std::sqrt(2 * params_->max_angular_accel * fabs(angular_distance_to_heading));
...
void RotationShimController::isCollisionFree(const geometry_msgs::msg::TwistStamped& cmd_vel, const double& angular_distance_to_heading,
                                             const geometry_msgs::msg::PoseStamped& pose) {
    ...
    double remaining_rotation_before_thresh = fabs(angular_distance_to_heading) - params_->angular_dist_threshold;
    ...
    while (simulated_time < params_->simulate_ahead_time) {
        ...
        footprint_cost = collision_checker_->footprintCostAtPose(pose.pose.position.x, pose.pose.position.y, yaw, costmap_ros_->getRobotFootprint());
