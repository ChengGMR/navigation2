...
ParameterHandler::ParameterHandler(const nav2::LifecycleNode::SharedPtr& node, std::string& plugin_name, rclcpp::Logger& logger) {
    ...
    try {
        params_.primary_controller = node->declare_or_get_parameter<std::string>(plugin_name_ + ".primary_controller.plugin");
        ...
        throw nav2_core::ControllerException("Failed to get 'primary_controller.plugin' parameter");
    }
...
void ParameterHandler::activate() {
    ...
    post_set_params_handler_ =
        node->add_post_set_parameters_callback(std::bind(&ParameterHandler::updateParametersCallback, this, std::placeholders::_1));
    on_set_params_handler_ =
        node->add_on_set_parameters_callback(std::bind(&ParameterHandler::validateParameterUpdatesCallback, this, std::placeholders::_1));
}
...
rcl_interfaces::msg::SetParametersResult ParameterHandler::validateParameterUpdatesCallback(const std::vector<rclcpp::Parameter>& parameters) {
    ...
    for (const auto& parameter : parameters) {
        ...
        if (param_type == ParameterType::PARAMETER_DOUBLE) {
            if (param_name == plugin_name_ + ".simulate_ahead_time" && parameter.as_double() < 0.0) {
                RCLCPP_WARN(logger_,
