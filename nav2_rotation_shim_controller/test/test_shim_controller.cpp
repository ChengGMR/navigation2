...
class RotationShimShim : public nav2_rotation_shim_controller::RotationShimController {
   public:
    ...
    bool isGoalChangedWrapper(const nav_msgs::msg::Path& path) { return isGoalChanged(path); }

    geometry_msgs::msg::Pose transformPoseToBaseFrameWrapper(geometry_msgs::msg::PoseStamped pt) { return transformPoseToBaseFrame(pt); }

    geometry_msgs::msg::TwistStamped computeRotateToHeadingCommandWrapper(const double& param, const geometry_msgs::msg::PoseStamped& pose,
...
TEST(RotationShimControllerTest, lifecycleTransitions) {
    ...
    auto results = rec_param->set_parameters_atomically({rclcpp::Parameter(
        "PathFollower.primary_controller.plugin", std::string("nav2_regulated_pure_pursuit_controller::RegulatedPurePursuitController"))});
...
TEST(RotationShimControllerTest, setPlanAndSampledPointsTests) {
    ...
    node->declare_parameter("PathFollower.primary_controller.plugin",
...
TEST(RotationShimControllerTest, rotationAndTransformTests) {
    ...
    const geometry_msgs::msg::Twist velocity;
    EXPECT_EQ(controller->computeRotateToHeadingCommandWrapper(0.7, path.poses[1], velocity).twist.angular.z, 1.8);
    EXPECT_EQ(controller->computeRotateToHeadingCommandWrapper(-0.7, path.poses[1], velocity).twist.angular.z, -1.8);

    EXPECT_EQ(controller->computeRotateToHeadingCommandWrapper(0.87, path.poses[1], velocity).twist.angular.z, 1.8);
...
TEST(RotationShimControllerTest, openLoopRotationTests) {
    ...
    node->declare_parameter("PathFollower.rotate_to_goal_heading", true);
    node->declare_parameter("PathFollower.closed_loop", false);
    ...
    geometry_msgs::msg::Twist velocity;
    nav2_controller::SimpleGoalChecker checker;
    node->declare_parameter("checker.xy_goal_tolerance", 1.0);
    checker.initialize(node, "checker", costmap);
...
TEST(RotationShimControllerTest, accelerationTests) {
    ...
    node->declare_parameter("controller_frequency", 20.0);
    node->declare_parameter("PathFollower.rotate_to_goal_heading", true);
    node->declare_parameter("PathFollower.max_angular_accel", 0.5);
...
TEST(RotationShimControllerTest, isGoalChangedTest) {
    ...
    node->declare_parameter("PathFollower.primary_controller.plugin",
                            std::string("nav2_regulated_pure_pursuit_controller::RegulatedPurePursuitController"));
    node->declare_parameter("PathFollower.rotate_to_heading_once", true);
...
TEST(RotationShimControllerTest, testDynamicParameter) {
    ...
    // set a valid primary controller so we can do lifecycle
    node->declare_parameter("test.primary_controller.plugin", std::string("nav2_regulated_pure_pursuit_controller::RegulatedPurePursuitController"));
    ...
    auto results = rec_param->set_parameters_atomically(
        {rclcpp::Parameter("test.angular_dist_threshold", 7.0), rclcpp::Parameter("test.forward_sampling_distance", 7.0),
         rclcpp::Parameter("test.rotate_to_heading_angular_vel", 7.0), rclcpp::Parameter("test.max_angular_accel", 7.0),
         rclcpp::Parameter("test.simulate_ahead_time", 7.0), rclcpp::Parameter("test.primary_controller.plugin", std::string("HI")),
         rclcpp::Parameter("test.rotate_to_goal_heading", true), rclcpp::Parameter("test.rotate_to_heading_once", true),
         rclcpp::Parameter("test.closed_loop", false), rclcpp::Parameter("test.use_path_orientations", true)});
    ...
    results = rec_param->set_parameters_atomically({rclcpp::Parameter("test.angular_dist_threshold", -1.0)});

    rclcpp::spin_until_future_complete(node->get_node_base_interface(), results);

    EXPECT_EQ(node->get_parameter("test.angular_dist_threshold").as_double(), 7.0);

    results = rec_param->set_parameters_atomically({rclcpp::Parameter("test.simulate_ahead_time", -0.1)});
