...
void pluginLoader(rclcpp::Node::SharedPtr node, bool& server_failed, const std::string& server_name, const std::string& plugin_type,
                  QComboBox* combo_box, rclcpp::Executor::SharedPtr executor) {
    ...
    if (executor) {
        if (executor->spin_until_future_complete(parameters) != rclcpp::FutureReturnCode::SUCCESS) {
            RCLCPP_ERROR(node->get_logger(), "Failed to get parameter '%s' from server '%s'", plugin_type.c_str(), server_name.c_str());
            return;
        }
    } else {
    ...
    if (result.empty()) {
        RCLCPP_ERROR(node->get_logger(), "Parameter '%s' not found on server '%s'", plugin_type.c_str(), server_name.c_str());
        return;
    }
...
QString getGoalStatusLabel(std::string title, int8_t status) {
    ...
    return QString(std::string("<table><tr><td width=150><b>" + title + ":</b></td><td>" + status_str + "</td></tr></table>").c_str());
}
