...
Nav2Panel::Nav2Panel(QWidget* parent) : Panel(parent), server_timeout_(100) {
    ...
    accumulated_nav_through_poses_->assignProperty(nr_of_loops_, "text", QString::fromStdString(loop_no_));
    accumulated_nav_through_poses_->assignProperty(nr_of_loops_, "enabled", false);
    accumulated_nav_through_poses_->assignProperty(store_initial_pose_checkbox_, "enabled", false);
    ...
    QObject::connect(nr_of_loops_, &QLineEdit::editingFinished, this, &Nav2Panel::loophandler);
    QObject::connect(store_initial_pose_checkbox_, &QCheckBox::stateChanged, this, &Nav2Panel::initialStateHandler);
    ...
    ROSActionQTransition* idleAccumulatedWpTransition = new ROSActionQTransition(QActionState::INACTIVE);
    ...
    ROSActionQTransition* idleAccumulatedNTPTransition = new ROSActionQTransition(QActionState::INACTIVE);
    ...
    auto options = rclcpp::NodeOptions().arguments({"--ros-args", "--remap", "__node:=rviz_navigation_dialog_action_client", "--"});
    client_node_ = std::make_shared<rclcpp::Node>("_", options);
    executor_ = std::make_shared<rclcpp::executors::SingleThreadedExecutor>();
    executor_->add_node(client_node_);

    client_nav_ = std::make_shared<nav2_lifecycle_manager::LifecycleManagerClient>("lifecycle_manager_navigation", client_node_);
    client_loc_ = std::make_shared<nav2_lifecycle_manager::LifecycleManagerClient>("lifecycle_manager_localization", client_node_);
    ...
    QSignalTransition* activeSignal = new QSignalTransition(initial_thread_, &InitialThread::navigationActive);
    activeSignal->setTargetState(idle_);
    pre_initial_->addTransition(activeSignal);
    QSignalTransition* inactiveSignal = new QSignalTransition(initial_thread_, &InitialThread::navigationInactive);
    ...
    QObject::connect(initial_thread_, &InitialThread::navigationActive,
                     [this, navigation_active] { navigation_status_indicator_->setText(navigation_active); });
    QObject::connect(initial_thread_, &InitialThread::navigationInactive, [this, navigation_inactive] {
    ...
    QObject::connect(initial_thread_, &InitialThread::localizationInactive,
                     [this, localization_inactive] { localization_status_indicator_->setText(localization_inactive); });
    ...
    imgDisplayLabel_->setPixmap(rviz_common::loadPixmap("package://nav2_rviz_plugins/icons/classes/nav2_logo_small.png"));
    ...
    navigation_action_client_ = rclcpp_action::create_client<nav2_msgs::action::NavigateToPose>(client_node_, "navigate_to_pose");
    ...
    nav_through_poses_action_client_ = rclcpp_action::create_client<nav2_msgs::action::NavigateThroughPoses>(client_node_, "navigate_through_poses");

    // Setting up tf for initial pose
    tf2_buffer_ = std::make_shared<tf2_ros::Buffer>(client_node_->get_clock());
    auto timer_interface =
        std::make_shared<tf2_ros::CreateTimerROS>(client_node_->get_node_base_interface(), client_node_->get_node_timers_interface());
...
bool Nav2Panel::isLoopValueValid(std::string& loop_value) {
    ...
    try {
        ...
        waypoint_status_indicator_->setText("<b> Note: </b> Loop value out of range, setting max possible value");
...
void Nav2Panel::handleGoalLoader() {
    ...
    QString file = QFileDialog::getOpenFileName(this, tr("Open File"), "", tr("yaml(*.yaml);;All Files (*)"));
...
geometry_msgs::msg::PoseStamped Nav2Panel::convert_to_msg(std::vector<double> pose, std::vector<double> orientation) {
...
void Nav2Panel::handleGoalSaver() {
    ...
    for (unsigned int i = 0; i < acummulated_poses_.goals.size(); ++i) {
        ...
        out << YAML::BeginMap;
        out << YAML::Key << "pose";
        std::vector<double> pose = {acummulated_poses_.goals[i].pose.position.x, acummulated_poses_.goals[i].pose.position.y,
                                    acummulated_poses_.goals[i].pose.position.z};
        out << YAML::Value << pose;
        out << YAML::Key << "orientation";
        std::vector<double> orientation = {acummulated_poses_.goals[i].pose.orientation.w, acummulated_poses_.goals[i].pose.orientation.x,
                                           acummulated_poses_.goals[i].pose.orientation.y, acummulated_poses_.goals[i].pose.orientation.z};
        out << YAML::Value << orientation;
        out << YAML::EndMap;
    }

    // open dialog to save it in a file
    QString file = QFileDialog::getSaveFileName(this, tr("Open File"), "", tr("yaml(*.yaml);;All Files (*)"));
...
void Nav2Panel::onInitialize() {
    ...
    if (node_ptr_ == nullptr) {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("nav2_panel"), "Underlying ROS node no longer exists, initialization failed");
        return;
    }
    ...
    navigation_feedback_sub_ = node->create_subscription<nav2_msgs::action::NavigateToPose::Impl::FeedbackMessage>(
        ...
        [this](const nav2_msgs::action::NavigateToPose::Impl::FeedbackMessage::SharedPtr msg) {
            if (stoi(nr_of_loops_->displayText().toStdString()) > 0) {
                ...
                navigation_feedback_indicator_->setText(
                    getNavToPoseFeedbackLabel(msg->feedback)
                    + QString(std::string("</td></tr><tr><td width=150>Waypoint:</td><td>" + toString(goal_index_ + 1)).c_str())
                    + QString(std::string("</td></tr><tr><td width=150>Loop:</td><td>" + toString(loop_count_)).c_str()));
            } else {
                navigation_feedback_indicator_->setText(getNavToPoseFeedbackLabel(msg->feedback));
            }
        });
    nav_through_poses_feedback_sub_ = node->create_subscription<nav2_msgs::action::NavigateThroughPoses::Impl::FeedbackMessage>(
    ...
    navigation_goal_status_sub_ = node->create_subscription<action_msgs::msg::GoalStatusArray>(
        "navigate_to_pose/_action/status", rclcpp::SystemDefaultsQoS(), [this](const action_msgs::msg::GoalStatusArray::SharedPtr msg) {
            navigation_goal_status_indicator_->setText(nav2_rviz_plugins::getGoalStatusLabel("Feedback", msg->status_list.back().status));
            // Clearing all the stored values once reaching the final goal
            if (loop_count_ == stoi(nr_of_loops_->displayText().toStdString()) && goal_index_ == static_cast<int>(store_poses_.goals.size()) - 1
    ...
    nav_through_poses_goal_status_sub_ = node->create_subscription<action_msgs::msg::GoalStatusArray>(
        "navigate_through_poses/_action/status", rclcpp::SystemDefaultsQoS(), [this](const action_msgs::msg::GoalStatusArray::SharedPtr msg) {
            navigation_goal_status_indicator_->setText(nav2_rviz_plugins::getGoalStatusLabel("Feedback", msg->status_list.back().status));
...
void Nav2Panel::onPause() {
    QFuture<bool> futureNav = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::pause, client_nav_.get(), std::placeholders::_1), server_timeout_);
    QFuture<bool> futureLoc = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::pause, client_loc_.get(), std::placeholders::_1), server_timeout_);
}

void Nav2Panel::onResume() {
    QFuture<bool> futureNav = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::resume, client_nav_.get(), std::placeholders::_1), server_timeout_);
    QFuture<bool> futureLoc = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::resume, client_loc_.get(), std::placeholders::_1), server_timeout_);
}

void Nav2Panel::onStartup() {
    QFuture<bool> futureNav = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::startup, client_nav_.get(), std::placeholders::_1), server_timeout_);
    QFuture<bool> futureLoc = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::startup, client_loc_.get(), std::placeholders::_1), server_timeout_);
}

void Nav2Panel::onShutdown() {
    QFuture<bool> futureNav = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::reset, client_nav_.get(), std::placeholders::_1), server_timeout_);
    QFuture<bool> futureLoc = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::reset, client_loc_.get(), std::placeholders::_1), server_timeout_);
    timer_.stop();
}
...
void Nav2Panel::onResumedWp() {
    ...
    loop_no_ = std::to_string(stoi(nr_of_loops_->displayText().toStdString()) - loop_count_);
    waypoint_status_indicator_->setText(QString(std::string("<b> Note: </b> Navigation is paused.").c_str()));
}
...
void Nav2Panel::onNewGoal(double x, double y, double theta, QString frame) {
    ...
    if (store_poses_.goals.empty()) {
        ...
        waypoint_status_indicator_->setText(QString(std::string("<b> Note: </b> Cannot set goal in pause state").c_str()));
    }
    updateWpNavigationMarkers();
}
...
void Nav2Panel::onCancelButtonPressed() {
    if (navigation_goal_handle_) {
        ...
        if (executor_->spin_until_future_complete(future_cancel, server_timeout_) != rclcpp::FutureReturnCode::SUCCESS) {
    ...
    if (nav_through_poses_goal_handle_) {
        auto future_cancel = nav_through_poses_action_client_->async_cancel_goal(nav_through_poses_goal_handle_);
...
void Nav2Panel::onAccumulatedWp() {
    if (acummulated_poses_.goals.empty()) {
        ...
        waypoint_status_indicator_->setText("<b> Note: </b> Uh oh! Someone forgot to select the waypoints");
        return;
    }
    ...
    if (store_poses_.goals.empty()) {
        ...
        if (store_initial_pose_) {
            try {
                init_transform = tf2_buffer_->lookupTransform(acummulated_poses_.goals[0].header.frame_id, base_frame_, tf2::TimePointZero);
            } catch (const tf2::TransformException& ex) {
                RCLCPP_INFO(client_node_->get_logger(), "Could not transform %s to %s: %s", acummulated_poses_.goals[0].header.frame_id.c_str(),
            ...
            acummulated_poses_.goals.erase(acummulated_poses_.goals.begin(), acummulated_poses_.goals.begin());
        }
    } else {
...
void Nav2Panel::startWaypointFollowing(std::vector<geometry_msgs::msg::PoseStamped> poses) {
    ...
    RCLCPP_DEBUG(client_node_->get_logger(), "Sending a path of %zu waypoints:", waypoint_follower_goal_.poses.size());
    ...
    auto send_goal_options = nav2::ActionClient<nav2_msgs::action::FollowWaypoints>::SendGoalOptions();
    ...
    if (executor_->spin_until_future_complete(future_goal_handle, server_timeout_) != rclcpp::FutureReturnCode::SUCCESS) {
...
void Nav2Panel::startNavThroughPoses(nav_msgs::msg::Goals poses) {
    auto is_action_server_ready = nav_through_poses_action_client_->wait_for_action_server(std::chrono::seconds(5));
    if (!is_action_server_ready) {
        RCLCPP_ERROR(client_node_->get_logger(),
    ...
    RCLCPP_INFO(client_node_->get_logger(), "NavigateThroughPoses will be called using the BT Navigator's default behavior tree.");

    RCLCPP_DEBUG(client_node_->get_logger(), "Sending a path of %zu waypoints:", nav_through_poses_goal_.poses.goals.size());
    for (auto waypoint : nav_through_poses_goal_.poses.goals) {
        RCLCPP_DEBUG(client_node_->get_logger(), "\t(%lf, %lf)", waypoint.pose.position.x, waypoint.pose.position.y);
    }

    // Enable result awareness by providing an empty lambda function
    auto send_goal_options = nav2::ActionClient<nav2_msgs::action::NavigateThroughPoses>::SendGoalOptions();
...
void Nav2Panel::startNavigation(geometry_msgs::msg::PoseStamped pose) {
    ...
    RCLCPP_INFO(client_node_->get_logger(), "NavigateToPose will be called using the BT Navigator's default behavior tree.");

    // Enable result awareness by providing an empty lambda function
    auto send_goal_options = nav2::ActionClient<nav2_msgs::action::NavigateToPose>::SendGoalOptions();
...
void Nav2Panel::save(rviz_common::Config config) const {
    Panel::save(config);
}

void Nav2Panel::load(const rviz_common::Config& config) {
    Panel::load(config);
}

void Nav2Panel::resetUniqueId() {
    unique_id = 0;
}
...
inline QString Nav2Panel::getNavThroughPosesFeedbackLabel(nav2_msgs::action::NavigateThroughPoses::Feedback msg) {
    return QString(std::string("<table><tr><td width=150>Poses remaining:</td><td>" + std::to_string(msg.number_of_poses_remaining) + "</td></tr>"
                               + toLabel(msg) + "</table>")
                       .c_str());
}
