...
RouteTool::RouteTool(QWidget* parent) : rviz_common::Panel(parent), ui_(std::make_unique<Ui::route_tool>()) {
    // Extend the widget with all attributes and children from UI file
    ui_->setupUi(this);
    node_ = std::make_shared<nav2::LifecycleNode>("route_tool_node", "", rclcpp::NodeOptions());
    node_->configure();
    graph_vis_publisher_ =
        node_->create_publisher<visualization_msgs::msg::MarkerArray>("route_graph", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
    node_->activate();
...
void RouteTool::onInitialize(void) {
    ...
    if (!ros_node_abstraction) {
        RCLCPP_ERROR(node_->get_logger(), "Unable to get ROS node abstraction");
        return;
    }
...
void RouteTool::on_load_button_clicked(void) {
    ...
    graph_.clear();
    QString filename = QFileDialog::getOpenFileName(this, tr("Open Address Book"), "", tr("Address Book (*.geojson);;All Files (*)"));
...
void RouteTool::on_save_button_clicked(void) {
    QString filename = QFileDialog::getSaveFileName(this, tr("Open Address Book"), "", tr("Address Book (*.geojson);;All Files (*)"));
...
void RouteTool::on_create_button_clicked(void) {
    if (ui_->add_field_1->toPlainText() == "" || ui_->add_field_2->toPlainText() == "") {
        return;
    }
...
void RouteTool::on_confirm_button_clicked(void) {
    if (ui_->edit_id->toPlainText() == "" || ui_->edit_field_1->toPlainText() == "" || ui_->edit_field_2->toPlainText() == "") {
        return;
    }
