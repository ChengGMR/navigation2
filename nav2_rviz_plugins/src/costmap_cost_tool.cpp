...
CostmapCostTool::CostmapCostTool() : qos_profile_(5) {
    shortcut_key_ = 'm';

    auto_deactivate_property_ = new rviz_common::properties::BoolProperty("Single click", true, "Switch away from this tool after one click.",
...
void CostmapCostTool::onInitialize() {
    ...
    if (node_ptr_ == nullptr) {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("costmap_cost_tool"), "Underlying ROS node no longer exists, initialization failed");
        return;
    }
    rclcpp::Node::SharedPtr node = node_ptr_->get_raw_node();
    local_cost_client_ = std::make_shared<nav2::ServiceClient<nav2_msgs::srv::GetCosts>>("local_costmap/get_cost_local_costmap", node,
                                                                                         false /* Does not create and spin an internal executor*/);
    global_cost_client_ = std::make_shared<nav2::ServiceClient<nav2_msgs::srv::GetCosts>>("global_costmap/get_cost_global_costmap", node,
                                                                                          false /* Does not create and spin an internal executor*/);
}
...
void CostmapCostTool::callCostService(float x, float y) {
    ...
    if (local_cost_client_->wait_for_service(std::chrono::seconds(1))) {
        local_cost_client_->async_call(request, std::bind(&CostmapCostTool::handleLocalCostResponse, this, std::placeholders::_1));
    }

    // Call global costmap service
    if (global_cost_client_->wait_for_service(std::chrono::seconds(1))) {
        global_cost_client_->async_call(request, std::bind(&CostmapCostTool::handleGlobalCostResponse, this, std::placeholders::_1));
    }
}

void CostmapCostTool::handleLocalCostResponse(rclcpp::Client<nav2_msgs::srv::GetCosts>::SharedFuture future) {
    rclcpp::Node::SharedPtr node = node_ptr_->get_raw_node();
    auto response = future.get();
    RCLCPP_INFO(node->get_logger(), "Local costmap cost: %.1f", response->costs[0]);
}

void CostmapCostTool::handleGlobalCostResponse(rclcpp::Client<nav2_msgs::srv::GetCosts>::SharedFuture future) {
    rclcpp::Node::SharedPtr node = node_ptr_->get_raw_node();
    auto response = future.get();
