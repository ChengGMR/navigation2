...
Selector::Selector(QWidget* parent) : Panel(parent) {
    ...
    pub_goal_checker_ = client_node_->create_publisher<std_msgs::msg::String>("goal_checker_selector", qos);
    pub_smoother_ = client_node_->create_publisher<std_msgs::msg::String>("smoother_selector", qos);
    pub_progress_checker_ = client_node_->create_publisher<std_msgs::msg::String>("progress_checker_selector", qos);
    ...
    connect(controller_, QOverload<int>::of(&QComboBox::activated), this, &Selector::setController);

    connect(planner_, QOverload<int>::of(&QComboBox::activated), this, &Selector::setPlanner);

    connect(goal_checker_, QOverload<int>::of(&QComboBox::activated), this, &Selector::setGoalChecker);

    connect(smoother_, QOverload<int>::of(&QComboBox::activated), this, &Selector::setSmoother);

    connect(progress_checker_, QOverload<int>::of(&QComboBox::activated), this, &Selector::setProgressChecker);
}
...
void Selector::setSelection(QComboBox* combo_box, rclcpp::Publisher<std_msgs::msg::String>::SharedPtr publisher) {
...
void Selector::loadPlugins() {
    load_plugins_thread_ = std::thread([this]() {
        ...
        while (rclcpp::ok() && !plugins_loaded_) {
            ...
            nav2_rviz_plugins::pluginLoader(client_node_, server_failed_, "controller_server", "controller_plugins", controller_);
            nav2_rviz_plugins::pluginLoader(client_node_, server_failed_, "planner_server", "planner_plugins", planner_);
            nav2_rviz_plugins::pluginLoader(client_node_, server_failed_, "controller_server", "goal_checker_plugins", goal_checker_);
            nav2_rviz_plugins::pluginLoader(client_node_, server_failed_, "smoother_server", "smoother_plugins", smoother_);
            nav2_rviz_plugins::pluginLoader(client_node_, server_failed_, "controller_server", "progress_checker_plugins", progress_checker_);
            if (controller_->count() > 0 && planner_->count() > 0 && goal_checker_->count() > 0 && smoother_->count() > 0
