...
DockingPanel::DockingPanel(QWidget* parent) : Panel(parent), server_timeout_(100) {
    ...
    QSignalTransition* activeDockSignal = new QSignalTransition(initial_thread_, &InitialDockThread::dockingActive);
    activeDockSignal->setTargetState(idle_);
    pre_initial_->addTransition(activeDockSignal);

    QSignalTransition* activeUndockSignal = new QSignalTransition(initial_thread_, &InitialDockThread::undockingActive);
    activeUndockSignal->setTargetState(idle_);
    pre_initial_->addTransition(activeUndockSignal);

    QObject::connect(initial_thread_, &InitialDockThread::dockingActive, [this] {
        // Load the plugins if not already loaded
        if (!plugins_loaded_) {
            RCLCPP_INFO(client_node_->get_logger(), "Loading dock plugins");
            nav2_rviz_plugins::pluginLoader(client_node_, server_failed_, "docking_server", "dock_plugins", dock_type_, executor_);
            plugins_loaded_ = true;
        }
    });

    // Connect buttons with functions
    QObject::connect(use_dock_id_checkbox_, &QCheckBox::stateChanged, this, &DockingPanel::dockIdCheckbox);
}
...
void DockingPanel::onInitialize() {
    ...
    if (node_ptr_ == nullptr) {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("docking_panel"), "Underlying ROS node no longer exists, initialization failed");
        return;
    }
    ...
    docking_feedback_sub_ = node->create_subscription<Dock::Impl::FeedbackMessage>(
        "dock_robot/_action/feedback", rclcpp::SystemDefaultsQoS(),
        [this](const Dock::Impl::FeedbackMessage::SharedPtr msg) { docking_feedback_indicator_->setText(getDockFeedbackLabel(msg->feedback)); });

    // Create action goal status subscribers
    docking_goal_status_sub_ = node->create_subscription<action_msgs::msg::GoalStatusArray>(
        "dock_robot/_action/status", rclcpp::SystemDefaultsQoS(), [this](const action_msgs::msg::GoalStatusArray::SharedPtr msg) {
            docking_goal_status_indicator_->setText(nav2_rviz_plugins::getGoalStatusLabel("Feedback", msg->status_list.back().status));
    ...
    undocking_goal_status_sub_ = node->create_subscription<action_msgs::msg::GoalStatusArray>(
        "undock_robot/_action/status", rclcpp::SystemDefaultsQoS(), [this](const action_msgs::msg::GoalStatusArray::SharedPtr msg) {
            docking_goal_status_indicator_->setText(nav2_rviz_plugins::getGoalStatusLabel("Feedback", msg->status_list.back().status));
        });
}
...
void DockingPanel::onDockingButtonPressed() {
    auto is_action_server_ready = dock_client_->wait_for_action_server(std::chrono::seconds(5));
    ...
    if (use_dock_id_) {
        ...
        RCLCPP_INFO(client_node_->get_logger(), "DockRobot will be called using dock id: %s", goal_msg.dock_id.c_str());

    } else {
        if (dock_pose_x_->text().isEmpty() || dock_pose_y_->text().isEmpty() || dock_pose_yaw_->text().isEmpty()) {
        ...
        goal_msg.dock_pose.pose.orientation = nav2_util::geometry_utils::orientationAroundZAxis(dock_pose_yaw_->text().toDouble());
        goal_msg.dock_type = combo_box->currentText().toStdString();

        RCLCPP_INFO(client_node_->get_logger(), "DockRobot will be called using dock pose: (%f, %f) and type: %s", goal_msg.dock_pose.pose.position.x,
                    goal_msg.dock_pose.pose.position.y, goal_msg.dock_type.c_str());
    }
    ...
    send_goal_options.result_callback = [this](const DockGoalHandle::WrappedResult& result) {
        ...
        if (result.result->success) {
            ...
            docking_result_indicator_->setText(QString(std::string("(" + dockErrorToString(result.result->error_code) + ")").c_str()));
        }
    };
...
void DockingPanel::onUndockingButtonPressed() {
    auto is_action_server_ready = undock_client_->wait_for_action_server(std::chrono::seconds(5));
    if (!is_action_server_ready) {
    ...
    RCLCPP_INFO(client_node_->get_logger(), "UndockRobot will be called using dock type: %s", goal_msg.dock_type.c_str());
