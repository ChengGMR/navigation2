...
class VelSmootherShim : public nav2_velocity_smoother::VelocitySmoother {
   public:
    ...
    nav2::CallbackReturn configure(const rclcpp_lifecycle::State& state) { return this->on_configure(state); }
    ...
    void sendCommandMsg(geometry_msgs::msg::TwistStamped::SharedPtr msg) { inputCommandStampedCallback(msg); }
};
...
TEST(VelocitySmootherTest, openLoopTestTimer6dof) {
    ...
    auto subscription = nav2_util::TwistSubscriber(
        smoother, "cmd_vel_smoothed", [&](geometry_msgs::msg::Twist::SharedPtr msg) { linear_vels.push_back(msg->linear.x); },
        [&](geometry_msgs::msg::TwistStamped::SharedPtr msg) { linear_vels.push_back(msg->twist.linear.x); });
...
TEST(VelocitySmootherTest, testDynamicParameter) {
    ...
    auto rec_param = std::make_shared<rclcpp::AsyncParametersClient>(smoother->get_node_base_interface(), smoother->get_node_topics_interface(),
    ...
    auto results = rec_param->set_parameters_atomically(
        {rclcpp::Parameter("smoothing_frequency", 100.0), rclcpp::Parameter("feedback", std::string("CLOSED_LOOP")),
         rclcpp::Parameter("scale_velocities", true), rclcpp::Parameter("max_velocity", max_vel), rclcpp::Parameter("min_velocity", min_vel),
         rclcpp::Parameter("max_accel", max_accel), rclcpp::Parameter("max_decel", min_accel), rclcpp::Parameter("odom_topic", std::string("TEST")),
         rclcpp::Parameter("odom_duration", 2.0), rclcpp::Parameter("velocity_timeout", 4.0), rclcpp::Parameter("deadband_velocity", deadband)});
    ...
    // Test reverting
    results = rec_param->set_parameters_atomically({rclcpp::Parameter("feedback", std::string("OPEN_LOOP"))});
    rclcpp::spin_until_future_complete(smoother->get_node_base_interface(), results);
