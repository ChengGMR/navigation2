...
nav2::CallbackReturn VelocitySmoother::on_configure(const rclcpp_lifecycle::State& state) {
    ...
    if ((size != 3 && size != 6) || min_velocities_.size() != size || max_accels_.size() != size || max_decels_.size() != size
    ...
    for (unsigned int i = 0; i != size; i++) {
        if (max_decels_[i] > 0.0) {
            RCLCPP_ERROR(get_logger(), "Positive values set of deceleration! These should be negative to slow down!");
            on_cleanup(state);
            return nav2::CallbackReturn::FAILURE;
        }
        if (max_accels_[i] < 0.0) {
            RCLCPP_ERROR(get_logger(), "Negative values set of acceleration! These should be positive to speed up!");
            on_cleanup(state);
            return nav2::CallbackReturn::FAILURE;
        }
        if (min_velocities_[i] > 0.0) {
            RCLCPP_ERROR(get_logger(), "Positive values set of min_velocities! These should be negative!");
            on_cleanup(state);
            return nav2::CallbackReturn::FAILURE;
        }
        if (max_velocities_[i] < 0.0) {
            RCLCPP_ERROR(get_logger(), "Negative values set of max_velocities! These should be positive!");
            on_cleanup(state);
            return nav2::CallbackReturn::FAILURE;
        }
    ...
    if (feedback_type == "OPEN_LOOP") {
        ...
        RCLCPP_ERROR(get_logger(), "Invalid feedback_type, options are OPEN_LOOP and CLOSED_LOOP.");
        on_cleanup(state);
        return nav2::CallbackReturn::FAILURE;
    }
    ...
    cmd_sub_ =
        std::make_unique<nav2_util::TwistSubscriber>(node, "cmd_vel", std::bind(&VelocitySmoother::inputCommandCallback, this, std::placeholders::_1),
...
nav2::CallbackReturn VelocitySmoother::on_activate(const rclcpp_lifecycle::State&) {
    RCLCPP_INFO(get_logger(), "Activating");
    smoothed_cmd_pub_->on_activate();
    double timer_duration_ms = 1000.0 / smoothing_frequency_;
    timer_ =
        this->create_wall_timer(std::chrono::milliseconds(static_cast<int>(timer_duration_ms)), std::bind(&VelocitySmoother::smootherTimer, this));
...
nav2::CallbackReturn VelocitySmoother::on_deactivate(const rclcpp_lifecycle::State&) {
...
nav2::CallbackReturn VelocitySmoother::on_cleanup(const rclcpp_lifecycle::State&) {
...
nav2::CallbackReturn VelocitySmoother::on_shutdown(const rclcpp_lifecycle::State&) {
    RCLCPP_INFO(get_logger(), "Shutting down");
    return nav2::CallbackReturn::SUCCESS;
}

void VelocitySmoother::inputCommandStampedCallback(const geometry_msgs::msg::TwistStamped::SharedPtr msg) {
...
void VelocitySmoother::inputCommandCallback(geometry_msgs::msg::Twist::SharedPtr msg) {
...
void VelocitySmoother::smootherTimer() {
    ...
    if (!is_6dof_) {
        command_->twist.linear.x = std::clamp(command_->twist.linear.x, min_velocities_[0], max_velocities_[0]);
        command_->twist.linear.y = std::clamp(command_->twist.linear.y, min_velocities_[1], max_velocities_[1]);
        command_->twist.angular.z = std::clamp(command_->twist.angular.z, min_velocities_[2], max_velocities_[2]);
    } else {
        command_->twist.linear.x = std::clamp(command_->twist.linear.x, min_velocities_[0], max_velocities_[0]);
        command_->twist.linear.y = std::clamp(command_->twist.linear.y, min_velocities_[1], max_velocities_[1]);
        command_->twist.linear.z = std::clamp(command_->twist.linear.z, min_velocities_[2], max_velocities_[2]);
        command_->twist.angular.x = std::clamp(command_->twist.angular.x, min_velocities_[3], max_velocities_[3]);
        command_->twist.angular.y = std::clamp(command_->twist.angular.y, min_velocities_[4], max_velocities_[4]);
        command_->twist.angular.z = std::clamp(command_->twist.angular.z, min_velocities_[5], max_velocities_[5]);
    }
    ...
    if (scale_velocities_) {
        ...
        if (!is_6dof_) {
            ...
            curr_eta = findEtaConstraint(current_.twist.linear.z, command_->twist.linear.z, max_accels_[2], max_decels_[2]);
            if (curr_eta > 0.0 && std::fabs(1.0 - curr_eta) > std::fabs(1.0 - eta)) {
                eta = curr_eta;
            }

            curr_eta = findEtaConstraint(current_.twist.angular.x, command_->twist.angular.x, max_accels_[3], max_decels_[3]);
            if (curr_eta > 0.0 && std::fabs(1.0 - curr_eta) > std::fabs(1.0 - eta)) {
                eta = curr_eta;
            }

            curr_eta = findEtaConstraint(current_.twist.angular.y, command_->twist.angular.y, max_accels_[4], max_decels_[4]);
            if (curr_eta > 0.0 && std::fabs(1.0 - curr_eta) > std::fabs(1.0 - eta)) {
                eta = curr_eta;
            }

            curr_eta = findEtaConstraint(current_.twist.angular.z, command_->twist.angular.z, max_accels_[5], max_decels_[5]);
            if (curr_eta > 0.0 && std::fabs(1.0 - curr_eta) > std::fabs(1.0 - eta)) {
                eta = curr_eta;
            }
        }
    ...
    if (!is_6dof_) {
        cmd_vel->twist.linear.x = applyConstraints(current_.twist.linear.x, command_->twist.linear.x, max_accels_[0], max_decels_[0], eta);
        cmd_vel->twist.linear.y = applyConstraints(current_.twist.linear.y, command_->twist.linear.y, max_accels_[1], max_decels_[1], eta);
        cmd_vel->twist.angular.z = applyConstraints(current_.twist.angular.z, command_->twist.angular.z, max_accels_[2], max_decels_[2], eta);
    } else {
        cmd_vel->twist.linear.x = applyConstraints(current_.twist.linear.x, command_->twist.linear.x, max_accels_[0], max_decels_[0], eta);
        cmd_vel->twist.linear.y = applyConstraints(current_.twist.linear.y, command_->twist.linear.y, max_accels_[1], max_decels_[1], eta);
        cmd_vel->twist.linear.z = applyConstraints(current_.twist.linear.z, command_->twist.linear.z, max_accels_[2], max_decels_[2], eta);
        cmd_vel->twist.angular.x = applyConstraints(current_.twist.angular.x, command_->twist.angular.x, max_accels_[3], max_decels_[3], eta);
        cmd_vel->twist.angular.y = applyConstraints(current_.twist.angular.y, command_->twist.angular.y, max_accels_[4], max_decels_[4], eta);
        cmd_vel->twist.angular.z = applyConstraints(current_.twist.angular.z, command_->twist.angular.z, max_accels_[5], max_decels_[5], eta);
    }
    ...
    if (!is_6dof_) {
        ...
        cmd_vel->twist.linear.x = fabs(cmd_vel->twist.linear.x) < deadband_velocities_[0] ? 0.0 : cmd_vel->twist.linear.x;
        cmd_vel->twist.linear.y = fabs(cmd_vel->twist.linear.y) < deadband_velocities_[1] ? 0.0 : cmd_vel->twist.linear.y;
        cmd_vel->twist.linear.z = fabs(cmd_vel->twist.linear.z) < deadband_velocities_[2] ? 0.0 : cmd_vel->twist.linear.z;
        cmd_vel->twist.angular.x = fabs(cmd_vel->twist.angular.x) < deadband_velocities_[3] ? 0.0 : cmd_vel->twist.angular.x;
        cmd_vel->twist.angular.y = fabs(cmd_vel->twist.angular.y) < deadband_velocities_[4] ? 0.0 : cmd_vel->twist.angular.y;
        cmd_vel->twist.angular.z = fabs(cmd_vel->twist.angular.z) < deadband_velocities_[5] ? 0.0 : cmd_vel->twist.angular.z;
    }
    ...
    for (auto parameter : parameters) {
        ...
        if (param_type == ParameterType::PARAMETER_DOUBLE) {
            if (param_name == "smoothing_frequency") {
                ...
                double timer_duration_ms = 1000.0 / smoothing_frequency_;
                timer_ = this->create_wall_timer(std::chrono::milliseconds(static_cast<int>(timer_duration_ms)),
            ...
            if (parameter.as_double_array().size() != size) {
                RCLCPP_WARN(get_logger(), "Invalid size of parameter %s. Must be size %ld", param_name.c_str(), size);
            ...
            if (param_name == "max_velocity") {
                for (unsigned int i = 0; i != size; i++) {
                    if (parameter.as_double_array()[i] < 0.0) {
                        RCLCPP_WARN(get_logger(), "Negative values set of max_velocity! These should be positive!");
                        result.successful = false;
                    }
                }
                ...
                for (unsigned int i = 0; i != size; i++) {
                    if (parameter.as_double_array()[i] > 0.0) {
                        RCLCPP_WARN(get_logger(), "Positive values set of min_velocity! These should be negative!");
                        result.successful = false;
                    }
                }
                if (result.successful) {
