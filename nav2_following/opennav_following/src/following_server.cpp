...
FollowingServer::FollowingServer(const rclcpp::NodeOptions& options) : nav2::LifecycleNode("following_server", "", options) {
...
nav2::CallbackReturn FollowingServer::on_configure(const rclcpp_lifecycle::State& /*state*/) {
    ...
    following_action_server_ = node->create_action_server<FollowObject>("follow_object", std::bind(&FollowingServer::followObject, this), nullptr,
    ...
    controller_ = std::make_unique<opennav_docking::Controller>(node, tf2_buffer_, fixed_frame_, base_frame_);
    ...
    filtered_dynamic_pose_pub_ = create_publisher<geometry_msgs::msg::PoseStamped>("filtered_dynamic_pose");
...
nav2::CallbackReturn FollowingServer::on_activate(const rclcpp_lifecycle::State& /*state*/) {
    ...
    dyn_params_handler_ = node->add_on_set_parameters_callback(std::bind(&FollowingServer::dynamicParametersCallback, this, _1));
...
nav2::CallbackReturn FollowingServer::on_deactivate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn FollowingServer::on_cleanup(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn FollowingServer::on_shutdown(const rclcpp_lifecycle::State&) {
    RCLCPP_INFO(get_logger(), "Shutting down %s", get_name());
    return nav2::CallbackReturn::SUCCESS;
}

template <typename ActionT>
void FollowingServer::getPreemptedGoalIfRequested(typename std::shared_ptr<const typename ActionT::Goal> goal,
...
template <typename ActionT>
bool FollowingServer::checkAndWarnIfCancelled(typename nav2::SimpleActionServer<ActionT>::SharedPtr& action_server, const std::string& name) {
...
template <typename ActionT>
bool FollowingServer::checkAndWarnIfPreempted(typename nav2::SimpleActionServer<ActionT>::SharedPtr& action_server, const std::string& name) {
    if (action_server->is_preempt_requested()) {
...
void FollowingServer::followObject() {
    ...
    try {
        ...
        if (target_frame.empty()) {
            if (pose_topic.empty()) {
                RCLCPP_ERROR(get_logger(), "Both pose topic and target frame are empty. Cannot follow object.");
                ...
                dynamic_pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
                    pose_topic, [this](const geometry_msgs::msg::PoseStamped::SharedPtr pose) { detected_dynamic_pose_ = *pose; },
        ...
        while (rclcpp::ok()) {
            try {
                ...
                if (approachObject(object_pose, target_frame)) {
                    ...
                    RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000, "Reached object. Stopping until goal is moved again.");
                    ...
                    if (static_object_timeout_ > 0.0) {
                        ...
                        if (static_duration.seconds() > static_object_timeout_) {
                            RCLCPP_INFO(get_logger(), "Object has been static for %.2f seconds (timeout: %.2f), stopping.", static_duration.seconds(),
                                        static_object_timeout_);
                            result->total_elapsed_time = this->now() - action_start_time_;
                            result->num_retries = num_retries_;
                            publishZeroVelocity();
                            following_action_server_->succeeded_current(result);
...
bool FollowingServer::approachObject(geometry_msgs::msg::PoseStamped& object_pose, const std::string& target_frame) {
    rclcpp::Rate loop_rate(controller_frequency_);
    while (rclcpp::ok()) {
        ...
        try {
            tf2_buffer_->transform(target_pose, target_pose, base_frame_, tf2::durationFromSec(transform_tolerance_));
        ...
        if (!nav2_util::getCurrentPose(robot_pose, *tf2_buffer_, target_pose.header.frame_id, base_frame_, transform_tolerance_,
...
bool FollowingServer::rotateToObject(geometry_msgs::msg::PoseStamped& object_pose, const std::string& target_frame) {
    const double dt = 1.0 / controller_frequency_;

    // Compute initial robot heading
    geometry_msgs::msg::PoseStamped robot_pose;
    if (!nav2_util::getCurrentPose(robot_pose, *tf2_buffer_, object_pose.header.frame_id, base_frame_, transform_tolerance_, iteration_start_time_)) {
        RCLCPP_WARN(get_logger(), "Failed to get current robot pose");
        return false;
    }
    ...
    for (const double& target_angle : angles) {
        ...
        while (rclcpp::ok()) {
            ...
            if (!nav2_util::getCurrentPose(robot_pose, *tf2_buffer_, object_pose.header.frame_id, base_frame_, transform_tolerance_,
                                           iteration_start_time_)) {
                RCLCPP_WARN(get_logger(), "Failed to get current robot pose");
                return false;
            }

            double angular_distance_to_heading = angles::shortest_angular_distance(tf2::getYaw(robot_pose.pose.orientation), target_angle);
            ...
            command->twist = controller_->computeRotateToHeadingCommand(angular_distance_to_heading, current_vel, dt);
...
bool FollowingServer::getRefinedPose(geometry_msgs::msg::PoseStamped& pose) {
    ...
    if (detected.header.frame_id != fixed_frame_) {
        try {
            tf2_buffer_->transform(detected, detected, fixed_frame_, tf2::durationFromSec(transform_tolerance_));
        } catch (const tf2::TransformException& ex) {
    ...
    if (skip_orientation_) {
        geometry_msgs::msg::PoseStamped robot_pose;
        if (!nav2_util::getCurrentPose(robot_pose, *tf2_buffer_, detected.header.frame_id, base_frame_, transform_tolerance_,
                                       iteration_start_time_)) {
            RCLCPP_WARN(get_logger(), "Failed to get current robot pose");
            return false;
        }
...
bool FollowingServer::getFramePose(geometry_msgs::msg::PoseStamped& pose, const std::string& frame_id) {
    try {
        // Get the transform from the target frame to the fixed frame
        auto transform = tf2_buffer_->lookupTransform(fixed_frame_, frame_id, iteration_start_time_, tf2::durationFromSec(transform_tolerance_));
        ...
        RCLCPP_WARN(get_logger(), "Failed to get transform for frame %s: %s", frame_id.c_str(), ex.what());
        return false;
    }
...
bool FollowingServer::getTrackingPose(geometry_msgs::msg::PoseStamped& pose, const std::string& frame_id) {
    // Use frame tracking if we have a target frame, otherwise use topic tracking
    if (!frame_id.empty()) {
        if (!getFramePose(pose, frame_id)) {
            throw opennav_docking_core::FailedToDetectDock("Failed to get pose in target frame: " + frame_id);
        }
    } else {
...
geometry_msgs::msg::PoseStamped FollowingServer::getPoseAtDistance(const geometry_msgs::msg::PoseStamped& pose, double distance) {
    geometry_msgs::msg::PoseStamped robot_pose;
    if (!nav2_util::getCurrentPose(robot_pose, *tf2_buffer_, pose.header.frame_id, base_frame_, transform_tolerance_, iteration_start_time_)) {
        RCLCPP_WARN(get_logger(), "Failed to get current robot pose");
...
bool FollowingServer::isGoalReached(const geometry_msgs::msg::PoseStamped& goal_pose) {
    geometry_msgs::msg::PoseStamped robot_pose;
    if (!nav2_util::getCurrentPose(robot_pose, *tf2_buffer_, goal_pose.header.frame_id, base_frame_, transform_tolerance_, iteration_start_time_)) {
        RCLCPP_WARN(get_logger(), "Failed to get current robot pose");
        return false;
    }
    const double dist = std::hypot(robot_pose.pose.position.x - goal_pose.pose.position.x, robot_pose.pose.position.y - goal_pose.pose.position.y);
    const double yaw = angles::shortest_angular_distance(tf2::getYaw(robot_pose.pose.orientation), tf2::getYaw(goal_pose.pose.orientation));
    return dist < linear_tolerance_ && abs(yaw) < angular_tolerance_;
}

rcl_interfaces::msg::SetParametersResult FollowingServer::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters) {
