...
ControllerServer::ControllerServer(const rclcpp::NodeOptions& options)
    ...
    costmap_ros_ = std::make_shared<nav2_costmap_2d::Costmap2DROS>("local_costmap", std::string{get_namespace()},
...
nav2::CallbackReturn ControllerServer::on_configure(const rclcpp_lifecycle::State& state) {
    auto node = shared_from_this();

    RCLCPP_INFO(get_logger(), "Configuring controller interface");

    RCLCPP_INFO(get_logger(), "getting progress checker plugins..");
    progress_checker_ids_ = declare_or_get_parameter("progress_checker_plugins", default_progress_checker_ids_);
    if (progress_checker_ids_ == default_progress_checker_ids_) {
        for (size_t i = 0; i < default_progress_checker_ids_.size(); ++i) {
            nav2::declare_parameter_if_not_declared(node, default_progress_checker_ids_[i] + ".plugin",
    ...
    if (goal_checker_ids_ == default_goal_checker_ids_) {
        for (size_t i = 0; i < default_goal_checker_ids_.size(); ++i) {
            nav2::declare_parameter_if_not_declared(node, default_goal_checker_ids_[i] + ".plugin",
    ...
    if (controller_ids_ == default_ids_) {
        for (size_t i = 0; i < default_ids_.size(); ++i) {
            nav2::declare_parameter_if_not_declared(node, default_ids_[i] + ".plugin", rclcpp::ParameterValue(default_types_[i]));
        }
    }
    ...
    std::string speed_limit_topic = declare_or_get_parameter("speed_limit_topic", std::string("speed_limit"));
    ...
    for (size_t i = 0; i != progress_checker_ids_.size(); i++) {
        try {
            progress_checker_types_[i] = nav2::get_plugin_type_param(node, progress_checker_ids_[i]);
            nav2_core::ProgressChecker::Ptr progress_checker = progress_checker_loader_.createUniqueInstance(progress_checker_types_[i]);
            RCLCPP_INFO(get_logger(), "Created progress_checker : %s of type %s", progress_checker_ids_[i].c_str(),
                        progress_checker_types_[i].c_str());
    ...
    RCLCPP_INFO(get_logger(), "Controller Server has %s progress checkers available.", progress_checker_ids_concat_.c_str());
    ...
    for (size_t i = 0; i != goal_checker_ids_.size(); i++) {
        try {
            ...
            RCLCPP_INFO(get_logger(), "Created goal checker : %s of type %s", goal_checker_ids_[i].c_str(), goal_checker_types_[i].c_str());
    ...
    for (size_t i = 0; i != controller_ids_.size(); i++) {
        try {
            ...
            RCLCPP_INFO(get_logger(), "Created controller : %s of type %s", controller_ids_[i].c_str(), controller_types_[i].c_str());
    ...
    try {
        action_server_ = create_action_server<Action>("follow_path", std::bind(&ControllerServer::computeControl, this), nullptr,
                                                      std::chrono::milliseconds(500), true /*spin thread*/, use_realtime_priority_ /*soft realtime*/);
...
nav2::CallbackReturn ControllerServer::on_activate(const rclcpp_lifecycle::State& /*state*/) {
    ...
    dyn_params_handler_ = node->add_on_set_parameters_callback(std::bind(&ControllerServer::dynamicParametersCallback, this, _1));
...
nav2::CallbackReturn ControllerServer::on_deactivate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn ControllerServer::on_cleanup(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn ControllerServer::on_shutdown(const rclcpp_lifecycle::State&) {
...
bool ControllerServer::findProgressCheckerId(const std::string& c_name, std::string& current_progress_checker) {
    if (progress_checkers_.size() == 0) {
        if (c_name.empty()) {
            RCLCPP_DEBUG(get_logger(), "No progress checker configured and none requested. Progress checking will be bypassed.");
    ...
    if (progress_checkers_.find(c_name) == progress_checkers_.end()) {
        if (progress_checkers_.size() == 1 && c_name.empty()) {
            RCLCPP_WARN_ONCE(get_logger(),
                             ...
                             " Server will use only plugin loaded %s. "
                             "This warning will appear once.",
                             progress_checker_ids_concat_.c_str());
        ...
        while (rclcpp::ok()) {
            ...
            if (action_server_->is_cancel_requested()) {
                if (controllers_[current_controller_]->cancel()) {
                    ...
                    RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000, "Waiting for the controller to finish cancellation");
                }
            }
            ...
            if (!loop_rate.sleep()) {
                RCLCPP_WARN(get_logger(), "Control loop missed its desired rate of %.4f Hz. Current loop rate is %.4f Hz.", controller_frequency_,
                            1 / cycle_duration.seconds());
                loop_rate.reset();
            }
        }
...
void ControllerServer::computeAndPublishVelocity() {
    ...
    try {
        cmd_vel_2d = controllers_[current_controller_]->computeVelocityCommands(pose, twist, goal_checkers_[current_goal_checker_].get());
        last_valid_cmd_time_ = now();
    ...
    if (!nav2_util::transformPoseInTargetFrame(end_pose_, transformed_end_pose_, *costmap_ros_->getTfBuffer(), costmap_ros_->getGlobalFrameID(),
                                               costmap_ros_->getTransformTolerance())) {
        throw nav2_core::ControllerTFError("Failed to transform end pose to global frame");
    }

    if (current_path_.poses.size() >= 2) {
        double current_distance_to_goal = nav2_util::geometry_utils::euclidean_distance(pose, transformed_end_pose_);

        // Transform robot pose to path frame for path tracking calculations
        geometry_msgs::msg::PoseStamped robot_pose_in_path_frame;
        if (!nav2_util::transformPoseInTargetFrame(pose, robot_pose_in_path_frame, *costmap_ros_->getTfBuffer(), current_path_.header.frame_id,
                                                   costmap_ros_->getTransformTolerance())) {
            throw nav2_core::ControllerTFError("Failed to transform robot pose to path frame");
        }

        const auto path_search_result = nav2_util::distance_from_path(current_path_, robot_pose_in_path_frame.pose, start_index_, search_window_);
        ...
        tracking_feedback_msg->remaining_path_length = nav2_util::geometry_utils::calculate_path_length(current_path_, start_index_);
...
void ControllerServer::updateGlobalPath() {
    if (action_server_->is_preempt_requested()) {
        ...
        if (findControllerId(goal->controller_id, current_controller)) {
            ...
            std::shared_ptr<Action::Result> result = std::make_shared<Action::Result>();
            result->error_code = Action::Result::INVALID_CONTROLLER;
            result->error_msg = "Terminating action, invalid controller " + goal->controller_id + " requested.";
            action_server_->terminate_current(result);
            return;
        }
        std::string current_goal_checker;
        if (findGoalCheckerId(goal->goal_checker_id, current_goal_checker)) {
            ...
            std::shared_ptr<Action::Result> result = std::make_shared<Action::Result>();
            result->error_code = Action::Result::INVALID_CONTROLLER;
            result->error_msg = "Terminating action, invalid goal checker " + goal->goal_checker_id + " requested.";
            action_server_->terminate_current(result);
            return;
        }
        std::string current_progress_checker;
        if (findProgressCheckerId(goal->progress_checker_id, current_progress_checker)) {
            if (current_progress_checker_ != current_progress_checker) {
                RCLCPP_INFO(get_logger(), "Change of progress checker %s requested, resetting it", goal->progress_checker_id.c_str());
                current_progress_checker_ = current_progress_checker;
                if (!current_progress_checker_.empty()) {
                    progress_checkers_[current_progress_checker_]->reset();
                }
            ...
            std::shared_ptr<Action::Result> result = std::make_shared<Action::Result>();
            result->error_code = Action::Result::INVALID_CONTROLLER;
            result->error_msg = "Terminating action, invalid progress checker " + goal->progress_checker_id + " requested.";
            action_server_->terminate_current(result);
            return;
        }
        setPlannerPath(goal->path);
...
bool ControllerServer::isGoalReached() {
    ...
    return goal_checkers_[current_goal_checker_]->isGoalReached(pose.pose, transformed_end_pose_.pose, velocity);
}
...
rcl_interfaces::msg::SetParametersResult ControllerServer::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters) {
