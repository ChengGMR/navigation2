...
SmootherServer::SmootherServer(const rclcpp::NodeOptions& options)
    ...
    declare_parameter("costmap_topic", rclcpp::ParameterValue(std::string("global_costmap/costmap_raw")));
    declare_parameter("footprint_topic", rclcpp::ParameterValue(std::string("global_costmap/published_footprint")));
...
nav2::CallbackReturn SmootherServer::on_configure(const rclcpp_lifecycle::State& state) {
    ...
    costmap_sub_ = std::make_shared<nav2_costmap_2d::CostmapSubscriber>(shared_from_this(), costmap_topic);
    footprint_sub_ =
        std::make_shared<nav2_costmap_2d::FootprintSubscriber>(shared_from_this(), footprint_topic, *tf_, robot_base_frame, transform_tolerance);

    collision_checker_ = std::make_shared<nav2_costmap_2d::CostmapTopicCollisionChecker>(*costmap_sub_, *footprint_sub_, this->get_name());
    ...
    action_server_ =
        create_action_server<Action>("smooth_path", std::bind(&SmootherServer::smoothPlan, this), nullptr, std::chrono::milliseconds(500), true);
...
bool SmootherServer::loadSmootherPlugins() {
    ...
    for (size_t i = 0; i != smoother_ids_.size(); i++) {
        try {
            smoother_types_[i] = nav2::get_plugin_type_param(node, smoother_ids_[i]);
            nav2_core::Smoother::Ptr smoother = lp_loader_.createUniqueInstance(smoother_types_[i]);
            RCLCPP_INFO(get_logger(), "Created smoother : %s of type %s", smoother_ids_[i].c_str(), smoother_types_[i].c_str());
    ...
    RCLCPP_INFO(get_logger(), "Smoother Server has %s smoothers available.", smoother_ids_concat_.c_str());

    return true;
}

nav2::CallbackReturn SmootherServer::on_activate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn SmootherServer::on_deactivate(const rclcpp_lifecycle::State&) {
...
nav2::CallbackReturn SmootherServer::on_cleanup(const rclcpp_lifecycle::State&) {
...
nav2::CallbackReturn SmootherServer::on_shutdown(const rclcpp_lifecycle::State&) {
...
void SmootherServer::smoothPlan() {
    ...
    try {
        ...
        if (goal->check_for_collisions) {
            ...
            for (const auto& p : result->path.poses) {
                ...
                if (!collision_checker_->isCollisionFree(pose, fetch_data)) {
                    RCLCPP_ERROR(get_logger(), "Smoothed path leads to a collision at x: %lf, y: %lf, theta: %lf", pose.position.x, pose.position.y,
                                 tf2::getYaw(pose.orientation));
                    throw nav2_core::SmoothedPathInCollision(
                        "Smoothed Path collided at"
                        "X: "
                        + std::to_string(pose.position.x) + "Y: " + std::to_string(pose.position.y)
        ...
        RCLCPP_DEBUG(get_logger(), "Smoother succeeded (time: %lf), setting result", rclcpp::Duration(result->smoothing_duration).seconds());
