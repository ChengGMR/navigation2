...
void SavitzkyGolaySmoother::configure(const nav2::LifecycleNode::WeakPtr& parent, std::string name, std::shared_ptr<tf2_ros::Buffer> /*tf*/,
    ...
    declare_parameter_if_not_declared(node, name + ".enforce_path_inversion", rclcpp::ParameterValue(true));
    declare_parameter_if_not_declared(node, name + ".window_size", rclcpp::ParameterValue(7));
    declare_parameter_if_not_declared(node, name + ".poly_order", rclcpp::ParameterValue(3));
    ...
    if (window_size_ % 2 == 0 || window_size_ <= 2) {
        throw nav2_core::SmootherException("Savitzky-Golay Smoother requires an odd window size of 3 or greater");
    }
    ...
    for (unsigned int i = 0; i != path_segments.size(); i++) {
        if (path_segments[i].end - path_segments[i].start > minimum_points) {
            ...
            if (time_remaining <= 0.0) {
                RCLCPP_WARN(logger_, "Smoothing time exceeded allowed duration of %0.2f.", max_time.seconds());
...
bool SavitzkyGolaySmoother::smoothImpl(nav_msgs::msg::Path& path, bool& reversing_segment) {
    ...
    auto toEigenVec = [](const geometry_msgs::msg::PoseStamped& pose) -> Eigen::Vector2d { return {pose.pose.position.x, pose.pose.position.y}; };

    auto applyFilterOverAxes = [&](std::vector<geometry_msgs::msg::PoseStamped>& plan_pts,
