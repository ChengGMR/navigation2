...
void SimpleSmoother::configure(const nav2::LifecycleNode::WeakPtr& parent, std::string name, std::shared_ptr<tf2_ros::Buffer> /*tf*/,
...
bool SimpleSmoother::smooth(nav_msgs::msg::Path& path, const rclcpp::Duration& max_time) {
    ...
    std::vector<nav2_util::PathSegment> path_segments{PathSegment{0u, static_cast<unsigned int>(path.poses.size() - 1)}};
...
void SimpleSmoother::smoothImpl(nav_msgs::msg::Path& path, bool& reversing_segment, const nav2_costmap_2d::Costmap2D* costmap,
...
double SimpleSmoother::getFieldByDim(const geometry_msgs::msg::PoseStamped& msg, const unsigned int& dim) {
    if (dim == 0) {
...
void SimpleSmoother::setFieldByDim(geometry_msgs::msg::PoseStamped& msg, const unsigned int dim, const double& value) {
    if (dim == 0) {
