...
TEST(SmootherTest, test_sg_smoother_basics) {
    nav2::LifecycleNode::SharedPtr node = std::make_shared<nav2::LifecycleNode>("SmacSGSmootherTest");

    std::shared_ptr<nav2_msgs::msg::Costmap> costmap_msg = std::make_shared<nav2_msgs::msg::Costmap>();
    ...
    for (uint i = 0; i != straight_regular_path.poses.size() - 1; i++) {
        ...
        EXPECT_NEAR(fabs(straight_regular_path.poses[i].pose.position.y - straight_regular_path_baseline.poses[i].pose.position.y), 0.0, 0.011);
    }
...
TEST(SmootherTest, test_sg_smoother_noisey_path) {
    ...
    for (unsigned int i = 0; i != noisey_path.poses.size() - 1; i++) {
        length += std::hypot(noisey_path.poses[i + 1].pose.position.x - noisey_path.poses[i].pose.position.x,
                             noisey_path.poses[i + 1].pose.position.y - noisey_path.poses[i].pose.position.y);
        base_length += std::hypot(noisey_path_baseline.poses[i + 1].pose.position.x - noisey_path_baseline.poses[i].pose.position.x,
                                  noisey_path_baseline.poses[i + 1].pose.position.y - noisey_path_baseline.poses[i].pose.position.y);
    }
    ...
    length = 0;
    for (unsigned int i = 0; i != noisey_path.poses.size() - 1; i++) {
        length += std::hypot(noisey_path_refined.poses[i + 1].pose.position.x - noisey_path_refined.poses[i].pose.position.x,
                             noisey_path_refined.poses[i + 1].pose.position.y - noisey_path_refined.poses[i].pose.position.y);
    }
...
TEST(SmootherTest, test_sg_smoother_reversing) {
    ...
    for (unsigned int i = 0; i != cusp_path.poses.size() - 1; i++) {
        length += std::hypot(cusp_path.poses[i + 1].pose.position.x - cusp_path.poses[i].pose.position.x,
                             cusp_path.poses[i + 1].pose.position.y - cusp_path.poses[i].pose.position.y);
        base_length += std::hypot(cusp_path_baseline.poses[i + 1].pose.position.x - cusp_path_baseline.poses[i].pose.position.x,
                                  cusp_path_baseline.poses[i + 1].pose.position.y - cusp_path_baseline.poses[i].pose.position.y);
    }
