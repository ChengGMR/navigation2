...
TEST(SmootherTest, test_simple_smoother) {
    ...
    for (uint i = 0; i != straight_irregular_path.poses.size() - 1; i++) {
        ...
        EXPECT_LT(fabs(straight_irregular_path.poses[i].pose.position.y - straight_irregular_path.poses[i + 1].pose.position.y), 0.50);
    }
    ...
    for (uint i = 0; i != straight_regular_path.poses.size() - 1; i++) {
        ...
        EXPECT_NEAR(fabs(straight_regular_path.poses[i].pose.position.y - straight_regular_path.poses[i + 1].pose.position.y), 0.1, 0.001);
    }
