...
class DummySmoother : public nav2_core::Smoother {
   public:
    ...
    virtual void configure(const nav2::LifecycleNode::WeakPtr&, std::string, std::shared_ptr<tf2_ros::Buffer>,
                           std::shared_ptr<nav2_costmap_2d::CostmapSubscriber>, std::shared_ptr<nav2_costmap_2d::FootprintSubscriber>) {}
    ...
    virtual bool smooth(nav_msgs::msg::Path& path, const rclcpp::Duration& max_time) {
        ...
        pose.pose.position.x = (path.poses.front().pose.position.x + path.poses.back().pose.position.x) / 2;
        pose.pose.position.y = (path.poses.front().pose.position.y + path.poses.back().pose.position.y) / 2;
...
template <>
pluginlib::UniquePtr<nav2_core::Smoother> pluginlib::ClassLoader<nav2_core::Smoother>::createUniqueInstance(const std::string& lookup_name) {
    if (lookup_name != "DummySmoother") {
        ...
        try {
            ...
            pluginlib::UniquePtr<nav2_core::Smoother> obj = lowlevel_class_loader_.createUniqueInstance<nav2_core::Smoother>(class_type);
            return obj;
    ...
    return std::unique_ptr<nav2_core::Smoother, class_loader::ClassLoader::DeleterType<nav2_core::Smoother>>(new DummySmoother(), onPluginDeletion);
}
...
class DummyFootprintSubscriber : public nav2_costmap_2d::FootprintSubscriber {
   public:
    DummyFootprintSubscriber(nav2::LifecycleNode::SharedPtr node, const std::string& topic_name, tf2_ros::Buffer& tf)
...
class SmootherTest : public ::testing::Test {
   public:
    ...
    void SetUp() override {
        node_ = std::make_shared<nav2::LifecycleNode>("LifecycleSmootherTestNode");

        smoother_server_ = std::make_shared<DummySmootherServer>();
        smoother_server_->set_parameter(
            rclcpp::Parameter("smoother_plugins", rclcpp::ParameterValue(std::vector<std::string>(1, "DummySmoothPath"))));
        smoother_server_->declare_parameter("DummySmoothPath.plugin", rclcpp::ParameterValue(std::string("DummySmoother")));
        smoother_server_->configure();
        smoother_server_->activate();

        client_ =
            rclcpp_action::create_client<SmoothAction>(node_->get_node_base_interface(), node_->get_node_graph_interface(),
    ...
    bool sendGoal(std::string smoother_id, double x_start, double y_start, double x_goal, double y_goal, std::chrono::milliseconds max_time,
...
TEST(SmootherConfigTest, testingConfigureSuccessWithValidSmootherPlugin) {
    ...
    smoother_server->set_parameter(rclcpp::Parameter("smoother_plugins", rclcpp::ParameterValue(std::vector<std::string>(1, "DummySmoothPath"))));
    smoother_server->declare_parameter("DummySmoothPath.plugin", rclcpp::ParameterValue(std::string("DummySmoother")));
...
TEST(SmootherConfigTest, testingConfigureFailureWithInvalidSmootherPlugin) {
    auto smoother_server = std::make_shared<DummySmootherServer>();
    smoother_server->set_parameter(rclcpp::Parameter("smoother_plugins", rclcpp::ParameterValue(std::vector<std::string>(1, "DummySmoothPath"))));
    smoother_server->declare_parameter("DummySmoothPath.plugin", rclcpp::ParameterValue(std::string("InvalidSmootherPlugin")));
    auto state = smoother_server->configure();
