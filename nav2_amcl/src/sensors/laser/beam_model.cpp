...
BeamModel::BeamModel(double z_hit, double z_short, double z_max, double z_rand, double sigma_hit, double lambda_short, double chi_outlier,
...
double BeamModel::sensorFunction(LaserData* data, pf_sample_set_t* set) {
    ...
    for (j = 0; j < set->sample_count; j++) {
        ...
        for (i = 0; i < data->range_count; i += step) {
            ...
            map_range = map_calc_range(self->map_, pose.v[0], pose.v[1], pose.v[2] + obs_bearing, data->range_max);
            pz = 0.0;
