...
AmclNode::AmclNode(const rclcpp::NodeOptions& options) : nav2::LifecycleNode("amcl", "", options) {
...
nav2::CallbackReturn AmclNode::on_configure(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn AmclNode::on_activate(const rclcpp_lifecycle::State& /*state*/) {
    ...
    post_set_params_handler_ = node->add_post_set_parameters_callback(std::bind(&AmclNode::updateParametersCallback, this, std::placeholders::_1));
    on_set_params_handler_ =
        node->add_on_set_parameters_callback(std::bind(&AmclNode::validateParameterUpdatesCallback, this, std::placeholders::_1));
...
nav2::CallbackReturn AmclNode::on_deactivate(const rclcpp_lifecycle::State& /*state*/) {
...
nav2::CallbackReturn AmclNode::on_cleanup(const rclcpp_lifecycle::State& /*state*/) {
    ...
    if (set_initial_pose_) {
        set_parameter(rclcpp::Parameter("initial_pose.x", rclcpp::ParameterValue(last_published_pose_.pose.pose.position.x)));
        set_parameter(rclcpp::Parameter("initial_pose.y", rclcpp::ParameterValue(last_published_pose_.pose.pose.position.y)));
        set_parameter(rclcpp::Parameter("initial_pose.z", rclcpp::ParameterValue(last_published_pose_.pose.pose.position.z)));
        set_parameter(rclcpp::Parameter("initial_pose.yaw", rclcpp::ParameterValue(tf2::getYaw(last_published_pose_.pose.pose.orientation))));
    }

    return nav2::CallbackReturn::SUCCESS;
}

nav2::CallbackReturn AmclNode::on_shutdown(const rclcpp_lifecycle::State& /*state*/) {
...
bool AmclNode::getOdomPose(geometry_msgs::msg::PoseStamped& odom_pose, double& x, double& y, double& yaw, const rclcpp::Time& sensor_timestamp,
...
void AmclNode::globalLocalizationCallback(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
    ...
    pf_init_model(pf_, (pf_init_model_fn_t)AmclNode::uniformPoseGenerator, reinterpret_cast<void*>(map_));
    RCLCPP_INFO(get_logger(), "Global initialisation done!");
    initial_pose_is_known_ = true;
    pf_init_ = false;
}

void AmclNode::initialPoseReceivedSrv(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
...
void AmclNode::nomotionUpdateCallback(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
                                      const std::shared_ptr<std_srvs::srv::Empty::Request> /*req*/,
                                      std::shared_ptr<std_srvs::srv::Empty::Response> /*res*/) {
...
void AmclNode::initialPoseReceived(geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg) {
    ...
    if (first_map_received_ && (abs(msg->pose.pose.position.x) > map_->size_x || abs(msg->pose.pose.position.y) > map_->size_y)) {
        RCLCPP_ERROR(get_logger(), "Received initialpose from message is out of the size of map. Rejecting.");
        return;
    }
...
void AmclNode::handleInitialPose(geometry_msgs::msg::PoseWithCovarianceStamped& msg) {
    ...
    try {
        ...
        tx_odom = tf_buffer_->lookupTransform(base_frame_id_, tf2_ros::fromMsg(msg.header.stamp), base_frame_id_, tf2_time, odom_frame_id_);
    } catch (tf2::TransformException& e) {
    ...
    RCLCPP_INFO(get_logger(), "Setting pose (%.6f): %.3f %.3f %.3f", now().nanoseconds() * 1e-9, pose_new.getOrigin().x(), pose_new.getOrigin().y(),
...
bool AmclNode::addNewScanner(int& laser_index, const sensor_msgs::msg::LaserScan::ConstSharedPtr& laser_scan, const std::string& laser_scan_frame_id,
                             geometry_msgs::msg::PoseStamped& laser_pose) {
...
bool AmclNode::updateFilter(const int& laser_index, const sensor_msgs::msg::LaserScan::ConstSharedPtr& laser_scan, const pf_vector_t& pose) {
    ...
    try {
        ...
        RCLCPP_WARN(get_logger(), "Unable to transform min/max laser angles into base frame: %s", e.what());
        return false;
    }
...
bool AmclNode::getMaxWeightHyp(std::vector<amcl_hyp_t>& hyps, amcl_hyp_t& max_weight_hyps, int& max_weight_hyp) {
    ...
    if (max_weight > 0.0) {
        RCLCPP_DEBUG(get_logger(), "Max weight pose: %.3f %.3f %.3f", hyps[max_weight_hyp].pf_pose_mean.v[0], hyps[max_weight_hyp].pf_pose_mean.v[1],
...
void AmclNode::publishAmclPose(const sensor_msgs::msg::LaserScan::ConstSharedPtr& laser_scan, const std::vector<amcl_hyp_t>& hyps,
    ...
    RCLCPP_DEBUG(get_logger(), "New pose: %6.3f %6.3f %6.3f", hyps[max_weight_hyp].pf_pose_mean.v[0], hyps[max_weight_hyp].pf_pose_mean.v[1],
                 hyps[max_weight_hyp].pf_pose_mean.v[2]);
}

void AmclNode::calculateMaptoOdomTransform(const sensor_msgs::msg::LaserScan::ConstSharedPtr& laser_scan, const std::vector<amcl_hyp_t>& hyps,
                                           const int& max_weight_hyp) {
...
nav2_amcl::Laser* AmclNode::createLaserObject() {
    ...
    if (sensor_model_type_ == "beam") {
        return new nav2_amcl::BeamModel(z_hit_, z_short_, z_max_, z_rand_, sigma_hit_, lambda_short_, 0.0, max_beams_, map_);
    }

    if (sensor_model_type_ == "likelihood_field_prob") {
        return new nav2_amcl::LikelihoodFieldModelProb(z_hit_, z_rand_, sigma_hit_, laser_likelihood_max_dist_, do_beamskip_, beam_skip_distance_,
                                                       beam_skip_threshold_, beam_skip_error_threshold_, max_beams_, map_);
    }

    return new nav2_amcl::LikelihoodFieldModel(z_hit_, z_rand_, sigma_hit_, laser_likelihood_max_dist_, max_beams_, map_);
}
...
void AmclNode::initParameters() {
    ...
    sensor_model_type_ = this->declare_or_get_parameter("laser_model_type", std::string{"likelihood_field"});
    ...
    robot_model_type_ = this->declare_or_get_parameter("robot_model_type", std::string{"nav2_amcl::DifferentialMotionModel"});
    ...
    allow_parameter_qos_overrides_ = this->declare_or_get_parameter("allow_parameter_qos_overrides", true);
...
rcl_interfaces::msg::SetParametersResult AmclNode::validateParameterUpdatesCallback(const std::vector<rclcpp::Parameter>& parameters) {
...
void AmclNode::updateParametersCallback(const std::vector<rclcpp::Parameter>& parameters) {
    std::lock_guard<std::recursive_mutex> cfl(mutex_);
    ...
    if (reinit_map) {
        map_sub_.reset();
        map_sub_ = create_subscription<nav_msgs::msg::OccupancyGrid>(map_topic_, std::bind(&AmclNode::mapReceived, this, std::placeholders::_1),
...
void AmclNode::handleMapMessage(const nav_msgs::msg::OccupancyGrid& msg) {
    ...
    RCLCPP_INFO(get_logger(), "Received a %d X %d map @ %.3f m/pix", msg.info.width, msg.info.height, msg.info.resolution);
    if (msg.header.frame_id != global_frame_id_) {
        RCLCPP_WARN(get_logger(),
...
map_t* AmclNode::convertMap(const nav_msgs::msg::OccupancyGrid& map_msg) {
    ...
    map->cells = reinterpret_cast<map_cell_t*>(malloc(sizeof(map_cell_t) * map->size_x * map->size_y));
...
void AmclNode::initMessageFilters() {
    auto sub_opt = nav2::interfaces::createSubscriptionOptions(scan_topic_, allow_parameter_qos_overrides_, callback_group_);

#if RCLCPP_VERSION_GTE(29, 6, 0)
    laser_scan_sub_ = std::make_unique<message_filters::Subscriber<sensor_msgs::msg::LaserScan>>(shared_from_this(), scan_topic_,
                                                                                                 nav2::qos::SensorDataQoS(), sub_opt);
#else
    laser_scan_sub_ = std::make_unique<message_filters::Subscriber<sensor_msgs::msg::LaserScan, rclcpp_lifecycle::LifecycleNode>>(
        std::static_pointer_cast<rclcpp_lifecycle::LifecycleNode>(shared_from_this()), scan_topic_, nav2::qos::SensorDataQoS().get_rmw_qos_profile(),
        sub_opt);
#endif

    laser_scan_filter_ = std::make_unique<tf2_ros::MessageFilter<sensor_msgs::msg::LaserScan>>(
        *laser_scan_sub_, *tf_buffer_, odom_frame_id_, 10, get_node_logging_interface(), get_node_clock_interface(), transform_tolerance_);

    laser_scan_connection_ = laser_scan_filter_->registerCallback(std::bind(&AmclNode::laserReceived, this, std::placeholders::_1));
}

void AmclNode::initPubSub() {
    RCLCPP_INFO(get_logger(), "initPubSub");

    particle_cloud_pub_ = create_publisher<nav2_msgs::msg::ParticleCloud>("particle_cloud", nav2::qos::SensorDataQoS());

    pose_pub_ = create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>("amcl_pose", nav2::qos::LatchedPublisherQoS());
...
void AmclNode::initServices() {
    ...
    initial_guess_srv_ = create_service<nav2_msgs::srv::SetInitialPose>(
        "set_initial_pose", std::bind(&AmclNode::initialPoseReceivedSrv, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
void AmclNode::initParticleFilter() {
    ...
    pf_ = pf_alloc(min_particles_, max_particles_, alpha_slow_, alpha_fast_, (pf_init_model_fn_t)AmclNode::uniformPoseGenerator);
    pf_->pop_err = pf_err_;
    pf_->pop_z = pf_z_;
