...
TEST(WPTest, test_dynamic_parameters) {
    ...
    auto rec_param = std::make_shared<rclcpp::AsyncParametersClient>(amcl->get_node_base_interface(), amcl->get_node_topics_interface(),
                                                                     amcl->get_node_graph_interface(), amcl->get_node_services_interface());

    auto results = rec_param->set_parameters_atomically({rclcpp::Parameter("alpha1", 1.0),
