   ...
   public:
    ...
    void startNavigating(const std::string& navigator_name) {
        ...
        if (!current_navigator_.empty()) {
            RCLCPP_ERROR(rclcpp::get_logger("NavigatorMutex"),
...
class NavigatorBase {
   public:
     ...
     */
    virtual bool on_configure(nav2::LifecycleNode::WeakPtr parent_node, const std::vector<std::string>& plugin_lib_names,
                              const FeedbackUtils& feedback_utils, nav2_core::NavigatorMuxer* plugin_muxer,
...
class BehaviorTreeNavigator : public NavigatorBase {
   public:
     ...
     * @return bool If successful
     */
    bool on_configure(nav2::LifecycleNode::WeakPtr parent_node, const std::vector<std::string>& plugin_lib_names, const FeedbackUtils& feedback_utils,
                      nav2_core::NavigatorMuxer* plugin_muxer, std::shared_ptr<nav2_util::OdomSmoother> odom_smoother) final {
        ...
        auto search_directories = node->declare_or_get_parameter(
            "bt_search_directories", std::vector<std::string>{ament_index_cpp::get_package_share_directory("nav2_bt_navigator") + "/behavior_trees"});

        // Create the Behavior Tree Action Server for this navigator
        bt_action_server_ = std::make_unique<nav2_behavior_tree::BtActionServer<ActionT, nav2::LifecycleNode>>(
            node, getName(), plugin_lib_names, default_bt_xml_filename,
            std::bind(&BehaviorTreeNavigator::onGoalReceived, this, std::placeholders::_1), std::bind(&BehaviorTreeNavigator::onLoop, this),
            std::bind(&BehaviorTreeNavigator::onPreempt, this, std::placeholders::_1),
            std::bind(&BehaviorTreeNavigator::onCompletion, this, std::placeholders::_1, std::placeholders::_2), search_directories);
   ...
   protected:
     ...
     */
    virtual bool configure(nav2::LifecycleNode::WeakPtr /*node*/, std::shared_ptr<nav2_util::OdomSmoother> /*odom_smoother*/) { return true; }
    ...
    std::unique_ptr<nav2_behavior_tree::BtActionServer<ActionT, nav2::LifecycleNode>> bt_action_server_;
